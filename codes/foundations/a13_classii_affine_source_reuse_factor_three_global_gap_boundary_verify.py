#!/usr/bin/env python3
"""Integrated verifier for the A13 R-155 affine source-reuse gap package."""
...
MANIFEST = CLAIM_DIR / "classii_affine_source_reuse_factor_three_global_gap_boundary_manifest.json"
...
NOTE = CLAIM_DIR / "notes/classii-affine-source-reuse-factor-three-global-gap-boundary-260803-v1.0.tex.txt"
...
EXPECTED_CHILD_COUNTS = {"primary": 49, "independent": 44}
EXPLORATION_IDS = ("EXP-000674", "EXP-000675", "EXP-000676")
...
def main() -> int:
    ...
    audit.check("children", "all child rows embedded once", embedded_rows == sum(EXPECTED_CHILD_COUNTS.values()), embedded_rows, sum(EXPECTED_CHILD_COUNTS.values()))
    ...
    audit.check("scope", "independent audit scope pinned", independent.get("scope") == EXPECTED_INDEPENDENT_SCOPE, independent.get("scope"), EXPECTED_INDEPENDENT_SCOPE)
    ...
    parity = {
        ...
        "H6 registered": (p["H6_registered"], i["H6_registered"]),
        "H6 upper": (p["H6_strict_upper"], i["H6_strict_upper"]),
        "response multipliers": (p["response_multipliers"], i["response_multipliers"]),
        "factor-two intersection": (p["factor_two_overlap_intersection"], i["factor_two_overlap_intersection"]),
        "recursive support": (p["recursive_connection_support"], i["recursive_connection_support"]),
        "resonance ratios": (p["distinct_resonance_ratios"], i["distinct_resonance_ratios"]),
        "edge components": (p["factor_three_edge_components"], i["factor_three_edge_components"]),
        ...
        "global loss target": (p["global_loss_target"], i["global_loss_target"]),
        "global gap": (p["certified_global_gap"], i["certified_global_gap"]),
        "dyadic target": (p["dyadic_diagonal_loss_target"], i["dyadic_diagonal_loss_target"]),
        "dyadic gap": (p["certified_pure_dyadic_gap"], i["certified_pure_dyadic_gap"]),
        "parent loss": (p["parent_row_loss_upper"], i["parent_loss"]),
        "parent margin": (p["parent_row_margin"], i["parent_margin"]),
    }
    ...
    audit.check("oracle", "global loss exact", F(p["global_loss_target"]) == F(109, 125), p["global_loss_target"], "109/125")
    audit.check("oracle", "global gap exact", F(p["certified_global_gap"]) == F(7, 250), p["certified_global_gap"], "7/250")
    audit.check("oracle", "dyadic target exact", F(p["dyadic_diagonal_loss_target"]) == F(753, 1000), p["dyadic_diagonal_loss_target"], "753/1000")
    audit.check("oracle", "dyadic gap exact", F(p["certified_pure_dyadic_gap"]) == F(147, 1000), p["certified_pure_dyadic_gap"], "147/1000")
    audit.check("oracle", "edge factor exact", F(p["edge_radial_factor"]) == 2256, p["edge_radial_factor"], 2256)
    audit.check("oracle", "parent arithmetic", F(p["parent_row_loss_upper"]) == F(201, 1000) and F(p["parent_row_margin"]) == F(699, 1000), [p["parent_row_loss_upper"], p["parent_row_margin"]], ["201/1000", "699/1000"])
    ...
    for token in (
        "source-target reuse", "factor-three", "7\\over250", "147\\over1000",
        "legal adjoint", "controlled-state", "Devil's-advocate", "T-050",
    ...
    for token in ("R-155", "source-target reuse", "factor-three", "7/250", "147/1000", "T-050", "phase/PDE"):
    ...
    if renderer is not None:
        with tempfile.TemporaryDirectory(prefix="tect-r155-render-") as temporary:
    ...
    audit.check("records", "R-155 ledger entry", '<a id="r-155"></a>' in results_text, LEDGER_ID, "registered")
    audit.check("records", "claim narrative", RESULT_ID in claim_text and "7/250" in claim_text and "147/1000" in claim_text, RESULT_ID, "registered")
    audit.check("records", "lineage narrative", "R-155" in lineage_text and "source reuse" in lineage_text.lower(), LEDGER_ID, "registered")
    audit.check("records", "status synchronization", status.get("no_overclaim") == manifest.get("no_overclaim") and "R-155" in status.get("statement", ""), status.get("no_overclaim"), manifest.get("no_overclaim"))
    audit.check("records", "TODO route", "R-155" in todo_text and "T-050" in todo_text and "T-054" in todo_text, "R-155/T-050/T-054", "registered")
    audit.check("records", "changelog entry", "R-155" in changelog_text and RESULT_ID in changelog_text, LEDGER_ID, "registered")
    audit.check("records", "theorem map", "R-155" in json.dumps(theorem_map, sort_keys=True) and RESULT_ID in json.dumps(theorem_map, sort_keys=True), LEDGER_ID, "registered")
    audit.check("records", "proof map", "R-155" in proof_map and all(identifier in proof_map for identifier in EXPLORATION_IDS), EXPLORATION_IDS, "registered")
    ...
    if all(identifier in exploration_lookup for identifier in EXPLORATION_IDS):
        audit.check("records", "exploration verdicts", [exploration_lookup[identifier]["verdict"] for identifier in EXPLORATION_IDS] == ["advanced", "advanced", "advanced"], [exploration_lookup[identifier]["verdict"] for identifier in EXPLORATION_IDS], ["advanced", "advanced", "advanced"])
