#!/usr/bin/env python3
"""Non-importing independent two-scale filter-removal corridor audit for EXP-001226 / R-384."""
...
def run() -> dict[str, Any]:
    ...
    check("R-383 source", source.get("verdict") == "PASS" and source.get("derived", {}).get("independent", {}).get("context_count") == int(fixture["expected_parent_contexts"]), [source.get("verdict"), source.get("derived", {}).get("independent", {}).get("context_count")], "R-383 PASS and expected parent contexts", "source linkage")
    ...
    check("filter power", [label for label, _power in filters] == [str(fixture["filter_power"])], [label for label, _power in filters], [fixture["filter_power"]], "filtered profile")
    ...
    return {
        ...
        "derived": {
            ...
            "low_frequency_uniformity_proved": False, "high_frequency_tail_uniformity_proved": False,
...
def main() -> int:
    ...
    print(f"INDEPENDENT TWO-SCALE-FILTER-REMOVAL-CORRIDOR PASS {payload['assertion_count']}/{payload['assertion_count']} contexts={payload['derived']['context_count']} ratio={payload['derived']['maximum_endpoint_to_cauchy_ratio']:.9f}")
    return 0
