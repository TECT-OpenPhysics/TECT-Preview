"""Integrated verifier for the R-184 two-block Douglas identity."""
...
PRIMARY = REPO / "verification" / "scripts" / "lean_a13_two_block_douglas_crosscheck.py"
INDEPENDENT = REPO / "codes" / "foundations" / "lean_a13_two_block_douglas_crosscheck_independent.py"
SCRIPT = Path(__file__).resolve()
LEAN = REPO / "verification" / "lean" / "Tect" / "R184.lean"
...
def accepts(derived: dict, manifest: dict) -> bool:
    ...
    return (
        ...
        and derived.get("wedge") == expected["wedge"]
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=REPO / "claims" / "A13-CLASSII-RELATIVE-PHASE-SOURCE-BUDGET-OBSTRUCTION" / "runs" / "2026-08-22-lean-r184-two-block-douglas-crosscheck" / "integrated.json")
    ...
    check("identity", manifest["audit_id"] == "A13-TWO-BLOCK-DOUGLAS-CROSSCHECK", manifest["audit_id"], "A13-TWO-BLOCK-DOUGLAS-CROSSCHECK")
    ...
    check("Lean theorem markers", all(marker in source for marker in manifest["theorem_markers"]), [marker for marker in manifest["theorem_markers"] if marker in source], manifest["theorem_markers"])
    ...
    with tempfile.TemporaryDirectory(prefix="r184-crosscheck-") as directory:
    ...
    check("acceptance predicate", accepts(primary["derived"], manifest), primary["derived"], "two-block Douglas identity")
    ...
    if not args.staged:
        ...
        check("R-184 authority", results.count("R-184") >= 2 and "two-block douglas" in results.lower(), results.count("R-184"), ">=2")
        check("strategy index authority", "R-184" in (REPO / "strategy/INDEX.md").read_text(encoding="utf-8"), True, True)
    ...
    print(f"INTEGRATED R-184 LEAN PASS {len(rows)}/{len(rows)}")
    return 0
