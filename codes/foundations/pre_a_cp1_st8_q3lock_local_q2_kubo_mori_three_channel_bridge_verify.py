#!/usr/bin/env python3
"""Integrated primary/independent/Lean verifier for EXP-001221 / R-379."""
...
LEAN = REPO / "verification/lean/Tect/R379.lean"
...
def compile_lean() -> dict[str, Any]:
    lake = lake_path()
    command = "lake env lean Tect/R379.lean"
    if lake is None:
        return {"status": "UNAVAILABLE", "command": command, "output": "pinned lake executable not found"}
    process = subprocess.run([str(lake), "env", "lean", "Tect/R379.lean"], cwd=LEAN_ROOT, text=True, encoding="utf-8", capture_output=True, check=False)
...
def run() -> dict[str, Any]:
    ...
    check("identity", manifest["exploration_id"] == "EXP-001221" and manifest["result_id"] == "R-379" and manifest["claim_bearing"] is False, [manifest["exploration_id"], manifest["result_id"], manifest["claim_bearing"]], "EXP-001221/R-379/false")
    ...
    markers = ("anticommutator_three_channel", "commutator_three_channel", "commutator_anticommutator_sum", "two_slice_am_gm", "two_slice_nonnegative", "scope_fixture")
    ...
    registry = json.loads(LEAN_REGISTRY.read_text(encoding="utf-8"))
    entries = [entry for entry in registry["entrypoints"] if entry["path"] == "verification/lean/Tect/R379.lean"]
    ...
    with tempfile.TemporaryDirectory(prefix="q3-three-channel-bridge-") as temporary:
        ...
        exact_fields = ("context_count", "expected_contexts", "theta", "three_channel_anticommutator_finite_checked", "three_channel_commutator_finite_checked", "two_slice_cross_envelope_finite_checked", "left_right_gns_separation_finite_checked", "beta_half_two_slice_interface_finite_checked", "uniform_half_density_channels_proved", "weighted_cutoff_uniformity_proved", "weighted_volume_uniformity_proved", "source_uniformity_proved", "shape_uniformity_proved", "common_core_closed", "common_alpha_closed", "kms_gns_gap_closed", "continuum_closed", "c6_closed", "sector_a_closed", "pre_a_closed")
        for field in exact_fields:
            check(f"agreement {field}", p[field] == i[field], [p[field], i[field]], "equal")
        numeric_fields = ("maximum_anticommutator_decomposition_residual", "maximum_commutator_decomposition_residual", "maximum_sum_decomposition_residual", "maximum_two_slice_spectral_residual", "maximum_left_spectral_residual", "maximum_right_spectral_residual", "maximum_two_slice_envelope_violation", "maximum_commutator_nonnegative_violation", "maximum_anticommutator_nonnegative_violation", "maximum_two_slice_fraction", "maximum_left_leg", "maximum_right_leg", "maximum_two_slice_leg", "maximum_commutator_leg", "maximum_anticommutator_leg", "minimum_two_slice_leg", "maximum_bond_gibbs_centering_error", "maximum_bond_unitary_factorization_error")
        ...
        regime_fields = ("shape", "volume", "contexts", "expected_contexts", "maximum_left_leg", "maximum_right_leg", "maximum_two_slice_leg", "maximum_commutator_leg", "maximum_anticommutator_leg")
        ...
        open_fields = ("uniform_half_density_channels_proved", "weighted_cutoff_uniformity_proved", "weighted_volume_uniformity_proved", "source_uniformity_proved", "shape_uniformity_proved", "common_core_closed", "common_alpha_closed", "kms_gns_gap_closed", "continuum_closed", "c6_closed", "sector_a_closed", "pre_a_closed")
...
def main() -> int:
    ...
    print(f"INTEGRATED THREE-CHANNEL-BRIDGE PASS {payload['assertion_count']}/{payload['assertion_count']}; Lean={payload['lean']['status']} diff={payload['derived']['max_primary_independent_numeric_difference']:.3e}")
    return 0
