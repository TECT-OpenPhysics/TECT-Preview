#!/usr/bin/env python3
"""Integrated primary/independent/Lean verifier for EXP-001210 / R-368."""
...
LEAN = REPO / "verification/lean/Tect/R368.lean"
...
def compile_lean() -> dict[str, Any]:
    lake = lake_path()
    command = "lake env lean Tect/R368.lean"
    if lake is None:
        return {"status": "UNAVAILABLE", "command": command, "output": "pinned lake executable not found"}
    process = subprocess.run([str(lake), "env", "lean", "Tect/R368.lean"], cwd=LEAN_ROOT, text=True, encoding="utf-8", capture_output=True, check=False)
...
def run() -> dict[str, Any]:
    ...
    check("identity", manifest["exploration_id"] == "EXP-001210" and manifest["result_id"] == "R-368" and not manifest["claim_bearing"], [manifest["exploration_id"], manifest["result_id"], manifest["claim_bearing"]], "EXP-001210/R-368/false")
    ...
    markers = ("kubo_mori_diagonal", "arithmetic_mean_symmetry", "half_envelope", "weighted_half_envelope", "prefix_fixture", "scope_fixture")
    ...
    registry = json.loads(LEAN_REGISTRY.read_text(encoding="utf-8"))
    entries = [entry for entry in registry["entrypoints"] if entry["path"] == "verification/lean/Tect/R368.lean"]
    ...
    with tempfile.TemporaryDirectory(prefix="q3-fractional-stress-") as temporary:
        ...
        exact_fields = (
            "context_count", "expected_contexts", "theta", "all_prefixes_finite_checked", "kubo_mori_weight_convention_checked",
            "weighted_fractional_bound_closed", "weighted_cutoff_uniformity_proved", "weighted_volume_uniformity_proved",
            "local_modular_dirichlet_comparison_proved", "common_core_closed", "common_alpha_closed", "kms_gns_gap_closed",
        ...
        numeric_fields = (
            "max_kubo_phase_error", "max_kubo_weighted_bound_violation", "maximum_kubo_fractional_norm", "maximum_kubo_fractional_bound",
            "maximum_kubo_to_bound_ratio", "minimum_nonzero_kubo_fractional_norm", "maximum_kubo_weight_asymmetry", "minimum_kubo_weight",
            "maximum_arithmetic_to_kubo_fractional_ratio", "max_phase_identity_error", "max_fractional_envelope_violation",
            "max_fractional_bound_violation", "max_state_trace_bound_violation", "max_bond_unitary_factorization_error",
            "max_density_hs_norm_excess", "max_spectral_reduction_error", "maximum_finite_time_change_norm", "maximum_fractional_bound",
            "maximum_fractional_norm", "maximum_state_trace_change", "maximum_fractional_to_bound_ratio", "minimum_nonzero_fractional_norm"
        )
        ...
        for regime_index, (pr, ir) in enumerate(zip(p["regimes"], i["regimes"])):
            for field in ("volume", "cutoffs", "sites", "contexts", "expected_contexts"):
                check(f"regime {regime_index} {field}", pr[field] == ir[field], [pr[field], ir[field]], "equal")
            for field in ("maximum_fractional_norm", "maximum_fractional_bound"):
        ...
        open_fields = (
            "weighted_cutoff_uniformity_proved", "weighted_volume_uniformity_proved", "local_modular_dirichlet_comparison_proved",
            "common_core_closed", "common_alpha_closed", "kms_gns_gap_closed", "continuum_closed", "c6_closed", "sector_a_closed", "pre_a_closed"
        )
...
def main() -> int:
    ...
    print(f"INTEGRATED KUBO-MORI FRACTIONAL PREFIX PASS {payload['assertion_count']}/{payload['assertion_count']}; Lean={payload['lean']['status']} diff={payload['derived']['max_primary_independent_numeric_difference']:.3e}")
    return 0
