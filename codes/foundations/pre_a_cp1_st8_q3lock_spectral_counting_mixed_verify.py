#!/usr/bin/env python3
"""Integrated primary/independent/hostile/Lean verifier for R-412."""
...
MANIFEST = ROOT / "strategy/pre-a-cp1-st8-q3lock-spectral-counting-mixed-manifest.json"
PRIMARY = ROOT / "codes/foundations/pre_a_cp1_st8_q3lock_spectral_counting_mixed.py"
INDEPENDENT = ROOT / "codes/foundations/pre_a_cp1_st8_q3lock_spectral_counting_mixed_independent.py"
HOSTILE = ROOT / "codes/foundations/pre_a_cp1_st8_q3lock_spectral_counting_mixed_hostile.py"
LEAN = ROOT / "verification/lean/Tect/R412.lean"
PRIMARY_OUTPUT = ROOT / "claims/C6-SPACETIME-SIGNATURE/runs/2026-08-31-primary-pre_a_cp1_st8_q3lock_spectral_counting_mixed/primary.json"
INDEPENDENT_OUTPUT = ROOT / "claims/C6-SPACETIME-SIGNATURE/runs/2026-08-31-independent-pre_a_cp1_st8_q3lock_spectral_counting_mixed/independent.json"
HOSTILE_OUTPUT = ROOT / "claims/C6-SPACETIME-SIGNATURE/runs/2026-08-31-hostile-pre_a_cp1_st8_q3lock_spectral_counting_mixed/hostile.json"
INTEGRATED_OUTPUT = ROOT / "claims/C6-SPACETIME-SIGNATURE/runs/2026-08-31-integrated-pre_a_cp1_st8_q3lock_spectral_counting_mixed/integrated.json"
...
def main() -> int:
    ...
    check("manifest identity", manifest["result_id"] == "R-412" and manifest["exploration_id"] == "EXP-001257" and manifest["claim_bearing"] is False, [manifest["result_id"], manifest["exploration_id"], manifest["claim_bearing"]], "R-412/EXP-001257/false", "identity")
    ...
    check("artifacts present", all(path.is_file() for path in artifacts), [str(path) for path in artifacts if not path.is_file()], "all R-412 artifacts", "provenance")
    ...
    lean = command([str(LAKE), "env", "lean", "Tect/R412.lean"], ROOT / "verification/lean")
    ...
    fields = ("system_count", "context_count", "comparison_row_count", "minimum_resistance_average", "maximum_resistance_average", "minimum_trace_inverse", "maximum_trace_inverse", "minimum_projector_trace", "maximum_projector_trace", "maximum_identity_abs_residual", "maximum_transformed_identity_abs_residual", "maximum_projector_identity_abs_residual", "minimum_positive_normalized_eigenvalue", "maximum_effective_resistance", "minimum_mode_constant", "maximum_mode_constant", "minimum_zeta_bound", "maximum_zeta_bound", "minimum_zeta_infinite_bound", "maximum_zeta_infinite_bound", "maximum_mode_envelope_abs_residual", "alpha_profiles", "mixed_selected_min_finite_bound", "mixed_selected_max_finite_bound", "mixed_selected_min_infinite_bound", "mixed_selected_max_infinite_bound", "mixed_selected_min_trace_slack", "mixed_selected_min_tail_slack", "mixed_candidate_count_per_row", "pair_count", "cutoff_dimensions")
    for field in fields:
    ...
    check("mixed envelope bounds", p["mixed_selected_min_finite_bound"] > 0.0 and p["mixed_selected_min_infinite_bound"] > 0.0 and p["mixed_selected_min_trace_slack"] >= -tolerance and p["mixed_selected_min_tail_slack"] >= -tolerance, [p["mixed_selected_min_finite_bound"], p["mixed_selected_max_finite_bound"], p["mixed_selected_min_infinite_bound"], p["mixed_selected_max_infinite_bound"], p["mixed_selected_min_trace_slack"], p["mixed_selected_min_tail_slack"]], f"selected mixed finite/infinite bounds and slacks >= -{tolerance}", "mixed zeta")
    h = hostile["derived"]
    hostile_spectral_condition = (
        all(item["mode_constant"] > 0.0 and item["trace_inverse"] <= item["mixed_profile"]["infinite_zeta_bound"] + tolerance and item["one_mode_inverse"] < item["trace_inverse"] - tolerance for item in h["selected"])
        ...
        and abs(h["toy_spectrum"]["split_bound_first"] - h["toy_spectrum"]["split_bound_last"]) > tolerance
        and all(item["status"] == "PASS" for item in hostile["checks"])
    )
    ...
    payload = {"schema": "tect/pre-a-r412-integrated/1.0", "manifest": str(MANIFEST.relative_to(ROOT)).replace("\\", "/"), "result_id": "R-412", "exploration_id": "EXP-001257", "verdict": "PASS", "checks": checks, "derived": {"primary": p, "independent": i, "hostile": h, "lean": "PASS", "command_outputs": outputs}, "scope": scope, "boundary": manifest["boundary"]}
    atomic_json(args.output if args.output.is_absolute() else ROOT / args.output, payload)
    print(f"INTEGRATED SPECTRAL-COUNTING-MIXED PASS {len(checks)}/{len(checks)} contexts={p['context_count']} rows={p['comparison_row_count']} alpha_pairs={p['mixed_candidate_count_per_row']} selected=[{p['mixed_selected_min_finite_bound']:.6g},{p['mixed_selected_max_infinite_bound']:.6g}] trace=[{p['minimum_trace_inverse']:.6g},{p['maximum_trace_inverse']:.6g}]")
    return 0
