#!/usr/bin/env python3
"""Integrated verifier for the R-436 d=18 finite interval package."""
...
MANIFEST = ROOT / "strategy/pre-a-cp1-st8-q3lock-original-source-interval-d18-manifest.json"
PRIMARY = ROOT / "codes/foundations/pre_a_cp1_st8_q3lock_original_source_interval_d18.py"
INDEPENDENT = ROOT / "codes/foundations/pre_a_cp1_st8_q3lock_original_source_interval_d18_independent.py"
HOSTILE = ROOT / "codes/foundations/pre_a_cp1_st8_q3lock_original_source_interval_d18_hostile.py"
LEAN = ROOT / "verification/lean/Tect/R436.lean"
SLUG = "original_source_interval_d18"
...
def main() -> int:
    ...
    check("manifest identity", manifest["result_id"] == "R-436" and manifest["exploration_id"] == "EXP-001281" and manifest["claim_bearing"] is False, [manifest["result_id"], manifest["exploration_id"], manifest["claim_bearing"]], "R-436/EXP-001281/false", "provenance")
    artifacts = [PRIMARY, INDEPENDENT, HOSTILE, LEAN]
    check("artifacts present", all(path.is_file() for path in artifacts), [path.relative_to(ROOT).as_posix() for path in artifacts if not path.is_file()], "all R-436 artifacts", "provenance")
    ...
    lean = command([os.fspath(LAKE), "env", "lean", "Tect/R436.lean"], ROOT / "verification/lean")
    ...
    check("primary interval certificate", primary["verdict"] == "ORIGINAL_SOURCE_INTERVAL_CERTIFIED" and primary["assertion_count"] == 58 and p["finite_positive_gap_certified"] is True and Decimal(p["maximum_residual_matrix_interval_width"]) < Decimal(manifest["interval_contract"]["maximum_matrix_interval_width"]) and Decimal(p["bracket_width"]) < Decimal(manifest["interval_contract"]["maximum_bracket_width"]), {key: p[key] for key in ("rayleigh_interval", "maximum_residual_matrix_interval_width", "bracket_width")}, "58/58 finite interval checks and bounded bracket", "primary")
    ...
    payload = {
        "schema": "tect/pre-a-r435-integrated/1.0",
        "result_id": "R-436",
        "exploration_id": "EXP-001281",
    ...
    print(f"R-436 INTEGRATED ORIGINAL_SOURCE_INTERVAL_CERTIFIED {len(checks)}/{len(checks)} bracket={p['rayleigh_interval']} Lean=PASS", flush=True)
    return 0
