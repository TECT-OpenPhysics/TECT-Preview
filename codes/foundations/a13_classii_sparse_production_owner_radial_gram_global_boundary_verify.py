#!/usr/bin/env python3
"""Integrated verifier for the A13 R-166 sparse-fibre global owner package."""
...
def main() -> int:
    ...
    if arguments.self_test:
        integrated_record = manifest.get("files", {}).get("integrated_result")
        if (
            not isinstance(integrated_record, dict)
            or integrated_record.get("path") != relative(arguments.output.resolve())
            or integrated_record.get("sha256") != sha256(arguments.output.resolve())
        ):
            raise AssertionError("manifest integrated-result pin is absent or stale")
    ...
    expected_child_pair = [EXPECTED_CHILD_COUNTS["primary"], EXPECTED_CHILD_COUNTS["independent"]]
    audit.check("manifest", "child counts", [manifest["verification"].get("primary_assertions"), manifest["verification"].get("independent_assertions")] == expected_child_pair, [manifest["verification"].get("primary_assertions"), manifest["verification"].get("independent_assertions")], expected_child_pair)

    children: dict[str, dict[str, Any]] = {}
    embedded = 0
    temp_parent = REPO / "internal/tmp"
    temp_parent.mkdir(parents=True, exist_ok=True)
    with tempfile.TemporaryDirectory(prefix="r166-child-", dir=temp_parent) as temporary:
    ...
    if renderer is not None:
        with tempfile.TemporaryDirectory(prefix="r166-render-", dir=temp_parent) as temporary:
    ...
    audit.check("records", "status open T4", status.get("tier") == "T4" and status.get("proof_complete") is False and status.get("no_overclaim", "").startswith(manifest.get("no_overclaim", "")), [status.get("tier"), status.get("proof_complete")], ["T4", False])
    audit.check("records", "TODO", any(row.get("id") == "T-050" and "R-166" in row.get("note", "") and "complete production cylinder" in row.get("note", "") for row in todo.get("tasks", [])), LEDGER_ID, "registered")
