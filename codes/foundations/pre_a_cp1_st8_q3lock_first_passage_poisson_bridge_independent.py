#!/usr/bin/env python3
"""Independent standard-library audit for EXP-001028."""
...
def run() -> dict[str, Any]:
    ...
    audit.check("hypothesis open", manifest["hypothesis"]["status"].startswith("OPEN"), manifest["hypothesis"]["status"], "OPEN", "scope")

    fixture = manifest["fixture"]
    z = Fraction(fixture["degree"])
    eta = Fraction(fixture["eta"])
    time = Fraction(fixture["time"])
    base = Fraction(fixture["weight_base"])
    distance = int(fixture["distance"])
    truncation = int(fixture["truncation_order"])
    source_mass = Fraction(fixture["source_mass"])
    rate = eta * z * time
    weighted_rate = rate * base
    partial = sum(rate**n / math.factorial(n) for n in range(truncation + 1))
    tail_partial = sum(rate**n / math.factorial(n) for n in range(distance, truncation + 1))
    spatial_factor = 1 / (base**distance)
    exp_weight = math.exp(float(weighted_rate))
    bound_numeric = float(spatial_factor) * exp_weight
    point_bound_repr = f"exp({weighted_rate})/{base**distance}"
    ...
    audit.check("short paths absent", sum(1 for n in range(distance)) == distance, distance, "no path length below d", "support")
    audit.check("weight base", base > 1, base, ">1", "weight")
    audit.check("source mass", source_mass == 1, source_mass, 1, "initial")
    audit.check("partial exponential bound", partial < exp_weight, partial, "< exp(4)", "series")
    audit.check("distance tail bound", tail_partial < bound_numeric, tail_partial, "<= 2^(-10)*exp(4)", "series")
    audit.check("point bound numeric", bound_numeric < 0.1, bound_numeric, "<0.1", "decay")
    audit.check("distance decay asymptotic", base > 1, base, "limit zero", "decay")
    ...
    return {"schema": "tect/foundation-audit/1.0", "run_kind": "independent", "verdict": "PASS", "passed": passed, "total": passed, "failed": 0, "assertions": audit.rows, "derived": {"degree": z, "eta": eta, "time": time, "weight_base": base, "distance": distance, "truncation_order": truncation, "rate": rate, "weighted_rate": weighted_rate, "point_bound": point_bound_repr, "conditional_boundary_decay_closed": True, "actual_q3_first_passage_closed": False, "exhaustion_cauchy_closed": False, "common_alpha_closed": False}, "series": {"partial": partial, "tail_partial": tail_partial, "weighted_constant": exp_weight}, "hypothesis": manifest["hypothesis"], "boundary": manifest["scope"], "exploration_id": manifest["exploration_id"], "provenance": {"script": str(SCRIPT.relative_to(REPO)).replace("\\", "/"), "script_sha256": sha256(SCRIPT), "manifest": str(MANIFEST.relative_to(REPO)).replace("\\", "/"), "manifest_sha256": sha256(MANIFEST)}}
...
def main() -> int:
    ...
    print(f"INDEPENDENT FIRST-PASSAGE-POISSON PASS {payload['passed']}/{payload['total']}")
    return 0
