#!/usr/bin/env python3
"""Independent, non-importing finite reconstruction for R-392."""
...
DEFAULT_OUTPUT = ROOT / "claims/C6-SPACETIME-SIGNATURE/runs" / "2026-08-30-independent-pre_a_cp1_st8_q3lock_quantum_markov_shell_telescope" / "independent.json"
...
def ladder(n: int) -> tuple[np.ndarray, np.ndarray]:
    ...
    for index in range(1, n): a[index - 1, index] = np.sqrt(float(index))
...
def hamiltonian(n: int, volume: int, cfg: dict[str, Any]) -> np.ndarray:
    ...
    for qj, pj in zip(qs, ps): h += pj @ pj / (2.0 * chi) + r * qj @ qj / 2.0 + g * qj @ qj @ qj @ qj / 4.0
...
def reduce_sites(rho: np.ndarray, n: int, volume: int, keep: list[int]) -> np.ndarray:
    ...
    kept = len(keep)
    for _ in rest:
        tensor = np.trace(tensor, axis1=kept, axis2=tensor.ndim // 2 + kept)
    size = n**kept
...
def layouts(volume: int, core_widths: list[int], buffer_width: int) -> list[dict[str, Any]]:
    answer: list[dict[str, Any]] = []
    for width in core_widths:
        for start in range(max(0, volume - width - buffer_width)):
            core = list(range(start, start + width)); buffer = list(range(start + width, start + width + buffer_width)); shell = list(range(start + width + buffer_width, volume))
            if shell: answer.append({"core": core, "buffer": buffer, "shell": shell, "orientation": "right", "core_width": width, "buffer_width": buffer_width})
        for end in range(width + buffer_width, volume + 1):
            core = list(range(end - width, end)); buffer = list(range(end - width - buffer_width, end - width)); shell = list(reversed(range(0, end - width - buffer_width)))
            if shell: answer.append({"core": core, "buffer": buffer, "shell": shell, "orientation": "left", "core_width": width, "buffer_width": buffer_width})
    return answer
...
def qcmI(S: Any, core: list[int], condition: list[int], shell: list[int]) -> float:
    return S(union(core, condition)) + S(union(condition, shell)) - S(condition) - S(union(core, condition, shell))
...
def profile(rows: list[dict[str, Any]], field: str, keys: tuple[str, ...]) -> dict[str, Any]:
    ...
    records = [{"key": list(key), "count": len(values), "minimum": min(values), "maximum": max(values), "range": max(values) - min(values)} for key, values in sorted(groups.items(), key=lambda item: str(item[0]))]
...
def run(path: Path) -> dict[str, Any]:
    ...
    q_values: list[float] = []; increments: list[float] = []; budgets: list[float] = []; residuals: list[float] = []
    negative_inc = 0; negative_cumulative = 0; partition_count = 0
    for volume, n in pairs:
        ...
        for beta in betas:
            ...
            def S(sites: list[int]) -> float:
                ...
                if key not in cache: cache[key] = entropy(reduce_sites(rho, n, volume, list(key)), tol)
                return cache[key]
            for layout in layouts_for_system:
                partial: list[float] = []
                for index, site in enumerate(layout["shell"]):
                    delta = qcmI(S, layout["core"], layout["buffer"] + layout["shell"][:index], [site])
                    cumulative = qcmI(S, layout["core"], layout["buffer"], layout["shell"][: index + 1])
                    budget = float(sum(partial) + delta); residual = cumulative - budget
                    q_values.append(cumulative); increments.append(delta); budgets.append(budget); residuals.append(residual)
                    negative_inc += int(delta < -tol); negative_cumulative += int(cumulative < -tol)
    ...
    if negative_inc or negative_cumulative or max(abs(value) for value in residuals) > tol: raise AssertionError("independent QCMI shell check failed")
    if not all(np.isfinite(value) for value in q_values + increments + budgets): raise AssertionError("non-finite independent value")
    derived = {"admissible_pairs": [{"volume": v, "dimension": n} for v, n in pairs], "system_count": len(pairs), "base_partition_count": partition_count, "beta_values": betas, "qcmI_record_count": len(q_values), "row_count": len(rows), "qcmI_min": min(q_values), "qcmI_max": max(q_values), "increment_min": min(increments), "increment_max": max(increments), "l1_budget_min": min(budgets), "l1_budget_max": max(budgets), "max_chain_rule_residual": max(abs(value) for value in residuals), "negative_increment_count": negative_inc, "negative_cumulative_count": negative_cumulative, "qcmI_shell_profile": profile(rows, "qcmI_increment", ("core_width", "shell_index")), "cumulative_shell_profile": profile(rows, "qcmI_cumulative", ("core_width", "shell_index")), "budget_shell_profile": profile(rows, "l1_budget_to_shell", ("core_width", "shell_index"))}
    payload = {"schema": "tect/pre-a-r392-independent/1.0", "manifest": str(MANIFEST.relative_to(ROOT)).replace("\\", "/"), "result_id": "R-392", "exploration_id": "EXP-001235", "verdict": "PASS", "checks": 6, "derived": derived}
    dump_atomic(path, payload)
    print(f"R-392 INDEPENDENT PASS 6/6 systems={len(pairs)} partitions={partition_count} qcmI={len(q_values)} rows={len(rows)} qcmI_max={max(q_values):.6g} residual={max(abs(value) for value in residuals):.3g}")
    return payload
