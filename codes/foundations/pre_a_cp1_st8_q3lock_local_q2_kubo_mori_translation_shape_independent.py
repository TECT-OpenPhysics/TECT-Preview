#!/usr/bin/env python3
"""Independent source-translation and shape stress for EXP-001211 / R-369."""
...
def run() -> dict[str, Any]:
    ...
    check("authority", manifest["exploration_id"] == "EXP-001211" and manifest["result_id"] == "R-369", [manifest["exploration_id"], manifest["result_id"]], "EXP-001211/R-369", "provenance")
    check("scope", manifest["claim_bearing"] is False and manifest["scope"]["shape_uniformity_proved"] is False, manifest["scope"], "finite-translation-shape-only", "scope")
    context_count = 0
    max_phase_error = 0.0
...
def main() -> int:
    ...
    print(f"INDEPENDENT KUBO-MORI TRANSLATION-SHAPE PASS {payload['assertion_count']}/{payload['assertion_count']} contexts={payload['derived']['context_count']} max_weighted={payload['derived']['maximum_weighted_fractional_norm']:.6f}")
    return 0
