#!/usr/bin/env python3
"""Integrated primary/independent/hostile/Lean verifier for R-408."""
...
MANIFEST = ROOT / "strategy/pre-a-cp1-st8-q3lock-effective-resistance-manifest.json"
PRIMARY = ROOT / "codes/foundations/pre_a_cp1_st8_q3lock_effective_resistance.py"
INDEPENDENT = ROOT / "codes/foundations/pre_a_cp1_st8_q3lock_effective_resistance_independent.py"
HOSTILE = ROOT / "codes/foundations/pre_a_cp1_st8_q3lock_effective_resistance_hostile.py"
LEAN = ROOT / "verification/lean/Tect/R408.lean"
PRIMARY_OUTPUT = ROOT / "claims/C6-SPACETIME-SIGNATURE/runs/2026-08-30-primary-pre_a_cp1_st8_q3lock_effective_resistance/primary.json"
INDEPENDENT_OUTPUT = ROOT / "claims/C6-SPACETIME-SIGNATURE/runs/2026-08-30-independent-pre_a_cp1_st8_q3lock_effective_resistance/independent.json"
HOSTILE_OUTPUT = ROOT / "claims/C6-SPACETIME-SIGNATURE/runs/2026-08-30-hostile-pre_a_cp1_st8_q3lock_effective_resistance/hostile.json"
INTEGRATED_OUTPUT = ROOT / "claims/C6-SPACETIME-SIGNATURE/runs/2026-08-30-integrated-pre_a_cp1_st8_q3lock_effective_resistance/integrated.json"
...
def main() -> int:
    ...
    check("manifest identity", manifest["result_id"] == "R-408" and manifest["exploration_id"] == "EXP-001253" and manifest["claim_bearing"] is False, [manifest["result_id"], manifest["exploration_id"], manifest["claim_bearing"]], "R-408/EXP-001253/false", "identity")
    ...
    check("artifacts present", all(path.is_file() for path in artifacts), [str(path) for path in artifacts if not path.is_file()], "all R-408 artifacts", "provenance")
    ...
    lean = command([str(LAKE), "env", "lean", "Tect/R408.lean"], ROOT / "verification/lean")
    ...
    fields = ("system_count", "context_count", "comparison_row_count", "minimum_resistance_bound", "maximum_resistance_bound", "minimum_intrinsic_gap", "maximum_intrinsic_gap", "minimum_resistance_average", "maximum_resistance_average", "minimum_positive_laplacian_eigenvalue", "maximum_effective_resistance", "minimum_residual", "maximum_residual", "minimum_tree_bound", "maximum_tree_bound", "cutoff_dimensions", "system_profiles")
    for field in fields:
    ...
    check("resistance bound positive", p["minimum_resistance_bound"] > 0.0, p["minimum_resistance_bound"], ">0 on finite grid", "effective resistance")
    check("resistance residual nonnegative", p["minimum_residual"] >= -float(fixture["numerical_tolerance"]), p["minimum_residual"], f">=-{fixture['numerical_tolerance']}", "effective resistance")
    check("bound is below exact graph gap", p["maximum_resistance_bound"] <= p["maximum_intrinsic_gap"] + tolerance, [p["maximum_resistance_bound"], p["maximum_intrinsic_gap"]], "bound <= exact gap envelope", "comparison")
    h = hostile["derived"]
    ...
    payload = {"schema": "tect/pre-a-r408-integrated/1.0", "manifest": str(MANIFEST.relative_to(ROOT)).replace("\\", "/"), "result_id": "R-408", "exploration_id": "EXP-001253", "verdict": "PASS", "checks": checks, "derived": {"primary": p, "independent": i, "hostile": h, "lean": "PASS", "command_outputs": outputs}, "scope": scope, "boundary": manifest["boundary"]}
    atomic_json(args.output if args.output.is_absolute() else ROOT / args.output, payload)
    print(f"INTEGRATED EFFECTIVE-RESISTANCE PASS {len(checks)}/{len(checks)} contexts={p['context_count']} rows={p['comparison_row_count']} bound=[{p['minimum_resistance_bound']:.6g},{p['maximum_resistance_bound']:.6g}] Rbar=[{p['minimum_resistance_average']:.6g},{p['maximum_resistance_average']:.6g}]")
    return 0
