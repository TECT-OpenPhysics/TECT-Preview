...
def main() -> int:
    ...
    a3_component = next(row for row in manifest["components"] if row["id"] == pert["id"])
    a4_component = next(row for row in manifest["components"] if row["id"] == constructive["id"])
    a3_evidence = load_json(component_path(a3_component, "evidence_path"))
    a4_evidence = load_json(component_path(a4_component, "evidence_path"))
    check(
    ...
    honesty_text = " ".join(manifest["honesty_boundary"]).lower()
    check(
        "bcc_route_b_infinite_volume_and_full_classii_measure_are_excluded",
        ("infinite volume" in no_overclaim_text or "infinite-volume" in no_overclaim_text)
        and "bcc" in honesty_text
        and "route b" in honesty_text
        and "infinite volume" in honesty_text
        and "full three-component derivative class-ii constructive measure" in honesty_text,
        {"component_no_overclaim": no_overclaim_text, "manifest_honesty_boundary": manifest["honesty_boundary"]},
        assertions,
    )
    check(
