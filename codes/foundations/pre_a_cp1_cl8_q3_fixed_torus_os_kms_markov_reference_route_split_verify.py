#!/usr/bin/env python3
"""Integrated verifier for the fixed-torus Q3 OS/KMS/Markov/reference theorem."""
...
def build_payload() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="tect-q3-os-kms-reference-") as directory:
    ...
    for name in ("sharp-time thermal covariance", "thermal kernel derivative jump", "free thermal RP Gram positive semidefinite", "local tilt preserves conditional factorization b0", "finite Gibbs KMS boundary identity", "Q3 constant-mode fourth chaos nonzero", "strict Jensen fixture", "relative entropy scalar gauge invariance", "scope firewall physical_empty_space_reference"):
        audit.check(f"primary coverage {name}", name in primary_names, name, "present", "coverage")
    for name in ("independent sharp-time continuity", "independent RP rank-one coefficients", "independent local Bayes factorization b0", "independent finite KMS identity", "independent Q3 fourth chaos strictness", "independent strict Jensen", "independent KL identity", "independent normalized scalar gauge", "independent scope firewall physical_empty_space_reference"):
        audit.check(f"independent coverage {name}", name in independent_names, name, "present", "coverage")
    audit.check("cross KMS equality", primary["derived"]["KMS"]["left"] == primary["derived"]["KMS"]["right"] and independent["derived"]["KMS"]["left"] == independent["derived"]["KMS"]["right"], (primary["derived"]["KMS"], independent["derived"]["KMS"]), "both exact", "cross")
    audit.check("cross two-sided Liouvillean", any(value.startswith("-") for value in primary["derived"]["KMS"]["Liouville_spectrum"]) and min(independent["derived"]["KMS"]["Liouville_spectrum"]) < 0, (primary["derived"]["KMS"]["Liouville_spectrum"], independent["derived"]["KMS"]["Liouville_spectrum"]), "negative present", "cross")
    audit.check("cross strict partition", "exp" in primary["derived"]["free_energy"]["partition"] and independent["derived"]["free_energy"]["partition"] > 1.0, (primary["derived"]["free_energy"]["partition"], independent["derived"]["free_energy"]["partition"]), ">1", "cross")
    audit.check("cross fourth-chaos sign", "-g - 3*lambda" in primary["derived"]["free_energy"]["fourth_coefficient"] and independent["derived"]["free_energy"]["fourth_coefficient"].startswith("-"), (primary["derived"]["free_energy"]["fourth_coefficient"], independent["derived"]["free_energy"]["fourth_coefficient"]), "negative", "cross")
    audit.check("distinct covariance fixtures", primary["derived"]["sharp_time"]["continuity_variances"] != independent["derived"]["continuity_variances"], "distinct", "distinct", "cross")
    audit.check("distinct Markov fixtures", primary["derived"]["Markov"] != independent["derived"]["Markov"], "distinct", "distinct", "cross")
    ...
    for phrase in ("Schwinger regularity", "Closed reflection-positive algebra", "Sharp-time fields and generation", "Two-sided germ-domain Markov theorem", "Periodic OS reconstruction", "Strict free-energy ordering", "Scalar-gauge-invariant statement", "Vacuum, Hadamard", "physical empty space", "Pre-A"):
    ...
    for key in ("full_E2_covariance", "ordinary_one_sided_circle_Markov_claimed", "beta_independent_Hamiltonian_family", "physical_beta_selected", "positive_energy_vacuum", "interacting_Hadamard_or_microlocal_spectrum", "physical_empty_space_reference", "absolute_vacuum_energy_fixed", "thermodynamic_limit", "ground_state_limit", "phase_transition_proved", "original_fixed_raw_CL8_family", "original_3D_Q3LOCK_parent", "C0_closed", "N1_through_N5_closed", "C6_advanced", "CP1_complete", "Sector_A_complete", "Pre_A_complete"):
