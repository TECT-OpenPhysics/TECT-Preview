#!/usr/bin/env python3
"""Non-importing independent audit of the R-426 finite Schur algebra."""
...
def run(output: Path = DEFAULT_OUTPUT) -> dict[str, Any]:
    ...
    finite_flags = ["finite_high_cutoff_schur_closed", "finite_harmonic_coarse_schur_closed", "finite_residual_reuse_closed", "finite_support_boundary_recorded"]
    ...
    check(
        "failure contract",
        failure["volume"] == 2
        and failure["cutoff_dimension"] == 16
        and failure["beta"] == "8"
        and failure["orientation"] == "right"
        and failure["conditional_row_index"] == 7
        and failure["core_size"] == 7
        and failure["tail_size"] == 9
        and failure["absolute_difference"] > failure["comparison_tolerance"]
        and failure["tolerance_relaxed"] is False
        and failure["clipping_applied"] is False,
        failure,
        "exact declared failing row and fixed tolerance",
        "failure",
    )
    ...
    derived = {"fixture_count": len(records), "minimum_coarse_gap": min(item["coarse_gap"] for item in records), "maximum_coarse_gap": max(item["coarse_gap"] for item in records), "minimum_residual_gap": min(item["residual_gap"] for item in records), "maximum_residual_gap": max(item["residual_gap"] for item in records), "minimum_combined_gap": min(item["combined_gap"] for item in records), "maximum_combined_gap": max(item["combined_gap"] for item in records), "maximum_energy_error": max(item["energy_error"] for item in records), "minimum_lower_margin": min(item["lower_margin"] for item in records), "maximum_residual_basis_difference": max(item["residual_basis_difference"] for item in records), "records": records, "route_outcome": "FAIL_ROUTE_LOCAL_EXPECTED", "failure_contract": failure}
    payload: dict[str, Any] = {"schema": "tect/pre-a-r426-independent/1.0", "result_id": manifest["result_id"], "exploration_id": manifest["exploration_id"], "claim_id": manifest["claim_ids"][0], "manifest": MANIFEST.relative_to(REPO).as_posix(), "run_kind": "independent", "verdict": "PASS", "assertion_count": assertions, "assertions": checks, "derived": derived, "assumptions": manifest["assumptions"], "missing_assumptions": manifest["missing_assumptions"], "evidence_level": manifest["evidence_level"], "non_claims": manifest["non_claims"], "boundary": manifest["boundary"], "independence_scope": "non-importing abstract reversible Schur fixtures; the primary high-cutoff Q3 grid is independently checked by the integrated output predicates but not reconstructed in this lane"}
    atomic_json(output, payload)
    print(f"R-426 INDEPENDENT CONTROL PASS {assertions}/{assertions} abstract_fixtures={len(records)} route_outcome=FAIL_ROUTE_LOCAL_EXPECTED coarse=[{derived['minimum_coarse_gap']:.6g},{derived['maximum_coarse_gap']:.6g}] residual=[{derived['minimum_residual_gap']:.6g},{derived['maximum_residual_gap']:.6g}] combined=[{derived['minimum_combined_gap']:.6g},{derived['maximum_combined_gap']:.6g}]")
    return payload
