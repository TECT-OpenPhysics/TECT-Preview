#!/usr/bin/env python3
"""Independent NumPy reconstruction for EXP-001086 (no primary import)."""
...
def hamiltonian(volume: int, n: int, fixture: dict[str, Any], bond_replacement: np.ndarray | None = None) -> tuple[list[np.ndarray], np.ndarray]:
    ...
    bond_single = q_single if bond_replacement is None else bond_replacement
    bond_qs = [embed(bond_single, site, volume, identity) for site in range(volume)]
    ...
    for left, right in graph_edges(volume):
        d = bond_qs[left] - bond_qs[right]
        terms.append(c * (d @ d) / 2.0 + lam * (d @ d) @ (bond_qs[left] @ bond_qs[left] + bond_qs[right] @ bond_qs[right]) / 4.0)
    raw = sum(terms, np.zeros_like(qs[0]))
    return qs, (raw + raw.conj().T) / 2.0
...
def unitary(hamiltonian_matrix: np.ndarray, time: float, hbar: float) -> np.ndarray:
    values, vectors = np.linalg.eigh((hamiltonian_matrix + hamiltonian_matrix.conj().T) / 2.0)
    return (vectors * np.exp(-1j * time * values / hbar)) @ vectors.conj().T


def state(hamiltonian_matrix: np.ndarray, beta: float) -> np.ndarray:
    values, vectors = np.linalg.eigh((hamiltonian_matrix + hamiltonian_matrix.conj().T) / 2.0)
...
def run() -> dict[str, Any]:
    ...
    for volume in map(int, fixture["volume_values"]):
        ...
        qs, h = hamiltonian(volume, n, fixture)
        rho = state(h, beta)
        observable = character(qs[0] + qs[1], amplitude, hbar)
        q0, _ = oscillator(n)
        reference = {float(time): unitary(h, float(time), hbar) @ observable @ unitary(h, float(time), hbar).conj().T for time in fixture["time_values"]}
        radius_rows: list[dict[str, Any]] = []
        for radius in map(float, fixture["radius_values"]):
            _, h_cut = hamiltonian(volume, n, fixture, cutoff(q0, radius))
            tail = h - h_cut
            ...
            for time in map(float, fixture["time_values"]):
                ...
                for sign in (-1, 1):
                    evolution = unitary(h + sign * tail, time, hbar)
                    difference = evolution @ observable @ evolution.conj().T - reference[time]
                    modular = -beta * (h @ difference - difference @ h)
                    values = {"D_norm": seminorm(difference, rho), "delta_D_norm": seminorm(modular, rho), "matrix_norm": float(np.linalg.svd(difference, compute_uv=False)[0])}
                ...
                times.append({"time": time, "orientations": orientations, "two_orientation_sum_of_norms": {key: orientations["1"][key] + orientations["-1"][key] for key in ("D_norm", "delta_D_norm")}})
                anchor = unitary(h, 0.0, hbar) @ observable @ unitary(h, 0.0, hbar).conj().T - observable
                check(f"V={volume} L={radius} t={time} t=0 anchor", seminorm(anchor, rho) <= float(fixture["tail_tolerance"]), "zero", "near zero", "two-site direct D")
        ...
        check(f"V={volume} radius sequence", [item["radius"] for item in radius_rows] == list(map(float, fixture["radius_values"])), [item["radius"] for item in radius_rows], fixture["radius_values"], "cutoff")
    check("volume sequence", [item["volume"] for item in volume_rows] == fixture["volume_values"], [item["volume"] for item in volume_rows], fixture["volume_values"], "volume")
...
def main() -> int:
    ...
    print(f"INDEPENDENT FINITE-TWO-SITE-D-DELTA-D PASS {payload['passed']}/{payload['assertion_count']}")
    return 0
