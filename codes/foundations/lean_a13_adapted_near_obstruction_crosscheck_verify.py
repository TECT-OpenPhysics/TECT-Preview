"""Integrated verifier for the R-187 adapted-NEAR obstruction fixtures."""
...
PRIMARY = REPO / "verification" / "scripts" / "lean_a13_adapted_near_obstruction_crosscheck.py"
INDEPENDENT = REPO / "codes" / "foundations" / "lean_a13_adapted_near_obstruction_crosscheck_independent.py"
SCRIPT = Path(__file__).resolve()
LEAN = REPO / "verification" / "lean" / "Tect" / "R187.lean"
...
def accepts(derived: dict, manifest: dict) -> bool:
    ...
    scalar_keys = [key for key, value in expected.items() if not isinstance(value, list) and not isinstance(value, bool)]
    return (
        all(derived.get(key) == expected[key] for key in scalar_keys)
        and derived.get("pair_slacks") == expected["pair_slacks"]
        and derived.get("doob_l6_holds") is True
        and derived.get("pair_slacks_all_negative") is True
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=REPO / "claims" / "A13-CLASSII-RELATIVE-PHASE-SOURCE-BUDGET-OBSTRUCTION" / "runs" / "2026-08-22-lean-r187-adapted-near-obstruction-crosscheck" / "integrated.json")
    ...
    check("identity", manifest["audit_id"] == "A13-ADAPTED-NEAR-OBSTRUCTION-CROSSCHECK", manifest["audit_id"], "A13-ADAPTED-NEAR-OBSTRUCTION-CROSSCHECK")
    ...
    with tempfile.TemporaryDirectory(prefix="r187-crosscheck-") as directory:
    ...
    check("acceptance predicate", accepts(primary["derived"], manifest), primary["derived"], "adapted NEAR fixtures")

    mutations = []
    for index, label in enumerate(manifest["hostile_mutations"]):
        mutated = copy.deepcopy(primary["derived"])
        if index == 0:
            mutated["conditional_mean_plus"] = "1/5"
        elif index == 1:
            mutated["root_deviation_plus"] = "1"
        elif index == 2:
            mutated["ledger_a"] = "1/2"
        elif index == 3:
            mutated["pair_slacks"] = ["0", "0", "0"]
            mutated["pair_slacks_all_negative"] = False
        elif index == 4:
            mutated["doob_terminal_l6"] = "2"
        elif index == 5:
    ...
    if not args.staged:
        ...
        check("R-187 authority", results.count("R-187") >= 2 and "adapted near obstruction" in results.lower(), results.count("R-187"), ">=2")
        check("strategy index authority", "R-187" in (REPO / "strategy/INDEX.md").read_text(encoding="utf-8"), True, True)
        ...
        check("exploration count", len([line for line in (REPO / "explorations/log.jsonl").read_text(encoding="utf-8").splitlines() if line.strip()]) == expected["explorations"], expected["explorations"], expected["explorations"])
    ...
    print(f"INTEGRATED R-187 LEAN PASS {len(rows)}/{len(rows)}")
    return 0
