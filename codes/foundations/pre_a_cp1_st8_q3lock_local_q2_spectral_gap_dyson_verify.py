#!/usr/bin/env python3
"""Integrated primary/independent/Lean verifier for EXP-001207 / R-365."""
...
LEAN = REPO / "verification/lean/Tect/R365.lean"
...
def compile_lean() -> dict[str, Any]:
    lake = lake_path()
    command = "lake env lean Tect/R365.lean"
    if lake is None:
        return {"status": "UNAVAILABLE", "command": command, "output": "pinned lake executable not found"}
    process = subprocess.run([str(lake), "env", "lean", "Tect/R365.lean"], cwd=LEAN_ROOT, text=True, encoding="utf-8", capture_output=True, check=False)
...
def run() -> dict[str, Any]:
    ...
    check("identity", manifest["exploration_id"] == "EXP-001207" and manifest["result_id"] == "R-365" and not manifest["claim_bearing"], [manifest["exploration_id"], manifest["result_id"], manifest["claim_bearing"]], "EXP-001207/R-365/false")
    ...
    markers = ("spectral_commutator_coefficient", "spectral_gap_square_fixture", "spectral_block_commutator_reduction", "scope_fixture")
    ...
    registry = json.loads(LEAN_REGISTRY.read_text(encoding="utf-8"))
    entries = [entry for entry in registry["entrypoints"] if entry["path"] == "verification/lean/Tect/R365.lean"]
    ...
    with tempfile.TemporaryDirectory(prefix="q3-spectral-gap-dyson-") as temporary:
        ...
        exact_fields = (
            ...
            "finite_spectral_gap_duhamel_bound_closed", "finite_state_trace_influence_bound_closed", "finite_prefix_fixture_checked",
        ...
        numeric_fields = (
            "max_commutator_spectral_formula_error", "max_phase_identity_error", "max_spectral_reduction_error", "max_duhamel_violation",
            "max_state_trace_violation", "max_bond_unitary_factorization_error", "max_density_hs_norm_excess", "maximum_commutator_norm",
            "maximum_finite_time_change_norm", "maximum_state_trace_change", "maximum_duhamel_bound", "maximum_state_trace_bound",
            "minimum_commutator_norm", "minimum_nonzero_commutator_norm", "maximum_finite_time_to_duhamel_ratio"
        )
...
def main() -> int:
    ...
    print(f"INTEGRATED LOCAL-Q2 SPECTRAL GAP DYSON PASS {payload['assertion_count']}/{payload['assertion_count']}; Lean={payload['lean']['status']} diff={payload['derived']['max_primary_independent_numeric_difference']:.3e}")
    return 0
