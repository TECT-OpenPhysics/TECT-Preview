"""Integrated verifier for the R-182 pulled cross-Hessian margin."""
...
PRIMARY = REPO / "verification" / "scripts" / "lean_a13_feedback_pulled_cross_hessian_margin.py"
INDEPENDENT = REPO / "codes" / "foundations" / "lean_a13_feedback_pulled_cross_hessian_margin_independent.py"
SCRIPT = Path(__file__).resolve()
LEAN = REPO / "verification" / "lean" / "Tect" / "R182.lean"
...
def accepts(derived: dict, manifest: dict) -> bool:
    return (
        derived.get("cross_coefficients") == {"field": "2", "current": "3", "ordered": "5"}
        and derived.get("active_weights") == {"c1": "1", "c2": "1", "s1": "0", "s2": "0", "w1": "1", "w2": "2"}
        and derived.get("feedback_gain") == "1/2"
        and derived.get("hessian_coefficient") == "-8"
        and derived.get("cross_hessian") == [["-8", "8"], ["8", "-8"]]
        and derived.get("pulled_hessian") == [["-2", "4"], ["4", "-8"]]
        and derived.get("cross_quadratic") == "-8*(x - y)**2"
        and derived.get("pulled_quadratic") == "-2*(x - 2*y)**2"
        and derived.get("cross_fixture") == "-8"
        and derived.get("pulled_fixture") == "-8"
        and derived.get("pulled_eigenvalues") == {"-10": 1, "0": 1}
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=REPO / "claims" / "A13-CLASSII-RELATIVE-PHASE-SOURCE-BUDGET-OBSTRUCTION" / "runs" / "2026-08-21-lean-r182-feedback-pulled-cross-hessian-margin" / "integrated.json")
    ...
    check("identity", manifest["audit_id"] == "A13-FEEDBACK-PULLED-CROSS-HESSIAN-MARGIN", manifest["audit_id"], "A13-FEEDBACK-PULLED-CROSS-HESSIAN-MARGIN")
    ...
    with tempfile.TemporaryDirectory(prefix="r182-crosscheck-") as directory:
    ...
    check("acceptance predicate", accepts(primary["derived"], manifest), primary["derived"], "pulled cross-Hessian margin")
    baseline = primary["derived"]
    mutations: list[dict] = []
    for index, label in enumerate(manifest["hostile_mutations"]):
        mutated = copy.deepcopy(baseline)
        if index == 0:
            mutated["hessian_coefficient"] = "-7"
        elif index == 1:
            mutated["feedback_gain"] = "1"
        elif index == 2:
            mutated["pulled_fixture"] = "0"
        elif index == 3:
            mutated["cross_hessian"] = [["0", "0"], ["0", "0"]]
        elif index == 4:
    ...
    if not args.staged:
        ...
        check("R-182 authority", results.count("R-182") >= 2 and "pulled cross-hessian" in results.lower(), results.count("R-182"), ">=2")
        check("strategy index authority", "R-182" in (REPO / "strategy/INDEX.md").read_text(encoding="utf-8"), True, True)
    ...
    print(f"INTEGRATED R-182 LEAN PASS {len(rows)}/{len(rows)}")
    return 0
