#!/usr/bin/env python3
"""Non-importing independent reconstruction of the R-459 source audit."""
...
def scan(root: Path) -> dict:
    ...
    return {
        ...
        "standalone_complete_rows": [
            row for row in complete if not row["merged_bundle"]
        ],
        "standalone_complete_paths": sum(
            not row["merged_bundle"] for row in complete
        ),
...
def run(output: Path = DEFAULT_OUTPUT, contents_root: Path | None = None) -> dict:
    ...
    check(
        "identity",
        [manifest["result_id"], manifest["exploration_id"], manifest["task_id"]]
        == ["R-459", "EXP-001332", "T-054"],
        [manifest["result_id"], manifest["exploration_id"], manifest["task_id"]],
        ["R-459", "EXP-001332", "T-054"],
        "provenance",
    )
    ...
    check(
        ...
        result["complete_semantic_paths"] == expected["complete_semantic_paths"],
        result["complete_semantic_paths"],
        expected["complete_semantic_paths"],
        "owner-applicability",
    )
    check(
        "standalone boundary",
        result["standalone_complete_paths"] == expected["standalone_complete_paths"],
        result["standalone_complete_paths"],
        expected["standalone_complete_paths"],
        "owner-applicability",
    )
    check(
        "exact token boundary",
        result["exact_owner_token_paths"] == expected["exact_owner_token_paths"],
        result["exact_owner_token_paths"],
        expected["exact_owner_token_paths"],
        "owner-applicability",
    )
    check(
        "merged boundary",
        result["all_complete_rows_are_merged"]
    ...
    check(
        "loose q-ledger control",
        result["loose_qledger_only_complete_rows_are_merged"]
    ...
    check(
        "scan cardinality",
        result["paths_scanned"] > 0
        and result["unique_content_hashes"] > 0
        and result["unique_content_hashes"] <= result["paths_scanned"],
        [
            result["paths_scanned"],
            result["unique_content_hashes"],
            result["duplicate_path_count"],
        ],
        "positive scan and deduplication cardinalities",
        "provenance",
    )
    check(
        "no promotion",
    ...
    payload = {
        "schema": "tect/pre-a-contents-owner-audit-independent/1.0",
        ...
        "source_hashes": {
            "manifest": raw_sha(MANIFEST),
            "script": raw_sha(Path(__file__)),
        },
    }
    ...
    print(
        "R-459 INDEPENDENT "
        f"{payload['verdict']} "
        f"standalone={result['standalone_complete_paths']} "
        f"exact={result['exact_owner_token_paths']} "
        f"complete={result['complete_semantic_paths']}",
        flush=True,
    )
    return payload
...
def main() -> int:
    ...
    if args.self_test:
        ...
        print("R-459 INDEPENDENT SELFTEST: PASS", flush=True)
    return 0
