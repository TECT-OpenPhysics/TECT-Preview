#!/usr/bin/env python3
"""Independent finite actual-Q3 split-step recurrence parameter stress for EXP-001195."""
...
def graph_edges(volume: int) -> list[tuple[int, int]]:
    ...
    if volume not in tables:
        raise ValueError("EXP-001195 uses volumes 2, 4, and 6")
    return tables[volume]
...
def observable(generator: np.ndarray, amplitude: float, hbar: float) -> np.ndarray:
...
def local_lengths(qs: list[np.ndarray], ps: list[np.ndarray], word: np.ndarray, rho: np.ndarray) -> list[float]:
...
def run() -> dict[str, Any]:
    ...
    check("independent construction", True, "separate implementation", "no primary import", "provenance")
    ...
    for volume in volumes:
        ...
        terms = onsite + bonds
        orders = {manifest["split_orders"][0]: list(range(len(terms))), manifest["split_orders"][1]: list(reversed(range(len(terms))))}
        step_operators = {(order_name, sign): product(terms, order, sign, delta, hbar) for order_name, order in orders.items() for sign in (-1, 1)}
        ...
        for beta_fraction in betas:
            ...
            for support in supports:
                ...
                base = observable(generator, amplitude, hbar)
                initial = local_lengths(qs, ps, base, rho)
                ...
                for order_name in orders:
                    for sign in (-1, 1):
                        step_operator = step_operators[(order_name, sign)]
                        for adjoint in (0, 1):
                            ...
                            for step in range(steps + 1):
                                ...
                                check(f"V={volume} beta={beta_fraction} support={support} {order_name} sign={sign} adj={adjoint} step={step} finite", all(np.isfinite(value) and value >= -tolerance for value in lengths), lengths, "finite/nonnegative", "commutator rows")
                                history.append(lengths)
                                lengths_count += volume
                                if step < steps:
                            ...
                            for step in range(steps):
                                for site in range(volume):
                                    ...
                                    if residual > recurrence_tolerance:
                                        violations.append({"volume": volume, "beta": str(beta_fraction), "support": list(support), "order": order_name, "sign": sign, "adjoint": adjoint, "step": step, "site": site, "lhs": history[step + 1][site], "rhs": rhs, "residual": residual, "neighbor_sum": neighbor_sum})
                            summaries.append({"volume": volume, "beta": str(beta_fraction), "support": list(support), "order": order_name, "context": f"sign{sign}_adjoint{adjoint}", "max_residual": max(residuals, default=0.0), "min_residual": min(residuals, default=0.0), "violation_count": len(violations), "violations": violations[:4]})
        check(f"V={volume} beta coverage", len([row for row in summaries if row["volume"] == volume]) == len(betas) * len(supports) * len(orders) * 4, len([row for row in summaries if row["volume"] == volume]), len(betas) * len(supports) * len(orders) * 4, "coverage")
    expected_contexts = len(volumes) * len(betas) * len(supports) * len(orders) * 4
    check("context coverage", len(summaries) == expected_contexts, len(summaries), expected_contexts, "coverage")
    status = "PASS_ALL_PARAMETER_ROWS" if all(row["violation_count"] == 0 for row in summaries) else "FAIL_PARAMETER_ROUTE_LOCAL"
    ...
    return {
        ...
        "derived": {
            ...
            "length_row_count": lengths_count,
...
def main() -> int:
    ...
    print(f"INDEPENDENT ACTUAL-SPLIT-STEP PARAMETER STRESS PASS {payload['passed']}/{payload['assertion_count']} status={payload['derived']['recurrence_status']}")
    return 0
