#!/usr/bin/env python3
"""Integrated primary/independent/Lean verifier for EXP-001172."""
...
def compare_summaries(primary: list[dict[str, Any]], independent: list[dict[str, Any]], tolerance: float) -> bool:
    fields = ("max_kms_residual", "min_kms_residual", "max_stationarity_defect", "min_stationarity_defect")
    left = sorted(primary, key=lambda row: (row.get("graph"), row.get("source_site")))
    right = sorted(independent, key=lambda row: (row.get("graph"), row.get("source_site")))
    if len(left) != len(right):
        return False
    for one, two in zip(left, right):
        if (one.get("graph"), one.get("source_site"), one.get("degree")) != (two.get("graph"), two.get("source_site"), two.get("degree")):
            return False
        if any(abs(float(one[field]) - float(two[field])) > tolerance * (1.0 + abs(float(one[field]))) for field in fields):
            return False
    return True


def compare_shapes(primary: list[dict[str, Any]], independent: list[dict[str, Any]], tolerance: float) -> bool:
    fields = ("max_kms_residual", "min_kms_residual", "max_stationarity_defect", "min_stationarity_defect", "degree_mean")
    left = sorted(primary, key=lambda row: row.get("graph"))
    right = sorted(independent, key=lambda row: row.get("graph"))
    if len(left) != len(right):
        return False
    for one, two in zip(left, right):
        if (one.get("graph"), one.get("volume"), one.get("edge_count"), one.get("degree_min"), one.get("degree_max")) != (two.get("graph"), two.get("volume"), two.get("edge_count"), two.get("degree_min"), two.get("degree_max")):
            return False
        if any(abs(float(one[field]) - float(two[field])) > tolerance * (1.0 + abs(float(one[field]))) for field in fields):
            return False
    return True
...
def main() -> int:
    ...
    check("identity", manifest["exploration_id"] == "EXP-001172" and manifest["task_id"] == "T-054", [manifest["exploration_id"], manifest["task_id"]], "EXP-001172/T-054")
    ...
    check("scope firewall", scope["finite_exact_gibbs_control_closed"] and scope["finite_periodic_source_stationarity_rows_closed"] and scope["finite_periodic_source_kms_residual_rows_closed"] and scope["source_uniformity_diagnostic_closed"] and scope["shape_degree_diagnostic_closed"] and not scope["source_uniform_direct_d_cauchy_closed"] and not scope["common_alpha_closed"], scope, "finite periodic diagnostic only")
    ...
    with tempfile.TemporaryDirectory(prefix="finite-periodic-split-gibbs-kms-source-stress-") as temporary:
        ...
        check("source summary crosscheck", compare_summaries(primary.get("source_summaries", []), independent.get("source_summaries", []), tolerance), "primary/independent source summaries", "within tolerance")
        ...
        check("shape summary count", len(primary.get("shape_summaries", [])) == len(independent.get("shape_summaries", [])) == primary.get("derived", {}).get("shape_count"), [len(primary.get("shape_summaries", [])), len(independent.get("shape_summaries", [])), primary.get("derived", {}).get("shape_count")], "equal")
        ...
        for flag in ("exact_gibbs_kms_control_closed", "finite_periodic_source_stationarity_rows_closed", "finite_periodic_source_kms_residual_rows_closed", "source_uniformity_diagnostic_closed", "shape_degree_diagnostic_closed", "mesh_decrease_diagnostic_closed", "source_uniform_direct_d_cauchy_closed", "volume_uniform_direct_d_cauchy_closed", "beta_uniform_direct_d_cauchy_closed", "modular_domain_transfer_closed", "product_core_density_closed", "exhaustion_independence_closed", "group_law_closed", "common_alpha_closed", "hamiltonian_os_identification_closed", "kms_gns_gap_closed", "continuum_closed", "c6_closed", "sector_a_closed", "pre_a_closed", "no_new_negative_result", "no_tier_change", "no_pdf"):
    ...
    print(f"INTEGRATED FINITE-PERIODIC-SPLIT-GIBBS-KMS-SOURCE-STRESS PASS {len(assertions)}/{len(assertions)}; Lean={lean['status']}")
    return 0
