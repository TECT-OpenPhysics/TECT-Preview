#!/usr/bin/env python3
"""Hostile mutations for the R-415 Schur-certified semigroup bridge."""
...
def profile_for(spectrum: list[float], split: int, alpha: float) -> dict[str, Any]:
    if not 1 <= split < len(spectrum) or not 0.0 < alpha < 1.0:
        raise ValueError("finite Schur profile requires an interior split and sublinear alpha")
...
def quantities(spectrum: list[float], profile: dict[str, Any], tau: float, certified_gap: float) -> dict[str, float]:
    ...
    if not 1 <= split < len(spectrum) or not 0.0 < alpha < 1.0 or tau <= 0.0 or certified_gap <= 0.0:
        raise ValueError("invalid finite Schur semigroup input")
    ...
    short_constant = head_power + uv_factor
    short_budget = short_constant * tau ** (1.0 - alpha) / (1.0 - alpha)
    late_budget = heat_tau / certified_gap
    return {"heat_tau": heat_tau, "short_actual": short_actual, "late_actual": late_actual, "trace": trace, "gap": min(spectrum), "short_budget": short_budget, "late_budget": late_budget, "green_bound": short_budget + late_budget, "uv_factor": uv_factor, "head_power": head_power}
...
def main() -> int:
    ...
    baseline = quantities(baseline_spectrum, baseline_profile, 1.0, 0.125)
    check("baseline Schur-certified criterion", baseline["green_bound"] >= baseline["trace"], [baseline["green_bound"], baseline["trace"]], "green bound covers trace")

    too_high_gap_bound = baseline["heat_tau"] / max(baseline_spectrum)
    too_high_gap_deficit = baseline["late_actual"] - too_high_gap_bound
    check("unverified larger gap rejected", too_high_gap_deficit > threshold, too_high_gap_deficit, f">{threshold}")

    head_heavy = [0.25, 0.5, 0.75, 1.0, 100.0]
    head_profile = profile_for(head_heavy, 4, 0.5)
    head_data = quantities(head_heavy, head_profile, 0.1, 0.125)
    ...
    tail_data = quantities(tail_heavy, tail_profile, 0.1, 0.125)
    ...
    check("omitted late Schur budget rejected", omitted_late_deficit > threshold, omitted_late_deficit, f">{threshold}")
    ...
    check("reversed heat-time ordering rejected", reverse_increase > threshold, reverse_increase, f">{threshold}")
    ...
    payload = {"schema": "tect/pre-a-r415-hostile/1.0", "manifest": str(MANIFEST.relative_to(REPO)).replace("\\", "/"), "result_id": "R-415", "exploration_id": "EXP-001260", "verdict": "PASS", "checks": checks, "assertion_count": len(checks), "derived": {"baseline": baseline, "unverified_larger_gap_late_deficit": too_high_gap_deficit, "omitted_head_short_deficit": omitted_head_deficit, "omitted_uv_short_deficit": omitted_uv_deficit, "omitted_late_green_deficit": omitted_late_deficit, "wrong_shift_late_deficit": wrong_shift_deficit, "reverse_heat_max_increase": reverse_increase, "wrong_sign_residual": wrong_sign_residual, "alpha_one_rejected": alpha_one_rejected, "toy_threshold": threshold}, "boundary": manifest["boundary"]}
    atomic_json(args.output if args.output.is_absolute() else REPO / args.output, payload)
    print(f"R-415 HOSTILE PASS {len(checks)}/{len(checks)} larger_gap={too_high_gap_deficit:.6g} omitted_uv={omitted_uv_deficit:.6g} omitted_head={omitted_head_deficit:.6g} reverse_increase={reverse_increase:.6g}")
    return 0
