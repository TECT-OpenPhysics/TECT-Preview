"""Integrated verifier for the R-178 complete cross-owner phase ledger."""
...
PRIMARY = REPO / "verification" / "scripts" / "lean_a13_two_root_complete_cross_owner_differentiation.py"
INDEPENDENT = REPO / "codes" / "foundations" / "lean_a13_two_root_complete_cross_owner_differentiation_independent.py"
SCRIPT = Path(__file__).resolve()
LEAN = REPO / "verification" / "lean" / "Tect" / "R178.lean"
...
def accepts(derived: dict, manifest: dict) -> bool:
    ...
    return (
        derived.get("root_labels") == inputs["root_labels"]
        and derived.get("cross_owner_blocks") == inputs["cross_owner_blocks"]
        and derived.get("field_current_ordered_blocks_retained") is True
        and str(derived.get("ordered_fixture")) == "-1"
        and str(derived.get("d1")) == "8"
        and str(derived.get("d2")) == "-8"
        and derived.get("phase_derivative_sum_zero") is True
        and derived.get("ordered_derivative_active") is True
        and derived.get("actual_a1_roots_from_r176") is True
        and derived.get("incidence_from_r177") is True
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=REPO / "claims" / "A13-CLASSII-RELATIVE-PHASE-SOURCE-BUDGET-OBSTRUCTION" / "runs" / "2026-08-21-lean-r178-two-root-complete-cross-owner-differentiation" / "integrated.json")
    ...
    check("identity", manifest["audit_id"] == "A13-TWO-ROOT-COMPLETE-CROSS-OWNER-DIFFERENTIATION", manifest["audit_id"], "A13-TWO-ROOT-COMPLETE-CROSS-OWNER-DIFFERENTIATION")
    ...
    with tempfile.TemporaryDirectory(prefix="r178-crosscheck-") as directory:
    ...
    common = ("root_labels", "cross_owner_blocks", "field_current_ordered_blocks_retained", "ordered_fixture", "d1", "d2", "phase_derivative_sum_zero", "ordered_derivative_active", "actual_a1_roots_from_r176", "incidence_from_r177", "a13_gate_closed", "sector_a_closed", "authority_hashes_ok", "lean_escape_tokens_absent", "boundary_present")
    ...
    check("acceptance predicate", accepts(primary["derived"], manifest), primary["derived"], "complete cross-owner derivative")
    ...
    for index, label in enumerate(manifest["hostile_mutations"]):
        ...
        if index == 0:
            mutated["field_current_ordered_blocks_retained"] = False
        elif index == 1:
            mutated["actual_a1_roots_from_r176"] = False
        elif index == 2:
            mutated["phase_derivative_sum_zero"] = False
        elif index == 3:
            mutated["cross_owner_blocks"] = ["field-field", "ordered-field-current"]
        elif index == 4:
            mutated["ordered_derivative_active"] = False
        elif index == 5:
    ...
    if not args.staged:
        ...
        check("R-178 authority", results.count("R-178") >= 2 and "cross-owner" in results.lower(), results.count("R-178"), ">=2")
        check("strategy index authority", "R-178" in (REPO / "strategy/INDEX.md").read_text(encoding="utf-8"), True, True)
    ...
    print(f"INTEGRATED R-178 LEAN PASS {len(rows)}/{len(rows)}")
    return 0
