#!/usr/bin/env python3
"""Integrated verifier for the A13 R-154 disjoint-multipair gap package."""
...
MANIFEST = CLAIM_DIR / "classii_disjoint_multipair_factor_three_resonance_gap_boundary_manifest.json"
...
NOTE = CLAIM_DIR / "notes/classii-disjoint-multipair-factor-three-resonance-gap-boundary-260803-v1.0.tex.txt"
...
EXPECTED_CHILD_COUNTS = {"primary": 34, "independent": 26}
EXPLORATION_IDS = ("EXP-000671", "EXP-000672", "EXP-000673")
...
def pdf_security(reader: PdfReader) -> list[str]:
    ...
    unsafe_keys = {
        "/JS", "/JavaScript", "/AA", "/Launch", "/AF", "/EF", "/EmbeddedFiles",
        "/RichMedia", "/Movie", "/Sound", "/XFA", "/SubmitForm", "/ImportData",
        "/URI", "/GoToR",
    }
...
def main() -> int:
    ...
    parity = {
        ...
        "resonance ratios": (p["surviving_distinct_ratios"], i["resonance_ratios"]),
        "lattice floor": (p["lattice_floor"], i["lattice_floor"]),
        "parent floor": (p["factor_three_parent_floor"], i["factor_three_parent_floor"]),
        "source": (p["source_hessian"], i["source_hessian"]),
        "gap": (p["certified_gap"], i["certified_gap"]),
        "parent loss": (p["parent_row_loss_upper"], i["parent_row_loss_upper"]),
        "parent margin": (p["parent_row_margin"], i["parent_row_margin"]),
    }
    ...
    audit.check("oracle", "certified gap exact", F(p["certified_gap"]) == F(1, 40), p["certified_gap"], "1/40")
    audit.check("oracle", "parent row arithmetic", F(p["parent_row_loss_upper"]) == F(201, 1000) and F(p["parent_row_margin"]) == F(699, 1000), [p["parent_row_loss_upper"], p["parent_row_margin"]], ["201/1000", "699/1000"])
    ...
    for token in (
        "factor-three path graph", "complete mixed block", "1\\over40",
        "global block-row", "Bernstein", "Devil's-advocate", "T-050, A13",
        "phase/PDE verdicts", "Result footer",
    ):
    ...
    for token in ("R-154", "factor-three", "1/40", "Bernstein", "T-050", "phase/PDE verdicts"):
    ...
    if renderer is not None:
        with tempfile.TemporaryDirectory(prefix="tect-r154-render-") as temporary:
    ...
    audit.check("records", "R-154 ledger entry", '<a id="r-154"></a>' in results_text, LEDGER_ID, "registered")
    audit.check("records", "claim narrative", RESULT_ID in claim_text and "1/40" in claim_text, RESULT_ID, "registered")
    audit.check("records", "lineage narrative", "R-154" in lineage_text and "factor-three" in lineage_text.lower(), LEDGER_ID, "registered")
    audit.check("records", "status synchronization", status.get("no_overclaim") == manifest.get("no_overclaim") and "R-154" in status.get("statement", ""), status.get("no_overclaim"), manifest.get("no_overclaim"))
    audit.check("records", "TODO route", "R-154" in todo_text and "T-050" in todo_text, "R-154/T-050", "registered")
    audit.check("records", "changelog entry", "R-154" in changelog_text and RESULT_ID in changelog_text, LEDGER_ID, "registered")
    audit.check("records", "theorem map", "R-154" in json.dumps(theorem_map, sort_keys=True), LEDGER_ID, "registered")
    audit.check("records", "proof map", "R-154" in proof_map and all(identifier in proof_map for identifier in EXPLORATION_IDS), EXPLORATION_IDS, "registered")
