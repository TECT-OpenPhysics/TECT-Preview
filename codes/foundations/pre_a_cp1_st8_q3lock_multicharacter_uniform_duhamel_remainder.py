#!/usr/bin/env python3
"""Primary exact audit for EXP-001072."""
...
def run() -> dict[str, Any]:
    ...
    check("finite support scope", manifest["model"]["scope"].startswith("Registered finite periodic Q3") and "fixed finite support" in manifest["model"]["scope"], manifest["model"]["scope"], "finite support", "scope")
    ...
    check("remainder upper reduced", remainder_upper == rational(fixture["derived_remainder_squared_upper"]), remainder_upper, fixture["derived_remainder_squared_upper"], "scaling")
    check("remainder upper unreduced", remainder_upper == rational(fixture["derived_remainder_squared_upper_unreduced"]), remainder_upper, fixture["derived_remainder_squared_upper_unreduced"], "scaling")
    ...
    passed = len(rows)
    return {"schema": "tect/foundation-audit/1.0", "run_kind": "primary", "audit_id": "PA-CP1-ST8-Q3LOCK-MULTICHARACTER-UNIFORM-DUHAMEL-REMAINDER", "exploration_id": manifest["exploration_id"], "task_id": manifest["task_id"], "verdict": "PASS", "passed": passed, "assertion_count": passed, "assertions": rows, "derived": derived, "boundary": scope}
...
def main() -> int:
    ...
    print(f"PRIMARY MULTICHARACTER-DUHAMEL PASS {payload['passed']}/{payload['assertion_count']}")
    return 0
