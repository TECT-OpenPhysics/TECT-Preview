#!/usr/bin/env python3
"""Compact exhaustive cross-check for the arbitrary-box R-443 theorem."""
...
MANIFEST = REPO / "strategy/pre-a-cp1-st8-q3lock-rectangular-matching-arbitrary-box-manifest.json"
DEFAULT_OUTPUT = REPO / "claims/C6-SPACETIME-SIGNATURE/runs/2026-08-30-primary-rectangular_matching_arbitrary_box/primary.json"
...
def run(output: Path = DEFAULT_OUTPUT) -> dict[str, Any]:
    ...
    check("identity", [manifest["result_id"], manifest["exploration_id"], manifest["task_id"], manifest["claim_bearing"], manifest["status"]] == ["R-443", "EXP-001288", "T-054", False, "ARBITRARY_BOX_MATCHING_THEOREM_AUDITED"], [manifest["result_id"], manifest["exploration_id"], manifest["task_id"], manifest["claim_bearing"], manifest["status"]], "R-443/EXP-001288/T-054/false/audited", "provenance")
    ...
    check("colour rule", fixture["lower_endpoint_colour"] == "(axis, lower_coordinate[axis] mod parity_modulus)", fixture["lower_endpoint_colour"], "lower endpoint parity", "contract")
    ...
    check("complete box family", len(boxes) == (side_max - side_min + 1) ** dimension, len(boxes), (side_max - side_min + 1) ** dimension, "coverage")
    check("six colour keys", len(colours) == dimension * modulus, len(colours), dimension * modulus, "graph")
    ...
    for box_index, sides in enumerate(boxes):
        ...
        for lower in vertices:
            for axis, side in enumerate(sides):
                if lower[axis] + 1 < side:
                    upper = list(lower)
                    upper[axis] += 1
                    edges.append((index[lower], index[tuple(upper)], axis, lower[axis] % modulus))
        expected_edges = sum((sides[axis] - 1) * math.prod(sides[:axis] + sides[axis + 1 :]) for axis in range(dimension))
        check(f"box {box_index} sides", all(side >= 2 for side in sides), sides, "all sides >=2", "graph")
        check(f"box {box_index} vertices", len(vertices) == math.prod(sides), len(vertices), math.prod(sides), "graph")
        ...
        check(f"box {box_index} degree", degree <= 2 * dimension, degree, 2 * dimension, "graph")
        for vertex in vertices:
            for axis, side in enumerate(sides):
                candidates = []
                if vertex[axis] + 1 < side:
                    candidates.append(vertex[axis] % modulus)
                if vertex[axis] > 0:
                    candidates.append((vertex[axis] - 1) % modulus)
        ...
        summaries.append({"sides": list(sides), "vertices": len(vertices), "edges": len(edges), "empty_layer_count": sum(int(not layer) for layer in layers.values()), "incidence_max": incidence_max})
    ...
    payload: dict[str, Any] = {
        "schema": "tect/pre-a-r443-primary/1.0", "manifest": MANIFEST.relative_to(REPO).as_posix(), "result_id": "R-443", "exploration_id": "EXP-001288", "claim_id": manifest["claim_ids"][0], "run_kind": "primary", "verdict": "ARBITRARY_BOX_MATCHING_THEOREM_AUDITED", "passed": check_count, "assertion_count": check_count, "assertions": check_samples, "assertion_samples_truncated": check_count > len(check_samples),
        ...
        "source_hashes": {"script": sha256(Path(__file__)), "manifest": sha256(MANIFEST)}, "assumptions": manifest["assumptions"], "missing_assumptions": manifest["missing_assumptions"], "evidence_level": manifest["evidence_level"], "non_claims": manifest["non_claims"], "boundary": manifest["boundary"]
    }
    destination = output if output.is_absolute() else REPO / output
    atomic_json(destination, payload)
    print(f"R-443 PRIMARY {payload['verdict']} {check_count}/{check_count} boxes={len(boxes)} vertices={total_vertices} edges={total_edges}", flush=True)
    return payload
...
def main() -> int:
    ...
    if args.self_test:
        assert payload["verdict"] == "ARBITRARY_BOX_MATCHING_THEOREM_AUDITED" and payload["derived"]["maximum_incidence"] <= 1
        print("R-443 PRIMARY SELFTEST: PASS", flush=True)
    return 0
