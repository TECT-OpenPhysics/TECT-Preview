#!/usr/bin/env python3
"""Independent field-order control for the R-441 blocked owner audit."""
...
DEFAULT_OUTPUT = ROOT / "claims/C6-SPACETIME-SIGNATURE/runs/2026-08-30-independent-pre-a-t055-reading-h-physical-empty-bounded-test-owner-execution/independent.json"
...
def run(output: Path = DEFAULT_OUTPUT) -> dict[str, Any]:
    ...
    check("identity", (manifest["result_id"], manifest["exploration_id"], manifest["status"], manifest["claim_bearing"]) == ("R-441", "EXP-001286", "BLOCKED", False), [manifest["result_id"], manifest["exploration_id"], manifest["status"], manifest["claim_bearing"]], ["R-441", "EXP-001286", "BLOCKED", False], "provenance")
    ...
    print(f"R-441 INDEPENDENT {payload['verdict']} {len(checks)}/{len(checks)}", flush=True)
    return payload
...
def main() -> int:
    ...
    if args.self_test:
        ...
        print("R-441 INDEPENDENT SELFTEST: PASS", flush=True)
    return 0
