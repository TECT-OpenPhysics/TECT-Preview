#!/usr/bin/env python3
"""Non-importing independent increasing-cutoff endpoint-energy stress audit for EXP-001224 / R-382."""
...
def run() -> dict[str, Any]:
    ...
    check("R-381 source", source.get("verdict") == "PASS" and source.get("derived", {}).get("independent", {}).get("context_count") == int(fixture["expected_parent_contexts"]), [source.get("verdict"), source.get("derived", {}).get("independent", {}).get("context_count")], "R-381 PASS and expected parent contexts", "source linkage")
...
def main() -> int:
    ...
    print(f"INDEPENDENT INCREASING-CUTOFF-ENDPOINT-STRESS PASS {payload['assertion_count']}/{payload['assertion_count']} contexts={payload['derived']['context_count']} ratio={payload['derived']['maximum_endpoint_to_cauchy_ratio']:.9f}")
    return 0
