"""Integrated verifier for the R-177 heat/incidence ledger."""
...
PRIMARY = REPO / "verification" / "scripts" / "lean_a13_two_root_heat_incidence_ledger.py"
INDEPENDENT = REPO / "codes" / "foundations" / "lean_a13_two_root_heat_incidence_ledger_independent.py"
SCRIPT = Path(__file__).resolve()
LEAN = REPO / "verification" / "lean" / "Tect" / "R177.lean"
...
def accepts(derived: dict, manifest: dict) -> bool:
    return (
        derived.get("root_labels") == manifest["registered_inputs"]["root_labels"]
        and derived.get("actual_a1_roots_from_r176") is True
        and derived.get("common_heat_shared") is True
        and derived.get("common_heat_cancels_from_replica_difference") is True
        and derived.get("independent_heat_would_survive") is True
        and derived.get("root2_feedback_from_root1_retained") is True
        and derived.get("two_replica_variance_identity") is True
        and derived.get("mean_only_variance_rejected") is True
        and derived.get("owner_order") == manifest["registered_inputs"]["owner_order"]
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=REPO / "claims" / "A13-CLASSII-RELATIVE-PHASE-SOURCE-BUDGET-OBSTRUCTION" / "runs" / "2026-08-21-lean-r177-two-root-heat-incidence-ledger" / "integrated.json")
    ...
    check("identity", manifest["audit_id"] == "A13-TWO-ROOT-HEAT-INCIDENCE-LEDGER", manifest["audit_id"], "A13-TWO-ROOT-HEAT-INCIDENCE-LEDGER")
    ...
    with tempfile.TemporaryDirectory(prefix="r177-crosscheck-") as directory:
    ...
    common = ("root_labels", "actual_a1_roots_from_r176", "common_heat_shared", "common_heat_cancels_from_replica_difference", "independent_heat_would_survive", "root2_feedback_from_root1_retained", "two_replica_variance_identity", "mean_only_variance_rejected", "owner_order", "a13_gate_closed", "sector_a_closed", "authority_hashes_ok", "lean_escape_tokens_absent", "boundary_present")
    ...
    check("acceptance predicate", accepts(primary["derived"], manifest), primary["derived"], "actual A1 two-root heat/incidence ledger")
    ...
    for index, label in enumerate(manifest["hostile_mutations"]):
        ...
        if index == 0:
            ...
            mutated["common_heat_shared"] = False
        elif index == 2:
            mutated["root2_feedback_from_root1_retained"] = False
        elif index == 3:
            mutated["owner_order"] = ["common_heat", "root_1", "future_residual", "root_2"]
        elif index == 4:
            mutated["mean_only_variance_rejected"] = False
        elif index == 5:
    ...
    if not args.staged:
        ...
        check("R-177 authority", results.count("R-177") >= 2 and "common-heat and root-incidence" in results, results.count("R-177"), ">=2")
        check("strategy index authority", "R-177" in (REPO / "strategy/INDEX.md").read_text(encoding="utf-8"), True, True)
    ...
    print(f"INTEGRATED R-177 LEAN PASS {len(rows)}/{len(rows)}")
    return 0
