#!/usr/bin/env python3
"""Integrated primary/independent/hostile/Lean verifier for R-404."""
...
MANIFEST = ROOT / "strategy/pre-a-cp1-st8-q3lock-hamiltonian-intrinsic-kinetic-poincare-manifest.json"
PRIMARY = ROOT / "codes/foundations/pre_a_cp1_st8_q3lock_hamiltonian_intrinsic_kinetic_poincare.py"
INDEPENDENT = ROOT / "codes/foundations/pre_a_cp1_st8_q3lock_hamiltonian_intrinsic_kinetic_poincare_independent.py"
HOSTILE = ROOT / "codes/foundations/pre_a_cp1_st8_q3lock_hamiltonian_intrinsic_kinetic_poincare_hostile.py"
LEAN = ROOT / "verification/lean/Tect/R404.lean"
PRIMARY_OUTPUT = ROOT / "claims/C6-SPACETIME-SIGNATURE/runs/2026-08-30-primary-pre_a_cp1_st8_q3lock_hamiltonian_intrinsic_kinetic_poincare/primary.json"
INDEPENDENT_OUTPUT = ROOT / "claims/C6-SPACETIME-SIGNATURE/runs/2026-08-30-independent-pre_a_cp1_st8_q3lock_hamiltonian_intrinsic_kinetic_poincare/independent.json"
HOSTILE_OUTPUT = ROOT / "claims/C6-SPACETIME-SIGNATURE/runs/2026-08-30-hostile-pre_a_cp1_st8_q3lock_hamiltonian_intrinsic_kinetic_poincare/hostile.json"
INTEGRATED_OUTPUT = ROOT / "claims/C6-SPACETIME-SIGNATURE/runs/2026-08-30-integrated-pre_a_cp1_st8_q3lock_hamiltonian_intrinsic_kinetic_poincare/integrated.json"
...
def main() -> int:
    ...
    check("manifest identity", manifest["result_id"] == "R-404" and manifest["exploration_id"] == "EXP-001249" and manifest["claim_bearing"] is False, [manifest["result_id"], manifest["exploration_id"], manifest["claim_bearing"]], "R-404/EXP-001249/false", "identity")
    ...
    check("artifacts present", all(path.is_file() for path in artifacts), [str(path) for path in artifacts if not path.is_file()], "all R-404 artifacts", "provenance")
    ...
    lean = command([str(LAKE), "env", "lean", "Tect/R404.lean"], ROOT / "verification/lean")
    ...
    fields = ("system_count", "context_count", "comparison_row_count", "nonzero_variance_row_count", "zero_variance_row_count", "minimum_intrinsic_gap", "maximum_intrinsic_gap", "minimum_kinetic_form", "maximum_kinetic_form", "minimum_variance", "maximum_variance", "minimum_kinetic_to_variance_ratio", "maximum_kinetic_to_variance_ratio", "minimum_graph_trace_residual", "maximum_graph_trace_residual", "minimum_poincare_residual", "maximum_poincare_residual", "cutoff_dimensions", "system_profiles")
    for field in fields:
    ...
    check("intrinsic gap remains positive", p["minimum_intrinsic_gap"] > 0.0, p["minimum_intrinsic_gap"], ">0 on finite grid", "comparison")
    check("intrinsic Poincare residuals nonnegative", p["minimum_poincare_residual"] >= -float(fixture["numerical_tolerance"]), p["minimum_poincare_residual"], f">=-{fixture['numerical_tolerance']}", "comparison")
    h = hostile["derived"]
    check("hostile q mutation caught", h["late"]["genuine_gap"] > 0.0 and h["late"]["mutated_max_edge_weight"] <= float(fixture["hostile_zero_threshold"]) and h["late"]["mutated_edge_count"] == 0, h["late"], "genuine gap positive and q mutation zero", "hostile")
    payload = {"schema": "tect/pre-a-r404-integrated/1.0", "manifest": str(MANIFEST.relative_to(ROOT)).replace("\\", "/"), "result_id": "R-404", "exploration_id": "EXP-001249", "verdict": "PASS", "checks": checks, "derived": {"primary": p, "independent": i, "hostile": h, "lean": "PASS", "command_outputs": outputs}, "scope": scope, "boundary": manifest["boundary"]}
    atomic_json(args.output if args.output.is_absolute() else ROOT / args.output, payload)
    print(f"INTEGRATED INTRINSIC-KINETIC PASS {len(checks)}/{len(checks)} contexts={p['context_count']} rows={p['comparison_row_count']} gap=[{p['minimum_intrinsic_gap']:.6g},{p['maximum_intrinsic_gap']:.6g}] ratio=[{p['minimum_kinetic_to_variance_ratio']:.6g},{p['maximum_kinetic_to_variance_ratio']:.6g}]")
    return 0
