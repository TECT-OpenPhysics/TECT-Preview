#!/usr/bin/env python3
"""Integrated primary/independent/Lean verifier for EXP-001211 / R-369."""
...
SLUG = "pre_a_cp1_st8_q3lock_local_q2_kubo_mori_translation_shape"
MANIFEST = REPO / "strategy/pre-a-cp1-st8-q3lock-local-q2-kubo-mori-translation-shape-manifest.json"
PRIMARY = REPO / f"codes/foundations/{SLUG}.py"
INDEPENDENT = REPO / f"codes/foundations/{SLUG}_independent.py"
LEAN = REPO / "verification/lean/Tect/R369.lean"
...
def compile_lean() -> dict[str, Any]:
    lake = lake_path()
    command = "lake env lean Tect/R369.lean"
    if lake is None:
        return {"status": "UNAVAILABLE", "command": command, "output": "pinned lake executable not found"}
    process = subprocess.run([str(lake), "env", "lean", "Tect/R369.lean"], cwd=LEAN_ROOT, text=True, encoding="utf-8", capture_output=True, check=False)
...
def run() -> dict[str, Any]:
    ...
    check("identity", manifest["exploration_id"] == "EXP-001211" and manifest["result_id"] == "R-369" and not manifest["claim_bearing"], [manifest["exploration_id"], manifest["result_id"], manifest["claim_bearing"]], "EXP-001211/R-369/false")
    ...
    markers = ("kubo_mori_diagonal", "arithmetic_mean_symmetry", "half_envelope", "weighted_half_envelope", "bond_translation_fixture", "square_shape_fixture", "prefix_fixture", "scope_fixture")
    ...
    registry = json.loads(LEAN_REGISTRY.read_text(encoding="utf-8"))
    entries = [entry for entry in registry["entrypoints"] if entry["path"] == "verification/lean/Tect/R369.lean"]
    ...
    with tempfile.TemporaryDirectory(prefix="q3-fractional-stress-") as temporary:
        ...
        exact_fields = (
            ...
            "edge_and_square_shapes_checked", "all_prefixes_finite_checked", "weighted_fractional_bound_closed",
        ...
        numeric_fields = (
            ...
            "maximum_arithmetic_to_kubo_fractional_ratio", "maximum_bond_unitary_factorization_error"
        )
    ...
    return {
        ...
        "run_kind": "integrated",
        "audit_id": "PA-CP1-ST8-Q3LOCK-LOCAL-Q2-KUBO-MORI-TRANSLATION-SHAPE",
...
def main() -> int:
    ...
    print(f"INTEGRATED KUBO-MORI TRANSLATION-SHAPE PASS {payload['assertion_count']}/{payload['assertion_count']}; Lean={payload['lean']['status']} diff={payload['derived']['max_primary_independent_numeric_difference']:.3e}")
    return 0
