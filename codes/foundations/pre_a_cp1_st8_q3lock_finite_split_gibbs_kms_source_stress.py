#!/usr/bin/env python3
"""Primary source-wise finite Q3 Gibbs/KMS residual audit for EXP-001171."""
...
def run() -> dict[str, Any]:
    ...
    check("source fixture", fixture["graphs"]["path4"]["source_sites"] == [0, 1, 2, 3] and fixture["graphs"]["path6"]["source_sites"] == [0, 1, 2, 3, 4, 5], fixture["graphs"], "all source sites", "fixture")
    check("scope firewall", scope["finite_exact_gibbs_control_closed"] and scope["finite_source_stationarity_rows_closed"] and scope["finite_source_kms_residual_rows_closed"] and scope["source_uniformity_diagnostic_closed"] and not scope["common_alpha_closed"] and not scope["pre_a_closed"], scope, "finite source diagnostic", "scope")
    ...
    for graph, declaration in fixture["graphs"].items():
        ...
        check(f"{graph} edge fixture", base.graph_edges(graph) == declared_edges, base.graph_edges(graph), declared_edges, "graph")
        terms, q_single, p_single = base.terms_for(graph, dimension, parameters)
        ...
        source_sites = [int(value) for value in declaration["source_sites"]]
        observables = {
            site: (
        ...
        for order_name in order_names:
            order = base.order_indices(order_name, len(terms))
            for delta in deltas:
                ...
                for delta in deltas:
                    ...
                    for sign in signs:
                        ...
                        for site, (observable_a, observable_b) in observables.items():
                            ...
                            rows.append({"graph": graph, "volume": volume, "source_site": site, "beta": beta, "delta": delta, "steps": steps, "sign": sign, "order": order_name, "stationarity_defect": stationarity, "kms_residual": float(kms_residual), "complex_inverse_error": inverse_error})
    ...
    expected_rows = sum(len(declaration["source_sites"]) for declaration in fixture["graphs"].values()) * len(beta_values) * len(deltas) * len(order_names) * len(signs)
    expected_exact = sum(len(declaration["source_sites"]) for declaration in fixture["graphs"].values()) * len(beta_values) * len(signs)
    expected_mesh = sum(len(declaration["source_sites"]) for declaration in fixture["graphs"].values()) * len(beta_values) * len(order_names) * len(signs)
    ...
    check("source summary coverage", len(source_summaries) == sum(len(declaration["source_sites"]) for declaration in fixture["graphs"].values()), len(source_summaries), "all declared sites", "coverage")
...
def main() -> int:
    ...
    print(f"PRIMARY FINITE-SPLIT-GIBBS-KMS-SOURCE-STRESS PASS {payload['passed']}/{payload['assertion_count']} rows={payload['derived']['row_count']} sources={payload['derived']['source_count']}")
    return 0
