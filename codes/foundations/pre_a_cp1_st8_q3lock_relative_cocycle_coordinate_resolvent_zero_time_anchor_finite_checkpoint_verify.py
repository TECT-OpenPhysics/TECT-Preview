#!/usr/bin/env python3
"""Integrated primary/independent/hostile/Lean verifier for R-386."""
...
LEAN = REPO / "verification/lean/Tect/R386.lean"
...
def compile_lean() -> dict[str, Any]:
    lake = lake_path()
    command = "lake env lean Tect/R386.lean"
    if lake is None:
        return {"status": "UNAVAILABLE", "command": command, "output": "pinned lake executable not found"}
    process = subprocess.run([str(lake), "env", "lean", "Tect/R386.lean"], cwd=LEAN_ROOT, text=True, encoding="utf-8", capture_output=True, check=False)
...
def run() -> dict[str, Any]:
    ...
    check("identity", manifest["exploration_id"] == "EXP-001229" and manifest["result_id"] == "R-386" and manifest["claim_bearing"] is False, [manifest["exploration_id"], manifest["result_id"], manifest["claim_bearing"]], "EXP-001229/R-386/false")
    ...
    markers = ["commute_inverse_of_commute", "commutator_difference", "anchored_first_variation", "anchored_second_variation", "scope_fixture"]
    ...
    registry = json.loads(REGISTRY.read_text(encoding="utf-8"))
    entries = [entry for entry in registry["entrypoints"] if entry["path"] == "verification/lean/Tect/R386.lean"]
    ...
    with tempfile.TemporaryDirectory(prefix="r386-") as temporary:
        temp = Path(temporary)
    ...
    pd, ider = primary["derived"], independent["derived"]
    comparable_numeric = ("zero_commutator_residual", "first_variation_residual", "second_variation_residual", "second_reduction_residual", "modular_first_variation_residual", "dynamic_norm", "dynamic_norm_over_time_squared")
    for field in comparable_numeric:
        check(f"agreement {field}", abs(float(pd["maximums"][field]) - float(ider["maximums"][field])) <= float(manifest["finite_fixture"]["agreement_tolerance"]), [pd["maximums"][field], ider["maximums"][field]], f"within {manifest['finite_fixture']['agreement_tolerance']}")
    for field in ("context_count", "expected_contexts", "bond_prefix_count", "zero_commutator_rows", "first_variation_rows", "second_variation_rows", "modular_first_variation_rows", "dynamic_rows"):
        check(f"agreement {field}", pd[field] == ider[field], [pd[field], ider[field]], "equal")
    for field in ("finite_position_boundary_commutes_with_coordinate_resolvent_closed", "finite_zero_first_variation_closed", "finite_second_variation_reduction_closed", "finite_modular_zero_first_variation_closed", "finite_quadratic_time_diagnostic_closed"):
        check(f"agreement {field}", pd[field] is True and ider[field] is True, [pd[field], ider[field]], "true")
    for field in ("phase_local_bkm_estimate_closed", "boundary_shell_l1_closed", "cutoff_uniformity_closed", "source_uniformity_closed", "volume_uniformity_closed", "shape_uniformity_closed", "operator_domain_embedding_closed", "direct_D_cauchy_closed", "delta_D_cauchy_closed", "common_alpha_closed", "hamiltonian_os_identification_closed", "kms_gns_gap_closed", "continuum_closed", "c6_closed", "sector_a_closed", "pre_a_closed"):
        check(f"scope {field}", pd[field] is False and ider[field] is False, [pd[field], ider[field]], "false")
    check("hostile rejected", hostile["derived"]["wrong_orientation_rejected"] is True, hostile["derived"], "wrong mutation rejected")
    check("hostile separation", float(hostile["derived"]["wrong_residual_min"]) > float(manifest["finite_fixture"]["hostile_threshold"]), hostile["derived"]["wrong_residual_min"], f">{manifest['finite_fixture']['hostile_threshold']}")
    lean = compile_lean()
    check("Lean compile", lean["status"] == "PASS", lean, "PASS")
    firewall = [field for field in ("phase_local_bkm_estimate_closed", "boundary_shell_l1_closed", "cutoff_uniformity_closed", "source_uniformity_closed", "volume_uniformity_closed", "shape_uniformity_closed", "operator_domain_embedding_closed", "direct_D_cauchy_closed", "delta_D_cauchy_closed", "common_alpha_closed", "hamiltonian_os_identification_closed", "kms_gns_gap_closed", "continuum_closed", "c6_closed", "sector_a_closed", "pre_a_closed") if pd[field]]
    check("scope firewall", firewall == [], firewall, "all open")
    numeric_difference = max(abs(float(pd["maximums"][field]) - float(ider["maximums"][field])) for field in comparable_numeric)
    return {"schema": "tect/foundation-audit/1.0", "run_kind": "integrated", "audit_id": "PA-CP1-ST8-Q3LOCK-RELATIVE-COCYCLE-COORDINATE-RESOLVENT-ZERO-TIME-ANCHOR-FINITE-CHECKPOINT", "claim_id": manifest["claim_ids"][0], "result_id": manifest["result_id"], "task_id": manifest["task_id"], "exploration_id": manifest["exploration_id"], "verdict": "PASS", "assertion_count": assertion_count, "assertions": rows, "lean": lean, "derived": {"primary": pd, "independent": ider, "hostile": hostile["derived"], "max_primary_independent_numeric_difference": numeric_difference}, "boundary": manifest["boundary"], "recorded_at": datetime.now(timezone.utc).isoformat().replace("+00:00", "Z")}
...
def main() -> int:
    ...
    print(f"INTEGRATED RELATIVE-COCYCLE ZERO-TIME-ANCHOR PASS {payload['assertion_count']}/{payload['assertion_count']}; Lean={payload['lean']['status']} diff={payload['derived']['max_primary_independent_numeric_difference']:.3e}")
    return 0
