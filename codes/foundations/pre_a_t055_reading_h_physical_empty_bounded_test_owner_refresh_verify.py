#!/usr/bin/env python3
"""Integrated verifier for the R-434 physical-empty owner refresh."""
...
SLUG = "pre-a-t055-reading-h-physical-empty-bounded-test-owner-refresh"
MANIFEST = ROOT / f"strategy/{SLUG}-manifest.json"
PRIMARY = ROOT / "codes/foundations/pre_a_t055_reading_h_physical_empty_bounded_test_owner_refresh.py"
INDEPENDENT = ROOT / "codes/foundations/pre_a_t055_reading_h_physical_empty_bounded_test_owner_refresh_independent.py"
HOSTILE = ROOT / "codes/foundations/pre_a_t055_reading_h_physical_empty_bounded_test_owner_refresh_hostile.py"
LEAN = ROOT / "verification/lean/Tect/R434.lean"
...
def main() -> int:
    ...
    check("manifest identity", [manifest["result_id"], manifest["exploration_id"], manifest["status"], manifest["claim_bearing"]] == ["R-434", "EXP-001279", "BLOCKED", False], [manifest["result_id"], manifest["exploration_id"], manifest["status"], manifest["claim_bearing"]], ["R-434", "EXP-001279", "BLOCKED", False], "provenance")
    ...
    check("artifacts present", all(path.is_file() for path in artifacts), [path.relative_to(ROOT).as_posix() for path in artifacts if not path.is_file()], "all R-434 artifacts", "provenance")
    ...
    lean = command([str(LAKE), "env", "lean", "Tect/R434.lean"], ROOT / "verification/lean")
    ...
    print(f"R-434 INTEGRATED BLOCKED_NOT_EVALUATED {len(checks)}/{len(checks)}; E preregistered, not admitted; Lean=PASS", flush=True)
    return 0
