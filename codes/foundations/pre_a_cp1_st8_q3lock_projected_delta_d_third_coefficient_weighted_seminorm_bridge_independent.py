#!/usr/bin/env python3
"""Independent Fraction-only audit for EXP-001075."""
...
def run() -> dict[str, Any]:
    ...
    c1 = frac(fixture["A1_sum"])
    c0 = frac(fixture["A0_sum"])
    check("A1 majorant authority", c1 == frac(previous["derived_majorants"]["A1_sum"]), c1, previous["derived_majorants"]["A1_sum"], "authority")
    check("A0 majorant authority", c0 == frac(previous["derived_majorants"]["A0_sum"]), c0, previous["derived_majorants"]["A0_sum"], "authority")
    check("positive coefficients", c1 > 0 and c0 > 0 and hbar > 0, [c1, c0, hbar], "positive", "algebra")

    names = ("P_plus", "Q_plus", "P_minus", "Q_minus", "P_delta_plus", "Q_delta_plus", "P_delta_minus", "Q_delta_minus")
    values = {name: frac(fixture[name]) for name in names}
    check("moment roots nonnegative", all(value >= 0 for value in values.values()), values, "nonnegative", "moments")
    m_plus = c1 * values["P_plus"] / hbar + c0 * values["Q_plus"]
    m_minus = c1 * values["P_minus"] / hbar + c0 * values["Q_minus"]
    m_two = m_plus + m_minus
    m_delta_plus = c1 * values["P_delta_plus"] / hbar + c0 * values["Q_delta_plus"]
    m_delta_minus = c1 * values["P_delta_minus"] / hbar + c0 * values["Q_delta_minus"]
    m_delta_two = m_delta_plus + m_delta_minus
    computed = {"M_plus": m_plus, "M_minus": m_minus, "M_two_orientation": m_two, "M_delta_plus": m_delta_plus, "M_delta_minus": m_delta_minus, "M_delta_two_orientation": m_delta_two}
    for label, value in computed.items():
        check(label, value == frac(fixture[f"derived_{label}"]), value, fixture[f"derived_{label}"], "bridge")

    time = frac(fixture["time"])
    t3_over_6 = time**3 / 6
    third_bound = t3_over_6 * m_two
    third_modular_bound = t3_over_6 * m_delta_two
    check("third Taylor scale", t3_over_6 == frac(fixture["derived_t3_over_6"]), t3_over_6, fixture["derived_t3_over_6"], "time")
    check("third two-orientation bound", third_bound == frac(fixture["derived_third_two_orientation_bound"]), third_bound, fixture["derived_third_two_orientation_bound"], "time")
    check("third modular bound", third_modular_bound == frac(fixture["derived_third_modular_bound"]), third_modular_bound, fixture["derived_third_modular_bound"], "time")
    check("triangle decomposition", m_two == m_plus + m_minus, m_two, "M_plus+M_minus", "bridge")
...
def main() -> int:
    ...
    print(f"INDEPENDENT PROJECTED-DELTA-D-THIRD-WEIGHTED-SEMINORM-BRIDGE PASS {payload['passed']}/{payload['assertion_count']}")
    return 0
