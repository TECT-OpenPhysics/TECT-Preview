#!/usr/bin/env python3
"""Primary finite effective-resistance audit for R-408.

The resistance average is tree-independent.  For the symmetric conductance
Laplacian L and conditional law pi, pairwise effective resistance gives

    Var_pi(f) <= (sum_{x<y} pi_x*pi_y*R_xy) * f^T L f.

The reciprocal of the displayed average is a constructive finite lower bound;
no uniform limit is inferred.
"""
...
import pre_a_cp1_st8_q3lock_canonical_path_resistance as r407  # noqa: E402
...
def effective_resistance_data(probabilities: np.ndarray, laplacian: np.ndarray, eigen_floor: float) -> dict[str, Any]:
    ...
    for left in range(pi.size):
        for right in range(left + 1, pi.size):
            rho += float(pi[left] * pi[right] * resistances[left, right])
    ...
    return {
        ...
        "minimum_effective_resistance": float(np.min(resistances[np.triu_indices(pi.size, 1)])),
        "maximum_effective_resistance": float(np.max(resistances[np.triu_indices(pi.size, 1)])),
        "resistance_average": rho,
        "resistance_bound": 1.0 / rho,
...
def run(output: Path = DEFAULT_OUTPUT) -> dict[str, Any]:
                        ...
                        for prefix_length, prefix in prefixes:
                            for history_adjoint in adjoints:
                                ...
                                for orientation in orientations:
                                    ...
                                    for conditional, likelihood in r407.conditional_rows(reference, sample, collar_order, dimension, floor):
                                        gap, laplacian, conductance = r407.intrinsic_graph(conditional, momentum, chi)
                                        ...
                                        tree = r407.canonical_data(conditional, conductance, 1e-300)
    ...
    payload = {"schema": "tect/pre-a-r408-primary/1.0", "manifest": str(MANIFEST.relative_to(REPO)).replace("\\", "/"), "result_id": "R-408", "exploration_id": "EXP-001253", "verdict": "PASS", "checks": checks, "assertion_count": check_count, "derived": derived, "scope": scope, "boundary": manifest["boundary"]}
    r407.atomic_json(output, payload)
    print(f"R-408 PRIMARY PASS {check_count}/{check_count} systems={len(pairs)} contexts={total_contexts} rows={total_rows} bound=[{min(bounds):.6g},{max(bounds):.6g}] Rbar=[{min(averages):.6g},{max(averages):.6g}]")
    return payload
