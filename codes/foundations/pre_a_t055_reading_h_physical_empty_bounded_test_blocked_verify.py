#!/usr/bin/env python3
"""Integrate the R-420 preregistration, independent, hostile and Lean lanes."""
...
PRIMARY = ROOT / "codes/foundations/pre_a_t055_reading_h_physical_empty_bounded_test_blocked.py"
INDEPENDENT = ROOT / "codes/foundations/pre_a_t055_reading_h_physical_empty_bounded_test_blocked_independent.py"
HOSTILE = ROOT / "codes/foundations/pre_a_t055_reading_h_physical_empty_bounded_test_blocked_hostile.py"
LEAN = ROOT / "verification/lean/Tect/R420.lean"
...
def main() -> int:
    ...
    check("test card identity", manifest["test_card_id"] == "PA-T055-READING-H-PHYSICAL-EMPTY-BOUNDED-TEST", manifest["test_card_id"], "PA-T055-READING-H-PHYSICAL-EMPTY-BOUNDED-TEST", "provenance")
    check("common owner blocked", owner["field_status"] == "NOT_FIXED_IN_ONE_COMMON_OWNER" and owner["finite_evaluation_allowed"] is False, [owner["field_status"], owner["finite_evaluation_allowed"]], ["NOT_FIXED_IN_ONE_COMMON_OWNER", False], "contract")
    check("E slot blocked", branch["preregistered_slot"] is True and branch["admitted"] is False and branch["admission_status"] == "BLOCKED_NOT_INSTANTIATED", [branch["preregistered_slot"], branch["admitted"], branch["admission_status"]], [True, False, "BLOCKED_NOT_INSTANTIATED"], "contract")
    check("contract matrix is complete", len(owner["required_fixed_fields"]) == 15 and set(owner["required_fixed_fields"]) == set(owner["field_matrix"]), len(owner["required_fixed_fields"]), 15, "contract")
    check("three verdicts blocked", all(verdicts[name]["status"] == "BLOCKED_NOT_EVALUATED" for name in ("sign", "reading_h_stationarity", "symmetry_projected_transverse_stability")), {name: verdicts[name]["status"] for name in verdicts}, "three BLOCKED_NOT_EVALUATED statuses", "verdict")
    ...
    check("R-420 artifacts present", all(path.is_file() for path in artifacts), [path.as_posix() for path in artifacts if not path.is_file()], "all R-420 artifacts", "provenance")
    ...
    check("Lean theorem markers", all(marker in lean_text for marker in ("evaluation_requires_owner_and_empty", "three_blocked", "preregistration_slot_is_not_admission")), True, True, "Lean")
    ...
    lean = command([str(LAKE), "env", "lean", "Tect/R420.lean"], ROOT / "verification/lean")
    ...
    for field in ("owner_contract_status", "physical_empty_slot", "required_field_count", "sign_status", "stationarity_status", "transverse_stability_status", "numeric_evaluation", "blocker_count"):
    ...
    check("hostile mutation count", len(hostile["assertions"]) == 7, len(hostile["assertions"]), 7, "hostile")

    payload = {
        "schema": "tect/pre-a-t055-physical-empty-bounded-test-blocked-integrated/1.0",
        "run_kind": "integrated",
    ...
    if args.self_test:
        ...
        print(f"R-420 INTEGRATED SELFTEST: PASS ({len(checks)}/{len(checks)} checks; three verdicts blocked; Lean PASS)")
        return 0
    output = args.output if args.output.is_absolute() else ROOT / args.output
    atomic_json(output, payload)
    print(f"R-420 INTEGRATED BLOCKED {len(checks)}/{len(checks)} checks; sign=BLOCKED_NOT_EVALUATED stationarity=BLOCKED_NOT_EVALUATED transverse=BLOCKED_NOT_EVALUATED Lean=PASS")
    return 0
