#!/usr/bin/env python3
"""Integrated verifier for EXP-001191."""
...
LEAN = REPO / "verification/lean/Tect/R350.lean"
...
def lean_run() -> dict[str, Any]:
    command = "lake env lean Tect/R350.lean"
    lake = lake_path()
    if lake is None:
        return {"status": "UNAVAILABLE", "command": command, "output": "pinned lake executable not found"}
    process = subprocess.run([str(lake), "env", "lean", "Tect/R350.lean"], cwd=LEAN_ROOT, text=True, encoding="utf-8", capture_output=True, check=False)
...
def compare_summaries(primary: list[dict[str, Any]], independent: list[dict[str, Any]], tolerance: float) -> list[dict[str, Any]]:
    ...
    keys = ("volume", "oscillator_dimension", "beta", "max_local_quotient_constant", "max_full_quotient_constant", "max_two_slice", "max_equal_time_fourth_moment", "max_local_trace_bound", "max_full_trace_bound", "min_local_trace_slack", "min_full_trace_slack")
...
def run() -> dict[str, Any]:
    ...
    check("identity", manifest["exploration_id"] == "EXP-001191" and manifest["task_id"] == "T-054", [manifest["exploration_id"], manifest["task_id"]], "EXP-001191/T-054")
    ...
    with tempfile.TemporaryDirectory(prefix="source-local-tail-envelope-") as temporary:
        ...
        for key in ("scalar_contraction_bond_envelope_closed", "scalar_local_fourth_tail_envelope_closed", "finite_source_edge_tail_rows_closed", "finite_local_quotient_rows_closed", "source_volume_uniform_scalar_constant_closed"):
            check(key, p.get(key) is True and i.get(key) is True, [p.get(key), i.get(key)], True)
        for key in ("operator_fourth_order_envelope_closed", "gibbs_local_fourth_moment_uniform_closed", "unbounded_common_core_closed", "common_alpha_closed", "qft_promoted"):
            check(key + " remains open", p.get(key) is False and i.get(key) is False, [p.get(key), i.get(key)], False)

    check("scope firewall", scope["scalar_contraction_bond_envelope_closed"] and scope["scalar_local_fourth_tail_envelope_closed"] and scope["finite_source_edge_tail_rows_closed"] and scope["finite_local_quotient_rows_closed"] and scope["source_volume_uniform_scalar_constant_closed"] and not scope["operator_fourth_order_envelope_closed"] and not scope["gibbs_local_fourth_moment_uniform_closed"] and not scope["common_alpha_closed"] and not scope["pre_a_closed"], scope, "scalar/finite only")
    lean = lean_run()
    check("Lean compile", lean["status"] == "PASS", lean, "PASS")
    return {"schema": "tect/foundation-audit/1.0", "run_kind": "integrated", "audit_id": "PA-CP1-ST8-Q3LOCK-SOURCE-LOCAL-TAIL-SCALAR-ENERGY-ENVELOPE", "claim_id": manifest["claim_ids"][0], "task_id": manifest["task_id"], "exploration_id": manifest["exploration_id"], "verdict": "PASS", "assertion_count": len(checks), "assertions": checks, "lean": lean, "boundary": scope, "derived": {"primary_summary_rows": p.get("summary_rows", []), "independent_summary_rows": i.get("summary_rows", []), "scalar_contraction_bond_envelope_closed": True, "scalar_local_fourth_tail_envelope_closed": True, "finite_source_edge_tail_rows_closed": True, "finite_local_quotient_rows_closed": True, "source_volume_uniform_scalar_constant_closed": True, "operator_fourth_order_envelope_closed": False, "gibbs_local_fourth_moment_uniform_closed": False, "unbounded_common_core_closed": False, "common_alpha_closed": False, "qft_promoted": False}, "recorded_at": datetime.now(timezone.utc).isoformat().replace("+00:00", "Z"), "provenance": {"primary_sha256": primary_hash, "independent_sha256": independent_hash, "manifest_sha256": sha256(MANIFEST), "lean_sha256": sha256(LEAN)}}
...
def main() -> int:
    ...
    print(f"INTEGRATED SOURCE-LOCAL-TAIL-SCALAR-ENERGY-ENVELOPE PASS {payload['assertion_count']}/{payload['assertion_count']}; Lean={payload['lean']['status']}")
    return 0
