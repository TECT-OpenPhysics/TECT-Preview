"""Integrated verifier for the R-181 feedback mixed-Gram cross-check."""
...
PRIMARY = REPO / "verification" / "scripts" / "lean_a13_two_root_feedback_mixed_gram_crosscheck.py"
INDEPENDENT = REPO / "codes" / "foundations" / "lean_a13_two_root_feedback_mixed_gram_crosscheck_independent.py"
SCRIPT = Path(__file__).resolve()
LEAN = REPO / "verification" / "lean" / "Tect" / "R181.lean"
...
def accepts(derived: dict, manifest: dict) -> bool:
    ...
    return (
        str(derived.get("feedback_gain")) == manifest["registered_inputs"]["feedback_gain"]
        and str(derived.get("envelope_constant")) == manifest["registered_inputs"]["envelope_constant"]
        and derived.get("owner_order") == ["common_heat", "root_1", "root_2", "future_residual"]
        and derived.get("root_multipliers") == [1, 2]
        and derived.get("fixture_output") == fixture["output_norm"]
        and derived.get("fixture_source") == fixture["source_norm"]
        and derived.get("fixture_defect") == fixture["defect"]
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=REPO / "claims" / "A13-CLASSII-RELATIVE-PHASE-SOURCE-BUDGET-OBSTRUCTION" / "runs" / "2026-08-21-lean-r181-two-root-feedback-mixed-gram-crosscheck" / "integrated.json")
    ...
    check("identity", manifest["audit_id"] == "A13-TWO-ROOT-FEEDBACK-MIXED-GRAM-LEAN-CROSSCHECK", manifest["audit_id"], "A13-TWO-ROOT-FEEDBACK-MIXED-GRAM-LEAN-CROSSCHECK")
    ...
    with tempfile.TemporaryDirectory(prefix="r181-crosscheck-") as directory:
    ...
    common = tuple(primary["derived"])
    check("derived agreement", all(primary["derived"].get(key) == independent["derived"].get(key) for key in common), primary["derived"], independent["derived"])
    check("acceptance predicate", accepts(primary["derived"], manifest), primary["derived"], "feedback mixed-Gram cross-check")
    ...
    for index, label in enumerate(manifest["hostile_mutations"]):
        ...
        if index == 0:
            ...
            mutated["envelope_constant"] = "1"
        elif index == 2:
            mutated["fixture_defect"] = "0"
        elif index == 3:
            mutated["owner_order"] = ["root_1", "common_heat", "root_2", "future_residual"]
        elif index == 4:
    ...
    if not args.staged:
        ...
        check("R-181 authority", results.count("R-181") >= 2 and "feedback mixed-gram" in results.lower(), results.count("R-181"), ">=2")
        check("strategy index authority", "R-181" in (REPO / "strategy/INDEX.md").read_text(encoding="utf-8"), True, True)
    ...
    print(f"INTEGRATED R-181 LEAN PASS {len(rows)}/{len(rows)}")
    return 0
