#!/usr/bin/env python3
"""Integrated verifier for EXP-001156."""
...
LEAN = REPO / "verification/lean/Tect/R326.lean"
...
def lean_run() -> dict[str, Any]:
    lake = lake_path()
    command = "lake env lean Tect/R326.lean"
    if lake is None:
        return {"status": "UNAVAILABLE", "command": command, "output": "pinned lake executable not found"}
    process = subprocess.run([str(lake), "env", "lean", "Tect/R326.lean"], cwd=LEAN_ROOT, text=True, encoding="utf-8", capture_output=True, check=False)
...
def run() -> dict[str, Any]:
    ...
    check("identity", manifest["exploration_id"] == "EXP-001156" and manifest["task_id"] == "T-054" and manifest["claim_bearing"] is False, [manifest["exploration_id"], manifest["task_id"], manifest["claim_bearing"]], "EXP-001156/T-054/false")
    check("Lean source", LEAN.is_file(), LEAN, "present")
    markers = ["weighted_step_fixture", "time_horizon_fixture", "order_count_fixture", "source_support_fixture", "context_count_fixture", "scope_fixture"]
    ...
    with tempfile.TemporaryDirectory(prefix="actual-split-step-recurrence-") as temporary:
        ...
        def row_key(row: dict[str, Any]) -> tuple[int, str, str]:
            ...
            return int(row["volume"]), order, context

        p_rows = {row_key(row): row for row in p.get("context_summaries", [])}
        i_rows = {row_key(row): row for row in i.get("context_summaries", [])}
        expected_contexts = sum(int(fixture["split_order_count"]) * int(fixture["orientation_count"]) for _ in fixture["volume_values"])
        check("context coverage", set(p_rows) == set(i_rows) and len(p_rows) == expected_contexts, [len(p_rows), len(i_rows)], expected_contexts)
        ...
        check("finite split scope", p.get("actual_split_recurrence_theorem_closed") is False and i.get("actual_split_recurrence_theorem_closed") is False and p.get("common_alpha_closed") is False and i.get("common_alpha_closed") is False, [p, i], "actual theorem and QFT gates remain open")
        check("route outcome", p.get("recurrence_status") == i.get("recurrence_status") and p.get("recurrence_status") in ("PASS_ALL_SPLIT_ROWS", "FAIL_SPLIT_ROUTE_LOCAL"), [p.get("recurrence_status"), i.get("recurrence_status")], "explicit finite outcome")
...
def main() -> int:
    ...
    print(f"INTEGRATED ACTUAL-SPLIT-STEP-COMMUTATOR-RECURRENCE PASS {payload['assertion_count']}/{payload['assertion_count']}; Lean={payload['lean']['status']}")
    return 0
