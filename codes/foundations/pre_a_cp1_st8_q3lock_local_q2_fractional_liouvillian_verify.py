#!/usr/bin/env python3
"""Integrated primary/independent/Lean verifier for EXP-001208 / R-366."""
...
LEAN = REPO / "verification/lean/Tect/R366.lean"
...
def compile_lean() -> dict[str, Any]:
    lake = lake_path()
    command = "lake env lean Tect/R366.lean"
    if lake is None:
        return {"status": "UNAVAILABLE", "command": command, "output": "pinned lake executable not found"}
    process = subprocess.run([str(lake), "env", "lean", "Tect/R366.lean"], cwd=LEAN_ROOT, text=True, encoding="utf-8", capture_output=True, check=False)
...
def run() -> dict[str, Any]:
    ...
    check("identity", manifest["exploration_id"] == "EXP-001208" and manifest["result_id"] == "R-366" and not manifest["claim_bearing"], [manifest["exploration_id"], manifest["result_id"], manifest["claim_bearing"]], "EXP-001208/R-366/false")
    ...
    markers = ("fractional_half_envelope", "fractional_theta_one_envelope", "scope_fixture")
    ...
    registry = json.loads(LEAN_REGISTRY.read_text(encoding="utf-8"))
    entries = [entry for entry in registry["entrypoints"] if entry["path"] == "verification/lean/Tect/R366.lean"]
    ...
    with tempfile.TemporaryDirectory(prefix="q3-fractional-liouvillian-") as temporary:
        ...
        exact_fields = (
            "context_count", "expected_contexts", "theta_count", "fractional_liouvillian_bound_closed", "fractional_theta_half_pilot_checked",
            "local_modular_square_function_proved", "local_collar_bound_proved", "positive_Euclidean_path_measure_constructed", "phase_weight_preservation_proved",
            "phase_conditioned_influence_contraction_proved", "cutoff_uniformity_proved", "volume_uniformity_proved", "history_uniformity_proved", "common_core_closed",
        ...
        numeric_fields = (
            ...
            "max_spectral_reduction_error", "max_bond_unitary_factorization_error", "max_density_hs_norm_excess", "maximum_finite_time_change_norm",
            "maximum_fractional_bound", "maximum_state_trace_change", "maximum_fractional_to_bound_ratio", "minimum_nonzero_fractional_norm", "maximum_theta_one_reduction_error"
        )
...
def main() -> int:
    ...
    print(f"INTEGRATED FRACTIONAL LIOUVILLIAN PASS {payload['assertion_count']}/{payload['assertion_count']}; Lean={payload['lean']['status']} diff={payload['derived']['max_primary_independent_numeric_difference']:.3e}")
    return 0
