#!/usr/bin/env python3
"""Hostile mutation firewall for the R-438 finite d=19 certificate."""
...
DEFAULT_OUTPUT = REPO / "claims/C6-SPACETIME-SIGNATURE/runs/2026-08-30-hostile-original_source_interval_d19/hostile.json"
...
def run(output: Path = DEFAULT_OUTPUT) -> dict[str, Any]:
    ...
    candidate = copy.deepcopy(manifest)
    candidate["source_contract"]["cutoff_dimension"] = 18
    mutations.append(("cutoff substitution", candidate["source_contract"]["cutoff_dimension"], 19))
    candidate = copy.deepcopy(manifest)
    ...
    payload = {
        "schema": "tect/pre-a-r438-hostile/1.0",
    ...
    print(f"R-438 HOSTILE {payload['verdict']} {len(checks)}/{len(checks)}", flush=True)
    return payload
...
def main() -> int:
    ...
    if args.self_test:
        ...
        print("R-438 HOSTILE SELFTEST: PASS", flush=True)
    return 0
