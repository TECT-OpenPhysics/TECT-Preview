#!/usr/bin/env python3
"""Integrated verifier for EXP-001193."""
...
LEAN = REPO / "verification/lean/Tect/R352.lean"
...
def lean_run() -> dict[str, Any]:
    command = "lake env lean Tect/R352.lean"
    lake = lake_path()
    if lake is None:
        return {"status": "UNAVAILABLE", "command": command, "output": "pinned lake executable not found"}
    process = subprocess.run([str(lake), "env", "lean", "Tect/R352.lean"], cwd=LEAN_ROOT, text=True, encoding="utf-8", capture_output=True, check=False)
...
def compare_summaries(primary: list[dict[str, Any]], independent: list[dict[str, Any]], tolerance: float) -> list[dict[str, Any]]:
    ...
    keys = ("volume", "oscillator_dimension", "max_kinetic_relative_quotient", "min_fourth_defect", "max_potential_fourth_moment", "max_full_fourth_moment", "min_trace_slack")
        ...
        for key in keys:
            if isinstance(left.get(key), (int, float)) and isinstance(right.get(key), (int, float)):
                if abs(float(left[key]) - float(right[key])) > tolerance * (1 + abs(float(left[key]))):
                    mismatches.append({"key": key, "primary": left[key], "independent": right[key], "row": left.get("volume")})
            elif left.get(key) != right.get(key):
                mismatches.append({"key": key, "primary": left.get(key), "independent": right.get(key), "row": left.get("volume")})
    return mismatches
...
def run() -> dict[str, Any]:
    ...
    check("identity", manifest["exploration_id"] == "EXP-001193" and manifest["task_id"] == "T-054", [manifest["exploration_id"], manifest["task_id"]], "EXP-001193/T-054")
    ...
    with tempfile.TemporaryDirectory(prefix="kinetic-inclusive-transfer-boundary-") as temporary:
        ...
        for key in ("generic_form_order_shortcut_refuted", "finite_q3_kinetic_diagnostic_closed", "finite_q3_relative_quotient_rows_closed", "finite_q3_trace_rows_closed"):
            check(key, p.get(key) is True and i.get(key) is True, [p.get(key), i.get(key)], True)
        for key in ("uniform_kinetic_inclusive_operator_order_closed", "uniform_gibbs_potential_moment_closed", "unbounded_common_core_closed", "common_alpha_closed", "qft_promoted"):
            check(key + " remains open", p.get(key) is False and i.get(key) is False, [p.get(key), i.get(key)], False)
        check("generic determinant", abs(float(p.get("generic_fourth_difference_determinant", 0.0)) + 168.0) <= 1.0e-7 and abs(float(i.get("generic_fourth_difference_determinant", 0.0)) + 168.0) <= 1.0e-7, [p.get("generic_fourth_difference_determinant"), i.get("generic_fourth_difference_determinant")], "-168")
        check("generic negative eigenvalue", p.get("generic_fourth_difference_min_eigenvalue", 0.0) < 0.0 and i.get("generic_fourth_difference_min_eigenvalue", 0.0) < 0.0, [p.get("generic_fourth_difference_min_eigenvalue"), i.get("generic_fourth_difference_min_eigenvalue")], "<0")

    check("scope firewall", scope["generic_form_order_shortcut_refuted"] and scope["finite_q3_kinetic_diagnostic_closed"] and scope["finite_q3_relative_quotient_rows_closed"] and scope["finite_q3_trace_rows_closed"] and not scope["uniform_kinetic_inclusive_operator_order_closed"] and not scope["uniform_gibbs_potential_moment_closed"] and not scope["common_alpha_closed"] and not scope["pre_a_closed"], scope, "finite/generic boundary only")
    lean = lean_run()
    check("Lean compile", lean["status"] == "PASS", lean, "PASS")
    return {"schema": "tect/foundation-audit/1.0", "run_kind": "integrated", "audit_id": "PA-CP1-ST8-Q3LOCK-KINETIC-INCLUSIVE-TRANSFER-BOUNDARY", "claim_id": manifest["claim_ids"][0], "task_id": manifest["task_id"], "exploration_id": manifest["exploration_id"], "verdict": "PASS", "assertion_count": len(checks), "assertions": checks, "lean": lean, "boundary": scope, "derived": {"primary_summary_rows": p.get("summary_rows", []), "independent_summary_rows": i.get("summary_rows", []), "generic_form_order_shortcut_refuted": True, "finite_q3_kinetic_diagnostic_closed": True, "finite_q3_relative_quotient_rows_closed": True, "finite_q3_trace_rows_closed": True, "uniform_kinetic_inclusive_operator_order_closed": False, "uniform_gibbs_potential_moment_closed": False, "unbounded_common_core_closed": False, "common_alpha_closed": False, "qft_promoted": False}, "recorded_at": datetime.now(timezone.utc).isoformat().replace("+00:00", "Z"), "provenance": {"primary_sha256": primary_hash, "independent_sha256": independent_hash, "manifest_sha256": sha256(MANIFEST), "lean_sha256": sha256(LEAN)}}
...
def main() -> int:
    ...
    print(f"INTEGRATED KINETIC-INCLUSIVE-TRANSFER-BOUNDARY PASS {payload['assertion_count']}/{payload['assertion_count']}; Lean={payload['lean']['status']}")
    return 0
