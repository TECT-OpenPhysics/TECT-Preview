#!/usr/bin/env python3
"""Integrated verifier for the R-435 d=17 finite interval package."""
...
MANIFEST = ROOT / "strategy/pre-a-cp1-st8-q3lock-original-source-interval-d17-manifest.json"
PRIMARY = ROOT / "codes/foundations/pre_a_cp1_st8_q3lock_original_source_interval_d17.py"
INDEPENDENT = ROOT / "codes/foundations/pre_a_cp1_st8_q3lock_original_source_interval_d17_independent.py"
HOSTILE = ROOT / "codes/foundations/pre_a_cp1_st8_q3lock_original_source_interval_d17_hostile.py"
LEAN = ROOT / "verification/lean/Tect/R435.lean"
SLUG = "original_source_interval_d17"
...
def main() -> int:
    ...
    check("manifest identity", manifest["result_id"] == "R-435" and manifest["exploration_id"] == "EXP-001280" and manifest["claim_bearing"] is False, [manifest["result_id"], manifest["exploration_id"], manifest["claim_bearing"]], "R-435/EXP-001280/false", "provenance")
    artifacts = [PRIMARY, INDEPENDENT, HOSTILE, LEAN]
    check("artifacts present", all(path.is_file() for path in artifacts), [path.relative_to(ROOT).as_posix() for path in artifacts if not path.is_file()], "all R-435 artifacts", "provenance")
    ...
    lean = command([os.fspath(LAKE), "env", "lean", "Tect/R435.lean"], ROOT / "verification/lean")
    ...
    check("primary interval certificate", primary["verdict"] == "ORIGINAL_SOURCE_INTERVAL_CERTIFIED" and primary["assertion_count"] == 57 and p["finite_positive_gap_certified"] is True and Decimal(p["maximum_residual_matrix_interval_width"]) < Decimal(manifest["interval_contract"]["maximum_matrix_interval_width"]) and Decimal(p["bracket_width"]) < Decimal(manifest["interval_contract"]["maximum_bracket_width"]), {key: p[key] for key in ("rayleigh_interval", "maximum_residual_matrix_interval_width", "bracket_width")}, "57/57 finite interval checks and bounded bracket", "primary")
    ...
    print(f"R-435 INTEGRATED ORIGINAL_SOURCE_INTERVAL_CERTIFIED {len(checks)}/{len(checks)} bracket={p['rayleigh_interval']} Lean=PASS", flush=True)
    return 0
