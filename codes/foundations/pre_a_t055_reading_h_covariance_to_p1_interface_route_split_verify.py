...
def formal_audit(audit: Audit, manifest: dict[str, Any], fresh_primary: dict[str, Any], fresh_independent: dict[str, Any]) -> None:
    ...
    audit.check("R-169 current authority", all(token in results for token in ("### R-169", "R-169 v1.2", "EXP-000858", "current proof-first authority", "R-169 v1.1 certificate", "prior proof-first authority", "No R-169 v1.2 PDF")), "v1.2 current and v1.1 prior", "v1.2 current and v1.1 prior", "formal")
    ...
    audit.check("event 641 identity", len(events) == contract["event_ordinal"] and event.get("id") == contract["event_id"] and event.get("header") == exact_header, {"count": len(events), "id": event.get("id"), "header": event.get("header")}, {"count": contract["event_ordinal"], "id": contract["event_id"], "header": exact_header}, "formal")
    ...
    map_ok = (
        ...
        and "EXP-000858" in priority.get("latest_cp1_checkpoint", "")
