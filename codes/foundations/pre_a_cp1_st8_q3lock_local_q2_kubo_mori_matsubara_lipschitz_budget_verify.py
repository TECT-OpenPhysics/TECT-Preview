#!/usr/bin/env python3
"""Integrated primary/independent/Lean verifier for EXP-001217 / R-375."""
...
LEAN = REPO / "verification/lean/Tect/R375.lean"
...
def compile_lean() -> dict[str, Any]:
    lake = lake_path()
    command = "lake env lean Tect/R375.lean"
    if lake is None:
        return {"status": "UNAVAILABLE", "command": command, "output": "pinned lake executable not found"}
    process = subprocess.run([str(lake), "env", "lean", "Tect/R375.lean"], cwd=LEAN_ROOT, text=True, encoding="utf-8", capture_output=True, check=False)
...
def run() -> dict[str, Any]:
    ...
    check("identity", manifest["exploration_id"] == "EXP-001217" and manifest["result_id"] == "R-375" and manifest["claim_bearing"] is False, [manifest["exploration_id"], manifest["result_id"], manifest["claim_bearing"]], "EXP-001217/R-375/false")
    ...
    markers = ("layer_nonnegative", "derivative_envelope", "layer_zero", "frequency_budget_term", "scope_fixture")
    ...
    registry = json.loads(LEAN_REGISTRY.read_text(encoding="utf-8"))
    entries = [entry for entry in registry["entrypoints"] if entry["path"] == "verification/lean/Tect/R375.lean"]
    ...
    with tempfile.TemporaryDirectory(prefix="q3-matsubara-budget-") as temporary:
        ...
        exact_fields = ("context_count", "expected_contexts", "series_terms", "sample_grid_points", "layer_derivative_bound_finite_checked", "partial_lipschitz_budget_finite_checked", "exact_capped_kernel_lipschitz_finite_checked", "odd_frequency_budget_identity_recorded", "first_liouvillian_variation_reduction_proved", "eigenvector_rotation_control_proved", "resolvent_locality_proved", "capped_dirichlet_uniformity_proved", "weighted_cutoff_uniformity_proved", "weighted_volume_uniformity_proved", "source_uniformity_proved", "shape_uniformity_proved", "common_core_closed", "common_alpha_closed", "kms_gns_gap_closed", "continuum_closed", "c6_closed", "sector_a_closed", "pre_a_closed")
        for field in exact_fields:
            check(f"agreement {field}", p[field] == i[field], [p[field], i[field]], "equal")
        numeric_fields = ("theta", "maximum_transition_energy", "maximum_layer_slope", "maximum_layer_slope_envelope_violation", "maximum_partial_lipschitz_ratio", "maximum_partial_budget_violation", "maximum_exact_lipschitz_ratio", "maximum_exact_slope", "maximum_exact_unit_slope_violation", "maximum_partial_monotonicity_violation", "maximum_finite_budget", "maximum_budget_tail_envelope", "minimum_layer_value", "minimum_partial_value")
        ...
        for regime_index, (pr, ir) in enumerate(zip(p["regimes"], i["regimes"])):
            for field in ("shape", "volume", "cutoffs", "contexts", "expected_contexts"):
                check(f"regime {regime_index} {field}", pr[field] == ir[field], [pr[field], ir[field]], "equal")
            for field in ("maximum_transition", "maximum_partial_ratio", "maximum_exact_ratio"):
            ...
            for bond_index, (pb, ib) in enumerate(zip(pr["bond_rows"], ir["bond_rows"])):
                for field in ("cutoff", "beta", "bond_term_index", "context_count"):
                    check(f"regime {regime_index} bond {bond_index} {field}", pb[field] == ib[field], [pb[field], ib[field]], "equal")
                for field in ("maximum_transition", "budget", "budget_tail_envelope", "maximum_partial_ratio", "maximum_exact_ratio"):
        ...
        open_fields = ("first_liouvillian_variation_reduction_proved", "eigenvector_rotation_control_proved", "resolvent_locality_proved", "capped_dirichlet_uniformity_proved", "weighted_cutoff_uniformity_proved", "weighted_volume_uniformity_proved", "source_uniformity_proved", "shape_uniformity_proved", "common_core_closed", "common_alpha_closed", "kms_gns_gap_closed", "continuum_closed", "c6_closed", "sector_a_closed", "pre_a_closed")
...
def main() -> int:
    ...
    print(f"INTEGRATED MATSUBARA-LIPSCHITZ-BUDGET PASS {payload['assertion_count']}/{payload['assertion_count']}; Lean={payload['lean']['status']} diff={payload['derived']['max_primary_independent_numeric_difference']:.3e}")
    return 0
