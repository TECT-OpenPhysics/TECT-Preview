...
DEFAULT_OUTPUT = CLAIM / "runs" / "2026-07-17-integrated-verifier" / "result.json"
AUDITS = (
...
def main() -> int:
    ...
    output = {"schema": "tect/a3-full-production-integrated-verifier-result/1.0", "claim_id": manifest["claim_id"], "script_version": __version__, "verdict": "A3-FULL-PRODUCTION-VERIFY-PASS" if passed else "A3-FULL-PRODUCTION-VERIFY-FAIL", "scope": f"{mode} of spatial, finite-time, Hessian/Ritz, independent-proxy, and solution-ball-order audits; hash validation of recorded CUDA evidence", "reports": reports, "assertion_summary": {"passed": assertion_total if passed else sum(int(report.get("assertions", {}).get("passed", 0)) for report in reports), "total": assertion_total}, "failures": failures, "not_closed_here": ["numerical enclosure of the solution-ball constant C(R,tau,T)", "historical Sector-B solver continuum validity", "independent external reproduction", "tier promotion"]}
