#!/usr/bin/env python3
"""Primary frequency-adapted endpoint-shell audit for EXP-001225 / R-383."""
...
def run() -> dict[str, Any]:
    ...
    check("R-382 source", source.get("verdict") == "PASS" and source.get("derived", {}).get("primary", {}).get("context_count") == int(fixture["expected_parent_contexts"]), [source.get("verdict"), source.get("derived", {}).get("primary", {}).get("context_count")], "R-382 PASS and expected parent contexts", "source linkage")
    ...
    return {
        "schema": "tect/foundation-audit/1.0", "run_kind": "primary", "audit_id": "PA-CP1-ST8-Q3LOCK-LOCAL-Q2-KUBO-MORI-FREQUENCY-ADAPTED-ENDPOINT-FILTER",
...
def main() -> int:
    ...
    print(f"PRIMARY FREQUENCY-ADAPTED-ENDPOINT-FILTER PASS {payload['assertion_count']}/{payload['assertion_count']} contexts={payload['derived']['context_count']} ratio={payload['derived']['maximum_endpoint_to_cauchy_ratio']:.9f}")
    return 0
