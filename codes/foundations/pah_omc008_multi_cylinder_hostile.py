#!/usr/bin/env python3
"""Adversarial review for the PAH-OMC-008 multi-cylinder replay."""
...
PRIMARY = ROOT / "claims/C6-SPACETIME-SIGNATURE/runs/2026-09-04-pah-omc008-multi-cylinder/primary.json"
INDEPENDENT = ROOT / "claims/C6-SPACETIME-SIGNATURE/runs/2026-09-04-pah-omc008-multi-cylinder/independent.json"
DEFAULT_OUTPUT = ROOT / "claims/C6-SPACETIME-SIGNATURE/runs/2026-09-04-pah-omc008-multi-cylinder/hostile.json"
...
def main() -> int:
    ...
    check("manifest-pins", pins.get("PAH-001") == hashes["PAH-001"] and pins.get("PAH-OMC-004") == hashes["PAH-OMC-004"] and pins.get("PAH-OMC-005") == hashes["PAH-OMC-005"] and pins.get("PAH-OMC-006") == hashes["PAH-OMC-006"] and pins.get("PAH-OMC-007") == hashes["PAH-OMC-007"] and manifest.get("contract", {}).get("sha256") == hashes["PAH-OMC-008"])
    ...
    mutation("promote-pure-link-u_d0", True, "the carried link shifts by g_d-g_a, while both closed-face products cancel their endpoint incidences")
    # Removing d0 leaves open two-edge paths in both split triangles.
    mutation("use-open-face-h00-v1", True, "without d0 the endpoint gauge increment at the diagonal vertex survives")
    joint_observable_text = contract.get("exact_scope", {}).get("joint_observable", "")
    mutation("omit-second-face-or-matter-coordinate", all(token in joint_observable_text for token in ("ell_d", "H_1")), "the accepted cylinder is the four-coordinate tuple and cannot be reduced to only ell_a,H_0")
    support = json.dumps(primary.get("support_audit", {}), ensure_ascii=True, sort_keys=True)
    mutation("omit-wilson-face-term", "face:0" in support and "face:1" in support, "the retained links change the existing split-triangle Wilson faces and both are included in the exact local delta")
    ...
    expected_rows = state_count * int(dimensions.get("patch_link_edges", 0)) * 2 + (state_count // int(dimensions.get("radial_placements", 1))) * int(dimensions.get("radial_degree_sum", 0))
    mutation("collapse-k2-link-channels", root_count == expected_rows and int(dimensions.get("patch_link_edges", 0)) == 5, "all five patch links retain both sigma=+1 and sigma=-1 as distinct PAH channels")
    ...
    payload: dict[str, Any] = {
        "schema": "tect/pah-omc008-multi-cylinder-hostile/1.0",
        ...
        "reproduction": {"command": "python codes/foundations/pah_omc008_multi_cylinder_hostile.py --output claims/C6-SPACETIME-SIGNATURE/runs/2026-09-04-pah-omc008-multi-cylinder/hostile.json"},
