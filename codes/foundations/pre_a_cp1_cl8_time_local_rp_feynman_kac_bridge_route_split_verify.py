#!/usr/bin/env python3
"""Integrated verifier for the CL8 time-local RP/Feynman--Kac route split."""
...
PARENT_EXPLORATION_ID = "EXP-000767"
...
def build_payload() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="tect-time-local-rp-integrated-") as temporary:
    ...
    primary_required = (
        "heat coefficient",
        "physical link coefficient",
        "Gaussian square completion",
        "Gaussian convolution prefactor",
        "exact half-step square",
        "site reflected form square",
        "link reflected form half-step square",
        "periodic path normalizer",
        "bounded path correlator trace",
        "Doob detailed balance",
        "Doob bounded-log no-go linked",
        "Strang orderings differ",
        "Strang link Gram",
        "quartic epsilon-cubed defect",
        "actual Q3 bi-Laplacian per node",
        "actual CL8 one-slice defect",
        "pointwise-positive operator sentinel",
        "scope below_empty_space_comparison",
    )
    independent_required = (
        "Strang jet oracle",
        "exact jet oracle",
        "defect jet oracle",
        "closed defect reconstruction",
        "Q3 edge raw bi-Laplacian",
        "actual CL8 defect factor",
        "independent transfer Gram",
        "six-ring path equals trace",
        "site direct Gram factor",
        "link direct Gram factor",
        "entrywise positivity not operator positivity",
        "rational physical link coefficient",
        "independent Strang ordering differs",
        "independent Strang Gram orientation",
        "stdlib import firewall",
        "scope below_empty_space_comparison",
    )
    ...
    primary_jet = primary["derived"]["quartic_jet"]
    independent_jet = independent["derived"]["quartic_jet"]
    primary_defect_flat = primary_jet["defect"].replace(" ", "")
    audit.check("cross origin defect", "-2*gamma*k**2" in primary_defect_flat and independent_jet["defect"].get("0,2,1") == "-2", {"primary": primary_jet["defect"], "independent": independent_jet["defect"]}, "-2 gamma kappa^2", "cross")
    audit.check("cross x6 defect", "-4*gamma**2*k*x**6/3" in primary_defect_flat and independent_jet["defect"].get("6,1,2") == "-4/3", {"primary": primary_jet["defect"], "independent": independent_jet["defect"]}, "-4/3 gamma^2 kappa x^6", "cross")
    primary_g = primary_jet["actual_CL8_per_node_g_coefficient"]
    primary_lambda = primary_jet["actual_CL8_per_node_lambda_coefficient"]
    independent_actual = independent["derived"]["actual_CL8"]
    audit.check("cross actual CL8 g coefficient", primary_g == independent_actual["per_node_g_coefficient"] == "48", primary_g, independent_actual["per_node_g_coefficient"], "cross")
    audit.check("cross actual CL8 lambda coefficient", primary_lambda == independent_actual["per_node_lambda_coefficient"] == "192", primary_lambda, independent_actual["per_node_lambda_coefficient"], "cross")
    audit.check("cross actual CL8 defect factor", primary_jet["actual_CL8_defect_factor"] == "-4", primary_jet["actual_CL8_defect_factor"], "-4", "cross")
    audit.check("cross scope exact", primary["scope"] == independent["scope"] == manifest["scope"], primary["scope"], manifest["scope"], "cross")
    audit.check("fixture diversity", primary["derived"]["exact_RP"]["transfer"] != independent["derived"]["RP"]["transfer"], primary["derived"]["exact_RP"]["transfer"], independent["derived"]["RP"]["transfer"], "cross")

    certificate_phrases = (
        "inverse-energy units",
        "continuous-time periodic loop law",
        "every lattice-preserving dihedral reflection axis",
        "The infinitesimal Markov generator is `-L_0`",
        "no bounded inverse",
        "B_\\epsilon=e^{\\epsilon\\kappa_a\\Delta/2}A_\\epsilon",
        "or `O(N^-2)` trace-norm claim is made",
        "actual CL8 one-step defect",
        "Euclidean heat support is not a Lorentzian light cone",
        "absolute or below-empty-space energy comparison",
    ...
    audit.check("exploration continues comparator", any(item.get("id") == PARENT_EXPLORATION_ID and item.get("relation") == "continues" for item in exploration.get("related", [])), exploration.get("related", []), f"continues {PARENT_EXPLORATION_ID}", "records")
    ...
    for phrase in ("lattice-preserving", "Doob", "epsilon^3", "full configuration-transition support", "C6", "Pre-A"):
        audit.check(f"exploration finding {phrase}", phrase in exploration["finding"] or phrase in exploration["boundary"], phrase, "present", "records")
    ...
    for key in ("C6_advanced", "CP1_complete", "Pre_A_complete", "physical_state_or_vacuum", "below_empty_space_comparison"):
    ...
    return {
        ...
        "cross_oracles": {
            "quartic_defect": primary_jet["defect"],
            "actual_CL8_per_node_g_coefficient": primary_g,
            "actual_CL8_per_node_lambda_coefficient": primary_lambda,
            "actual_CL8_defect_factor": primary_jet["actual_CL8_defect_factor"],
            "primary_RP_Z": primary["derived"]["exact_RP"]["Z"],
            "independent_RP_Z": independent["derived"]["RP"]["Z"],
        },
