#!/usr/bin/env python3
"""Primary finite cancellation-aware signed Q3 commutator audit.

Absolute pair norms can hide cancellations in the first Lie--Trotter defect.
This audit therefore groups commutators with the same exact union support U,
retains their order signs, and evaluates the signed group with the local Gibbs
and K_U^(1/2)-Gibbs four-leg seminorms.  The groups are finite proxies only;
they are not a global-state Trotter estimate.
"""
...
import pre_a_cp1_st8_q3lock_state_weighted_commutator_sum_audit as base  # noqa: E402
...
def term_groups(order: list[dict[str, Any]], volume: int, cutoff: int, fixture: dict[str, Any]) -> tuple[dict[tuple[int, ...], list[np.ndarray]], float]:
    ...
    for left_index, left_spec in enumerate(order):
        ...
        for right_spec in order[left_index + 1 :]:
            ...
            left = base.support_term(left_spec, list(union), cutoff, fixture)
            right = base.support_term(right_spec, list(union), cutoff, fixture)
            value = base.commutator(left, right)
...
def window_metric(matrix: np.ndarray, weight_power: np.ndarray, rho_sqrt: np.ndarray, projector: np.ndarray, rho: np.ndarray, energy_values: np.ndarray, threshold: float, volume: int, tolerance: float) -> dict[str, Any]:
    ...
    return {
        ...
        "rank": rank,
        "window_mass": mass,
        "tail_mass": max(0.0, 1.0 - mass),
        "signed_weighted": signed,
        "signed_weighted_per_site": signed / volume,
        "conditional_signed_weighted": conditional,
        "conditional_signed_weighted_per_site": conditional / volume,
    }
...
def row(volume: int, cutoff: int, beta: float, fixture: dict[str, Any], source_manifest: dict[str, Any], exponent: float, energy_windows: list[float], tolerance: float, positivity_tolerance: float, orientation_tolerance: float, norm_floor: float) -> dict[str, Any]:
    declared = base.term_specs(volume)
    ...
    orientation_raw_residual = 0.0
    orientation_gibbs_difference = 0.0
    orientation_weighted_difference = 0.0
    ...
    for union in sorted(forward_groups):
        ...
        for threshold in energy_windows:
            ...
            projector = energy_vectors[:, shifted <= threshold + tolerance] @ energy_vectors[:, shifted <= threshold + tolerance].conj().T
            ...
            window_rows[key]["signed_weighted"] += signed_window["signed_weighted"]
            window_rows[key]["absolute_weighted"] += absolute_window
            window_rows[key]["conditional_signed_weighted"] += signed_window["conditional_signed_weighted"]
            window_rows[key]["group_count"] += 1
        ...
        for key, aggregate in source_aggregates.items():
            ...
            for threshold in energy_windows:
                window_key = format(threshold, "g")
                aggregate["window_signed_weighted"].setdefault(window_key, 0.0)
                aggregate["window_absolute_weighted"].setdefault(window_key, 0.0)
                aggregate["window_signed_weighted"][window_key] += group_windows[window_key]["signed_weighted"]
                aggregate["window_absolute_weighted"][window_key] += group_windows[window_key]["absolute_weighted"]
    ...
    for key, metric in window_rows.items():
        metric["signed_weighted_per_site"] = metric["signed_weighted"] / volume
        metric["absolute_weighted_per_site"] = metric["absolute_weighted"] / volume
        metric["conditional_signed_weighted_per_site"] = metric["signed_weighted_per_site"] / max(np.sqrt(metric["window_mass"]), np.finfo(float).tiny); metric["signed_to_absolute"] = metric["signed_weighted"] / max(metric["absolute_weighted"], np.finfo(float).tiny)
    for aggregate in source_aggregates.values():
        ...
        aggregate["absolute_weighted_sum_per_source_site"] = aggregate["absolute_weighted_sum"] / source_size
...
def run() -> dict[str, Any]:
    ...
    check("graph volume", volumes == [2] and all(base.q3.graph_edges(volume) for volume in volumes), volumes, "fixed two-site Q3 graph", "fixture")
    ...
    for volume in volumes:
        ...
        for beta in betas:
            for cutoff in cutoffs:
                value = row(volume, cutoff, beta, fixture, source_manifest, exponent, energy_windows, tolerance, positivity_tolerance, orientation_tolerance, norm_floor)
                all_group = value["all_group"]
    ...
    for beta in betas:
        for energy in energy_windows:
            key = format(energy, "g")
            selected = [item for item in rows if float(item["beta"]) == beta]
            window_values = [item["windows"][key] for item in selected]
            tail = [item["windows"][key] for item in selected if int(item["cutoff"]) >= tail_cutoff_start]
            signed_tail = [float(item["signed_weighted_per_site"]) for item in tail]
            conditional_tail = [float(item["conditional_signed_weighted_per_site"]) for item in tail]
            ratio = max(signed_tail) / max(min(signed_tail), np.finfo(float).tiny)
            conditional_ratio = max(conditional_tail) / max(min(conditional_tail), np.finfo(float).tiny)
            endpoint = float(window_values[-1]["signed_weighted_per_site"]) / max(float(window_values[0]["signed_weighted_per_site"]), np.finfo(float).tiny)
            stable = ratio <= threshold and conditional_ratio <= threshold
            summary = {"beta": beta, "energy_threshold": energy, "cutoff_first": cutoffs[0], "cutoff_last": cutoffs[-1], "tail_cutoff_start": tail_cutoff_start, "tail_row_count": len(tail), "signed_weighted_tail_max_per_site": max(signed_tail), "signed_weighted_tail_min_per_site": min(signed_tail), "tail_stability_ratio": ratio, "conditional_tail_stability_ratio": conditional_ratio, "full_endpoint_ratio": endpoint, "window_mass_min": min(float(item["window_mass"]) for item in window_values), "window_mass_max": max(float(item["window_mass"]) for item in window_values), "rank_min": min(int(item["rank"]) for item in window_values), "rank_max": max(int(item["rank"]) for item in window_values), "stability_threshold": threshold, "tail_stable": stable}
            summaries.append(summary)
            check(f"beta={beta} E={energy} summary coverage", len(selected) == len(cutoffs) and len(tail) == sum(int(cutoff >= tail_cutoff_start) for cutoff in cutoffs) and np.isfinite(ratio) and np.isfinite(conditional_ratio), summary, "finite summary", "scaling")
            check(f"beta={beta} E={energy} tail stability", stable, summary, f"<= {threshold} for signed and conditional tails", "window stability")
...
def main() -> int:
    ...
    print(f"PRIMARY LOW-ENERGY-SPECTRAL-WINDOW PASS {payload['passed']}/{payload['assertion_count']} rows={payload['derived']['row_count']} windows={payload['derived']['window_row_count']}")
    return 0
