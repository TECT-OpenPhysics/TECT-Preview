#!/usr/bin/env python3
"""Integrated primary/independent/Lean verifier for EXP-001225 / R-383."""
...
LEAN = REPO / "verification/lean/Tect/R383.lean"
...
def compile_lean() -> dict[str, Any]:
    lake = lake_path()
    command = "lake env lean Tect/R383.lean"
    if lake is None:
        return {"status": "UNAVAILABLE", "command": command, "output": "pinned lake executable not found"}
    process = subprocess.run([str(lake), "env", "lean", "Tect/R383.lean"], cwd=LEAN_ROOT, text=True, encoding="utf-8", capture_output=True, check=False)
...
def run() -> dict[str, Any]:
    ...
    check("identity", manifest["exploration_id"] == "EXP-001225" and manifest["result_id"] == "R-383" and manifest["claim_bearing"] is False, [manifest["exploration_id"], manifest["result_id"], manifest["claim_bearing"]], "EXP-001225/R-383/false")
    ...
    markers = ("filtered_weight_nonnegative", "filtered_weight_le_one", "filtered_endpoint_half_factor", "filtered_endpoint_unit_factor", "filtered_m2_unit_factor", "scope_fixture")
    ...
    registry = json.loads(LEAN_REGISTRY.read_text(encoding="utf-8"))
    entries = [entry for entry in registry["entrypoints"] if entry["path"] == "verification/lean/Tect/R383.lean"]
    ...
    with tempfile.TemporaryDirectory(prefix="q3-frequency-adapted-filter-") as temporary:
        ...
        open_fields = ("filter_removal_proved", "source_uniformity_proved", "weighted_cutoff_uniformity_proved", "weighted_volume_uniformity_proved", "shape_uniformity_proved", "common_core_closed", "common_alpha_closed", "kms_gns_gap_closed", "continuum_closed", "c6_closed", "sector_a_closed", "pre_a_closed")
...
def main() -> int:
    ...
    print(f"INTEGRATED FREQUENCY-ADAPTED-ENDPOINT-FILTER PASS {payload['assertion_count']}/{payload['assertion_count']}; Lean={payload['lean']['status']} diff={payload['derived']['max_primary_independent_numeric_difference']:.3e}")
    return 0
