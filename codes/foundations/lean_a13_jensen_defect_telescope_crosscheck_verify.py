"""Integrated verifier for the R-188 signed Jensen-defect telescope."""
...
PRIMARY = REPO / "verification" / "scripts" / "lean_a13_jensen_defect_telescope_crosscheck.py"
INDEPENDENT = REPO / "codes" / "foundations" / "lean_a13_jensen_defect_telescope_crosscheck_independent.py"
SCRIPT = Path(__file__).resolve()
LEAN = REPO / "verification" / "lean" / "Tect" / "R188.lean"
...
def accepts(derived: dict, manifest: dict) -> bool:
    ...
    return (
        all(derived.get(key) == (value if not isinstance(value, bool) else value) for key, value in expected.items())
        and derived.get("absolute_payment_strict") is True
        and derived.get("signed_endpoint_mean") == "0"
        and derived.get("a13_gate_closed") is False
        and derived.get("overlap_src_closed") is False
        and derived.get("production_map_identified") is False
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=REPO / "claims" / "A13-CLASSII-RELATIVE-PHASE-SOURCE-BUDGET-OBSTRUCTION" / "runs" / "2026-08-22-lean-r188-jensen-defect-telescope-crosscheck" / "integrated.json")
    ...
    check("identity", manifest["audit_id"] == "A13-JENSEN-DEFECT-TELESCOPE-CROSSCHECK", manifest["audit_id"], "A13-JENSEN-DEFECT-TELESCOPE-CROSSCHECK")
    ...
    with tempfile.TemporaryDirectory(prefix="r188-crosscheck-") as directory:
    ...
    check("acceptance predicate", accepts(primary["derived"], manifest), primary["derived"], "signed Jensen-defect telescope")

    mutations = []
    for index, label in enumerate(manifest["hostile_mutations"]):
        mutated = copy.deepcopy(primary["derived"])
        if index == 0:
            mutated["j0"] = "1"
        elif index == 1:
            mutated["dH1_plus"] = "0"
        elif index == 2:
            mutated["dH2_all_zero"] = False
        elif index == 3:
            mutated["signed_endpoint_mean"] = "4/5"
        elif index == 4:
            mutated["absolute_first_defect_mean"] = "0"
            mutated["absolute_payment_strict"] = False
        elif index == 5:
            mutated["production_map_identified"] = True
        elif index == 6:
    ...
    if not args.staged:
        ...
        check("R-188 authority", results.count("R-188") >= 2 and "Jensen-defect telescope" in results, results.count("R-188"), ">=2")
        check("strategy index authority", "R-188" in (REPO / "strategy/INDEX.md").read_text(encoding="utf-8"), True, True)
        expected_counts = integration["expected_counts"]
        summary = json.loads((REPO / "verification" / "catalog-summary.json").read_text(encoding="utf-8"))
        catalog_actual = summary.get("total")
        bootstrap_catalog = (not args.output.exists()) and catalog_actual == expected_counts["catalog"] - 1
        check("catalog count", catalog_actual == expected_counts["catalog"] or bootstrap_catalog, catalog_actual, expected_counts["catalog"])
        check("result count", len(re.findall(r"^### R-\d+\b", results, re.MULTILINE)) == expected_counts["results"], len(re.findall(r"^### R-\d+\b", results, re.MULTILINE)), expected_counts["results"])
        check("exploration count", len([line for line in (REPO / "explorations/log.jsonl").read_text(encoding="utf-8").splitlines() if line.strip()]) == expected_counts["explorations"], expected_counts["explorations"], expected_counts["explorations"])
    ...
    print(f"INTEGRATED R-188 LEAN PASS {len(rows)}/{len(rows)}")
    return 0
