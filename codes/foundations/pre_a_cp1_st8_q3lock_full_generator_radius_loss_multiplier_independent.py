#!/usr/bin/env python3
"""Independent Fraction/scalar audit for EXP-001116."""
...
def run() -> dict[str, Any]:
    ...
    audit.check("delta fixture", delta == F(1, 10) and delta > 0, delta, "1/10", "fixture")
    ...
    for n in orders:
        critical_y = F(n) / delta
        exact_max_without_e = critical_y**n
        multiplier_rows.append({"n": n, "critical_y": critical_y, "max_value": f"({critical_y}/e)^{n}", "max_without_e_power": exact_max_without_e})
        audit.check(f"critical point n={n}", critical_y == F(10 * n), critical_y, 10 * n, "radius-loss")
        audit.check(f"multiplier formula n={n}", exact_max_without_e == F(10 * n) ** n, exact_max_without_e, (10 * n) ** n, "radius-loss")
...
def main() -> int:
    ...
    print(f"INDEPENDENT FULL-GENERATOR-RADIUS-LOSS PASS {payload['passed']}/{payload['total']}")
    return 0
