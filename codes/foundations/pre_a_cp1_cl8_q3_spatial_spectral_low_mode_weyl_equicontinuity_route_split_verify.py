#!/usr/bin/env python3
"""Integrated verifier for the spatial-spectral Q3 low-mode Weyl checkpoint."""
...
PARENT_EXPLORATION_ID = "EXP-000770"
...
def summary(stdout: str) -> tuple[int, int]:
...
def modules(path: Path) -> set[str]:
    ...
    direct |= {node.module or "" for node in ast.walk(tree) if isinstance(node, ast.ImportFrom)}
    return direct
...
def build_payload() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="tect-q3-low-mode-") as directory:
        primary, primary_out = run_child(PRIMARY, Path(directory) / "primary.json")
        independent, independent_out = run_child(INDEPENDENT, Path(directory) / "independent.json")
    child_summaries = {"primary": summary(primary_out), "independent": summary(independent_out)}
    for label, child in (("primary", primary), ("independent", independent)):
        audit.check(f"{label} all pass", child_summaries[label][0] == child_summaries[label][1], child_summaries[label], "all pass", "children")
    ...
    imported = modules(INDEPENDENT)
    audit.check("independent no primary import", PRIMARY.stem not in imported, sorted(imported), f"not {PRIMARY.stem}", "independence")
    audit.check("independent stdlib only", not ({"sympy", "numpy", "scipy"} & imported), sorted(imported), "no numeric import", "independence")
    audit.check("source diversity", sha256(PRIMARY) != sha256(INDEPENDENT), sha256(PRIMARY), sha256(INDEPENDENT), "independence")

    primary_names = {row["name"] for row in primary["assertions"]}
    independent_names = {row["name"] for row in independent["assertions"]}
    for name in ("Q3 force coefficient sentinel", "Wick derivative commutes", "Wick cubic contraction", "restored virial commutator", "Mehler virial fixture", "seam curvature equals momentum variance", "hostile oscillator momentum diverges", "scope firewall below_empty_space_comparison"):
        audit.check(f"primary coverage {name}", name in primary_names, name, "present", "coverage")
    for name in ("independent edge derivative coefficients", "independent Wick differentiation", "independent Wick cubic norm", "independent HQ commutator sign", "independent HP commutator sign", "independent harmonic virial", "independent hostile P failure", "independent scope firewall below_empty_space_comparison"):
        audit.check(f"independent coverage {name}", name in independent_names, name, "present", "coverage")
    audit.check("cross cube edges", primary["derived"]["edges"] == independent["derived"]["edges"], primary["derived"]["edges"], independent["derived"]["edges"], "cross")
    audit.check("cross Q3 degrees", primary["derived"]["degrees"] == independent["derived"]["degrees"] == [3] * 8, primary["derived"]["degrees"], [3] * 8, "cross")
    audit.check("distinct covariance fixtures", primary["derived"]["covariance_L3"] != independent["derived"]["covariance_L3"], primary["derived"]["covariance_L3"], independent["derived"]["covariance_L3"], "cross")
    audit.check("cross hostile direction", primary["derived"]["hostile"] != independent["derived"]["hostile"], "distinct", "distinct", "cross")
    ...
    for phrase in ("renormalized Q3 force", "virial identity", "regular Weyl clusters", "full-sequence seam", "energy below empty space", "Pre-A"):
    ...
    audit.check("exploration claim nonbearing", exploration["formal_refs"].get("results", []) == [], exploration["formal_refs"], "no result card", "records")
    audit.check("exploration parent", any(item.get("id") == PARENT_EXPLORATION_ID and item.get("relation") == "continues" for item in exploration.get("related", [])), exploration.get("related"), PARENT_EXPLORATION_ID, "records")
    ...
    return {
        ...
        "child_summaries": {key: {"passed": value[0], "total": value[1]} for key, value in child_summaries.items()},
