#!/usr/bin/env python3
"""Non-importing NumPy control for the R-435 d=17 finite row."""
...
DEFAULT_OUTPUT = REPO / "claims/C6-SPACETIME-SIGNATURE/runs/2026-08-30-independent-original_source_interval_d17/independent.json"
...
def run(output: Path = DEFAULT_OUTPUT) -> dict[str, Any]:
    ...
    check("manifest identity", manifest["result_id"] == "R-435" and manifest["exploration_id"] == "EXP-001280" and not manifest["claim_bearing"], [manifest["result_id"], manifest["exploration_id"], manifest["claim_bearing"]], "R-435/EXP-001280/false", "provenance")
    ...
    check("residual compression", np.isfinite(gap) and compressed.shape == (n - 2, n - 2), [gap, compressed.shape], "finite 15x15 compression", "residual")
    ...
    payload = {
        "schema": "tect/pre-a-r435-independent/1.0",
    ...
    print(f"R-435 INDEPENDENT {payload['verdict']} {len(checks)}/{len(checks)} gap={gap:.15g}", flush=True)
    return payload
...
def main() -> int:
    ...
    if args.self_test:
        ...
        print("R-435 INDEPENDENT SELFTEST: PASS", flush=True)
    return 0
