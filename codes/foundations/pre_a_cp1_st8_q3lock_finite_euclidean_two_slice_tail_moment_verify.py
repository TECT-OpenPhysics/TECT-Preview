#!/usr/bin/env python3
"""Integrated verifier for EXP-001189."""
...
LEAN = REPO / "verification/lean/Tect/R348.lean"
...
def lean_run() -> dict[str, Any]:
    command = "lake env lean Tect/R348.lean"
    lake = lake_path()
    if lake is None:
        return {"status": "UNAVAILABLE", "command": command, "output": "pinned lake executable not found"}
    process = subprocess.run([str(lake), "env", "lean", "Tect/R348.lean"], cwd=LEAN_ROOT, text=True, encoding="utf-8", capture_output=True, check=False)
...
def compare_rows(primary: list[dict[str, Any]], independent: list[dict[str, Any]], tolerance: float) -> list[dict[str, Any]]:
    ...
    keys = ("volume", "oscillator_dimension", "beta", "max_holder_moment", "max_two_slice", "max_equal_time_fourth_moment", "max_operator_fourth_bound", "max_identity_residual", "max_spectral_residual", "min_amgm_slack", "min_operator_slack")
...
def run() -> dict[str, Any]:
    ...
    check("identity", manifest["exploration_id"] == "EXP-001189" and manifest["task_id"] == "T-054", [manifest["exploration_id"], manifest["task_id"]], "EXP-001189/T-054")
    check("fixture lineage", source_manifest["exploration_id"] == "EXP-001188" and source_manifest["task_id"] == "T-054", [source_manifest["exploration_id"], source_manifest["task_id"]], "EXP-001188/T-054")
    ...
    with tempfile.TemporaryDirectory(prefix="euclidean-two-slice-tail-moment-") as temporary:
        ...
        for key in ("finite_tail_fourth_leg_identity_closed", "finite_euclidean_two_slice_identity_closed", "finite_spectral_two_slice_crosscheck_closed", "finite_equal_time_coercive_envelope_closed"):
    ...
    check("scope firewall", scope["finite_tail_fourth_leg_identity_closed"] and scope["finite_euclidean_two_slice_identity_closed"] and scope["finite_spectral_two_slice_crosscheck_closed"] and scope["finite_equal_time_coercive_envelope_closed"] and not scope["tail_fourth_moment_uniform_closed"] and not scope["local_coercive_common_core_closed"] and not scope["common_alpha_closed"] and not scope["pre_a_closed"], scope, "finite bridge; QFT gates open")
    lean = lean_run()
    check("Lean compile", lean["status"] == "PASS", lean, "PASS")
    return {"schema": "tect/foundation-audit/1.0", "run_kind": "integrated", "audit_id": "PA-CP1-ST8-Q3LOCK-FINITE-EUCLIDEAN-TWO-SLICE-TAIL-MOMENT", "claim_id": manifest["claim_ids"][0], "task_id": manifest["task_id"], "exploration_id": manifest["exploration_id"], "verdict": "PASS", "assertion_count": len(rows), "assertions": rows, "lean": lean, "boundary": scope, "derived": {"primary_summary_rows": p.get("summary_rows", []), "independent_summary_rows": i.get("summary_rows", []), "finite_tail_fourth_leg_identity_closed": True, "finite_euclidean_two_slice_identity_closed": True, "finite_spectral_two_slice_crosscheck_closed": True, "finite_equal_time_coercive_envelope_closed": True, "tail_fourth_moment_uniform_closed": False, "local_coercive_common_core_closed": False, "common_alpha_promoted": False, "qft_promoted": False}, "recorded_at": datetime.now(timezone.utc).isoformat().replace("+00:00", "Z"), "provenance": {"primary_sha256": primary_hash, "independent_sha256": independent_hash, "manifest_sha256": sha256(MANIFEST), "lean_sha256": sha256(LEAN)}}
...
def main() -> int:
    ...
    print(f"INTEGRATED FINITE-EUCLIDEAN-TWO-SLICE-TAIL-MOMENT PASS {payload['assertion_count']}/{payload['assertion_count']}; Lean={payload['lean']['status']}")
    return 0
