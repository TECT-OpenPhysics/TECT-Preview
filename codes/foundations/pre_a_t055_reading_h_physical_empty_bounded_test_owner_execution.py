...
MANIFEST = REPO / "strategy/pre-a-t055-reading-h-physical-empty-bounded-test-owner-execution-manifest.json"
DEFAULT_OUTPUT = REPO / "claims/C6-SPACETIME-SIGNATURE/runs/2026-08-30-primary-pre-a-t055-reading-h-physical-empty-bounded-test-owner-execution/primary.json"
...
def run(output: Path = DEFAULT_OUTPUT) -> dict[str, Any]:
    ...
    check("manifest identity", [manifest["result_id"], manifest["exploration_id"], manifest["claim_bearing"], manifest["status"]] == ["R-441", "EXP-001286", False, "BLOCKED"], [manifest["result_id"], manifest["exploration_id"], manifest["claim_bearing"], manifest["status"]], ["R-441", "EXP-001286", False, "BLOCKED"], "provenance")
    check("test card identity", manifest["test_card_id"] == "PA-T055-READING-H-PHYSICAL-EMPTY-BOUNDED-TEST-OWNER-EXECUTION", manifest["test_card_id"], "PA-T055-READING-H-PHYSICAL-EMPTY-BOUNDED-TEST-OWNER-EXECUTION", "provenance")
    ...
    print(f"R-441 PRIMARY BLOCKED_NOT_EVALUATED {len(checks)}/{len(checks)}; E preregistered, not admitted", flush=True)
    return payload
...
def main() -> int:
    ...
    if args.self_test:
        ...
        print("R-441 PRIMARY SELFTEST: PASS", flush=True)
    return 0
