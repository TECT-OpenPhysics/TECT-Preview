...
def row(volume: int, cutoff: int, beta: float, fixture: dict[str, Any], source_manifest: dict[str, Any], exponent: float, energy_windows: list[float], max_union_size: int, tolerance: float, positivity_tolerance: float, orientation_tolerance: float, norm_floor: float) -> dict[str, Any]:
    ...
    for union in selected_unions:
        ...
        orientation_raw_residual=max(orientation_raw_residual,base.operator_norm(forward+reverse)); orientation_gibbs_difference=max(orientation_gibbs_difference,abs(signed_gibbs-reverse_gibbs)); orientation_weighted_difference=max(orientation_weighted_difference,abs(signed_weighted-reverse_weighted))
        for key,value in (("signed_raw_sum",signed_raw),("signed_gibbs_sum",signed_gibbs),("signed_weighted_sum",signed_weighted),("absolute_raw_sum",absolute_raw),("absolute_gibbs_sum",absolute_gibbs),("absolute_weighted_sum",absolute_weighted)): union_sum[key]+=value
        group_windows: dict[str,dict[str,Any]]={}
        for threshold in energy_windows:
            key=format(threshold,"g"); selector=shifted<=threshold+tolerance; projector=energy_vectors[:,selector]@energy_vectors[:,selector].conj().T
    ...
    for key in ("signed_raw_sum","signed_gibbs_sum","signed_weighted_sum","absolute_raw_sum","absolute_gibbs_sum","absolute_weighted_sum"): union_sum[key+"_per_union_count"] = union_sum[key] / len(selected_unions)
...
def run() -> dict[str, Any]:
    ...
    check("graph volumes",volumes==[2,4,6] and all(base.q3.graph_edges(volume) for volume in volumes),volumes,"registered Q3 volumes","fixture")
    ...
    summaries=[]; threshold=float(audit["tail_stability_ratio_threshold"])
    keys=sorted({(int(row["volume"]),float(row["beta"]),tuple(group["union"])) for row in rows for group in row["groups"]})
    for volume,beta,union in keys:
        for energy in energy_windows:
            key=format(energy,"g"); selected=[]
            for item in rows:
                if int(item["volume"])!=volume or float(item["beta"])!=beta: continue
            ...
            signed=[float(item["signed_weighted_per_site"]) for item in tail]; conditional=[float(item["conditional_signed_weighted_per_site"]) for item in tail]
            ratio=max(signed)/max(min(signed),np.finfo(float).tiny); conditional_ratio=max(conditional)/max(min(conditional),np.finfo(float).tiny)
            ...
            summaries.append(summary); check(f"V={volume} beta={beta} U={union} E={energy} summary",len(selected)==len(cutoffs) and len(tail)==sum(int(cutoff>=tail_cutoff_start) for cutoff in cutoffs) and np.isfinite(ratio) and np.isfinite(conditional_ratio),summary,"finite union summary","scaling")
...
def main() -> int:
    ...
    print(f"PRIMARY SPATIAL-SOURCE-SPECTRAL-WINDOW PASS {payload['passed']}/{payload['assertion_count']} rows={payload['derived']['row_count']} summaries={payload['derived']['summary_count']}"); return 0
