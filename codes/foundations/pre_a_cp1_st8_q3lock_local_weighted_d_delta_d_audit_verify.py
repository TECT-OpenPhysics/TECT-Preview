#!/usr/bin/env python3
"""Integrated verifier for EXP-001147 actual-Q3 local-weighted D(t) rows."""
...
LEAN = REPO / "verification/lean/Tect/R317.lean"
...
def lean_run() -> dict[str, Any]:
    command = "lake env lean Tect/R317.lean"
    lake = lake_path()
    if lake is None:
        return {"status": "UNAVAILABLE", "command": command, "output": "pinned lake executable not found"}
    process = subprocess.run([str(lake), "env", "lean", "Tect/R317.lean"], cwd=LEAN_ROOT, text=True, encoding="utf-8", capture_output=True, check=False)
...
def compare_summaries(primary: list[dict[str, Any]], independent: list[dict[str, Any]], tolerance: float) -> list[dict[str, Any]]:
    keys = ("volume", "beta", "max_local_four_leg_sum", "max_full_four_leg_sum", "max_local_left_D", "max_local_right_D", "max_full_left_D", "max_full_right_D")
...
def run() -> dict[str, Any]:
    ...
    check("identity", manifest["exploration_id"] == "EXP-001147" and manifest["task_id"] == "T-054", [manifest["exploration_id"], manifest["task_id"]], "EXP-001147/T-054")
    ...
    with tempfile.TemporaryDirectory(prefix="local-weighted-d-delta-d-audit-") as temporary:
        ...
        expected_rows = len(fixture["volume_values"]) * len(fixture["beta_values"]) * len(fixture["radius_values"]) * len(fixture["time_values"]) * len(fixture["orientation_values"])
        check("row count agreement", p.get("row_count") == i.get("row_count") == expected_rows, [p.get("row_count"), i.get("row_count")], expected_rows)
        mismatches = compare_summaries(p.get("summary_rows", []), i.get("summary_rows", []), tolerance)
        check("trace summary agreement", not mismatches, mismatches, "within tolerance")
        check("finite local/full rows", p.get("local_four_leg_rows_closed") is True and i.get("local_four_leg_rows_closed") is True and p.get("full_global_baseline_closed") is True and i.get("full_global_baseline_closed") is True, [p, i], "finite local and global four-leg rows")
        check("uniformity firewall", p.get("local_weight_volume_uniformity_proved") is False and i.get("local_weight_volume_uniformity_proved") is False and p.get("global_weight_volume_uniformity_proved") is False and i.get("global_weight_volume_uniformity_proved") is False, [p, i], False)

    check("scope firewall", scope["actual_q3_local_four_leg_rows_closed"] and scope["independent_trace_reconstruction_closed"] and not scope["local_weight_volume_uniformity_proved"] and not scope["direct_d_delta_d_cauchy_closed"] and not scope["pre_a_closed"], scope, "finite local-weight rows; QFT gates open")
    lean = lean_run()
    ...
    return {
        ...
        "derived": {
            ...
            "local_four_leg_rows_promoted": True,
            "full_global_baseline_promoted": True,
            "local_weight_uniformity_promoted": False,
            "qft_promoted": False,
        },
...
def main() -> int:
    ...
    print(f"INTEGRATED LOCAL-WEIGHTED-D-DELTA-D-AUDIT PASS {payload['assertion_count']}/{payload['assertion_count']}; Lean={payload['lean']['status']}")
    return 0
