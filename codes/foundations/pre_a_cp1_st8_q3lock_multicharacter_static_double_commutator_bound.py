#!/usr/bin/env python3
"""Primary exact audit for EXP-001071."""
...
def run() -> dict[str, Any]:
    ...
    amplitudes = tuple(rational(value) for value in fixture["amplitudes"])
    ...
    shifted_bounds = tuple(sp.factor(32 * chi**2 * m5 + amplitude**4 / 2) for amplitude in amplitudes)
    kinetic_bound = sp.factor(2 * support_size**3 * sum((amplitude / (chi * hbar))**4 * shifted for amplitude, shifted in zip(amplitudes, shifted_bounds)))
    l1_amplitude = sp.factor(sum(abs(amplitude) for amplitude in amplitudes))
    force_sqrt_ceiling = rational(fixture["force_sqrt_ceiling"])
    force_fourth = rational(static["finite_fixture"]["derived_force_fourth_moment"])
    check("force fourth ceiling input", force_fourth < force_sqrt_ceiling**2, force_fourth, f"<{force_sqrt_ceiling**2}", "force")
    force_upper = sp.factor(2 * (l1_amplitude / (chi * hbar))**2 * force_sqrt_ceiling)
    ...
    check("positive parameters", chi > 0 and hbar > 0 and m5 >= 0, [chi, hbar, m5], "positive", "parameters")
    check("shifted fourth bounds", shifted_bounds == tuple(rational(value) for value in fixture["derived_shifted_fourth_bounds"]), shifted_bounds, fixture["derived_shifted_fourth_bounds"], "kinetic")
    check("l1 amplitude", l1_amplitude == rational(fixture["derived_l1_amplitude"]), l1_amplitude, fixture["derived_l1_amplitude"], "force")
    check("kinetic bound", kinetic_bound == rational(fixture["derived_kinetic_squared_norm"]), kinetic_bound, fixture["derived_kinetic_squared_norm"], "kinetic")
    check("kinetic ceiling", kinetic_bound < kinetic_sqrt_ceiling**2, kinetic_bound, f"<{kinetic_sqrt_ceiling**2}", "kinetic")
    check("force upper", force_upper == rational(fixture["derived_force_squared_upper"]), force_upper, fixture["derived_force_squared_upper"], "force")
    check("force ceiling", force_upper < force_norm_sqrt_ceiling**2, force_upper, f"<{force_norm_sqrt_ceiling**2}", "force")
    check("full triangle upper", full_upper == rational(fixture["derived_full_squared_norm_upper"]), full_upper, fixture["derived_full_squared_norm_upper"], "triangle")
    ...
    passed = len(rows)
    return {"schema": "tect/foundation-audit/1.0", "run_kind": "primary", "audit_id": "PA-CP1-ST8-Q3LOCK-MULTICHARACTER-STATIC-DOUBLE-COMMUTATOR-BOUND", "exploration_id": manifest["exploration_id"], "task_id": manifest["task_id"], "verdict": "PASS", "passed": passed, "assertion_count": passed, "assertions": rows, "derived": derived, "boundary": scope}
...
def main() -> int:
    ...
    print(f"PRIMARY MULTICHARACTER-STATIC-DOUBLE-COMMUTATOR PASS {payload['passed']}/{payload['assertion_count']}")
    return 0
