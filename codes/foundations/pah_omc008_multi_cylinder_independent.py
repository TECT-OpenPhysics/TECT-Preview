#!/usr/bin/env python3
"""Non-importing independent replay for PAH-OMC-008.
...
DEFAULT_OUTPUT = ROOT / (
    ...
    "2026-09-04-pah-omc008-multi-cylinder/independent.json"
)
...
def holonomies(state: dict[str, Any]) -> tuple[int, int]:
    h0 = z2(state["links"]["h00"]) * z2(state["links"]["v1"]) * z2(state["links"]["d0"])
    h1 = z2(state["links"]["d0"]) * z2(state["links"]["h01"]) * z2(state["links"]["v0"])
    return h0, h1


def observable(state: dict[str, Any]) -> tuple[int, int, int, int]:
    return state["ell"][(0, 0)], state["ell"][(1, 1)], *holonomies(state)
    ...
    for name, left, right in carrier(level)["edges"]:
        ...
        for source, target in ((left, right), (right, left)):
            ...
            rows.append({
                ...
                "delta_ell_a": after_observable[0] - before_observable[0], "delta_ell_d": after_observable[1] - before_observable[1],
                "delta_H_0": after_observable[2] - before_observable[2], "delta_H_1": after_observable[3] - before_observable[3],
...
def support(level: int) -> dict[str, list[str]]:
    ...
    link_h01 = link_move(state, "h01", 1)
    ...
    return {
        ...
        "link_h01_plus": sorted(label for label, value in before.items() if value != terms(level, link_h01)[label]),
    }
...
def gauge_check() -> dict[str, Any]:
    ...
    for links in itertools.product(range(K), repeat=LINK_BITS):
        ...
        for gauges in itertools.product(range(K), repeat=APERTURE_BITS):
            ...
            if holonomies(base) != holonomies(transformed) or observable(base) != observable(transformed):
...
def main() -> int:
    ...
    hashes = {"PAH-001": sha(SOURCE), "PAH-OMC-004": sha(GEOMETRY), "PAH-OMC-005": sha(PREDECESSOR), "PAH-OMC-006": sha(PARENT), "PAH-OMC-007": sha(ROOT / "strategy/pa-hyp/PAH-OMC-007-joint-holonomy-cylinder-v1.json"), "PAH-OMC-008": sha(CONTRACT), "PAH-OMC-008-MANIFEST": sha(MANIFEST)}
    ...
    level2_edges = carrier(2)["edges"]
    placement_degree_sum = sum(sum(1 for _name, left, right in level2_edges if RADIAL_ANCHORS.intersection((left, right)) and vertex in (left, right)) for vertex in PATCH_VERTICES)
    expected_link = len(domain) * len(PATCH_LINK_EDGES) * 2
    expected_radial = (len(domain) // RADIAL_PLACEMENTS) * placement_degree_sum
    ...
    check("joint-increment-support", all(any(root[key] != 0 for key in ("delta_ell_a", "delta_ell_d", "delta_H_0", "delta_H_1")) for record in rows_2["bounded_samples"] for root in record["roots"]))
    ...
    check("channel-multiplicity", all(sum(root["family"] == "LK" for root in record["roots"]) == len(PATCH_LINK_EDGES) * 2 for record in rows_2["bounded_samples"]))
    ...
    payload: dict[str, Any] = {
        "schema": "tect/pah-omc008-multi-cylinder-independent/1.0",
        ...
        "fixture_dimensions": {"aperture_bits": APERTURE_BITS, "link_bits": LINK_BITS, "phase_bits": PHASE_BITS, "radial_placements": RADIAL_PLACEMENTS, "patch_link_edges": len(PATCH_LINK_EDGES), "radial_degree_sum": placement_degree_sum, "state_count": len(domain), "root_count": rows_2["root_rows"]},
        ...
        "reproduction": {"command": "python codes/foundations/pah_omc008_multi_cylinder_independent.py --output claims/C6-SPACETIME-SIGNATURE/runs/2026-09-04-pah-omc008-multi-cylinder/independent.json"},
