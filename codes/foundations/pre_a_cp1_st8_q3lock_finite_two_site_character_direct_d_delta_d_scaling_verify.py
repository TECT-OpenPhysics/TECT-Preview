#!/usr/bin/env python3
"""Integrated verifier for EXP-001086."""
...
LEAN = REPO / "verification/lean/Tect/R268.lean"
...
def lean_run() -> dict[str, Any]:
    command = "lake env lean Tect/R268.lean"
    lake = lake_path()
    if lake is None:
        return {"status": "UNAVAILABLE", "command": command, "output": "pinned lake executable not found"}
    process = subprocess.run([str(lake), "env", "lean", "Tect/R268.lean"], cwd=LEAN_ROOT, text=True, encoding="utf-8", capture_output=True, check=False)
...
def run() -> dict[str, Any]:
    ...
    check("identity", manifest["exploration_id"] == "EXP-001086" and manifest["task_id"] == "T-054", [manifest["exploration_id"], manifest["task_id"]], "EXP-001086/T-054")
    check("claim nonbearing", manifest["claim_bearing"] is False, manifest["claim_bearing"], False)
    source = LEAN.read_text(encoding="utf-8") if LEAN.is_file() else ""
    markers = ["direct_fixture", "modular_fixture", "two_site_support_fixture", "orientation_sum_fixture", "zero_tail_fixture", "scope_fixture"]
    ...
    with tempfile.TemporaryDirectory(prefix="finite-two-site-direct-d-") as temporary:
        ...
        check("volume sequence", [item.get("volume") for item in p_rows] == [item.get("volume") for item in i_rows] == fixture["volume_values"], [[item.get("volume") for item in p_rows], [item.get("volume") for item in i_rows]], fixture["volume_values"])
        ...
        p_summary, i_summary = primary["derived"]["summary"], independent["derived"]["summary"]
        check("summary agreement", all(close(a[key], b[key]) for a, b in zip(p_summary, i_summary) for key in ("max_D_norm_sum", "max_delta_D_norm_sum")), [p_summary, i_summary], "within 1e-7 relative")
        d_growth = p_summary[-1]["max_delta_D_norm_sum"] / p_summary[0]["max_delta_D_norm_sum"]
        direct_growth = p_summary[-1]["max_D_norm_sum"] / p_summary[0]["max_D_norm_sum"]
        check("support scaling captured", d_growth > 1.0 and direct_growth > 1.0, [direct_growth, d_growth], ">1 for V=4/V=2")
...
def main() -> int:
    ...
    print(f"INTEGRATED FINITE-TWO-SITE-D-DELTA-D PASS {payload['assertion_count']}/{payload['assertion_count']}; Lean={payload['lean']['status']}")
    return 0
