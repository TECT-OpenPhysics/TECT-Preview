#!/usr/bin/env python3
"""Finite d=18 original-source interval enclosure for the Q3LOCK route.
...
POINT_CACHE = REPO / ".tmp/r436-d18-point-cache.json"
DEFAULT_OUTPUT = REPO / "claims/C6-SPACETIME-SIGNATURE/runs/2026-08-30-primary-original_source_interval_d18/primary.json"
...
def run(output: Path = DEFAULT_OUTPUT) -> dict[str, Any]:
    ...
    check(
        "manifest identity",
        manifest["result_id"] == "R-436" and manifest["exploration_id"] == "EXP-001281" and manifest["claim_bearing"] is False,
        [manifest["result_id"], manifest["exploration_id"], manifest["claim_bearing"]],
        "R-436/EXP-001281/false",
        "provenance",
    )
    ...
    payload = {
        "schema": "tect/pre-a-r436-primary/1.0",
    ...
    print(f"R-436 PRIMARY ORIGINAL_SOURCE_INTERVAL_CERTIFIED {len(checks)}/{len(checks)} d={dimension} row=unconditional bracket=[{derived['rayleigh_interval'][0]},{derived['rayleigh_interval'][1]}]", flush=True)
    return payload
...
def main() -> int:
    ...
    if args.self_test:
        ...
        print("R-436 PRIMARY SELFTEST: PASS", flush=True)
    return 0
