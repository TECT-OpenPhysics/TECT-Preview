#!/usr/bin/env python3
"""Exact finite joint-cylinder replay for PAH-OMC-007.

The unchanged PAH-001 functional is evaluated on the PAH-OMC-004 diagonal
strip.  The audited cylinder is the pair (ell_a,H_0), where H_0 is the
closed Z_2 holonomy of the first split triangle.  Every Q=1 patch state and
every retained radial/link root is enumerated.  Root differences are reduced
to the exact affected terms of the displayed functional; bounded samples
also compare that reduction with a complete energy recomputation.  The
canonical digest is streamed so the run artefact stays small.
"""
...
DEFAULT_OUTPUT = ROOT / (
    ...
    "2026-09-04-pah-omc007-joint-holonomy-cylinder/primary.json"
)
...
def triangle_holonomy(state: dict[str, Any]) -> int:
    return (
        sign_z2(state["links"]["h00"])
        * sign_z2(state["links"]["v1"])
        * sign_z2(state["links"]["d0"])
    )


def joint_observable(state: dict[str, Any]) -> tuple[int, int]:
    anchor = PATCH_VERTICES[0]
    return state["ell"][anchor], triangle_holonomy(state)
            ...
            rows.append(
                {
                    ...
                    "delta_H_0": after_obs[1] - before_obs[1],
...
def gauge_audit() -> dict[str, Any]:
    ...
    for link_bits in link_configs:
        ...
        for gauge_bits in gauge_configs:
            ...
            if triangle_holonomy(base_state) != triangle_holonomy(transformed_state):
                return {"passed": False, "checked": checked, "counterexample": [base, gauge_bits]}
            if joint_observable(base_state)[0] != joint_observable(transformed_state)[0]:
                return {"passed": False, "checked": checked, "counterexample": [base, gauge_bits]}
            checked += 1
...
def run(output: Path = DEFAULT_OUTPUT) -> dict[str, Any]:
    ...
    hashes = {
        ...
        "PAH-OMC-007": digest(CONTRACT),
        "PAH-OMC-007-MANIFEST": digest(MANIFEST),
    }
    ...
    check("joint-observable-declaration", "ell_a" in contract.get("exact_scope", {}).get("joint_observable", "") and "H_0" in contract.get("exact_scope", {}).get("joint_observable", ""))

    rows_2 = stream_records(2, states)
    rows_3 = stream_records(3, states)
    expected_link_rows = len(states) * len(TRIANGLE_EDGES) * 2
    expected_radial_rows = (len(states) // RADIAL_PLACEMENTS) * 6
    ...
    check("link-channel-multiplicity", all(sum(root["family"] == "LK" for root in record["roots"]) == len(TRIANGLE_EDGES) * 2 for record in rows_2["bounded_samples"]))
    check("joint-increments", all(root["delta_ell_a"] != 0 or root["delta_H_0"] != 0 for record in rows_2["bounded_samples"] for root in record["roots"]))
    check("radial-holonomy-zero", all(root["delta_H_0"] == 0 for record in rows_2["bounded_samples"] for root in record["roots"] if root["family"] == "TR"))
    check("link-density-zero", all(root["delta_ell_a"] == 0 for record in rows_2["bounded_samples"] for root in record["roots"] if root["family"] == "LK"))
    ...
    check("triangle-edge-set", TRIANGLE_EDGES == {"h00", "v1", "d0"})
    ...
    payload: dict[str, Any] = {
        "schema": "tect/pah-omc007-joint-holonomy-cylinder-primary/1.0",
        "run_kind": "primary",
        "audit_id": AUDIT_ID,
        ...
        "scope": {
            ...
            "model": "PAH-001 + PAH-OMC-007 researcher successor",
        ...
        "row_identity": {
            ...
            "exact_tuple": ["patch_state", "family", "edge", "sigma/source/target", "delta_F", "mobility_square", "delta_ell_a", "delta_H_0", "before_observable", "after_observable", "rate_exponent"],
            "observable_basis": ["ell_a", "H_0", "bounded functions of (ell_a,H_0)"],
        },
        ...
        "reproduction": {
            "command": "python codes/foundations/pah_omc007_joint_holonomy_cylinder.py --output claims/C6-SPACETIME-SIGNATURE/runs/2026-09-04-pah-omc007-joint-holonomy-cylinder/primary.json",
            "independent_command": "python codes/foundations/pah_omc007_joint_holonomy_cylinder_independent.py --output claims/C6-SPACETIME-SIGNATURE/runs/2026-09-04-pah-omc007-joint-holonomy-cylinder/independent.json",
        },
