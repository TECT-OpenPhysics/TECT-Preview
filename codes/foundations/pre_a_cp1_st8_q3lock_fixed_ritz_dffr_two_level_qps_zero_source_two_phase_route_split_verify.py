#!/usr/bin/env python3
"""Integrated verifier for the R-167 v3.3 fixed-Ritz DFFR package."""
...
INDEPENDENT = REPO / "codes/foundations/pre_a_cp1_st8_q3lock_fixed_ritz_dffr_two_level_qps_zero_source_two_phase_route_split_independent.py"
...
EVENT_TITLE = "R-167 v3.3 fixed-Ritz DFFR two-level QPS zero-source two-phase theorem"
EVENT_HEADER = f"[{EVENT_TITLE}] - 2026-08-13"
EVENT_ID = "20260813-r-167-v3-3-fixed-ritz-dffr-two-level-qps-zero-s"
EVENT_KEYWORDS = [
    "DFFR-two-level-QPS",
    "EXP-000837",
    "R-167-v3.3",
...
def execute_child(path: Path, staged: bool) -> dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix="tect-v33-") as temporary:
...
def build_payload(staged: bool) -> dict[str, Any]:
    ...
    audit.check(
        ...
        ("exact schema", SLUG, "R-167 v3.3", "2026-08-13"),
        "manifest",
    )
    audit.check(
        ...
        and manifest["reused_negative_ids"] == EXPECTED_REUSED_NEGATIVE
        ...
        (EXPECTED_CLOSED, [], EXPECTED_REUSED_NEGATIVE, EXPECTED_HISTORICAL_OPEN, EXPECTED_PARENTS),
        "manifest",
    )
    expected_paths = [
    ...
    audit.check(
        ...
        == {
            ...
            "pdf_issued": False,
            "workflow": "Proof-first manifest, certificate and three executable verifiers only. No v3.3 PDF is issued.",
        },
        manifest["checkpoint_synthesis"],
    ...
    for label, payload, owner, expected_total in (
        ("primary", primary, PRIMARY, 19 if staged else 20),
        ("independent", independent, INDEPENDENT, 20 if staged else 21),
    ):
    ...
    for group in (
        ...
        audit.check(
            f"cross exact {group}",
            primary["derived"][group] == independent["derived"][group] == oracle,
            (primary["derived"][group], independent["derived"][group]),
            oracle,
            "cross",
        )
    ...
    masked_literals = ('"gamma": "N^2"', '"high_penalty": "(N^2-1)/16"')
    audit.check(
        "derived-display hardcode masking firewall",
    ...
    theorem_tokens = (
        "DFFR Theorem 5.2",
        "strict DFFR two-level Peierls condition",
        "Q_(X,N)(lambda)",
    ...
    if not staged:
        ...
        records = [record for record in exploration_records if record.get("id") == "EXP-000837"]
        ...
        exploration_ok = (
            ...
            and records[0].get("related") == [{"id": "EXP-000836", "relation": "continues"}]
            and records[0].get("gate_ids") == expected_gate_links
            and records[0].get("formal_refs", {}).get("negatives") == EXPECTED_REUSED_NEGATIVE
        ...
        audit.check("EXP-000837 exact continuation", exploration_ok, records, "one exact proof-first record", "formal")

        child_section = markdown_h3_section(texts["gates"], EXPECTED_CLOSED[0])
        child_ok = bool(child_section) and all(
            token in child_section
            for token in (
                "**Status:** CLOSED",
                "EXP-000837 / R-167 v3.3",
                "fixed Ritz",
                "DFFR",
                "zero source",
                "not uniform in `M`",
        ...
        audit.check("one exact fixed-Ritz CLOSED child", child_ok, child_section[:500], "unique scoped CLOSED section", "formal")
        ...
        audit.check(
            ...
            all(
                ...
                and "EXP-000837 / R-167 v3.3" in section
        ...
        negative_section = markdown_result_section(texts["negatives"], EXPECTED_REUSED_NEGATIVE[0])
        negative_ok = bool(negative_section) and all(
            token in " ".join(negative_section.split())
            for token in ("theta_N/r_N=N", "common positive lower radius", "not a no-go for a future two-phase theorem")
        )
        audit.check("reused shrinking-radius boundary remains registered", negative_ok, negative_section[:500], "existing exact boundary", "formal")
        ...
        result_ok = all(
            ...
            for token in (
                ...
                "fixed-Ritz",
                "No v3.3 PDF",
                "R-167 v3.2 certificate",
                "R-167 v2.9 certificate",
                "remain the prior proof-first authority",
            )
        )
        audit.check("R-167 v3.3 current authority", result_ok, "result tokens", "present in bounded R-167 section", "formal")

        todo_data = json.loads(texts["todo"])
        todo_records = [item for item in todo_data.get("tasks", []) if item.get("id") == "T-054"]
        audit.check(
            "T-054 remains in progress with v3.3",
            len(todo_records) == 1
            and todo_records[0].get("status") == "in_progress"
            and "EXP-000837" in todo_records[0].get("note", "")
        ...
        linkage_ok = (
            "EXP-000837" in texts["roadmap"]
            and "EXP-000837" in texts["strategy"]
            and theorem_map.get("version") == "1.25.0"
            and "EXP-000837" in research.get("latest_cp1_checkpoint", "")
            and research.get("closed_v3_3_scoped_gates") == EXPECTED_CLOSED
            and research.get("closed_v3_2_scoped_gates") == EXPECTED_V32_CLOSED
        )
        audit.check(
            ...
            (
                ...
                research.get("closed_v3_2_scoped_gates"),
            ),
            ("1.25.0", EXPECTED_CLOSED, EXPECTED_V32_CLOSED),
            "formal",
        )
        ...
        event_candidates = [
            event
            for event in events
            if "EXP-000837" in event.get("claim_ids", []) or "R-167-v3.3" in event.get("keywords", [])
        ]
        event_ok = (
            len(events) == manifest["formal_integration_contract"]["event_id"] == 629
            ...
            and event_candidates[0].get("claim_ids") == ["C6-SPACETIME-SIGNATURE", "EXP-000837", "R-167"]
            and event_candidates[0].get("keywords") == sorted(EVENT_KEYWORDS)
            and event_candidates[0].get("neg_results") == []
            ...
            and all(
                ...
                for token in (
                    "DFFR Theorem 5.2",
                    "fixed complete spectral-cluster Ritz",
                    "zero source",
                    "beta-to-infinity ground-state limits",
                    "All five active parent gates remain OPEN",
                    "historical beta-infinity selection gate remains OPEN",
                    "No v3.3 PDF is issued",
                )
            )
        )
        audit.check("unique proof-first event 629", event_ok, event_candidates, "one exact no-PDF event", "formal")

        generated_ok = all("EXP-000837" in texts[key] for key in ("proof_map_md", "proof_map_json"))
    ...
    return {
        ...
        "derived": {group: primary["derived"][group] for group in manifest["exact_fixture"]}
...
def main() -> int:
    ...
    print(f"R-167 v3.3 INTEGRATED PASS {total}/{total}")
    if args.no_store:
        print("NO-STORE")
    return 0
