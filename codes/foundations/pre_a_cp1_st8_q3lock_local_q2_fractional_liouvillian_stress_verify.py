#!/usr/bin/env python3
"""Integrated primary/independent/Lean verifier for EXP-001209 / R-367."""
...
LEAN = REPO / "verification/lean/Tect/R367.lean"
...
def compile_lean() -> dict[str, Any]:
    lake = lake_path()
    command = "lake env lean Tect/R367.lean"
    if lake is None:
        return {"status": "UNAVAILABLE", "command": command, "output": "pinned lake executable not found"}
    process = subprocess.run([str(lake), "env", "lean", "Tect/R367.lean"], cwd=LEAN_ROOT, text=True, encoding="utf-8", capture_output=True, check=False)
...
def run() -> dict[str, Any]:
    ...
    check("identity", manifest["exploration_id"] == "EXP-001209" and manifest["result_id"] == "R-367" and not manifest["claim_bearing"], [manifest["exploration_id"], manifest["result_id"], manifest["claim_bearing"]], "EXP-001209/R-367/false")
    ...
    markers = ("fractional_half_envelope_stress", "scope_fixture")
    ...
    registry = json.loads(LEAN_REGISTRY.read_text(encoding="utf-8"))
    entries = [entry for entry in registry["entrypoints"] if entry["path"] == "verification/lean/Tect/R367.lean"]
    ...
    with tempfile.TemporaryDirectory(prefix="q3-fractional-stress-") as temporary:
        ...
        exact_fields = (
            "context_count", "expected_contexts", "theta", "larger_cutoff_volume_stress_checked", "fractional_liouvillian_bound_closed", "uniform_fractional_bound_proved",
            "local_modular_square_function_proved", "local_collar_bound_proved", "phase_weight_preservation_proved", "common_core_closed", "common_alpha_closed",
        ...
        numeric_fields = (
            "max_phase_identity_error", "max_fractional_envelope_violation", "max_fractional_bound_violation", "max_state_trace_bound_violation",
            "max_bond_unitary_factorization_error", "max_density_hs_norm_excess", "max_spectral_reduction_error", "maximum_finite_time_change_norm",
            "maximum_fractional_bound", "maximum_fractional_norm", "maximum_state_trace_change", "maximum_fractional_to_bound_ratio", "minimum_nonzero_fractional_norm"
        )
        ...
        open_fields = (
            "uniform_fractional_bound_proved", "local_modular_square_function_proved", "local_collar_bound_proved", "phase_weight_preservation_proved",
...
def main() -> int:
    ...
    print(f"INTEGRATED FRACTIONAL STRESS PASS {payload['assertion_count']}/{payload['assertion_count']}; Lean={payload['lean']['status']} diff={payload['derived']['max_primary_independent_numeric_difference']:.3e}")
    return 0
