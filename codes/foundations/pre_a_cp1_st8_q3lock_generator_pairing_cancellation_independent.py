#!/usr/bin/env python3
"""Independent eigenbasis reproduction for EXP-001140."""
...
def run() -> dict[str, Any]:
    ...
    check("provenance", manifest["exploration_id"] == "EXP-001140" and manifest["claim_bearing"] is False, manifest["exploration_id"], "EXP-001140/nonbearing", "provenance")
    check("geometry", q3.graph_edges(volumes[0]) == [(0, 1)] and len(q3.graph_edges(volumes[-1])) == 4, [q3.graph_edges(volumes[0]), len(q3.graph_edges(volumes[-1]))], "edge and square", "geometry")
            ...
            for radius in radii:
                ...
                for time in times:
                    for orientation in orientations:
                        ...
                        check(f"beta={beta} V={volume} L={radius} t={time} sign={orientation} positivity", float(np.real(d_inner)) >= -tolerance and float(np.real(first_inner)) >= -tolerance and float(np.real(delta2_inner)) >= -tolerance, [d_inner, first_inner, delta2_inner], ">=-tolerance", "Kubo-Mori positivity")
    ...
    for beta in betas:
        ...
        check(f"beta={beta} growth finite", np.isfinite(ratio), ratio, "finite", "volume diagnostic")

    check("all rows", len(rows) == len(betas) * len(volumes) * len(radii) * len(times) * len(orientations), len(rows), "manifest product", "coverage")
    return {"schema": "tect/foundation-audit/1.0", "run_kind": "independent", "audit_id": "PA-CP1-ST8-Q3LOCK-GENERATOR-PAIRING-CANCELLATION", "claim_id": manifest["claim_ids"][0], "task_id": manifest["task_id"], "exploration_id": manifest["exploration_id"], "verdict": "PASS", "passed": len(checks), "assertion_count": len(checks), "assertions": checks[:14] + [{"name": "assertion_summary", "group": "summary", "status": "PASS", "actual": str(len(checks)), "expected": "all executed assertions passed"}], "derived": {"summary_rows": summary_rows, "growth_rows": growth_rows, "row_count": len(rows), "finite_actual_d_delta_rows_closed": True, "full_generator_pairing_identity_closed": True, "two_orientation_history_closed": True, "finite_volume_growth_diagnostic_closed": True, "uniform_beta_volume_direct_d_delta_d_closed": False, "modular_domain_closed": False, "common_alpha_closed": False, "pre_a_closed": False}, "boundary": scope}
...
def main() -> int:
    ...
    print(f"INDEPENDENT GENERATOR-PAIRING-CANCELLATION PASS {payload['passed']}/{payload['assertion_count']} rows={payload['derived']['row_count']}")
    return 0
