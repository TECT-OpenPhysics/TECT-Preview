...
SLUG = "pre_a_cp1_st8_q3lock_cancellation_aware_signed_commutator_audit"
MANIFEST = REPO / "strategy/pre-a-cp1-st8-q3lock-cancellation-aware-signed-commutator-audit-manifest.json"
...
def row(volume: int, cutoff: int, beta: float, fixture: dict[str, Any], source_manifest: dict[str, Any], exponent: float, positivity_tolerance: float, norm_floor: float) -> dict[str, Any]:
    ...
    for union in sorted(forward_groups):
        ...
        minimum = float(np.min(np.linalg.eigvalsh(base.positive_weight(local_h))))
        context_cache[union] = (rho, rho_sqrt, k_power, minimum)
        ...
        absolute_reverse_raw = sum(base.operator_norm(value) for value in reverse_groups[union])
        if not all(np.isfinite(value) and value >= -norm_floor for value in (signed_raw, signed_gibbs, signed_weighted, reverse_raw, reverse_gibbs, reverse_weighted, absolute_gibbs, absolute_weighted, absolute_reverse_raw)):
        ...
        for key, aggregate in source_aggregates.items():
            ...
            aggregate["absolute_raw_sum"] += sum(base.operator_norm(value) for value in forward_groups[union])
...
def run() -> dict[str, Any]:
    ...
    check("identity", manifest["exploration_id"] == "EXP-001165" and manifest["task_id"] == "T-054" and manifest["claim_bearing"] is False, [manifest["exploration_id"], manifest["task_id"], manifest["claim_bearing"]], "EXP-001165/T-054/false", "provenance")
    ...
    check("graph volumes", volumes == [2, 4, 6] and all(base.q3.graph_edges(volume) for volume in volumes), volumes, "registered Q3 graph volumes", "fixture")
    ...
    diagnostic = {"interpretation": "finite union-group signed cancellation diagnostic; not a global-state or asymptotic bound", "maximum_signed_weighted_sum_per_site": max(float(value["all_group"]["signed_weighted_sum_per_site"]) for value in rows), "maximum_absolute_weighted_sum_per_site": max(float(value["all_group"]["absolute_weighted_sum_per_site"]) for value in rows), "any_cutoff_growth_threshold_crossed": any(item["growth_threshold_crossed"] for item in summary), "candidate_cutoff_volume_beta_uniform_bound": "not established by this audit", "global_gibbs_state_transfer": "open", "common_core_operator_embedding": "open", "actual_q3_trotter_defect": "open"}
...
def main() -> int:
    ...
    print(f"PRIMARY CANCELLATION-AWARE-SIGNED-COMMUTATOR PASS {payload['passed']}/{payload['assertion_count']} rows={payload['derived']['row_count']}")
    return 0
