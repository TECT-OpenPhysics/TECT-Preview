#!/usr/bin/env python3
"""Primary exact audit for EXP-001115.

This connects the R286 full-generator top coefficient to a coefficientwise
entire source-weight threshold.  It does not construct an actual Q3 seminorm or
history bound.
"""
...
def run() -> dict[str, Any]:
    ...
    time = sp.Rational(fixture["t"])
    sigma_good = sp.Rational(fixture["sigma_good"])
    sigma_bad = sp.Rational(fixture["sigma_bad"])
    G = sp.factor(g + 3 * lam)
    rate = sp.factor(time * G / 4)
    good_margin = sp.factor(sigma_good - rate)
    bad_margin = sp.factor(rate - sigma_bad)
    prefactor = sp.factor(coupling * time)
    ...
    audit.check("R286 full-generator link", manifest["exact_statement"]["full_generator_link"].startswith("R286"), manifest["exact_statement"]["full_generator_link"], "R286", "provenance")
    audit.check("G fixture", G == sp.Rational(51, 35), G, "51/35", "fixture")
    audit.check("rate fixture", rate == sp.Rational(17, 140), rate, "17/140", "threshold")
    audit.check("good margin", good_margin == sp.Rational(11, 140) and good_margin > 0, good_margin, "11/140", "threshold")
    audit.check("bad margin", bad_margin == sp.Rational(3, 140) and bad_margin > 0, bad_margin, "3/140", "threshold")
    audit.check("prefactor", prefactor == sp.Rational(2, 9), prefactor, "2/9", "threshold")
    audit.check("threshold order", sigma_good >= rate and sigma_bad < rate, [sigma_good, rate, sigma_bad], "good>=rate>bad", "threshold")
    coefficient_rows: list[dict[str, Any]] = []
    top_nonzero = True
    for m in orders:
        coefficient = sp.factor(m * coupling * (G / 4) ** (m - 1))
        degree = 4 * m - 3
        top_nonzero = top_nonzero and coefficient > 0
        coefficient_rows.append({"m": m, "coefficient": coefficient, "degree": degree})
        audit.check(f"top coefficient positive m={m}", coefficient > 0, coefficient, ">0", "full-generator")
        audit.check(f"top degree m={m}", degree == 4 * m - 3, degree, 4 * m - 3, "full-generator")
    audit.check("all top rows nonzero", top_nonzero, top_nonzero, True, "full-generator")
...
def main() -> int:
    ...
    print(f"PRIMARY FULL-GENERATOR-ENTIRE-WEIGHT PASS {payload['passed']}/{payload['total']}")
    return 0
