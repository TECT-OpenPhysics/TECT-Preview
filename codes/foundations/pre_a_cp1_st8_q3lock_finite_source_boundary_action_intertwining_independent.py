#!/usr/bin/env python3
"""Non-importing independent finite Q3 source-boundary audit for EXP-001183."""
...
def finite_state(declaration: dict[str, Any], dimension: int, parameters: dict[str, str]) -> dict[str, Any]:
    ...
    identity = np.eye(dimension, dtype=complex)
    return {"volume": volume, "dimension": int(dimension**volume), "edges": edges, "hamiltonian": hamiltonian, "energies": shifted, "vectors": vectors, "q_single": q_single, "p_single": p_single, "identity_single": identity}
...
def observable(state: dict[str, Any], kind: str, amplitude: float, hbar: float) -> np.ndarray:
    ...
    return embed(character(generator, amplitude, hbar), 0, state["volume"], state["identity_single"])
...
def heisenberg(eigenvalues: np.ndarray, eigenvectors: np.ndarray, seconds: float, operator: np.ndarray, hbar: float) -> np.ndarray:
    ...
    return eigenvectors @ (phase[:, None] * basis * phase.conj()[None, :]) @ eigenvectors.conj().T
...
def interpolation_spectra(h0: np.ndarray, delta: np.ndarray, order: int) -> list[tuple[float, float, np.ndarray, np.ndarray]]:
    ...
    for node, weight in zip(nodes, weights):
        ...
        spectra.append((parameter, float(weight) / 2.0, values, vectors))
    return spectra
...
def direct_action_difference(small: dict[str, Any], large: dict[str, Any], identity_extension: np.ndarray, operator_small: np.ndarray, operator_large: np.ndarray, seconds: float, hbar: float) -> np.ndarray:
...
def duhamel_action_difference(h0: np.ndarray, delta: np.ndarray, operator_large: np.ndarray, seconds: float, hbar: float, spectra: list[tuple[float, float, np.ndarray, np.ndarray]], u_order: int) -> np.ndarray:
    if seconds == 0.0:
    ...
    for _parameter, s_weight, values, vectors in spectra:
        ...
        for u, u_weight in interval_nodes(seconds, u_order):
            evolved = heisenberg(values, vectors, seconds - u, operator_large, hbar)
            inner += u_weight * heisenberg(values, vectors, u, commutator(delta, evolved), hbar)
...
def euclidean_word(state: dict[str, Any], beta: float, tau_fraction: float, operator: np.ndarray, hbar: float) -> tuple[np.ndarray, float]:
    ...
    return transfer(period / 2.0 - tau) @ operator @ transfer(tau) / np.sqrt(partition), partition
...
def run() -> dict[str, Any]:
    ...
    check("independent implementation", not any(line.lstrip().startswith(("import ", "from ")) and "pre_a_cp1_st8_q3lock_finite_source_boundary_action_intertwining" in line for line in source_text.splitlines()), "non-importing", "no primary import", "independence")
    ...
    for small_name, large_name in fixture["nested_pairs"]:
        ...
        identity_extension = np.eye(large["dimension"] // small["dimension"], dtype=complex)
        ...
        for kind in fixture["observable_kinds"]:
            ...
            source_commutator_norm = float(np.linalg.norm(commutator(delta, operator_large), ord="fro"))
    ...
    return {"schema": "tect/foundation-audit/1.0", "run_kind": "independent", "audit_id": "PA-CP1-ST8-Q3LOCK-FINITE-SOURCE-BOUNDARY-ACTION-INTERTWINING", "claim_id": manifest["claim_ids"][0], "task_id": manifest["task_id"], "exploration_id": manifest["exploration_id"], "verdict": "PASS", "passed": len(checks), "assertion_count": len(checks), "assertions": checks, "source_boundary_rows": source_boundary_rows, "order_rows": order_rows, "action_rows": action_rows, "os_rows": os_rows, "scope": scope, "boundary": manifest["boundary"], "derived": {"pair_count": len(source_boundary_rows), "order_row_count": len(order_rows), "action_row_count": len(action_rows), "os_row_count": len(os_rows), "max_source_commutator": max(max(row["q_source_commutator"], row["p_source_commutator"]) for row in source_boundary_rows), "max_action_defect_relative": max(row["direct_defect_relative"] for row in action_rows), "max_duhamel_residual_relative": max(row["duhamel_residual_relative"] for row in action_rows), "max_os_defect_relative": max(row["defect_relative"] for row in os_rows), "first_nonzero_orders": {f"{row['small_graph']}->{row['large_graph']}": row["first_nonzero_orders"] for row in source_boundary_rows}, "finite_source_boundary_commutator_zero_closed": True, "finite_source_distance_order_diagnostic_closed": True, "finite_duhamel_action_intertwining_identity_closed": True, "finite_euclidean_source_boundary_diagnostic_closed": True, "source_volume_cutoff_beta_uniform_closed": False, "common_alpha_closed": False, "hamiltonian_os_identification_closed": False, "kms_gns_gap_closed": False, "continuum_closed": False, "c6_closed": False, "sector_a_closed": False, "pre_a_closed": False, "no_new_negative_result": True, "no_tier_change": True, "no_pdf": True}, "provenance": {"script": str(Path(__file__).resolve().relative_to(REPO)).replace("\\", "/"), "script_sha256": sha256(Path(__file__).resolve()), "manifest": str(MANIFEST.relative_to(REPO)).replace("\\", "/"), "manifest_sha256": sha256(MANIFEST), "lean": str(LEAN.relative_to(REPO)).replace("\\", "/"), "lean_sha256": sha256(LEAN)}}
...
def main() -> int:
    ...
    print(f"INDEPENDENT FINITE-SOURCE-BOUNDARY-ACTION-INTERTWINING PASS {payload['passed']}/{payload['assertion_count']} pairs={payload['derived']['pair_count']}")
    return 0
