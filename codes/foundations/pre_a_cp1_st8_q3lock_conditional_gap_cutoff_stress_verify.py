#!/usr/bin/env python3
"""Integrated verifier for the R-400 increasing-cutoff static stress."""
...
MANIFEST = ROOT / "strategy/pre-a-cp1-st8-q3lock-conditional-gap-cutoff-stress-manifest.json"
PRIMARY = ROOT / "codes/foundations/pre_a_cp1_st8_q3lock_conditional_gap_cutoff_stress.py"
INDEPENDENT = ROOT / "codes/foundations/pre_a_cp1_st8_q3lock_conditional_gap_cutoff_stress_independent.py"
HOSTILE = ROOT / "codes/foundations/pre_a_cp1_st8_q3lock_conditional_gap_cutoff_stress_hostile.py"
LEAN = ROOT / "verification/lean/Tect/R400.lean"
PRIMARY_OUTPUT = ROOT / "claims/C6-SPACETIME-SIGNATURE/runs/2026-08-30-primary-pre_a_cp1_st8_q3lock_conditional_gap_cutoff_stress/primary.json"
INDEPENDENT_OUTPUT = ROOT / "claims/C6-SPACETIME-SIGNATURE/runs/2026-08-30-independent-pre_a_cp1_st8_q3lock_conditional_gap_cutoff_stress/independent.json"
HOSTILE_OUTPUT = ROOT / "claims/C6-SPACETIME-SIGNATURE/runs/2026-08-30-hostile-pre_a_cp1_st8_q3lock_conditional_gap_cutoff_stress/hostile.json"
INTEGRATED_OUTPUT = ROOT / "claims/C6-SPACETIME-SIGNATURE/runs/2026-08-30-integrated-pre_a_cp1_st8_q3lock_conditional_gap_cutoff_stress/integrated.json"
...
def main() -> int:
    ...
    crosscheck_tolerance = float(fixture.get("crosscheck_tolerance", tolerance * 1000.0))
    ...
    check("manifest identity", manifest["result_id"] == "R-400" and manifest["exploration_id"] == "EXP-001245" and manifest["claim_bearing"] is False, [manifest["result_id"], manifest["exploration_id"], manifest["claim_bearing"]], "R-400/EXP-001245/false", "identity")
    ...
    check("artifacts present", all(path.is_file() for path in artifacts), [str(path) for path in artifacts if not path.is_file()], "all R-400 artifacts", "provenance")
    ...
    lean = command([str(LAKE), "env", "lean", "Tect/R400.lean"], ROOT / "verification/lean")
    ...
    fields = (
        "system_count", "profile_count", "ratio_count", "minimum_conditional_gap",
        "maximum_conditional_gap", "minimum_conditional_atom", "minimum_reference_atom",
        "minimum_adjacent_gap_ratio", "maximum_adjacent_gap_ratio", "profiles", "adjacent_ratios",
    )
    for field in fields:
        check(f"primary-independent {field}", close(p[field], i[field], crosscheck_tolerance), [p[field], i[field]], f"within {crosscheck_tolerance}", "independence")
    ...
    check("profile rows positive", p["profile_count"] > 0 and p["ratio_count"] > 0, [p["profile_count"], p["ratio_count"]], ">0", "coverage")
    check("finite gap positive", p["minimum_conditional_gap"] > 0.0, p["minimum_conditional_gap"], ">0", "conditional gap")
    check("adjacent ratios finite", p["minimum_adjacent_gap_ratio"] > 0.0 and math.isfinite(p["maximum_adjacent_gap_ratio"]), [p["minimum_adjacent_gap_ratio"], p["maximum_adjacent_gap_ratio"]], "positive finite", "cutoff profile")
    hostile_derived = hostile["derived"]
    check("hostile mutations caught", hostile_derived["genuine_minimum_gap"] > 0.0 and hostile_derived["naive_deficit"] > float(fixture["hostile_threshold"]) and hostile_derived["wrong_parent_mismatch"] > float(fixture["hostile_threshold"]), hostile_derived, "finite gap plus mutation gaps", "hostile")
    payload = {
        "schema": "tect/pre-a-r400-integrated/1.0",
        "manifest": str(MANIFEST.relative_to(ROOT)).replace("\\", "/"),
        "result_id": "R-400",
        "exploration_id": "EXP-001245",
        "verdict": "PASS",
        "checks": checks,
        "derived": {"primary": p, "independent": i, "hostile": hostile_derived, "lean": "PASS", "command_outputs": outputs},
        "scope": scope,
        "boundary": manifest["boundary"],
    }
    atomic_json(args.output if args.output.is_absolute() else ROOT / args.output, payload)
    print(f"INTEGRATED CUTOFF-STRESS PASS {len(checks)}/{len(checks)} profiles={p['profile_count']} min_gap={p['minimum_conditional_gap']:.6g} max_ratio={p['maximum_adjacent_gap_ratio']:.6g}")
    return 0
