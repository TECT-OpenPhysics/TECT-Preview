#!/usr/bin/env python3
"""Integrate and lifecycle-audit the R-169 v1.3 native BCC termination.

Purpose: compare exact SymPy and stdlib/Fraction derivations, enforce source
discipline, and audit the staged or formal repository topology.
Convention: one native zero-phase branch closes one scoped child; the generic
Reading-H-to-P1 interface, empty-reference sign, C6 and Round-1 stay open.
Formula: both lanes must derive the two BCC cosets, volume ell^3/2,
M_Bohr(phi^2)=I, side-16 8-vs-12 split, and the exact P1 quadratic mismatch.
"""
...
PRIMARY = REPO / "codes/foundations/pre_a_t055_reading_h_native_owner_bcc_route_termination.py"
INDEPENDENT = REPO / "codes/foundations/pre_a_t055_reading_h_native_owner_bcc_route_termination_independent.py"
...
CLOSED = ["PA-T055-READING-H-NATIVE-ZERO-PHASE-BCC-ORBIT-CELL-AND-GSTAR-SIGN"]
STRENGTHENED = ["PA-T055-READING-H-PINNED-P1-NONLINEAR-CONVENTION-AND-TORUS-COMMENSURABILITY-CROSSWALK"]
OPEN = [
...
def catalog_inventory_count() -> int:
    """Count the current catalog-eligible tree without mutating generated views."""
...
def source_discipline(audit: Audit) -> None:
    ...
    for path, tree in trees.items():
        ...
        audit.check(f"docstring contract {path.name}", all(token in docstring for token in ("Purpose:", "Convention:", "Formula:")), docstring.splitlines()[:4], "purpose/convention/formula", "code")

    for path in PACKAGE_PATHS:
        data = path.read_bytes()
        format_ok = data.endswith(b"\n") and b"\r" not in data and all(byte < 128 for byte in data)
        audit.check(f"format {path.name}", format_ok, "ASCII LF final-LF" if format_ok else "bad format", "ASCII LF final-LF", "code")

    primary_source = PRIMARY.read_text(encoding="utf-8")
    independent_source = INDEPENDENT.read_text(encoding="utf-8")
    primary_derivation = ast.get_source_segment(primary_source, next(node for node in trees[PRIMARY].body if isinstance(node, ast.FunctionDef) and node.name == "exact_derivation")) or ""
    independent_derivation = ast.get_source_segment(independent_source, next(node for node in trees[INDEPENDENT].body if isinstance(node, ast.FunctionDef) and node.name == "exact_derivation")) or ""
    masking_firewall = (
        ...
        and "quadratic_gap =" in primary_source
        and "quadratic_gap =" in independent_source
        and "equality_corners" in primary_source
        and "equality_corners" in independent_source
    )
    audit.check("derived-value masking firewall", masking_firewall, masking_firewall, True, "code")
...
def formal_audit(audit: Audit, manifest: dict[str, Any], fresh_primary: dict[str, Any], fresh_independent: dict[str, Any]) -> None:
    ...
    strengthened_sections = [gate_section(gates, identifier) for identifier in STRENGTHENED]
    open_sections = [gate_section(gates, identifier) for identifier in OPEN]
    audit.check("one scoped native child", all("EXP-000860" in value and "R-169 v1.3" in value and "CLOSED" in value for value in closed_sections), "one unique CLOSED native child", "one unique CLOSED native child", "formal")
    audit.check("v1.2 interface child strengthened", all("EXP-000860" in value and "R-169 v1.3" in value and "CLOSED" in value for value in strengthened_sections), "one unique retained-CLOSED section", "one unique retained-CLOSED section", "formal")
    ...
    audit.check("unique EXP-000860", len(records) == 1, len(records), 1, "formal")
    record = records[0]
    expected_gates = [*CLOSED, *STRENGTHENED, *OPEN]
    topology_ok = (
        ...
        and record.get("related") == [{"id": "EXP-000858", "relation": "continues"}]
        and record.get("formal_refs", {}).get("results") == ["R-169"]
        and record.get("formal_refs", {}).get("negatives") == REUSED_NEGATIVES
    )
    audit.check("EXP exact topology", topology_ok, {key: record.get(key) for key in ("task_id", "verdict", "claim_ids", "gate_ids", "related", "formal_refs")}, "exact v1.3 exploration topology", "formal")

    events = parse_json_lines(REPO / "changelog/log.jsonl")
    contract = manifest["formal_integration"]
    matches = [
        (ordinal, candidate)
        for ordinal, candidate in enumerate(events, start=1)
        if candidate.get("id") == contract["event_id"]
    ]
    ...
    audit.check("event 644 identity", identity_ok, {"total_events": len(events), "ordinal": ordinal, "id": event.get("id"), "header": event.get("header")}, {"ordinal": contract["event_ordinal"], "id": contract["event_id"], "header": exact_header}, "formal")
    linkage_ok = event.get("claim_ids") == contract["event_claim_ids"] and event.get("keywords") == contract["event_keywords"] and event.get("notes") == contract["event_notes"] and event.get("scripts") == contract["event_scripts"] and event.get("neg_results") == []
    ...
    map_ok = (
        ...
        and priority.get("r169_v1_3_new_negatives") == []
        and priority.get("r169_v1_3_reused_negatives") == REUSED_NEGATIVES
    )
    audit.check("theorem map v1.38", map_ok, {"version": theorem_map.get("version"), "latest": priority.get("latest_cp1_checkpoint", "")[:40]}, "v1.38 exact arrays", "formal")
...
def run(staged: bool) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="r169-v13-") as directory:
        root = Path(directory)
    ...
    child_ok = fresh_primary["verdict"] == fresh_independent["verdict"] == "PASS" and fresh_primary["mode"] == fresh_independent["mode"] == expected_mode
    ...
    common_keys = (
        ...
        "support_antipodal",
        "support_zero_sum",
        "minimum_angle_cosine",
        "corner_maximum",
        "corner_minimum",
        "absolute_corner_maximum",
        "equality_corners",
        "equality_values",
        "center_residues_mod_two",
        "voronoi_vertex_count",
        "square_face_count",
        "hexagon_face_count",
        "edge_count",
        "cell_volume_coefficient",
        "centers_per_conventional_cube",
        "production_amplitude_square",
        "bohr_mean",
        "natural_torus_center_count",
        "natural_torus_norm_coefficient",
        "side16_shell_three_gap",
        "side16_q0_above_shell_three",
        "side16_nearest_radial_count",
        "side16_bcc_direction_count",
        "bohr_snap_error_coefficient",
        "angle_membership",
        "packing_membership",
        ...
        "reading_h_quadratic",
        "quadratic_gap",
        "quadratic_contradiction",
        "production_p1_charge",
        "r158_intensity_ratio",
    )
    ...
    boundary_ok = (
        "loses to the selected isotropic G_*" in manifest["native_owner_sign"]["meaning"]
        and "No sign for F_RH[G_*]-F_empty" in manifest["native_owner_sign"]["boundary"]
        and "only the natural constant-polarization" in manifest["global_rescaling_no_go"]["scope"]
        and manifest["new_negative_ids"] == []
    )
    audit.check("native-owner and P1 scope firewalls", boundary_ok, boundary_ok, True, "scope")
    ...
    return {
        "schema": "tect/pre-a-t055-reading-h-native-owner-bcc-route-termination-integrated/1.0",
        "version": __version__,
        "mode": expected_mode,
