"""Integrated verifier for the R-180 scalar triangular majorant cross-check."""
...
PRIMARY = REPO / "verification" / "scripts" / "lean_a13_scalar_triangular_majorant_crosscheck.py"
INDEPENDENT = REPO / "codes" / "foundations" / "lean_a13_scalar_triangular_majorant_crosscheck_independent.py"
SCRIPT = Path(__file__).resolve()
LEAN = REPO / "verification" / "lean" / "Tect" / "R180.lean"
...
def accepts(derived: dict, manifest: dict) -> bool:
    ...
    return (
        str(derived.get("near_fixture")) == fixture["near"]
        and str(derived.get("far_high_fixture")) == fixture["far_high"]
        and str(derived.get("h_five_fixture")) == fixture["hFive"]
        and str(derived.get("geom_fixture")) == fixture["geom_value"]
        and str(derived.get("beta_half_minus_gamma")) == margins["beta_half_minus_gamma"]
        and str(derived.get("s_minus_gamma")) == margins["s_minus_gamma"]
        and derived.get("h_five_positive") is True
        and derived.get("geom_closed") is True
        and derived.get("production_exponent_margins") is True
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=REPO / "claims" / "A13-CLASSII-RELATIVE-PHASE-SOURCE-BUDGET-OBSTRUCTION" / "runs" / "2026-08-21-lean-r180-scalar-triangular-majorant-crosscheck" / "integrated.json")
    ...
    check("identity", manifest["audit_id"] == "A13-SCALAR-TRIANGULAR-MAJORANT-LEAN-CROSSCHECK", manifest["audit_id"], "A13-SCALAR-TRIANGULAR-MAJORANT-LEAN-CROSSCHECK")
    ...
    with tempfile.TemporaryDirectory(prefix="r180-crosscheck-") as directory:
    ...
    common = ("near_fixture", "far_high_fixture", "h_five_fixture", "geom_fixture", "beta_half_minus_gamma", "s_minus_gamma", "h_five_positive", "geom_closed", "production_exponent_margins", "a13_gate_closed", "sector_a_closed", "authority_hashes_ok", "lean_escape_tokens_absent", "boundary_present")
    ...
    check("acceptance predicate", accepts(primary["derived"], manifest), primary["derived"], "scalar triangular majorant cross-check")
    baseline = primary["derived"]
    mutations: list[dict] = []
    for index, label in enumerate(manifest["hostile_mutations"]):
        mutated = copy.deepcopy(baseline)
        if index == 0:
            mutated["near_fixture"] = "0"
        elif index == 1:
            mutated["far_high_fixture"] = "0"
        elif index == 2:
            mutated["h_five_fixture"] = "1"
        elif index == 3:
            mutated["geom_closed"] = False
        elif index == 4:
            mutated["beta_half_minus_gamma"] = "0"
        elif index == 5:
            mutated["s_minus_gamma"] = "0"
        elif index == 6:
    ...
    if not args.staged:
        ...
        check("R-180 authority", results.count("R-180") >= 2 and "scalar triangular" in results.lower(), results.count("R-180"), ">=2")
        check("strategy index authority", "R-180" in (REPO / "strategy/INDEX.md").read_text(encoding="utf-8"), True, True)
    ...
    print(f"INTEGRATED R-180 LEAN PASS {len(rows)}/{len(rows)}")
    return 0
