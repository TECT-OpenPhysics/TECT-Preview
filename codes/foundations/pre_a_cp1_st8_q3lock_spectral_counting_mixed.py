#!/usr/bin/env python3
"""Primary finite audit of the mixed IR/UV mode-counting family for R-412.

For D=diag(pi), W=D^(-1/2)L D^(-1/2), order the d-1 positive eigenvalues
as lambda_k.  For a split 1<=r<m and exponents alpha_IR,alpha_UV in (0,1),
separate counting constants control the low and high mode sums.  The UV
series receives an explicit integral tail comparison.
"""
...
def run(output: Path = DEFAULT_OUTPUT) -> dict[str, Any]:
    ...
    payload = {"schema": "tect/pre-a-r412-primary/1.0", "manifest": str(MANIFEST.relative_to(REPO)).replace("\\", "/"), "result_id": "R-412", "exploration_id": "EXP-001257", "verdict": "PASS", "checks": checks, "assertion_count": check_count, "derived": derived, "scope": scope, "boundary": manifest["boundary"]}
    r408.r407.atomic_json(output, payload)
    print(f"R-412 PRIMARY PASS {check_count}/{check_count} systems={len(pairs)} contexts={total_contexts} rows={total_rows} mixed_candidates={len(exponents) * len(exponents)} selected_bounds=[{min(mixed_finite_bounds):.6g},{max(mixed_finite_bounds):.6g}] trace=[{min(traces):.6g},{max(traces):.6g}]")
    return payload
