#!/usr/bin/env python3
"""Primary finite high-cutoff stress of the R-425 Schur assembly.
...
MANIFEST = REPO / "strategy/pre-a-cp1-st8-q3lock-high-cutoff-schur-stress-manifest.json"
...
R425_MANIFEST = REPO / "strategy/pre-a-cp1-st8-q3lock-expanded-grid-schur-stress-manifest.json"
...
# The high-cutoff lane intentionally records one declared route-local failure.
# These globals let the command-line wrapper preserve the exact failing check
# while still returning exit 0 for an expected, reproducible audit boundary.
...
def run(output: Path = DEFAULT_OUTPUT) -> dict[str, Any]:
    global LAST_FAILURE, LAST_ASSERTION_COUNT, LAST_CHECKS
    LAST_FAILURE = None
    LAST_ASSERTION_COUNT = 0
    LAST_CHECKS = []
    ...
    def check(name: str, condition: bool, actual: Any, expected: Any, group: str) -> None:
        ...
        if not condition:
            LAST_FAILURE = {"name": name, "group": group, "actual": actual, "expected": expected}
            raise AssertionError(f"{name}: actual={actual!r}, expected={expected!r}")
        if len(checks) < 1000:
            record = {"name": name, "group": group, "status": "PASS", "actual": str(actual), "expected": str(expected)}
            checks.append(record)
            LAST_CHECKS.append(record)

    finite_flags = [
        "finite_high_cutoff_schur_closed",
        "finite_harmonic_coarse_schur_closed",
        "finite_residual_reuse_closed",
        "finite_support_boundary_recorded",
    ]
    promoted = {key: value for key, value in manifest["scope"].items() if key.endswith("_closed") and key not in finite_flags}
    check("manifest identity", manifest["result_id"] == "R-426" and manifest["exploration_id"] == "EXP-001271" and manifest["claim_bearing"] is False, [manifest["result_id"], manifest["exploration_id"], manifest["claim_bearing"]], "R-426/EXP-001271/false", "provenance")
    check(
        "scope firewall",
        manifest["status"] == "FAIL_ROUTE_LOCAL"
        and manifest["scope"]["finite_high_cutoff_schur_closed"] is False
        and manifest["scope"]["finite_harmonic_coarse_schur_closed"] is False
        and manifest["scope"]["finite_residual_reuse_closed"] is False
        and manifest["scope"]["finite_support_boundary_recorded"] is True
        and not any(promoted.values()),
        {"status": manifest["status"], "finite_flags": {key: manifest["scope"][key] for key in finite_flags}, "promoted": promoted},
        "declared route-local failure with no promotion",
        "scope",
    )
    check("expanded grid", pairs == expected_pairs and len(pairs) == len(set(pairs)), pairs, expected_pairs, "fixture")
    check("beta/orientation", betas == [0.5, 2.0, 8.0] and orientations == ["right", "left"], [betas, orientations], "fixed R-425 inputs", "fixture")
    ...
    check("upstream hashes", sha256(R419_MANIFEST) == manifest["upstream_authority"]["r419_sha256"] and sha256(R416_MANIFEST) == manifest["upstream_authority"]["r416_sha256"] and sha256(R422_MANIFEST) == manifest["upstream_authority"]["r422_sha256"] and sha256(R425_MANIFEST) == manifest["upstream_authority"]["r425_sha256"], "hash-pinned parents", "declared SHA-256 values", "authority")
    ...
    check("support boundary accounting", all(item["eligible_row_count"] >= 0 for item in systems) and any(item["eligible_row_count"] > 0 for item in systems), [item["eligible_row_count"] for item in systems], "eligible rows and explicit boundary accounting", "boundary")
    ...
    payload: dict[str, Any] = {
        "schema": "tect/pre-a-r426-primary/1.0",
        ...
        "assertions": checks,
        "derived": derived,
        "source_hashes": {"primary": sha256(Path(__file__)), "manifest": sha256(MANIFEST), "r419_manifest": sha256(R419_MANIFEST), "r416_manifest": sha256(R416_MANIFEST), "r422_manifest": sha256(R422_MANIFEST), "r425_manifest": sha256(R425_MANIFEST)},
    ...
    print(f"R-426 PRIMARY PASS {assertion_count}/{assertion_count} systems={len(pairs)} rows={row_count} eligible={eligible_rows} combined={combined_rows} coarse=[{min(coarse_gaps):.6g},{max(coarse_gaps):.6g}] residual=[{min(residual_gaps):.6g},{max(residual_gaps):.6g}] combined=[{min(combined_gaps):.6g},{max(combined_gaps):.6g}]")
    return payload
...
def main() -> int:
    ...
    try:
        ...
        manifest = json.loads(MANIFEST.read_text(encoding="utf-8"))
        failure = manifest.get("failure_contract", {})
        failure_name = (LAST_FAILURE or {}).get("name", "")
        expected_name = f"V={failure.get('volume')} d={failure.get('cutoff_dimension')} {failure.get('orientation')} residual reuse"
        if not failure or failure_name != expected_name:
            raise
        payload: dict[str, Any] = {
            "schema": "tect/pre-a-r426-primary/1.0",
            "result_id": manifest["result_id"],
            ...
            "manifest": MANIFEST.relative_to(REPO).as_posix(),
            "run_kind": "primary",
            "verdict": "FAIL_ROUTE_LOCAL",
            "assertion_count": LAST_ASSERTION_COUNT,
            "assertions": LAST_CHECKS,
            "failure": {
                **failure,
                "check_name": failure_name,
                "error": str(error),
            ...
            "source_hashes": {
                ...
                "r416_manifest": sha256(R416_MANIFEST),
                "r422_manifest": sha256(R422_MANIFEST),
                "r425_manifest": sha256(R425_MANIFEST),
            },
            "assumptions": manifest["assumptions"],
            "missing_assumptions": manifest["missing_assumptions"],
            "evidence_level": manifest["evidence_level"],
            ...
            "boundary": manifest["boundary"],
            "derived": {
                "failure_mode": "residual_reuse_above_fixed_tolerance",
                "numeric_evaluation": True,
                "promoted_result": False,
                "rows_checked_before_failure": LAST_ASSERTION_COUNT,
            },
        }
        atomic_json(output, payload)
        print(
            "R-426 PRIMARY FAIL_ROUTE_LOCAL "
            f"check={failure_name} difference={failure['absolute_difference']:.12g} "
            f"tolerance={failure['comparison_tolerance']:.12g}"
        )
        return 0
    return 0
