#!/usr/bin/env python3
"""Non-importing NumPy control for the R-436 d=18 finite row."""
...
MANIFEST = REPO / "strategy/pre-a-cp1-st8-q3lock-original-source-interval-d18-manifest.json"
R419_MANIFEST = REPO / "strategy/pre-a-cp1-st8-q3lock-growing-volume-lyapunov-core-tail-stress-manifest.json"
DEFAULT_OUTPUT = REPO / "claims/C6-SPACETIME-SIGNATURE/runs/2026-08-30-independent-original_source_interval_d18/independent.json"
...
def run(output: Path = DEFAULT_OUTPUT) -> dict[str, Any]:
    ...
    check("manifest identity", manifest["result_id"] == "R-436" and manifest["exploration_id"] == "EXP-001281" and not manifest["claim_bearing"], [manifest["result_id"], manifest["exploration_id"], manifest["claim_bearing"]], "R-436/EXP-001281/false", "provenance")
    ...
    check("residual compression", np.isfinite(gap) and compressed.shape == (n - 2, n - 2), [gap, compressed.shape], "finite 16x16 compression", "residual")
    ...
    payload = {
        "schema": "tect/pre-a-r436-independent/1.0",
    ...
    print(f"R-436 INDEPENDENT {payload['verdict']} {len(checks)}/{len(checks)} gap={gap:.15g}", flush=True)
    return payload
...
def main() -> int:
    ...
    if args.self_test:
        ...
        print("R-436 INDEPENDENT SELFTEST: PASS", flush=True)
    return 0
