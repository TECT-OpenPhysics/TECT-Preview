#!/usr/bin/env python3
"""Primary larger-cutoff/volume stress for EXP-001209 / R-367."""
...
SLUG = "pre_a_cp1_st8_q3lock_local_q2_fractional_liouvillian_stress"
MANIFEST = REPO / "strategy/pre-a-cp1-st8-q3lock-local-q2-fractional-liouvillian-stress-manifest.json"
...
def selected_prefixes(terms: list[np.ndarray], order: list[int], sign: int, delta: float, hbar: float, labels: list[str]) -> list[tuple[str, int, np.ndarray]]:
    ...
    for position in range(len(order) + 1):
        ...
        for label in labels:
            if wanted[label] == position and not any(old_position == position for _, old_position, _ in rows):
...
def run() -> dict[str, Any]:
    ...
    if len(thetas) != 1 or not (0.0 < thetas[0] <= 1.0):
        raise AssertionError("stress fixture must contain one theta in (0,1]")
    theta = thetas[0]
    ...
    check("authority", manifest["exploration_id"] == "EXP-001209" and manifest["result_id"] == "R-367", [manifest["exploration_id"], manifest["result_id"]], "EXP-001209/R-367", "provenance")
    check("scope", manifest["claim_bearing"] is False and manifest["scope"]["uniform_fractional_bound_proved"] is False, manifest["scope"], "finite-stress-only", "scope")
    context_count = 0
    max_phase_error = 0.0
    ...
    for regime in regimes:
            ...
            for beta_fraction in betas:
                ...
                for site in sites:
                    ...
                    for order_name, order in orders.items():
                        for sign in signs:
                            prefixes = selected_prefixes(terms, order, sign, delta, hbar, labels)
                            for prefix_label, prefix_length, prefix in prefixes:
                                for history_adjoint in adjoints:
                                    ...
                                    spectral_off = hermitian(centered - pinching(centered, eigenvectors, eigenvalues, tolerance))
                                    ...
                                    for current_theta in thetas:
                                        ...
                                        fractional_norm = float(np.sqrt(max(float(np.sum(np.abs(deltas) ** (2.0 * current_theta) * np.abs(transformed) ** 2)), 0.0)))
                                        ...
                                        check(f"V={volume} d={size} beta={beta_fraction} site={site} {order_name} {prefix_label} sign={sign} adj={history_adjoint} envelope", envelope_violation <= tolerance * 800, envelope_violation, f"<={tolerance * 800}", "fractional envelope")
                                        check(f"V={volume} d={size} beta={beta_fraction} site={site} {order_name} {prefix_label} sign={sign} adj={history_adjoint} bound", bound_violation <= tolerance * 800 * (1.0 + bound), [change_norm, bound], "fractional bound", "fractional bound")
                                        check(f"V={volume} d={size} beta={beta_fraction} site={site} {order_name} {prefix_label} sign={sign} adj={history_adjoint} trace", trace_violation <= tolerance * 800 * (1.0 + trace_bound), [trace_change, trace_bound], "state trace", "state trace")
                                        check(f"V={volume} d={size} beta={beta_fraction} site={site} {order_name} {prefix_label} sign={sign} adj={history_adjoint} phase", phase_error <= tolerance * 800, phase_error, f"<={tolerance * 800}", "spectral phase")
                                        context_count += 1
        ...
        expected_regime = sum(len(cutoffs) * len(betas) * len(sites) * len(orders) * len(signs) * len(selected_prefixes(base.split_system(volume, cutoffs[0], parameters)[2], list(range(len(base.split_system(volume, cutoffs[0], parameters)[2]))), signs[0], delta, hbar, labels)) * len(adjoints) for _ in [0])
        per_regime.append({"volume": volume, "cutoffs": cutoffs, "sites": sites, "contexts": context_count - regime_start, "expected_contexts": expected_regime, "maximum_fractional_norm": regime_max_norm, "maximum_fractional_bound": regime_max_bound})
    ...
    return {
        ...
        "run_kind": "primary",
        "audit_id": "PA-CP1-ST8-Q3LOCK-LOCAL-Q2-FRACTIONAL-LIOUVILLIAN-STRESS",
        ...
        "derived": {
            ...
            "larger_cutoff_volume_stress_checked": True,
            "fractional_liouvillian_bound_closed": True,
            "uniform_fractional_bound_proved": False,
...
def main() -> int:
    ...
    print(f"PRIMARY FRACTIONAL STRESS PASS {payload['assertion_count']}/{payload['assertion_count']} contexts={payload['derived']['context_count']} max_norm={payload['derived']['maximum_fractional_norm']:.6f}")
    return 0
