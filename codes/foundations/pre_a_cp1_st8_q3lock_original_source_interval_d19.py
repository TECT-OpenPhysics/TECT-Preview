#!/usr/bin/env python3
"""Finite d=19 original-source interval enclosure for the Q3LOCK route.
...
POINT_CACHE = REPO / ".tmp/r438-d19-point-cache.json"
DEFAULT_OUTPUT = REPO / "claims/C6-SPACETIME-SIGNATURE/runs/2026-08-30-primary-original_source_interval_d19/primary.json"
...
def run(output: Path = DEFAULT_OUTPUT) -> dict[str, Any]:
    ...
    check(
        "manifest identity",
        manifest["result_id"] == "R-438" and manifest["exploration_id"] == "EXP-001283" and manifest["claim_bearing"] is False,
        [manifest["result_id"], manifest["exploration_id"], manifest["claim_bearing"]],
        "R-438/EXP-001283/false",
        "provenance",
    )
    ...
    payload = {
        "schema": "tect/pre-a-r438-primary/1.0",
    ...
    print(f"R-438 PRIMARY ORIGINAL_SOURCE_INTERVAL_CERTIFIED {len(checks)}/{len(checks)} d={dimension} row=unconditional bracket=[{derived['rayleigh_interval'][0]},{derived['rayleigh_interval'][1]}]", flush=True)
    return payload
...
def main() -> int:
    ...
    if args.self_test:
        ...
        print("R-438 PRIMARY SELFTEST: PASS", flush=True)
    return 0
