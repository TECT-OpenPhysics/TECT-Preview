#!/usr/bin/env python3
"""Integrated primary/independent/hostile/Lean verifier for R-409."""
...
MANIFEST = ROOT / "strategy/pre-a-cp1-st8-q3lock-resistance-trace-manifest.json"
PRIMARY = ROOT / "codes/foundations/pre_a_cp1_st8_q3lock_resistance_trace.py"
INDEPENDENT = ROOT / "codes/foundations/pre_a_cp1_st8_q3lock_resistance_trace_independent.py"
HOSTILE = ROOT / "codes/foundations/pre_a_cp1_st8_q3lock_resistance_trace_hostile.py"
LEAN = ROOT / "verification/lean/Tect/R409.lean"
PRIMARY_OUTPUT = ROOT / "claims/C6-SPACETIME-SIGNATURE/runs/2026-08-30-primary-pre_a_cp1_st8_q3lock_resistance_trace/primary.json"
INDEPENDENT_OUTPUT = ROOT / "claims/C6-SPACETIME-SIGNATURE/runs/2026-08-30-independent-pre_a_cp1_st8_q3lock_resistance_trace/independent.json"
HOSTILE_OUTPUT = ROOT / "claims/C6-SPACETIME-SIGNATURE/runs/2026-08-30-hostile-pre_a_cp1_st8_q3lock_resistance_trace/hostile.json"
INTEGRATED_OUTPUT = ROOT / "claims/C6-SPACETIME-SIGNATURE/runs/2026-08-30-integrated-pre_a_cp1_st8_q3lock_resistance_trace/integrated.json"
...
def main() -> int:
    ...
    check("manifest identity", manifest["result_id"] == "R-409" and manifest["exploration_id"] == "EXP-001254" and manifest["claim_bearing"] is False, [manifest["result_id"], manifest["exploration_id"], manifest["claim_bearing"]], "R-409/EXP-001254/false", "identity")
    ...
    check("artifacts present", all(path.is_file() for path in artifacts), [str(path) for path in artifacts if not path.is_file()], "all R-409 artifacts", "provenance")
    ...
    lean = command([str(LAKE), "env", "lean", "Tect/R409.lean"], ROOT / "verification/lean")
    ...
    h = hostile["derived"]
    check("hostile pair mutation caught", all(abs(item["directed_pair_average"] - item["trace_inverse"]) > tolerance and abs(item["raw_laplacian_trace"] - item["trace_inverse"]) > tolerance and item["one_mode_inverse"] < item["trace_inverse"] - tolerance for item in h["selected"]), h["selected"], "factor, normalization and one-mode mutations rejected", "hostile")
    ...
    payload = {"schema": "tect/pre-a-r409-integrated/1.0", "manifest": str(MANIFEST.relative_to(ROOT)).replace("\\", "/"), "result_id": "R-409", "exploration_id": "EXP-001254", "verdict": "PASS", "checks": checks, "derived": {"primary": p, "independent": i, "hostile": h, "lean": "PASS", "command_outputs": outputs}, "scope": scope, "boundary": manifest["boundary"]}
    atomic_json(args.output if args.output.is_absolute() else ROOT / args.output, payload)
    print(f"INTEGRATED RESISTANCE-TRACE PASS {len(checks)}/{len(checks)} contexts={p['context_count']} rows={p['comparison_row_count']} Rbar=[{p['minimum_resistance_average']:.6g},{p['maximum_resistance_average']:.6g}] trace=[{p['minimum_trace_inverse']:.6g},{p['maximum_trace_inverse']:.6g}]")
    return 0
