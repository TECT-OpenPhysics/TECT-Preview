#!/usr/bin/env python3
"""Integrated verifier for the R-395 finite gentle projection bridge."""
...
PRIMARY_OUTPUT = ROOT / "claims/C6-SPACETIME-SIGNATURE/runs" / "2026-08-30-primary-pre_a_cp1_st8_q3lock_gibbs_gentle_projection_bridge" / "primary.json"
INDEPENDENT_OUTPUT = ROOT / "claims/C6-SPACETIME-SIGNATURE/runs" / "2026-08-30-independent-pre_a_cp1_st8_q3lock_gibbs_gentle_projection_bridge" / "independent.json"
HOSTILE_OUTPUT = ROOT / "claims/C6-SPACETIME-SIGNATURE/runs" / "2026-08-30-hostile-pre_a_cp1_st8_q3lock_gibbs_gentle_projection_bridge" / "hostile.json"
LEAN_ROOT = ROOT / "verification/lean"
LEAN_FILE = LEAN_ROOT / "Tect/R395.lean"
...
def main() -> None:
    ...
    parser.add_argument("--output", type=Path, default=ROOT / "claims/C6-SPACETIME-SIGNATURE/runs" / "2026-08-30-integrated-pre_a_cp1_st8_q3lock_gibbs_gentle_projection_bridge" / "integrated.json")
    ...
    check("manifest identity", manifest["candidate_id"].endswith("FINITE-v0") and manifest["result_id"] == "R-395" and manifest["exploration_id"] == "EXP-001238" and manifest["claim_bearing"] is False, [manifest["candidate_id"], manifest["result_id"], manifest["exploration_id"], manifest["claim_bearing"]], "R-395 finite false", "identity")
    check("coverage", all(coverage.values()), coverage, "all declared gentle-bridge rows", "coverage")
    ...
    check("artifacts present", all(path.is_file() for path in paths), [str(path) for path in paths if not path.is_file()], "all R-395 artifacts", "provenance")
    ...
    lean = run([str(LAKE), "env", "lean", "Tect/R395.lean"], LEAN_ROOT)
    ...
    profiles = derived["cutoff_profiles"]
    declared_max_dimension = max(int(dimension) for item in fixture["admissible_pairs"] for dimension in item["cutoff_dimensions"])
    ...
    check("disturbances", derived["trace_disturbance_min"] >= -tolerance and derived["trace_disturbance_max"] >= -tolerance and derived["tail_mass_min"] >= -tolerance and derived["tail_mass_max"] <= 1.0 + tolerance, [derived["trace_disturbance_min"], derived["trace_disturbance_max"], derived["tail_mass_min"], derived["tail_mass_max"]], "finite nonnegative disturbances and tails", "bridge")
    check("bridge inequalities", derived["markov_violation_count"] == 0 and derived["gentle_violation_count"] == 0 and derived["composition_violation_count"] == 0 and derived["gentle_bound_max"] >= derived["trace_disturbance_max"] - tolerance and derived["gentle_markov_bound_max"] >= derived["trace_disturbance_max"] - tolerance, [derived["markov_violation_count"], derived["gentle_violation_count"], derived["composition_violation_count"], derived["gentle_bound_max"], derived["gentle_markov_bound_max"]], "zero violations and finite envelopes", "bridge")
    ...
    ratios = [float(ratio["disturbance_ratio"]) for row in profiles["profiles"] for ratio in row["adjacent_ratios"]]
    ...
    check("hostile factor mutation", hderived["trace_disturbance"] > float(fixture["hostile_threshold"]) and hderived["trace_disturbance"] <= hderived["genuine_bound"] + tolerance and hderived["trace_disturbance"] > hderived["mutated_factor_one_bound"] + float(fixture["hostile_threshold"]), hderived, "genuine factor two holds and factor one is rejected", "hostile")
    payload = {"schema": "tect/pre-a-r395-integrated/1.0", "manifest": str(MANIFEST.relative_to(ROOT)).replace("\\", "/"), "result_id": "R-395", "exploration_id": "EXP-001238", "verdict": "PASS", "checks": checks, "derived": {"primary": derived, "independent": independent["derived"], "hostile": hderived, "lean": "PASS", "command_outputs": outputs}, "scope": scope}
    atomic_json(args.output, payload)
    print(f"INTEGRATED GENTLE-PROJECTION-BRIDGE PASS {len(checks)}/{len(checks)} Lean=PASS disturbance_max={derived['trace_disturbance_max']:.6g} composed_max={derived['gentle_markov_bound_max']:.6g} profiles={profiles['count']}")
