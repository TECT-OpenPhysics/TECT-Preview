#!/usr/bin/env python3
"""Independent Fraction audit for EXP-001053."""
...
def run() -> dict[str, Any]:
    ...
    coefficients = slice_coefficients(P, F(1, 4))
    expected_C = F(manifest["weighted_target"]["majorant"].split("=")[-1])
    coefficient_sum = sum(abs(coefficients.get(degree, F(0))) for degree in range(4))
    check("slice degree at most three", max(coefficients) <= 3, max(coefficients), "<=3", "slice")
    ...
    samples = [F(value) for value in manifest["finite_fixture"]["q_values"]]
    sample_rows: list[dict[str, Any]] = []
    for sample in samples:
        value = evaluate(P, (sample, F(0), F(1, 4)))
        energy = 1 + sample**4
    ...
    return {
        ...
        "assertions": checks, "sample_rows": sample_rows,
        "derived": {
            "slice_coefficients": coefficients, "slice_degree": max(coefficients), "coefficient_majorant_C": coefficient_sum,
            "energy_exponent": "3/4", "sample_count": len(samples), "scalar_energy_weighted_majorant_closed": True,
            "actual_q3_common_core_map_proved": False, "operator_domain_closure_proved": False, "multivariate_weighted_bound_proved": False,
...
def main() -> int:
    ...
    print(f"INDEPENDENT Q3-SCALAR-ENERGY-WEIGHT PASS {payload['passed']}/{payload['total']}")
    return 0
