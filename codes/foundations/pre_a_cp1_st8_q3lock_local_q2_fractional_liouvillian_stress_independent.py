#!/usr/bin/env python3
"""Independent larger-cutoff/volume stress for EXP-001209 / R-367."""
...
def selected_prefixes(terms: list[np.ndarray], order: list[int], sign: int, delta: float, hbar: float, labels: list[str]) -> list[tuple[str, int, np.ndarray]]:
    ...
    target = {"zero": 0, "first": 1, "full": len(order)}
    ...
    for position in range(len(order) + 1):
        ...
        for label in labels:
            if target[label] == position and not any(old == position for _, old, _ in rows):
...
def run() -> dict[str, Any]:
    ...
    check("authority", manifest["exploration_id"] == "EXP-001209" and manifest["result_id"] == "R-367" and not manifest["claim_bearing"], [manifest["exploration_id"], manifest["result_id"], manifest["claim_bearing"]], "EXP-001209/R-367/false", "provenance")
    ...
    for regime in fixture["regimes"]:
            ...
            for beta_fraction in betas:
                ...
                for site in sites:
                    ...
                    for order_name, order in orders.items():
                        for sign in signs:
                            for prefix_label, prefix_length, prefix in selected_prefixes(terms, order, sign, delta, hbar, labels):
        ...
        expected_regime = len(cutoffs) * len(betas) * len(sites) * 2 * len(signs) * len(labels) * len(adjoints) * len(thetas)
        per_regime.append({"volume": volume, "cutoffs": cutoffs, "sites": sites, "contexts": context_count - start_count, "expected_contexts": expected_regime, "maximum_fractional_norm": regime_max_norm, "maximum_fractional_bound": regime_max_bound})
...
def main() -> int:
    ...
    print(f"INDEPENDENT FRACTIONAL STRESS PASS {payload['assertion_count']}/{payload['assertion_count']} contexts={payload['derived']['context_count']} max_norm={payload['derived']['maximum_fractional_norm']:.6f}")
    return 0
