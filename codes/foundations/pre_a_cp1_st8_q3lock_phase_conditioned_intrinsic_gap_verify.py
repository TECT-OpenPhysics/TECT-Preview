#!/usr/bin/env python3
"""Integrated verifier for the R-405 phase-conditioned graph split."""
...
MANIFEST = ROOT / "strategy/pre-a-cp1-st8-q3lock-phase-conditioned-intrinsic-gap-manifest.json"
PRIMARY = ROOT / "codes/foundations/pre_a_cp1_st8_q3lock_phase_conditioned_intrinsic_gap.py"
INDEPENDENT = ROOT / "codes/foundations/pre_a_cp1_st8_q3lock_phase_conditioned_intrinsic_gap_independent.py"
HOSTILE = ROOT / "codes/foundations/pre_a_cp1_st8_q3lock_phase_conditioned_intrinsic_gap_hostile.py"
LEAN = ROOT / "verification/lean/Tect/R405.lean"
PRIMARY_OUTPUT = ROOT / "claims/C6-SPACETIME-SIGNATURE/runs/2026-08-30-primary-pre_a_cp1_st8_q3lock_phase_conditioned_intrinsic_gap/primary.json"
INDEPENDENT_OUTPUT = ROOT / "claims/C6-SPACETIME-SIGNATURE/runs/2026-08-30-independent-pre_a_cp1_st8_q3lock_phase_conditioned_intrinsic_gap/independent.json"
HOSTILE_OUTPUT = ROOT / "claims/C6-SPACETIME-SIGNATURE/runs/2026-08-30-hostile-pre_a_cp1_st8_q3lock_phase_conditioned_intrinsic_gap/hostile.json"
INTEGRATED_OUTPUT = ROOT / "claims/C6-SPACETIME-SIGNATURE/runs/2026-08-30-integrated-pre_a_cp1_st8_q3lock_phase_conditioned_intrinsic_gap/integrated.json"
...
def main() -> int:
    ...
    check("manifest identity", manifest["result_id"] == "R-405" and manifest["exploration_id"] == "EXP-001250" and manifest["claim_bearing"] is False, [manifest["result_id"], manifest["exploration_id"], manifest["claim_bearing"]], "R-405/EXP-001250/false", "identity")
    ...
    check("artifacts present", all(path.is_file() for path in artifacts), [str(path) for path in artifacts if not path.is_file()], "all R-405 artifacts", "provenance")
    ...
    expected_outputs = ((PRIMARY, PRIMARY_OUTPUT), (INDEPENDENT, INDEPENDENT_OUTPUT), (HOSTILE, HOSTILE_OUTPUT))
    for script, expected in expected_outputs:
    ...
    lean = command([str(LAKE), "env", "lean", "Tect/R405.lean"], ROOT / "verification/lean")
    ...
    scalar_fields = ("system_count", "profile_count", "row_count", "minimum_full_gap", "maximum_full_gap", "minimum_sector_gap", "maximum_sector_gap", "minimum_cross_capacity", "maximum_cross_capacity", "minimum_phase_mass", "maximum_neutral_mass")
    for field in scalar_fields:
        check(f"primary-independent {field}", close(p[field], i[field], tolerance), [p[field], i[field]], f"within {tolerance}", "independence")
    check("beta profile agreement", close(p["beta_profiles"], i["beta_profiles"], tolerance), [p["beta_profiles"], i["beta_profiles"]], f"within {tolerance}", "independence")
    ...
    check("sector gap positive", p["minimum_sector_gap"] > float(fixture["gap_floor"]), p["minimum_sector_gap"], f">{fixture['gap_floor']}", "within phase")
    check("cross capacity positive", p["minimum_cross_capacity"] > float(fixture["capacity_floor"]), p["minimum_cross_capacity"], f">{fixture['capacity_floor']}", "phase bottleneck")
    check("phase split observed", p["minimum_sector_gap"] > p["minimum_full_gap"], [p["minimum_full_gap"], p["minimum_sector_gap"]], "sector floor above global floor", "route split")
    ...
    check("hostile cross deletion", hostile_derived["cross_deleted_gap"] <= float(fixture["eigen_zero_threshold"]), hostile_derived["cross_deleted_gap"], f"<={fixture['eigen_zero_threshold']}", "hostile")
    payload = {"schema": "tect/pre-a-r405-integrated/1.0", "manifest": str(MANIFEST.relative_to(ROOT)).replace("\\", "/"), "result_id": "R-405", "exploration_id": "EXP-001250", "verdict": "PASS", "checks": checks, "derived": {"primary": p, "independent": i, "hostile": hostile_derived, "lean": "PASS", "command_outputs": outputs}, "scope": scope, "boundary": manifest["boundary"]}
    atomic_json(args.output if args.output.is_absolute() else ROOT / args.output, payload)
    print(f"INTEGRATED PHASE-CONDITIONED-GRAPH PASS {len(checks)}/{len(checks)} profiles={p['profile_count']} rows={p['row_count']} full_min={p['minimum_full_gap']:.6g} sector_min={p['minimum_sector_gap']:.6g} cross_min={p['minimum_cross_capacity']:.6g}")
    return 0
