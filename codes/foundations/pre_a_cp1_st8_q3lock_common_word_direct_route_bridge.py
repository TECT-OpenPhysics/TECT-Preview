#!/usr/bin/env python3
"""Primary provenance-linked bridge between finite OS and direct D routes."""
...
def analyse(os_run: dict[str, Any], direct_run: dict[str, Any], weighted_run: dict[str, Any], fixture: dict[str, Any]) -> dict[str, Any]:
    ...
    weighted_volumes = [int(row["volume"]) for row in weighted_rows]
...
def run() -> dict[str, Any]:
    ...
    os_run = read_input(inputs["direction_source_primary"], "EXP-001175")
    direct_run = read_input(inputs["extended_direct_primary"], "EXP-001169")
    weighted_run = read_input(inputs["source_weight_primary"], "EXP-001149")
...
def main() -> int:
    ...
    print(f"PRIMARY COMMON-WORD-DIRECT-ROUTE-BRIDGE PASS {payload['passed']}/{payload['assertion_count']} mixed_signal={payload['bridge_metrics']['mixed_finite_signal']}")
    return 0
