#!/usr/bin/env python3
"""Independent Fraction-free reconstruction of the finite Q3 cutoff audit.

This lane intentionally does not import the primary audit.  It rebuilds the
truncated oscillator, graph, local terms, support commutators, and fixed-step
coefficients independently, then emits the same row schema for the integrated
cross-check.
"""
...
def local_term(spec: tuple[str, tuple[int, ...]], union: list[int], dimension: int, fixture: dict[str, Any]) -> np.ndarray:
    ...
    left, right = support
    return hermitian(bond_term(q_ops[left], q_ops[right], fixture))


def reference_localization(volume: int, dimension: int, fixture: dict[str, Any], tolerance: float) -> float:
    q_single, p_single = oscillator(dimension)
    identity = np.eye(dimension, dtype=complex)
    ...
    for left_index, left_spec in enumerate(declared):
        for right_index in range(left_index + 1, len(declared)):
            ...
            left_support, right_support = set(left_spec[1]), set(right_spec[1])
            if left_support.isdisjoint(right_support):
                continue
            union = sorted(left_support | right_support)
            local_norm = operator_norm(commutator(local_term(left_spec, union, dimension, fixture), local_term(right_spec, union, dimension, fixture)))
...
def row(volume: int, cutoff: int, fixture: dict[str, Any], horizon: Fraction, step_count: int) -> dict[str, Any]:
    declared = specs(volume)
    q_single, p_single = oscillator(cutoff)
    term_norms: list[float] = []
    onsite_norms: list[float] = []
    bond_norms: list[float] = []
    for kind, support in declared:
        value = operator_norm(local_term((kind, support), sorted(support), cutoff, fixture))
        term_norms.append(value)
        (onsite_norms if kind == "onsite" else bond_norms).append(value)
    total = 0.0
    overlap_count = 0
    max_pair = 0.0
    ...
    for left_index, left_spec in enumerate(declared):
        for right_spec in declared[left_index + 1 :]:
            ...
            max_union_dimension = max(max_union_dimension, cutoff ** len(union))
            value = operator_norm(commutator(local_term(left_spec, union, cutoff, fixture), local_term(right_spec, union, cutoff, fixture)))
            total += value
            max_pair = max(max_pair, value)
    coarse = 2.0 * overlap_count * max(term_norms) ** 2
    delta = float(horizon) / step_count
    return {
        "volume": volume,
        "cutoff": cutoff,
        ...
        "commutator_sum": total,
        "commutator_sum_per_site": total / volume,
        "overlap_pair_count": overlap_count,
        "max_overlap_commutator": max_pair,
        "coarse_fixed_cutoff_envelope": coarse,
        "unitary_bound_at_step_count": float(horizon * horizon) * total / (2.0 * step_count),
        "history_unit_multiplier_coefficient": delta ** 3 * total * step_count * (step_count - 1) / 2.0,
    }
...
def run() -> dict[str, Any]:
    ...
    horizon = Fraction(audit["horizon"])
    ...
    check("graph volumes", volumes == [2, 4, 6] and all(len(graph_edges(volume)) > 0 for volume in volumes), volumes, "registered Q3 graph volumes", "fixture")
    ...
    check("fixed-step fixture", horizon == Fraction(1, 3) and step_count > 0, [str(horizon), step_count], ["1/3", ">0"], "fixture")
    ...
    for volume in volumes:
        volume_rows = [row(volume, cutoff, fixture, horizon, step_count) for cutoff in cutoffs]
        for value in volume_rows:
            check(f"V={volume} n={value['cutoff']} finite norms", all(np.isfinite(float(value[key])) for key in ("q_norm", "p_norm", "term_norm_max", "commutator_sum", "unitary_bound_at_step_count", "history_unit_multiplier_coefficient")), value, "finite", "numeric")
            check(f"V={volume} n={value['cutoff']} nonnegative commutator", value["commutator_sum"] >= 0.0, value["commutator_sum"], f">{floor}", "commutator")
            check(f"V={volume} n={value['cutoff']} fixed envelope", value["commutator_sum"] <= value["coarse_fixed_cutoff_envelope"] + tolerance * (1.0 + value["coarse_fixed_cutoff_envelope"]), [value["commutator_sum"], value["coarse_fixed_cutoff_envelope"]], "C_V<=2*overlap*K_n^2", "finite envelope")
            rows.append(value)
        commutators = [float(value["commutator_sum"]) for value in volume_rows]
        terms = [float(value["term_norm_max"]) for value in volume_rows]
        qs = [float(value["q_norm"]) for value in volume_rows]
        ...
        summary = {
            ...
            "term_norm_growth_ratio": terms[-1] / terms[0],
            "q_norm_growth_ratio": qs[-1] / qs[0],
...
def main() -> int:
    ...
    print(f"INDEPENDENT TROTTER-CUTOFF-SCALING PASS {payload['passed']}/{payload['assertion_count']} rows={payload['derived']['row_count']}")
    return 0
