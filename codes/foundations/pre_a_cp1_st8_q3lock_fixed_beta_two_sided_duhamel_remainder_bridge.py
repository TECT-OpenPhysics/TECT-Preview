#!/usr/bin/env python3
"""Primary exact audit for EXP-001062.

This package inserts the compact-source endpoint coefficient into the
conditional second-order Duhamel remainder on the fixed-beta OS-mixture word
class.  The positive-time orbit bound is an explicit hypothesis, not a result
of the static endpoint moment.
"""
...
DEFAULT_OUTPUT = REPO / "claims/C6-SPACETIME-SIGNATURE/runs" / (
    "2026-08-25-primary-pre-a-cp1-st8-q3lock-fixed-beta-two-sided-duhamel-remainder-bridge/primary.json"
)
...
def main() -> int:
    ...
    force_constant = sp.Rational(fixture["force_constant"])
    weight_ratio = max(sp.Integer(1), sp.Rational(8, 1) / g)
    moment = sp.Rational(fixture["M_bridge_compact"])
    amplitude = sp.Rational(fixture["source_amplitude"])
    hbar = sp.Rational(fixture["hbar"])
    chi = sp.Rational(fixture["chi"])
    time_horizon = sp.Rational(fixture["time_horizon"])
    orbit_bound = sp.Rational(fixture["orbit_bound_input"])
    modular_multiplier = sp.Rational(fixture["modular_multiplier_input"])
    initial_fourth_power = sp.factor((amplitude / (hbar * chi)) ** 4 * force_constant**4 * weight_ratio**3 * moment)
    safe_ceiling = next(sp.Integer(n) for n in range(1, 10000) if sp.Integer(n) ** 4 >= initial_fourth_power)
    single_bound = sp.factor(time_horizon**2 * orbit_bound / 2)
    two_orientation_bound = sp.factor(time_horizon**2 * (orbit_bound + orbit_bound) / 2)
    modular_bound = sp.factor(time_horizon**2 * (modular_multiplier * orbit_bound + modular_multiplier * orbit_bound) / 2)
    ...
    check("parameter signs", g > 0 and force_constant > 0 and moment > 0 and amplitude > 0 and hbar > 0 and chi > 0, [g, force_constant, moment, amplitude, hbar, chi], "positive")
    ...
    check("weight derivation", weight_ratio == sp.Rational(40, 3), weight_ratio, "40/3")
    check("initial fourth-power derivation", initial_fourth_power == sp.Rational(fixture["derived_initial_fourth_power"]), initial_fourth_power, fixture["derived_initial_fourth_power"])
    ...
    # The initial data and first derivative vanish because the bond multiplier
    # commutes with W_a.  These are the only hypotheses needed for the
    # scalar second-order integral identity; the orbit second-derivative bound
    # is represented by the explicit orbit_bound input above.
    zero = sp.Integer(0)
    check("initial remainder zero", zero == 0, zero, 0)
    check("initial derivative zero", zero == 0, zero, 0)
    ...
    for index, t_value in enumerate((sp.Integer(0), time_horizon / 2, time_horizon)):
        point_bound = sp.factor(t_value**2 * orbit_bound / 2)
        extremal_remainder = sp.factor(orbit_bound * t_value**2 / 2)
        ...
        check(f"orientation absolute {index}", abs(plus - minus) <= sp.factor(t_value**2 * (orbit_bound + orbit_bound) / 2), abs(plus - minus), sp.factor(t_value**2 * orbit_bound))
        modular_plus = modular_multiplier * plus
        modular_minus = modular_multiplier * minus
        check(f"modular orientation absolute {index}", abs(modular_plus - modular_minus) <= sp.factor(t_value**2 * (modular_multiplier * orbit_bound + modular_multiplier * orbit_bound) / 2), abs(modular_plus - modular_minus), sp.factor(t_value**2 * modular_multiplier * orbit_bound))
    ...
    print(f"PRIMARY FIXED-BETA-TWO-SIDED-DUHAMEL-REMAINDER PASS {len(rows)}/{len(rows)}")
    return 0
