#!/usr/bin/env python3
"""Integrated verifier for EXP-001120."""
...
LEAN = REPO / "verification/lean/Tect/R292.lean"
...
def lean_run() -> dict[str, Any]:
    command = "lake env lean Tect/R292.lean"
    lake = lake_path()
    if lake is None: return {"status": "UNAVAILABLE", "command": command, "output": "pinned lake executable not found"}
    process = subprocess.run([str(lake), "env", "lean", "Tect/R292.lean"], cwd=LEAN_ROOT, text=True, encoding="utf-8", capture_output=True, check=False)
...
def run() -> dict[str, Any]:
    ...
    check("identity", manifest["exploration_id"] == "EXP-001120" and manifest["task_id"] == "T-054", [manifest["exploration_id"], manifest["task_id"]], "EXP-001120/T-054")
    check("claim nonbearing", manifest["claim_bearing"] is False, manifest["claim_bearing"], False)
    source = LEAN.read_text(encoding="utf-8") if LEAN.is_file() else ""
    markers = ["fourth_word_fixture", "modular_fourth_word_fixture", "graph_fixture", "growth_threshold_fixture", "scope_fixture"]
    ...
    with tempfile.TemporaryDirectory(prefix="weighted-fourth-commutator-") as temporary:
        ...
        numeric = ("fourth_gibbs", "fourth_weighted", "triple_weighted", "tail_operator_norm", "fourth_identity_error")
        for p_volume, i_volume in zip(p_volumes, i_volumes):
            ...
            for p_row, i_row in zip(p_volume["radius_rows"], i_volume["radius_rows"]):
                check(f"V={p_volume['volume']} L={p_row['radius']} locality", close(p_row["source_commutator_norm"], i_row["source_commutator_norm"]) and close(p_row["disjoint_tail_commutator_norm"], i_row["disjoint_tail_commutator_norm"]), [p_row["source_commutator_norm"], i_row["source_commutator_norm"]], "within 1e-7 relative")
        ...
        for field in ("local_fourth_weighted_maxima", "full_fourth_weighted_maxima", "local_triple_weighted_maxima", "full_triple_weighted_maxima"):
        ...
        check("growth agreement", close(primary["derived"]["local_fourth_volume_growth"], independent["derived"]["local_fourth_volume_growth"]) and close(primary["derived"]["full_fourth_volume_growth"], independent["derived"]["full_fourth_volume_growth"]), [primary["derived"]["local_fourth_volume_growth"], independent["derived"]["local_fourth_volume_growth"], primary["derived"]["full_fourth_volume_growth"], independent["derived"]["full_fourth_volume_growth"]], "within 1e-7 relative")
        check("candidate growth", primary["derived"]["local_fourth_volume_growth"] >= float(fixture["growth_threshold"]) and primary["derived"]["full_fourth_volume_growth"] >= float(fixture["growth_threshold"]), [primary["derived"]["local_fourth_volume_growth"], primary["derived"]["full_fourth_volume_growth"]], f">={fixture['growth_threshold']}")
