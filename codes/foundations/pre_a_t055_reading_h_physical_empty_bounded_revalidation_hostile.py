#!/usr/bin/env python3
"""Hostile mutation checks for the R-427 preregistration firewall."""
...
DEFAULT_OUTPUT = ROOT / "claims/C6-SPACETIME-SIGNATURE/runs" / f"2026-08-30-hostile-{SLUG}/hostile.json"
...
def admissible_for_evaluation(doc: dict[str, Any]) -> bool:
    ...
    statuses = [doc["verdicts"][name]["status"] for name in names]
...
def main() -> int:
    ...
    drifted_contract = copy.deepcopy(original)
    drifted_contract["revalidation"]["same_contract_identity"] = False
    mutation("break R-420 identity lock", drifted_contract, "a revalidation cannot silently change the frozen contract")

    payload = {
        "schema": "tect/pre-a-t055-physical-empty-bounded-revalidation-hostile/1.0",
        "run_kind": "hostile",
        ...
        "controls": {
            ...
            "contract_identity_lock": original["revalidation"]["same_contract_identity"],
        },
    ...
    if args.self_test:
        ...
        print("R-427 HOSTILE SELFTEST: PASS (8 invalid mutations rejected)")
        return 0
    output = args.output if args.output.is_absolute() else ROOT / args.output
    write_json(output, payload)
    print(f"R-427 HOSTILE PASS {len(assertions)}/{len(assertions)} invalid mutations rejected")
    return 0
