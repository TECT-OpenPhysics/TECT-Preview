#!/usr/bin/env python3
"""Integrated verifier for the R-401 coordinate-metric checkpoint."""
...
MANIFEST = ROOT / "strategy/pre-a-cp1-st8-q3lock-coordinate-metric-gap-manifest.json"
PRIMARY = ROOT / "codes/foundations/pre_a_cp1_st8_q3lock_coordinate_metric_gap.py"
INDEPENDENT = ROOT / "codes/foundations/pre_a_cp1_st8_q3lock_coordinate_metric_gap_independent.py"
HOSTILE = ROOT / "codes/foundations/pre_a_cp1_st8_q3lock_coordinate_metric_gap_hostile.py"
LEAN = ROOT / "verification/lean/Tect/R401.lean"
PRIMARY_OUTPUT = ROOT / "claims/C6-SPACETIME-SIGNATURE/runs/2026-08-30-primary-pre_a_cp1_st8_q3lock_coordinate_metric_gap/primary.json"
INDEPENDENT_OUTPUT = ROOT / "claims/C6-SPACETIME-SIGNATURE/runs/2026-08-30-independent-pre_a_cp1_st8_q3lock_coordinate_metric_gap/independent.json"
HOSTILE_OUTPUT = ROOT / "claims/C6-SPACETIME-SIGNATURE/runs/2026-08-30-hostile-pre_a_cp1_st8_q3lock_coordinate_metric_gap/hostile.json"
INTEGRATED_OUTPUT = ROOT / "claims/C6-SPACETIME-SIGNATURE/runs/2026-08-30-integrated-pre_a_cp1_st8_q3lock_coordinate_metric_gap/integrated.json"
...
def main() -> int:
    ...
    check("manifest identity", manifest["result_id"] == "R-401" and manifest["exploration_id"] == "EXP-001246" and manifest["claim_bearing"] is False, [manifest["result_id"], manifest["exploration_id"], manifest["claim_bearing"]], "R-401/EXP-001246/false", "identity")
    ...
    check("artifacts present", all(path.is_file() for path in artifacts), [str(path) for path in artifacts if not path.is_file()], "all R-401 artifacts", "provenance")
    ...
    lean = command([str(LAKE), "env", "lean", "Tect/R401.lean"], ROOT / "verification/lean")
    ...
    fields = (
        "system_count", "profile_count", "index_ratio_count", "coordinate_ratio_count",
        "minimum_index_gap", "maximum_index_gap", "minimum_coordinate_gap", "maximum_coordinate_gap",
        "minimum_metric_gain", "maximum_metric_gain", "minimum_conditional_atom", "minimum_reference_atom",
        "minimum_coordinate_spacing", "minimum_adjacent_index_ratio", "maximum_adjacent_index_ratio",
        "minimum_adjacent_coordinate_ratio", "maximum_adjacent_coordinate_ratio", "profiles",
        "index_ratios", "coordinate_ratios",
    )
    for field in fields:
    ...
    check("profile rows positive", p["profile_count"] > 0 and p["index_ratio_count"] > 0 and p["coordinate_ratio_count"] > 0, [p["profile_count"], p["index_ratio_count"], p["coordinate_ratio_count"]], ">0", "coverage")
    check("finite gaps positive", p["minimum_index_gap"] > 0.0 and p["minimum_coordinate_gap"] > 0.0, [p["minimum_index_gap"], p["minimum_coordinate_gap"]], ">0", "conditional gaps")
    check("spacing positive", p["minimum_coordinate_spacing"] > 0.0, p["minimum_coordinate_spacing"], ">0", "coordinate metric")
    hostile_derived = hostile["derived"]
    check("hostile mutation caught", hostile_derived["index_gap"] > 0.0 and hostile_derived["coordinate_gap"] > 0.0 and hostile_derived["metric_gain"] > float(fixture["hostile_ratio_threshold"]), hostile_derived, f"gain>{fixture['hostile_ratio_threshold']}", "hostile")
    payload = {
        "schema": "tect/pre-a-r401-integrated/1.0",
    ...
    print(f"INTEGRATED COORDINATE-METRIC PASS {len(checks)}/{len(checks)} profiles={p['profile_count']} index_min={p['minimum_index_gap']:.6g} coordinate_min={p['minimum_coordinate_gap']:.6g} gain={p['minimum_metric_gain']:.6g}")
    return 0
