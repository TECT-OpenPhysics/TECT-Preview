#!/usr/bin/env python3
"""Integrated verifier for the A13 R-156 shifted-state local-gap package."""
...
MANIFEST = CLAIM_DIR / "classii_shifted_state_nonzero_neighborhood_gap_boundary_manifest.json"
PRIMARY = REPO / "codes/foundations/a13_classii_shifted_state_nonzero_neighborhood_gap_boundary.py"
INDEPENDENT = REPO / "codes/foundations/a13_classii_shifted_state_nonzero_neighborhood_gap_boundary_independent.py"
NOTE = CLAIM_DIR / "notes/classii-shifted-state-nonzero-neighborhood-gap-boundary-260803-v1.0.tex.txt"
...
EXPECTED_CHILD_COUNTS = {"primary": 29, "independent": 21}
EXPLORATION_IDS = ("EXP-000677", "EXP-000678", "EXP-000679")
NEGATIVE_ID = "NG-2026-08-03-A13-NONLINEAR-SHIFTED-STATE-PULLBACK-SOURCE-CONVEXITY"
...
def run_child(path: Path) -> subprocess.CompletedProcess[str]:
    return subprocess.run([sys.executable, str(path)], cwd=REPO, capture_output=True, text=True, encoding="utf-8", errors="replace", timeout=180)
...
def main() -> int:
    ...
    embedded_rows = 0
    for name, path, output in (("primary", PRIMARY, PRIMARY_OUTPUT), ("independent", INDEPENDENT, INDEPENDENT_OUTPUT)):
        run = run_child(path)
    ...
    for name in (
        "origin_pure_dyadic_gap", "target_gap", "continuation_headroom",
        "chosen_continuity_perturbation_allowance", "retained_gap_under_allowance",
        "fixture_tangent_gram", "fixture_source_pullback_mixed_hessian", "schur_fixture",
    ):
        audit.check("parity", name, normalize_exact(p[name]) == normalize_exact(i[name]), normalize_exact(p[name]), normalize_exact(i[name]))
    audit.check("oracle", "origin gap exact", F(p["origin_pure_dyadic_gap"]) == F(147, 1000), p["origin_pure_dyadic_gap"], "147/1000")
    audit.check("oracle", "target exact", F(p["target_gap"]) == F(1, 10), p["target_gap"], "1/10")
    audit.check("oracle", "headroom exact", F(p["continuation_headroom"]) == F(47, 1000), p["continuation_headroom"], "47/1000")
    audit.check("oracle", "retained gap exact", F(p["retained_gap_under_allowance"]) == F(247, 2000), p["retained_gap_under_allowance"], "247/2000")
    audit.check("oracle", "nonlinear chart counterexample", i["aligned_ray_hessian_eigenvalues_R2"] == ["-27/10", "117/10"], i["aligned_ray_hessian_eigenvalues_R2"], ["-27/10", "117/10"])

    artifacts = {"primary": PRIMARY, "independent": INDEPENDENT, "verifier": Path(__file__).resolve(), "note": NOTE, "pdf": PDF, "primary_result": PRIMARY_OUTPUT, "independent_result": INDEPENDENT_OUTPUT}
    ...
    for token in ("nonzero shifted-state neighbourhood", "intrinsic physical-source Hessian", "projected force", "legal reverse", "no new low", "47\\over1000", "247\\over2000", "Nonlinear pullback curvature no-go", "Devil's-advocate", "T-050", "phase", "PDE", "Result footer"):
    ...
    for token in ("R-156", "nonzero", "pullback curvature", "T-050", "PDE"):
        audit.check("pdf", f"text contains {token}", token in extracted, token, "present")
    compact_lines = " ".join(extracted.split())
    audit.check("pdf", "text contains 47 over 1000", "47 1000" in compact_lines, "47/1000", "present as extracted numerator/denominator")
    ...
    if renderer is not None:
        with tempfile.TemporaryDirectory(prefix="tect-r156-render-") as temporary:
    ...
    audit.check("records", "R-156 ledger entry", '<a id="r-156"></a>' in results_text, LEDGER_ID, "registered")
    audit.check("records", "negative registered", NEGATIVE_ID in negative_text, NEGATIVE_ID, "registered")
    audit.check("records", "claim narrative", RESULT_ID in claim_text and "47/1000" in claim_text, RESULT_ID, "registered")
    audit.check("records", "status synchronized", status.get("no_overclaim") == manifest.get("no_overclaim") and "R-156" in status.get("statement", ""), status.get("no_overclaim"), manifest.get("no_overclaim"))
    audit.check("records", "TODO route", "R-156" in todo_text and "T-050" in todo_text and "T-054" in todo_text, "R-156/T-050/T-054", "registered")
    audit.check("records", "changelog entry", "R-156" in changelog_text and "shifted-state nonzero-neighbourhood" in changelog_text.lower(), LEDGER_ID, "registered")
    audit.check("records", "theorem map", "R-156" in json.dumps(theorem_map, sort_keys=True), LEDGER_ID, "registered")
    audit.check("records", "proof map", "R-156" in proof_map and all(identifier in proof_map for identifier in EXPLORATION_IDS), EXPLORATION_IDS, "registered")
    ...
    if all(identifier in exploration_lookup for identifier in EXPLORATION_IDS):
        audit.check("records", "exploration verdicts", [exploration_lookup[x]["verdict"] for x in EXPLORATION_IDS] == ["advanced", "advanced", "failed"], [exploration_lookup[x]["verdict"] for x in EXPLORATION_IDS], ["advanced", "advanced", "failed"])
