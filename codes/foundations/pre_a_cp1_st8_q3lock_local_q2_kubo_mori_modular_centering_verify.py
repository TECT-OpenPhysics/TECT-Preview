#!/usr/bin/env python3
"""Integrated primary/independent/Lean verifier for EXP-001214 / R-372."""
...
LEAN = REPO / "verification/lean/Tect/R372.lean"
...
def compile_lean() -> dict[str, Any]:
    lake = lake_path()
    command = "lake env lean Tect/R372.lean"
    if lake is None:
        return {"status": "UNAVAILABLE", "command": command, "output": "pinned lake executable not found"}
    process = subprocess.run([str(lake), "env", "lean", "Tect/R372.lean"], cwd=LEAN_ROOT, text=True, encoding="utf-8", capture_output=True, check=False)
...
def run() -> dict[str, Any]:
    ...
    check("identity", manifest["exploration_id"] == "EXP-001214" and manifest["result_id"] == "R-372" and not manifest["claim_bearing"], [manifest["exploration_id"], manifest["result_id"], manifest["claim_bearing"]], "EXP-001214/R-372/false")
    ...
    markers = ("weighted_mean_real", "weighted_variance_decomposition", "weighted_variance_nonnegative", "centered_shell_pair_invariant", "gibbs_difference_pair_bound", "centered_row_nonnegative", "fixture_edge", "fixture_square", "scope_fixture")
    ...
    registry = json.loads(LEAN_REGISTRY.read_text(encoding="utf-8"))
    entries = [entry for entry in registry["entrypoints"] if entry["path"] == "verification/lean/Tect/R372.lean"]
    ...
    with tempfile.TemporaryDirectory(prefix="q3-modular-centering-") as temporary:
        ...
        exact_fields = ("context_count", "expected_contexts", "theta", "modular_centering_identity_finite_checked", "thermal_variance_bound_finite_checked", "shell_centering_invariance_finite_checked", "local_second_moment_uniformity_proved", "local_variance_uniformity_proved", "weighted_cutoff_uniformity_proved", "weighted_volume_uniformity_proved", "source_uniformity_proved", "shape_uniformity_proved", "local_modular_dirichlet_comparison_proved", "common_core_closed", "common_alpha_closed", "kms_gns_gap_closed", "continuum_closed", "c6_closed", "sector_a_closed", "pre_a_closed")
        for field in exact_fields:
            check(f"agreement {field}", p[field] == i[field], [p[field], i[field]], "equal")
        numeric_fields = ("maximum_gibbs_theta_half_identity_error", "maximum_centering_identity_error", "maximum_shell_centering_invariance_error", "maximum_thermal_variance_bound_violation", "maximum_raw_second_moment_bound_violation", "maximum_raw_local_gibbs_second_moment", "maximum_modular_local_gibbs_variance", "minimum_modular_local_gibbs_variance", "maximum_variance_to_raw_second_moment_ratio", "maximum_bond_gibbs_center_shift", "maximum_kubo_weight_asymmetry", "minimum_kubo_weight", "maximum_bond_unitary_factorization_error")
        ...
        for regime_index, (pr, ir) in enumerate(zip(p["regimes"], i["regimes"])):
            ...
            for field in ("maximum_raw_second_moment", "maximum_modular_variance"):
            ...
            for bond_index, (pb, ib) in enumerate(zip(pr["bond_rows"], ir["bond_rows"])):
                ...
                for field in ("maximum_raw_second_moment", "maximum_modular_variance", "maximum_variance_to_raw_ratio"):
        ...
        open_fields = ("local_second_moment_uniformity_proved", "local_variance_uniformity_proved", "weighted_cutoff_uniformity_proved", "weighted_volume_uniformity_proved", "source_uniformity_proved", "shape_uniformity_proved", "local_modular_dirichlet_comparison_proved", "common_core_closed", "common_alpha_closed", "kms_gns_gap_closed", "continuum_closed", "c6_closed", "sector_a_closed", "pre_a_closed")
...
def main() -> int:
    ...
    print(f"INTEGRATED MODULAR-CENTERING PASS {payload['assertion_count']}/{payload['assertion_count']}; Lean={payload['lean']['status']} diff={payload['derived']['max_primary_independent_numeric_difference']:.3e}")
    return 0
