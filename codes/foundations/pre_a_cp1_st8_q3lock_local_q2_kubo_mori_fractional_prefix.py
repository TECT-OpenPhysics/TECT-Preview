#!/usr/bin/env python3
"""Primary local Kubo--Mori weighted fractional-prefix audit for EXP-001210 / R-368."""
...
def run() -> dict[str, Any]:
    ...
    check("authority", manifest["exploration_id"] == "EXP-001210" and manifest["result_id"] == "R-368", [manifest["exploration_id"], manifest["result_id"]], "EXP-001210/R-368", "provenance")
    ...
    for regime in regimes:
            ...
            for beta_fraction in betas:
                ...
                for site in sites:
                    ...
                    for order_name, order in orders.items():
                        for sign in signs:
                            prefixes = all_prefixes(terms, order, sign, delta, hbar)
                            for prefix_length, prefix in prefixes:
                                for history_adjoint in adjoints:
                                    ...
                                    coefficient_phase_error = float(np.linalg.norm(change_transformed - phases * transformed, ord="fro"))
                                    trace_change = abs(complex(np.trace(omega @ change)))
                                    kubo_fractional_norm = float(np.sqrt(max(float(2.0 * np.sum(kubo_weights * np.abs(deltas) ** (2.0 * theta) * np.abs(transformed) ** 2)), 0.0)))
                                    kubo_change_norm = float(np.sqrt(max(float(2.0 * np.sum(kubo_weights * np.abs(change_transformed) ** 2)), 0.0)))
                                    kubo_phase_norm = float(np.sqrt(max(float(2.0 * np.sum(kubo_weights * np.abs(phases * transformed) ** 2)), 0.0)))
                                    ...
                                    arithmetic_fractional_norm = float(np.sqrt(max(float(2.0 * np.sum(arithmetic_weights * np.abs(deltas) ** (2.0 * theta) * np.abs(transformed) ** 2)), 0.0)))
        ...
        expected_regime = sum(len(cutoffs) * len(betas) * len(sites) * len(orders) * len(signs) * len(all_prefixes(base.split_system(volume, cutoffs[0], parameters)[2], list(range(len(base.split_system(volume, cutoffs[0], parameters)[2]))), signs[0], delta, hbar)) * len(adjoints) for _ in [0])
        per_regime.append({"volume": volume, "cutoffs": cutoffs, "sites": sites, "contexts": context_count - regime_start, "expected_contexts": expected_regime, "maximum_fractional_norm": regime_max_norm, "maximum_fractional_bound": regime_max_bound, "maximum_kubo_fractional_norm": regime_max_kubo_norm, "maximum_kubo_fractional_bound": regime_max_kubo_bound})
...
def main() -> int:
    ...
    print(f"PRIMARY KUBO-MORI FRACTIONAL PREFIX PASS {payload['assertion_count']}/{payload['assertion_count']} contexts={payload['derived']['context_count']} max_kubo={payload['derived']['maximum_kubo_fractional_norm']:.6f}")
    return 0
