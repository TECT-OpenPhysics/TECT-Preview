#!/usr/bin/env python3
"""Integrated verifier for EXP-001133."""
...
LEAN = REPO / "verification/lean/Tect/R304.lean"
...
def lean_run() -> dict[str, Any]:
    lake = lake_path()
    command = "lake env lean Tect/R304.lean"
    if lake is None:
        return {"status": "UNAVAILABLE", "command": command, "output": "pinned lake executable not found"}
    process = subprocess.run([str(lake), "env", "lean", "Tect/R304.lean"], cwd=LEAN_ROOT, text=True, encoding="utf-8", capture_output=True, check=False)
...
def run() -> dict[str, Any]:
    ...
    check("identity", manifest["exploration_id"] == "EXP-001133" and manifest["task_id"] == "T-054", [manifest["exploration_id"], manifest["task_id"]], "EXP-001133/T-054")
    check("claim nonbearing", manifest["claim_bearing"] is False, manifest["claim_bearing"], False)
    source = LEAN.read_text(encoding="utf-8") if LEAN.is_file() else ""
    markers = ["regularized_coordinate_square_fixture", "support_bound_fixture", "cross_orientation_multiplier_fixture"]
    ...
    with tempfile.TemporaryDirectory(prefix="bounded-lipschitz-cylinder-") as temporary:
        ...
        check("support count", p.get("support_count") == i.get("support_count") == 4, [p.get("support_count"), i.get("support_count")], 4)
        ...
        check("coordinate agreement", p.get("coordinate_rows") == i.get("coordinate_rows"), "exact regularized-coordinate rows", "equal")
        check("support agreement", p.get("support_rows") == i.get("support_rows"), "exact support rows", "equal")
        check("bounded class agreement", all(p.get(key) is True and i.get(key) is True for key in ("bounded_lipschitz_cylinder_form_invariance_closed", "bounded_lipschitz_weighted_square_root_products_closed", "weyl_subclass_closed", "fixed_support_volume_uniformity_closed")), [p, i], True)
        check("unbounded firewall", p.get("all_bounded_local_observables_closed") is False and p.get("unbounded_polynomial_products_closed") is False and p.get("weighted_product_domain_closed") is False, p, "bounded Lipschitz class only")

    check("QFT firewall", all(scope[key] is False for key in ("all_bounded_local_observables_closed", "unbounded_polynomial_products_closed", "weighted_product_domain_closed", "modular_domain_transfer_closed", "direct_d_cauchy_closed", "common_alpha_closed", "kms_gns_gap_closed", "continuum_closed", "pre_a_closed")), scope, "bounded cylinder bridge only")
    lean = lean_run()
    check("Lean compile", lean["status"] == "PASS", lean, "PASS")
    return {"schema": "tect/foundation-audit/1.0", "run_kind": "integrated", "audit_id": "PA-CP1-ST8-Q3LOCK-BOUNDED-LIPSCHITZ-CYLINDER", "claim_id": manifest["claim_ids"][0], "task_id": manifest["task_id"], "exploration_id": manifest["exploration_id"], "verdict": "PASS", "assertion_count": len(rows), "assertions": rows, "lean": lean, "boundary": scope, "recorded_at": datetime.now(timezone.utc).isoformat().replace("+00:00", "Z"), "provenance": {"primary_sha256": sha256(PRIMARY), "independent_sha256": sha256(INDEPENDENT), "manifest_sha256": sha256(MANIFEST), "lean_sha256": sha256(LEAN)}}
...
def main() -> int:
    ...
    print(f"INTEGRATED BOUNDED-LIPSCHITZ-CYLINDER PASS {payload['assertion_count']}/{payload['assertion_count']}; Lean={payload['lean']['status']}")
    return 0
