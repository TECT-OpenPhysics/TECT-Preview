#!/usr/bin/env python3
"""Integrated verifier for EXP-001143 finite energy-moment transfer."""
...
LEAN = REPO / "verification/lean/Tect/R313.lean"
...
def lean_run() -> dict[str, Any]:
    command = "lake env lean Tect/R313.lean"
    lake = lake_path()
    if lake is None:
        return {"status": "UNAVAILABLE", "command": command, "output": "pinned lake executable not found"}
    process = subprocess.run([str(lake), "env", "lean", "Tect/R313.lean"], cwd=LEAN_ROOT, text=True, encoding="utf-8", capture_output=True, check=False)
...
def compare_summary(primary: list[dict[str, Any]], independent: list[dict[str, Any]], tolerance: float) -> tuple[bool, list[dict[str, Any]]]:
    ...
    keys = (
        "volume", "beta", "M2", "max_spectral_second_moment",
        "max_operator_candidate_bound", "max_safe_hilbert_schmidt_bound",
        "max_operator_transfer_ratio", "max_hilbert_schmidt_transfer_ratio",
    )
...
def run() -> dict[str, Any]:
    ...
    check("identity", manifest["exploration_id"] == "EXP-001143" and manifest["task_id"] == "T-054", [manifest["exploration_id"], manifest["task_id"]], "EXP-001143/T-054")
    ...
    with tempfile.TemporaryDirectory(prefix="finite-energy-moment-transfer-") as temporary:
        ...
        check("finite transfer agreement", p.get("finite_energy_moment_transfer_closed") is True and i.get("finite_energy_moment_transfer_closed") is True and p.get("finite_hilbert_schmidt_transfer_closed") is True and i.get("finite_hilbert_schmidt_transfer_closed") is True, [p, i], "finite transfer only")
        check("operator candidate remains diagnostic", p.get("general_operator_norm_transfer_theorem_closed") is False and i.get("general_operator_norm_transfer_theorem_closed") is False, [p.get("general_operator_norm_transfer_theorem_closed"), i.get("general_operator_norm_transfer_theorem_closed")], False)

    check("scope firewall", scope["finite_energy_moment_transfer_closed"] and scope["termwise_arithmetic_mean_bound_closed"] and scope["actual_q3_rows_closed"] and scope["two_orientation_history_closed"] and not scope["uniform_local_M2_common_core_closed"] and not scope["modular_domain_transfer_closed"] and not scope["common_alpha_closed"] and not scope["pre_a_closed"], scope, "finite transfer; QFT gates open")
    lean = lean_run()
    ...
    return {
        ...
        "derived": {
            ...
            "finite_energy_moment_transfer_promoted": True,
            "finite_hilbert_schmidt_transfer_promoted": True,
            "operator_norm_candidate_empirical_only": True,
            "uniformity_promoted": False,
            "common_core_promoted": False,
            "qft_promoted": False,
        },
...
def main() -> int:
    ...
    print(f"INTEGRATED FINITE-ENERGY-MOMENT-TRANSFER PASS {payload['assertion_count']}/{payload['assertion_count']}; Lean={payload['lean']['status']}")
    return 0
