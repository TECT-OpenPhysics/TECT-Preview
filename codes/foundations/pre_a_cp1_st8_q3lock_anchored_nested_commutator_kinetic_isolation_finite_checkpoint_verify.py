#!/usr/bin/env python3
"""Integrated primary/independent/hostile/Lean verifier for R-387."""
...
LEAN = REPO / "verification/lean/Tect/R387.lean"
...
def compile_lean() -> dict[str, Any]:
    lake = pinned_lake()
    command = "lake env lean Tect/R387.lean"
    if lake is None:
        return {"status": "UNAVAILABLE", "command": command, "output": "pinned lake executable not found"}
    process = subprocess.run([str(lake), "env", "lean", "Tect/R387.lean"], cwd=LEAN_ROOT, text=True, encoding="utf-8", capture_output=True, check=False)
...
def run() -> dict[str, Any]:
    ...
    check("identity", manifest["exploration_id"] == "EXP-001230" and manifest["result_id"] == "R-387" and manifest["claim_bearing"] is False, [manifest["exploration_id"], manifest["result_id"], manifest["claim_bearing"]], "EXP-001230/R-387/false")
    ...
    markers = ["commutator_addition", "kinetic_isolation", "potential_scale_invariance", "scope_fixture"]
    ...
    registry = json.loads(REGISTRY.read_text(encoding="utf-8"))
    entries = [entry for entry in registry["entrypoints"] if entry["path"] == "verification/lean/Tect/R387.lean"]
    ...
    with tempfile.TemporaryDirectory(prefix="r387-") as temporary:
        temp = Path(temporary)
    ...
    tolerance = float(manifest["finite_fixture"]["isolation_tolerance"])
    for field in ("potential_commutator_residual", "boundary_commutator_residual", "inner_isolation_residual", "nested_isolation_residual", "scale_invariance_residual", "weighted_isolation_residual"):
        check(f"agreement {field}", abs(float(pd["maximums"][field]) - float(indep["maximums"][field])) <= tolerance, [pd["maximums"][field], indep["maximums"][field]], f"within {tolerance}")
    for field in ("context_count", "expected_contexts", "bond_prefix_count", "seed_rows", "weighted_rows"):
        check(f"agreement {field}", pd[field] == indep[field], [pd[field], indep[field]], "equal")
    for field in ("finite_coordinate_potential_commutation_closed", "finite_kinetic_isolation_closed", "finite_potential_scale_invariance_closed", "finite_weighted_isolation_closed"):
        check(f"agreement {field}", pd[field] is True and indep[field] is True, [pd[field], indep[field]], "true")
    ...
    check("hostile rejected", hostile["derived"]["wrong_orientation_rejected"] is True, hostile["derived"], "momentum mutation rejected")
    ...
    numeric_difference = max(abs(float(pd["maximums"][field]) - float(indep["maximums"][field])) for field in ("potential_commutator_residual", "boundary_commutator_residual", "inner_isolation_residual", "nested_isolation_residual", "scale_invariance_residual", "weighted_isolation_residual"))
    return {"schema": "tect/foundation-audit/1.0", "run_kind": "integrated", "audit_id": "PA-CP1-ST8-Q3LOCK-ANCHORED-NESTED-COMMUTATOR-KINETIC-ISOLATION-FINITE-CHECKPOINT", "claim_id": manifest["claim_ids"][0], "result_id": manifest["result_id"], "task_id": manifest["task_id"], "exploration_id": manifest["exploration_id"], "verdict": "PASS", "assertion_count": assertion_count, "assertions": rows, "lean": lean, "derived": {"primary": pd, "independent": indep, "hostile": hostile["derived"], "max_primary_independent_numeric_difference": numeric_difference}, "boundary": manifest["boundary"], "recorded_at": datetime.now(timezone.utc).isoformat().replace("+00:00", "Z")}
...
def main() -> int:
    ...
    print(f"INTEGRATED ANCHORED NESTED-COMMUTATOR KINETIC-ISOLATION PASS {payload['assertion_count']}/{payload['assertion_count']}; Lean={payload['lean']['status']} diff={payload['derived']['max_primary_independent_numeric_difference']:.3e}")
    return 0
