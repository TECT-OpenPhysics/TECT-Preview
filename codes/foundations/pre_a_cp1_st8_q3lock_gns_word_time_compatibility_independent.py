#!/usr/bin/env python3
"""Independent finite Q3 word/time enlargement compatibility audit for EXP-001182."""
...
def run() -> dict[str, Any]:
    ...
    for small_name, large_name in fixture["nested_pairs"]:
        ...
        for beta in beta_values:
            ...
            base_residual = np.linalg.norm(base_transport.conj().T @ g_large @ base_transport - g_small, ord="fro") / max(np.linalg.norm(g_small, ord="fro"), partition_floor)
            expanded_residual = np.linalg.norm(expanded_transport.conj().T @ g_large_expanded @ expanded_transport - g_small_expanded, ord="fro") / max(np.linalg.norm(g_small_expanded, ord="fro"), partition_floor)
            principal_residual = np.linalg.norm(g_small_expanded[np.ix_(base_indices, base_indices)] - g_small, ord="fro") / max(np.linalg.norm(g_small, ord="fro"), partition_floor)
...
def main() -> int:
    ...
    print(f"INDEPENDENT GNS-WORD-TIME-COMPATIBILITY PASS {payload['passed']}/{payload['assertion_count']} blocks={len(payload['rows'])}")
    return 0
