#!/usr/bin/env python3
"""Independent Fraction-only audit for EXP-001070."""
...
def run() -> dict[str, Any]:
    ...
    check("remainder upper", remainder_upper == frac(fixture["derived_remainder_squared_upper"]), remainder_upper, fixture["derived_remainder_squared_upper"], "scaling")
    ...
    passed = len(rows)
    return {"schema": "tect/foundation-audit/1.0", "run_kind": "independent", "audit_id": "PA-CP1-ST8-Q3LOCK-SINGLE-CHARACTER-UNIFORM-DUHAMEL-REMAINDER", "exploration_id": manifest["exploration_id"], "task_id": manifest["task_id"], "verdict": "PASS", "passed": passed, "assertion_count": passed, "assertions": rows, "derived": derived, "boundary": scope}
...
def main() -> int:
    ...
    print(f"INDEPENDENT SINGLE-CHARACTER-DUHAMEL PASS {payload['passed']}/{payload['assertion_count']}")
    return 0
