#!/usr/bin/env python3
"""Integrated verifier for the centered-nodal Q3 Nelson/affine-seam theorem."""
...
PARENT_EXPLORATIONS = ("EXP-000770", "EXP-000771")
...
def build_payload() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="tect-centered-nelson-") as directory:
    ...
    for name in ("Q3 coercive coefficient", "whole-Wick quartic scalar", "normalized scalar-shift invariance", "Nelson Young exponent table", "finite BD Gaussian value", "Nyquist lift included", "discrete shifted Leibniz identity", "seam spatial cost beta/24", "scope firewall below_empty_space_comparison"):
        audit.check(f"primary coverage {name}", name in primary_names, name, "present", "coverage")
    for name in ("independent Q3 coercivity fixtures", "independent whole-Wick scalar coefficients", "independent Hermite translation n4", "independent Nelson q table", "independent cyclic lift M10d4", "independent discrete Leibniz", "independent seam spatial coefficient", "independent seam Ax mutation", "independent scope firewall below_empty_space_comparison"):
        audit.check(f"independent coverage {name}", name in independent_names, name, "present", "coverage")
    audit.check("cross Q3 edges", primary["derived"]["edges"] == independent["derived"]["edges"], primary["derived"]["edges"], independent["derived"]["edges"], "cross")
    primary_q = [row["q"] for row in primary["derived"]["exponents"]]
    independent_q = [row[-1] for row in independent["derived"]["exponents"]]
    audit.check("cross Nelson exponents", primary_q == independent_q == ["16/11", "16/7", "16/3"], primary_q, independent_q, "cross")
    audit.check("cross seam beta/24", primary["derived"]["seam"]["spatial"].endswith("/24") and independent["derived"]["seam"]["spatial"] == "1/4", (primary["derived"]["seam"], independent["derived"]["seam"]), "symbolic and beta=6", "cross")
    audit.check("distinct symbol fixtures", primary["derived"]["symbol"] != independent["derived"]["symbol"], primary["derived"]["symbol"], independent["derived"]["symbol"], "cross")
    audit.check("distinct convolution fixtures", primary["derived"]["convolution_tails"] != independent["derived"]["convolution_tails"], "distinct", "distinct", "cross")
    ...
    for phrase in ("sup_M", "Boue--Dupuis", "all-M hybrid-lattice Wick estimate", "total-variation convergence", "exact affine seam", "regular state", "energy below empty space", "Pre-A"):
    ...
    for parent in PARENT_EXPLORATIONS:
        audit.check(f"exploration parent {parent}", any(item.get("id") == parent and item.get("relation") in {"continues", "composes"} for item in exploration.get("related", [])), exploration.get("related"), parent, "records")
