#!/usr/bin/env python3
"""Primary exact 2x2 stress test for EXP-001064."""
...
DEFAULT_OUTPUT = REPO / "claims/C6-SPACETIME-SIGNATURE/runs" / (
    "2026-08-25-primary-pre-a-cp1-st8-q3lock-fixed-beta-mixture-seminorm-orbit-contract-stress-test/primary.json"
)
...
def main() -> int:
    ...
    p = sp.Rational(fixture["polarization"])
    c = sp.Rational(3, 5)
    s = sp.Rational(4, 5)
    sigma_x = sp.Matrix([[0, 1], [1, 0]])
    sigma_z = sp.Matrix([[1, 0], [0, -1]])
    identity = sp.eye(2)
    rho = (identity + p * sigma_z) / 2
    U = c * identity - sp.I * s * sigma_x
    P_minus = (identity - sigma_z) / 2
    X_evolved = sp.conjugate(U).T * P_minus * U
    norm_squared = lambda X: sp.simplify(sp.trace(rho * (sp.conjugate(X).T * X)) + sp.trace(rho * (X * sp.conjugate(X).T)))
    initial = norm_squared(P_minus)
    evolved = norm_squared(X_evolved)
    gap = sp.factor(evolved - initial)
    ...
    check("polarization", p == sp.Rational(fixture["polarization"]), p, fixture["polarization"])
    check("unitarity", sp.simplify(sp.conjugate(U).T * U - identity) == sp.zeros(2), sp.conjugate(U).T * U, identity)
    check("initial norm", initial == sp.Rational(fixture["initial_squared_norm"]), initial, fixture["initial_squared_norm"])
    check("evolved diagonal", [X_evolved[0, 0], X_evolved[1, 1]] == [sp.Rational(value) for value in fixture["rotation_diagonal"]], [X_evolved[0, 0], X_evolved[1, 1]], fixture["rotation_diagonal"])
    check("evolved norm", evolved == sp.Rational(fixture["evolved_squared_norm"]), evolved, fixture["evolved_squared_norm"])
    check("growth gap", gap == sp.Rational(fixture["growth_gap"]), gap, fixture["growth_gap"])
    ...
    print(f"PRIMARY MIXTURE-SEMINORM-ORBIT-STRESS PASS {len(rows)}/{len(rows)}")
    return 0
