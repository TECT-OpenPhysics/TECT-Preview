#!/usr/bin/env python3
"""Integrated verifier for the A13 R-162 recursive-chain package."""
...
MANIFEST = CLAIM_DIR / "classii_resolvent_pure_dyadic_recursive_chain_uniform_neighborhood_gap_boundary_manifest.json"
PRIMARY = REPO / "codes/foundations/a13_classii_resolvent_pure_dyadic_recursive_chain_uniform_neighborhood_gap_boundary.py"
INDEPENDENT = REPO / "codes/foundations/a13_classii_resolvent_pure_dyadic_recursive_chain_uniform_neighborhood_gap_boundary_independent.py"
NOTE = CLAIM_DIR / "notes/classii-resolvent-pure-dyadic-recursive-chain-uniform-neighborhood-gap-boundary-260803-v1.0.tex.txt"
...
EXPECTED_CHILD_COUNTS = {"primary": 46, "independent": 41}
EXPECTED_INTEGRATOR_ONLY = 141
EXPECTED_INTEGRATED = 228
EXPLORATION_IDS = ("EXP-000708", "EXP-000709", "EXP-000710")
...
def main() -> int:
    ...
    expected_count_contract = {
        "primary_assertions": EXPECTED_CHILD_COUNTS["primary"],
        "independent_assertions": EXPECTED_CHILD_COUNTS["independent"],
        "embedded_child_assertions": sum(EXPECTED_CHILD_COUNTS.values()),
        "integrator_only_assertions": EXPECTED_INTEGRATOR_ONLY,
        "integrated_assertions": EXPECTED_INTEGRATED,
    }
    actual_count_contract = {key: manifest["verification"].get(key) for key in expected_count_contract}
    audit.check("manifest", "frozen assertion-count contract", actual_count_contract == expected_count_contract, actual_count_contract, expected_count_contract)

    children: dict[str, dict[str, Any]] = {}
    embedded_rows = 0
    temp_parent = REPO / "internal" / "tmp"
    temp_parent.mkdir(parents=True, exist_ok=True)
    with tempfile.TemporaryDirectory(prefix="r162-child-", dir=temp_parent) as temporary:
    ...
    audit.check("scope", "exact common no-overclaim", manifest.get("no_overclaim") == primary.get("no_overclaim") == independent.get("no_overclaim"), [manifest.get("no_overclaim"), primary.get("no_overclaim"), independent.get("no_overclaim")], "exactly equal")
    for label, text in (("manifest", manifest.get("no_overclaim", "")), ("primary", primary.get("no_overclaim", "")), ("independent", independent.get("no_overclaim", ""))):
        for token in ("single", "shifted-state", "controller-pullback", "intrinsic", "multi-chain", "random", "conditional", "T-050/A13", "phase/PDE", "Sector-A"):
            audit.check("scope", f"{label} boundary token {token}", token.lower() in str(text).lower(), token, "present")
    ...
    parity = {
        "synthesis g": (pd["g"], ider["g"]),
        "value synthesis": (pd["B_S"], ider["B_S"]),
        "current synthesis": (pd["B_D"], ider["B_D"]),
        "data map squared": (pd["c_Z_squared"], ider["c_Z_squared"]),
        "covariance jet factors": (pd["covariance_jet_factors_at_r0"], ider["jet_factors_at_r0"]),
        "terminal D3 coefficients": (pd["terminal_D3_coefficients_M1_M2_M3"], ider["terminal_D3_coefficients_M1_M2_M3"]),
        "source third derivative": (pd["CM_D3_bound_at_r0"], ider["CM_D3_bound_at_r0"]),
    ...
    audit.check("oracle", "data-map contraction", F(pd["c_Z_squared"]) < 1, pd["c_Z_squared"], "<1")
    audit.check("oracle", "metric guard exact", F(100, 97) ** 4 < F(13, 10), F(100, 97) ** 4, "<13/10")
    audit.check("oracle", "actual recursive read load-bearing", primary["scope"].get("actual_shifted_state_read_at_each_stage") is True and primary["scope"].get("deterministic_matrix_coefficients") is True, primary["scope"], "actual deterministic shifted-state recursion")
    audit.check("oracle", "complete connections retained", primary["scope"].get("projected_force_connection_included") is True and primary["scope"].get("sextic_connection_included") is True, primary["scope"], "both true")
    ...
    for token in (
        "Resolvent-controlled pure-dyadic", "actual shifted state", "projected-force connection",
        "D^mT_A", "13\\over100", "Dropping the sextic as positive failed",
        "Q_A-Q_0", "K_0=0", "{9\\over20}\\|B_A\\|", "forward block and its legal reverse",
        "not independent low", "single-chain", "Devil's-advocate", "T-050", "phase/PDE", "Result footer",
    ):
    ...
    malformed_qquad = re.search(r"(?<!\\)\bqquad\b", note_text)
    audit.check("note", "no literal malformed qquad", malformed_qquad is None, malformed_qquad.group(0) if malformed_qquad else "absent", "absent")
    ...
    for token in ("R-162", "Resolvent-controlled", "13/100", "controller-pullback", "T-050", "phase/PDE"):
    ...
    if renderer is not None:
        with tempfile.TemporaryDirectory(prefix="tect-r162-render-", dir=temp_parent) as temporary:
    ...
    audit.check("records", "status falsifier extension", all(token in status.get("falsifier", "") for token in ("R-162", "M1J3", "unrelated forest")), "R-162" in status.get("falsifier", ""), "R-162/M1J3/forest boundary")
    audit.check("records", "TODO T-050 boundary", any(row.get("id") == "T-050" and "R-162" in row.get("note", "") for row in todo.get("tasks", [])), LEDGER_ID, "registered")
    ...
    catalogued_package = [relative(MANIFEST)] + [relative(path) for path in artifact_paths.values()]
    audit.check("records", "catalog authored package", all(path in catalog_text for path in catalogued_package), catalogued_package, "all registered")
