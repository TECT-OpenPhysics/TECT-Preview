#!/usr/bin/env python3
"""Primary finite spectral-sum audit for EXP-001142."""
...
def spectral_positive(matrix: np.ndarray, energies: np.ndarray, vectors: np.ndarray, weights: np.ndarray, factor: float, hbar: float) -> float:
    matrix_hat = vectors.conj().T @ matrix @ vectors
    gaps = energies[:, None] - energies[None, :]
    terms = factor * weights * (gaps / hbar) ** 2 * np.abs(matrix_hat) ** 2
    return float(np.real(np.sum(terms)))
...
def run() -> dict[str, Any]:
    ...
    check("provenance", manifest["exploration_id"] == "EXP-001142" and manifest["task_id"] == "T-054", [manifest["exploration_id"], manifest["task_id"]], "EXP-001142/T-054", "provenance")
            ...
            for radius in radii:
                ...
                for time in times:
                    for orientation in orientations:
                        ...
                        spectral_sum = spectral_positive(difference, energies, vectors, weights, factor, hbar)
                        identity_error = abs(second_inner + first_inner)
                        spectral_error = abs(float(np.real(first_inner)) - spectral_sum)
                        signed_spectral_error = abs(float(np.real(second_inner)) + spectral_sum)
                        ...
                        values = {
                            "D_km_norm": d_km,
                            "delta_D_km_norm": delta_km,
                            "delta2_D_km_norm": delta2_km,
                            "spectral_second_moment": spectral_sum,
                        ...
                        check(f"V={volume} beta={beta} L={radius} t={time} sign={orientation} positivity", spectral_sum >= -spectral_tolerance and float(np.real(first_inner)) >= -spectral_tolerance and float(np.real(delta2_inner)) >= -spectral_tolerance, [spectral_sum, first_inner], ">=-tolerance", "nonnegative spectral moment")
    ...
    check("all spectral identities", all(row["values"]["spectral_sum_error"] <= spectral_tolerance and row["values"]["signed_spectral_error"] <= spectral_tolerance for row in all_rows), len(all_rows), "all <= tolerance", "spectral sum")
    return {
        "schema": "tect/foundation-audit/1.0",
        "run_kind": "primary",
        "audit_id": "PA-CP1-ST8-Q3LOCK-FINITE-SPECTRAL-PAIRING-SUM",
        ...
        "assertions": rows[:16] + [{"name": "assertion_summary", "group": "summary", "status": "PASS", "actual": str(len(rows)), "expected": "all executed assertions passed"}],
        "derived": {"summary_rows": summary_rows, "row_count": len(all_rows), "finite_spectral_sum_identity_closed": True, "nonnegative_spectral_second_moment_closed": True, "edge_square_sixsite_rows_closed": True, "two_orientation_history_closed": True, "uniform_beta_volume_direct_d_delta_d_closed": False, "common_core_spectral_tail_closed": False, "common_alpha_closed": False, "pre_a_closed": False},
        "boundary": scope,
    }
...
def main() -> int:
    ...
    print(f"PRIMARY FINITE-SPECTRAL-PAIRING-SUM PASS {payload['passed']}/{payload['assertion_count']} rows={payload['derived']['row_count']}")
    return 0
