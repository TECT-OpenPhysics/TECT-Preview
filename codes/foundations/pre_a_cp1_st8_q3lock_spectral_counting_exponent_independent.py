#!/usr/bin/env python3
"""Independent reconstruction of the R-411 sublinear mode envelope."""
...
def run(output: Path = DEFAULT_OUTPUT) -> dict[str, Any]:
    ...
    payload = {"schema": "tect/pre-a-r411-independent/1.0", "manifest": str(MANIFEST.relative_to(REPO)).replace("\\", "/"), "result_id": "R-411", "exploration_id": "EXP-001256", "verdict": "PASS", "checks": checks, "assertion_count": check_count, "derived": derived, "scope": scope, "boundary": manifest["boundary"]}
    r407i.atomic_json(output, payload)
    print(f"R-411 INDEPENDENT PASS {check_count}/{check_count} systems={len(pairs)} contexts={total_contexts} rows={total_rows} alpha={len(exponents)} c2=[{min(mode_constants):.6g},{max(mode_constants):.6g}] trace=[{min(traces):.6g},{max(traces):.6g}]")
    return payload
