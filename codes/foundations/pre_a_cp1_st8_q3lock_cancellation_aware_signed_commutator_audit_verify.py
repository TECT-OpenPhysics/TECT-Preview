#!/usr/bin/env python3
"""Integrated verifier for EXP-001165, including Lean R335."""
...
LEAN = REPO / "verification/lean/Tect/R335.lean"
...
def lean_run() -> dict[str, Any]:
    command = "lake env lean Tect/R335.lean"
    lake = lake_path()
    if lake is None:
        return {"status": "UNAVAILABLE", "command": command, "output": "pinned lake executable not found"}
    process = subprocess.run([str(lake), "env", "lean", "Tect/R335.lean"], cwd=LEAN_ROOT, text=True, encoding="utf-8", capture_output=True, check=False)
...
def run() -> dict[str, Any]:
    ...
    check("identity", manifest["exploration_id"] == "EXP-001165" and manifest["task_id"] == "T-054" and manifest["claim_bearing"] is False, [manifest["exploration_id"], manifest["task_id"], manifest["claim_bearing"]], "EXP-001165/T-054/false")
    ...
    markers = ["signed_pair_fixture", "reverse_antisymmetry_fixture", "weighted_proxy_fixture", "scope_fixture"]
    ...
    with tempfile.TemporaryDirectory(prefix="cancellation-aware-signed-commutator-") as temporary:
        ...
        check("diagnostic semantics", diagnostic.get("interpretation") == "finite union-group signed cancellation diagnostic; not a global-state or asymptotic bound" and diagnostic.get("candidate_cutoff_volume_beta_uniform_bound") == "not established by this audit" and diagnostic.get("actual_q3_trotter_defect") == "open", diagnostic, "finite-only interpretation")
    lean = lean_run()
    check("Lean compile", lean["status"] == "PASS", lean, "PASS")
    return {"schema": "tect/foundation-audit/1.0", "run_kind": "integrated", "audit_id": "PA-CP1-ST8-Q3LOCK-CANCELLATION-AWARE-SIGNED-COMMUTATOR-AUDIT", "claim_id": manifest["claim_ids"][0], "task_id": manifest["task_id"], "exploration_id": manifest["exploration_id"], "verdict": "PASS", "assertion_count": len(checks), "assertions": checks, "lean": lean, "boundary": manifest["scope"], "provenance": {"manifest_sha256": sha256(MANIFEST), "primary_sha256": sha256(PRIMARY), "independent_sha256": sha256(INDEPENDENT), "lean_sha256": sha256(LEAN)}}
...
def main() -> int:
    ...
    print(f"INTEGRATED CANCELLATION-AWARE-SIGNED-COMMUTATOR PASS {payload['assertion_count']}/{payload['assertion_count']}; Lean={payload['lean']['status']}")
    return 0
