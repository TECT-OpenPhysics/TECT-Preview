#!/usr/bin/env python3
"""Primary finite audit of the quadratic mode-counting envelope for R-410.

For D=diag(pi), W=D^(-1/2)L D^(-1/2), order the d-1 positive eigenvalues
as lambda_k.  The finite constant c2=min_k lambda_k/k^2 gives the exact
finite envelope tr(W^+) <= H_(d-1)^(2)/c2 <= pi^2/(6*c2).  The underlying
resistance/trace identity is retained as a conditioning cross-check.
"""
...
SLUG = "pre_a_cp1_st8_q3lock_spectral_counting"
...
def run(output: Path = DEFAULT_OUTPUT) -> dict[str, Any]:
    ...
    check("identity", manifest["result_id"] == "R-410" and manifest["exploration_id"] == "EXP-001255" and manifest["claim_bearing"] is False, [manifest["result_id"], manifest["exploration_id"], manifest["claim_bearing"]], "R-410/EXP-001255/false", "provenance")
    ...
    payload = {"schema": "tect/pre-a-r410-primary/1.0", "manifest": str(MANIFEST.relative_to(REPO)).replace("\\", "/"), "result_id": "R-410", "exploration_id": "EXP-001255", "verdict": "PASS", "checks": checks, "assertion_count": check_count, "derived": derived, "scope": scope, "boundary": manifest["boundary"]}
    r408.r407.atomic_json(output, payload)
    print(f"R-410 PRIMARY PASS {check_count}/{check_count} systems={len(pairs)} contexts={total_contexts} rows={total_rows} c2=[{min(mode_constants):.6g},{max(mode_constants):.6g}] trace=[{min(traces):.6g},{max(traces):.6g}] zeta=[{min(zeta_bounds):.6g},{max(zeta_bounds):.6g}]")
    return payload
