#!/usr/bin/env python3
"""Integrated primary/independent/Lean verifier for EXP-001175."""
...
def main() -> int:
    ...
    check("identity", manifest["exploration_id"] == "EXP-001175" and manifest["task_id"] == "T-054", [manifest["exploration_id"], manifest["task_id"]], "EXP-001175/T-054")
    check("claim nonbearing", manifest["claim_bearing"] is False, manifest["claim_bearing"], False)
    check("graph fixture", list(manifest["finite_fixture"]["graphs"]) == ["path4", "path5", "path6"], list(manifest["finite_fixture"]["graphs"]), "path4/path5/path6")
    ...
    with tempfile.TemporaryDirectory(prefix="finite-common-word-direction-source-") as temporary:
        ...
        gram_ok, gram_message = compare_rows(primary.get("gram_rows", []), independent.get("gram_rows", []), ("graph", "source_role", "source_site", "beta"), ("min_eigenvalue", "max_eigenvalue", "diagonal_min", "reflection_error", "partition_function", "matrix_real", "matrix_imag"), tolerance)
        check("OS Gram crosscheck", gram_ok, gram_message, "same role contexts and values")
        thermal_ok, thermal_message = compare_rows(primary.get("thermal_rows", []), independent.get("thermal_rows", []), ("graph", "source_role", "source_site", "beta", "fraction"), ("forward_real", "forward_imag", "reverse_real", "reverse_imag", "cyclicity_residual", "witness", "seconds"), tolerance)
        check("thermal word crosscheck", thermal_ok, thermal_message, "same role contexts and values")
        comparison_ok, comparison_message = compare_rows(primary.get("comparisons", []), independent.get("comparisons", []), ("from_graph", "to_graph", "source_role", "beta"), ("from_site", "to_site", "gram_max_delta", "gram_frobenius_delta", "gram_relative_max_delta", "thermal_max_delta", "thermal_relative_max_delta", "max_context_delta"), tolerance)
        check("direction/source delta crosscheck", comparison_ok, comparison_message, "same adjacent role deltas")
        ...
        derived_fields = ("context_count", "role_count", "nested_pair_count", "gram_row_count", "thermal_row_count", "comparison_count", "min_context_delta", "max_context_delta", "finite_q3_transfer_closed", "finite_os_gram_closed", "finite_thermal_cyclicity_closed", "source_role_coverage_closed", "direction_context_comparison_closed", "source_uniform_common_word_closed", "volume_uniform_os_cauchy_closed", "two_orientation_common_alpha_closed", "exhaustion_independence_closed", "common_alpha_closed", "hamiltonian_os_identification_closed", "kms_gns_gap_closed", "continuum_closed", "c6_closed", "sector_a_closed", "pre_a_closed", "no_new_negative_result", "no_tier_change", "no_pdf")
        for field in derived_fields:
            one, two = primary.get("derived", {}).get(field), independent.get("derived", {}).get(field)
            if isinstance(one, bool) or isinstance(two, bool):
                check(f"lane agreement {field}", one == two, [one, two], "equal")
            elif isinstance(one, (int, float)) and isinstance(two, (int, float)):
                check(f"lane agreement {field}", abs(float(one) - float(two)) <= tolerance * (1.0 + abs(float(one))), [one, two], "within tolerance")
            else:
                check(f"lane agreement {field}", one == two, [one, two], "equal")
