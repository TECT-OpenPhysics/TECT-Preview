#!/usr/bin/env python3
"""Hostile mutation lane for the R-426 high-cutoff Schur stress."""
...
DEFAULT_OUTPUT = REPO / "claims/C6-SPACETIME-SIGNATURE/runs" / f"2026-08-30-hostile-{SLUG}" / "hostile.json"
...
def run(output: Path = DEFAULT_OUTPUT) -> dict[str, Any]:
    ...
    if not (manifest["result_id"] == "R-426" and manifest["exploration_id"] == "EXP-001271" and manifest["claim_bearing"] is False):
    ...
    if not (
        ...
        raise AssertionError("route-local failure contract was altered")
    payload: dict[str, Any] = {"schema": "tect/pre-a-r426-hostile/1.0", "result_id": manifest["result_id"], "exploration_id": manifest["exploration_id"], "claim_id": manifest["claim_ids"][0], "manifest": MANIFEST.relative_to(REPO).as_posix(), "run_kind": "hostile", "verdict": "PASS", "assertion_count": assertions, "assertions": checks, "controls": {"all_mutations_rejected": True, "mutation_count": assertions, "numeric_evaluation": False, "physical_promotion": False, "route_local_failure_preserved": True, "failure_contract": failure}, "non_claims": manifest["non_claims"]}
    atomic_json(output, payload)
    print(f"R-426 HOSTILE PASS {assertions}/{assertions} invalid mutations rejected")
    return payload
