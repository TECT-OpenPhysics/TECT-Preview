#!/usr/bin/env python3
"""Primary finite actual-Q3 split-step recurrence parameter stress for EXP-001195."""
...
SLUG = "pre_a_cp1_st8_q3lock_split_step_parameter_stress"
MANIFEST = REPO / "strategy/pre-a-cp1-st8-q3lock-split-step-parameter-stress-manifest.json"
...
def split_system(volume: int, size: int, parameters: dict[str, str]) -> tuple[list[np.ndarray], list[np.ndarray], list[tuple[int, int]], np.ndarray, list[np.ndarray], list[np.ndarray]]:
    ...
    return qs, ps, edges, full, onsite, bonds
...
def run() -> dict[str, Any]:
    ...
    check("identity", manifest["exploration_id"] == "EXP-001195" and manifest["task_id"] == "T-054", [manifest["exploration_id"], manifest["task_id"]], "EXP-001195/T-054", "provenance")
    ...
    check("parameter counts", len(betas) == 3 and len(supports) == 2, [len(betas), len(supports)], [3, 2], "parameter grid")
    ...
    check("scope firewall", scope["finite_parameter_rows_closed"] and scope["source_beta_volume_stress_closed"] and scope["actual_split_recurrence_theorem_closed"] is False and scope["common_alpha_closed"] is False, scope, "finite parameter stress only", "scope")

    all_rows: list[dict[str, Any]] = []
    context_summaries: list[dict[str, Any]] = []
    sample_rows: list[dict[str, Any]] = []
    order_names = list(manifest["split_orders"])
    expected_contexts = len(volumes) * len(betas) * len(supports) * len(order_names) * 4
    for volume in volumes:
        ...
        for order_name, order in orders.items():
            ...
            for sign in (-1, 1):
                ...
                unitarity_error = float(np.linalg.norm(operator.conj().T @ operator - np.eye(operator.shape[0]), ord=2))
                check(f"V={volume} {order_name} sign={sign} unitary", unitarity_error <= tolerance, unitarity_error, f"<={tolerance}", "split product")
        ...
        for beta_fraction in betas:
            ...
            for support in supports:
                observable = q3.character(sum((qs[site] for site in support), np.zeros_like(qs[0])), amplitude, hbar)
                initial = local_lengths(qs, ps, observable, rho)
                outside = [initial[site] for site in range(volume) if site not in support]
                check(f"V={volume} beta={beta_fraction} support={support} locality", max(outside, default=0.0) <= tolerance, max(outside, default=0.0), f"<={tolerance}", "support locality")
                    ...
                    for sign in (-1, 1):
                        ...
                        for adjoint in (0, 1):
                            ...
                            for step in range(steps + 1):
                                ...
                                check_name = f"V={volume} beta={beta_fraction} support={support} {order_name} sign={sign} adj={adjoint} step={step} finite"
                                check(check_name, all(np.isfinite(value) and value >= -tolerance for value in lengths), lengths, "finite/nonnegative", "commutator rows")
                                history.append(lengths)
                                for site, value in enumerate(lengths):
                                    row = {"volume": volume, "beta": str(beta_fraction), "support": list(support), "order": order_name, "sign": sign, "adjoint": adjoint, "step": step, "site": site, "length": value}
                                    all_rows.append(row)
                            ...
                            for step in range(steps):
                                for site in range(volume):
                                    ...
                                    row = {"volume": volume, "beta": str(beta_fraction), "support": list(support), "order": order_name, "sign": sign, "adjoint": adjoint, "step": step, "site": site, "lhs": history[step + 1][site], "rhs": rhs, "residual": residual, "neighbor_sum": neighbor_sum}
                                    all_rows.append(row)
                                    if residual > recurrence_tolerance:
                                        violations.append(row)
                            context_summaries.append({"volume": volume, "beta": str(beta_fraction), "support": list(support), "order": order_name, "context": f"sign{sign}_adjoint{adjoint}", "max_residual": max(residuals, default=0.0), "min_residual": min(residuals, default=0.0), "violation_count": len(violations), "violations": violations[:4]})
        check(f"V={volume} beta coverage", len([row for row in context_summaries if row["volume"] == volume]) == len(betas) * len(supports) * len(order_names) * 4, len([row for row in context_summaries if row["volume"] == volume]), len(betas) * len(supports) * len(order_names) * 4, "coverage")

    check("context coverage", len(context_summaries) == expected_contexts, len(context_summaries), expected_contexts, "coverage")
    expected_length_rows = sum(len(betas) * len(supports) * len(order_names) * 4 * volume * (steps + 1) for volume in volumes)
    expected_recurrence_rows = sum(len(betas) * len(supports) * len(order_names) * 4 * volume * steps for volume in volumes)
    check("length row coverage", len(all_rows) >= expected_length_rows + expected_recurrence_rows, len(all_rows), f">={expected_length_rows + expected_recurrence_rows}", "coverage")
    violations = [row for row in context_summaries if row["violation_count"] > 0]
    status = "PASS_ALL_PARAMETER_ROWS" if not violations else "FAIL_PARAMETER_ROUTE_LOCAL"
    check("candidate outcome recorded", status in ("PASS_ALL_PARAMETER_ROWS", "FAIL_PARAMETER_ROUTE_LOCAL"), status, "explicit finite outcome", "route decision")
    ...
    maxima = {}
    for beta_fraction in betas:
        rows = [row for row in context_summaries if row["beta"] == str(beta_fraction)]
        maxima[str(beta_fraction)] = max((float(row["max_residual"]) for row in rows), default=0.0)
    return {
        "schema": "tect/foundation-audit/1.0",
        "run_kind": "primary",
        "audit_id": "PA-CP1-ST8-Q3LOCK-SPLIT-STEP-PARAMETER-STRESS",
        ...
        "derived": {
            ...
            "length_row_count": sum(1 for row in all_rows if "length" in row),
            "recurrence_row_count": sum(1 for row in all_rows if "residual" in row),
            "parameter_context_count": len(context_summaries),
            "recurrence_status": status,
            "all_parameter_rows_pass": status == "PASS_ALL_PARAMETER_ROWS",
            "max_residual_by_beta": maxima,
            "finite_parameter_rows_closed": True,
            "source_beta_volume_stress_closed": True,
...
def main() -> int:
    ...
    print(f"PRIMARY ACTUAL-SPLIT-STEP PARAMETER STRESS PASS {payload['passed']}/{payload['assertion_count']} status={payload['derived']['recurrence_status']}")
    return 0
