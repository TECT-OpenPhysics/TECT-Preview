#!/usr/bin/env python3
"""Integrated primary/independent/Lean verifier for EXP-001066."""
...
PRIMARY = REPO / "codes/foundations/pre_a_cp1_st8_q3lock_finite_gibbs_isometric_duhamel_remainder.py"
INDEPENDENT = REPO / "codes/foundations/pre_a_cp1_st8_q3lock_finite_gibbs_isometric_duhamel_remainder_independent.py"
LEAN = REPO / "verification/lean/Tect/R248.lean"
...
DEFAULT_OUTPUT = REPO / "claims/C6-SPACETIME-SIGNATURE/runs" / (
    "2026-08-25-integrated-pre-a-cp1-st8-q3lock-finite-gibbs-isometric-duhamel-remainder/integrated.json"
)
...
def lean_run() -> dict[str, Any]:
    lake = lake_path()
    command = "lake env lean Tect/R248.lean"
    if lake is None:
        return {"status": "UNAVAILABLE", "command": command, "output": "pinned lake executable not found"}
    process = subprocess.run([str(lake), "env", "lean", "Tect/R248.lean"], cwd=LEAN_ROOT, text=True, encoding="utf-8", capture_output=True, check=False)
...
def main() -> int:
    ...
    check("finite theorem flags", manifest["scope"]["finite_member_duhamel_remainder_closed"] is True and manifest["scope"]["all_time_orbit_hypothesis_removed_finite"] is True, manifest["scope"], "finite remainder closed")
    ...
    markers = ["gibbs_weight_normalization", "first_commutator_fixture", "second_commutator_fixture", "initial_seminorm_fixture", "double_seminorm_fixture", "finite_remainder_bound_fixture", "finite_scope_fixture"]
    ...
    with tempfile.TemporaryDirectory(prefix="finite-gibbs-duhamel-") as temporary:
        ...
        keys = ("initial_squared_seminorm", "delta_squared_squared_seminorm", "time", "remainder_squared_bound", "finite_gibbs_isometry_closed", "finite_member_duhamel_remainder_closed", "all_time_orbit_hypothesis_removed_finite", "volume_uniform_direct_d_cauchy_closed", "delta_d_cauchy_closed", "finite_hamiltonian_os_embedding_closed")
        for key in keys:
        ...
        for key in ("initial_squared_seminorm", "delta_squared_squared_seminorm", "remainder_squared_bound"):
            check(f"fixture {key}", derived.get(key) == expected[key], derived.get(key), expected[key])

    lean = {"status": "SKIPPED", "command": "lake env lean Tect/R248.lean"} if args.skip_lean else lean_run()
    ...
    print(f"INTEGRATED FINITE-GIBBS-DUHAMEL PASS {len(rows)}/{len(rows)}; Lean={lean['status']}")
    return 0
