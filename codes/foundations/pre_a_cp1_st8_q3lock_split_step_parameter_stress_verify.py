#!/usr/bin/env python3
"""Integrated verifier for EXP-001195 finite Q3 parameter stress."""
...
LEAN = REPO / "verification/lean/Tect/R354.lean"
...
def lean_run() -> dict[str, Any]:
    command = "lake env lean Tect/R354.lean"
    lake = lake_path()
    if lake is None:
        return {"status": "UNAVAILABLE", "command": command, "output": "pinned lake executable not found"}
    process = subprocess.run([str(lake), "env", "lean", "Tect/R354.lean"], cwd=LEAN_ROOT, text=True, encoding="utf-8", capture_output=True, check=False)
...
def row_key(row: dict[str, Any]) -> tuple[int, str, tuple[int, ...], str, str]:
    return (int(row["volume"]), str(row["beta"]), tuple(int(site) for site in row["support"]), str(row["order"]), str(row["context"]))
...
def run() -> dict[str, Any]:
    ...
    check("identity", manifest["exploration_id"] == "EXP-001195" and manifest["task_id"] == "T-054" and manifest["claim_bearing"] is False, [manifest["exploration_id"], manifest["task_id"], manifest["claim_bearing"]], "EXP-001195/T-054/false")
    ...
    markers = ["parameter_grid_fixture", "time_horizon_fixture", "weighted_step_fixture", "order_context_fixture", "scope_fixture"]
    ...
    with tempfile.TemporaryDirectory(prefix="q3-parameter-stress-") as temporary:
        ...
        check("parameter context count", p.get("parameter_context_count") == i.get("parameter_context_count") == 144, [p.get("parameter_context_count"), i.get("parameter_context_count")], 144)
        ...
        check("context keys", set(p_rows) == set(i_rows) and len(p_rows) == 144, [len(p_rows), len(i_rows)], 144)
        ...
        check("route outcome", p.get("recurrence_status") == i.get("recurrence_status") and p.get("recurrence_status") in ("PASS_ALL_PARAMETER_ROWS", "FAIL_PARAMETER_ROUTE_LOCAL"), [p.get("recurrence_status"), i.get("recurrence_status")], "explicit finite outcome")
    lean = lean_run()
...
def main() -> int:
    ...
    print(f"INTEGRATED ACTUAL-SPLIT-STEP PARAMETER STRESS PASS {payload['assertion_count']}/{payload['assertion_count']}; Lean={payload['lean']['status']}")
    return 0
