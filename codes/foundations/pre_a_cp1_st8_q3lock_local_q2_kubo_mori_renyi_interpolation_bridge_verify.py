#!/usr/bin/env python3
"""Integrated primary/independent/Lean verifier for EXP-001222 / R-380."""
...
LEAN = REPO / "verification/lean/Tect/R380.lean"
...
def compile_lean() -> dict[str, Any]:
    lake = lake_path()
    command = "lake env lean Tect/R380.lean"
    if lake is None:
        return {"status": "UNAVAILABLE", "command": command, "output": "pinned lake executable not found"}
    process = subprocess.run([str(lake), "env", "lean", "Tect/R380.lean"], cwd=LEAN_ROOT, text=True, encoding="utf-8", capture_output=True, check=False)
...
def run() -> dict[str, Any]:
    ...
    check("identity", manifest["exploration_id"] == "EXP-001222" and manifest["result_id"] == "R-380" and manifest["claim_bearing"] is False, [manifest["exploration_id"], manifest["result_id"], manifest["claim_bearing"]], "EXP-001222/R-380/false")
    ...
    markers = ("midpoint_arithmetic_envelope", "symmetric_chord_midpoint", "symmetric_sample_chord", "interpolation_nonnegative", "scope_fixture")
    ...
    registry = json.loads(LEAN_REGISTRY.read_text(encoding="utf-8"))
    entries = [entry for entry in registry["entrypoints"] if entry["path"] == "verification/lean/Tect/R380.lean"]
    ...
    with tempfile.TemporaryDirectory(prefix="q3-renyi-interpolation-bridge-") as temporary:
        ...
        exact_fields = ("context_count", "expected_contexts", "theta", "sample_points", "quadrature_nodes", "renyi_interpolation_finite_checked", "midpoint_shell_lower_bound_finite_checked", "endpoint_shell_upper_bound_finite_checked", "shell_integral_identity_finite_checked", "source_uniformity_proved", "weighted_cutoff_uniformity_proved", "weighted_volume_uniformity_proved", "shape_uniformity_proved", "common_core_closed", "common_alpha_closed", "kms_gns_gap_closed", "continuum_closed", "c6_closed", "sector_a_closed", "pre_a_closed")
        for field in exact_fields:
            check(f"agreement {field}", p[field] == i[field], [p[field], i[field]], "equal")
        numeric_fields = ("maximum_endpoint_symmetry_error", "maximum_sample_symmetry_error", "maximum_convex_chord_violation", "maximum_midpoint_lower_envelope_violation", "maximum_integral_upper_envelope_violation", "maximum_shell_integral_identity_error", "maximum_quadrature_reconstruction_error", "maximum_kubo_modular_identity_error", "maximum_shell", "maximum_endpoint_modular_moment", "maximum_midpoint_modular_moment", "maximum_integrated_modular_shell", "minimum_endpoint_modular_moment", "minimum_midpoint_modular_moment", "minimum_integrated_modular_shell", "maximum_midpoint_to_integral_ratio", "maximum_integral_to_endpoint_ratio", "maximum_bond_unitary_factorization_error", "maximum_kubo_weight_asymmetry", "minimum_kubo_weight")
        ...
        regime_fields = ("shape", "volume", "contexts", "expected_contexts", "maximum_endpoint", "maximum_midpoint", "maximum_integral", "maximum_shell")
...
def main() -> int:
    ...
    print(f"INTEGRATED RENYI-INTERPOLATION-BRIDGE PASS {payload['assertion_count']}/{payload['assertion_count']}; Lean={payload['lean']['status']} diff={payload['derived']['max_primary_independent_numeric_difference']:.3e}")
    return 0
