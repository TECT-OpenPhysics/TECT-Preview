#!/usr/bin/env python3
"""Integrated primary/independent/hostile/Lean verifier for R-410."""
...
MANIFEST = ROOT / "strategy/pre-a-cp1-st8-q3lock-spectral-counting-manifest.json"
PRIMARY = ROOT / "codes/foundations/pre_a_cp1_st8_q3lock_spectral_counting.py"
INDEPENDENT = ROOT / "codes/foundations/pre_a_cp1_st8_q3lock_spectral_counting_independent.py"
HOSTILE = ROOT / "codes/foundations/pre_a_cp1_st8_q3lock_spectral_counting_hostile.py"
LEAN = ROOT / "verification/lean/Tect/R410.lean"
PRIMARY_OUTPUT = ROOT / "claims/C6-SPACETIME-SIGNATURE/runs/2026-08-31-primary-pre_a_cp1_st8_q3lock_spectral_counting/primary.json"
INDEPENDENT_OUTPUT = ROOT / "claims/C6-SPACETIME-SIGNATURE/runs/2026-08-31-independent-pre_a_cp1_st8_q3lock_spectral_counting/independent.json"
HOSTILE_OUTPUT = ROOT / "claims/C6-SPACETIME-SIGNATURE/runs/2026-08-31-hostile-pre_a_cp1_st8_q3lock_spectral_counting/hostile.json"
INTEGRATED_OUTPUT = ROOT / "claims/C6-SPACETIME-SIGNATURE/runs/2026-08-31-integrated-pre_a_cp1_st8_q3lock_spectral_counting/integrated.json"
...
def main() -> int:
    ...
    check("manifest identity", manifest["result_id"] == "R-410" and manifest["exploration_id"] == "EXP-001255" and manifest["claim_bearing"] is False, [manifest["result_id"], manifest["exploration_id"], manifest["claim_bearing"]], "R-410/EXP-001255/false", "identity")
    finite_flags = ("finite_mode_ordering_closed", "finite_quadratic_mode_envelope_closed", "finite_zeta_trace_upper_bound_closed", "finite_likelihood_row_coverage_closed")
    ...
    check("artifacts present", all(path.is_file() for path in artifacts), [str(path) for path in artifacts if not path.is_file()], "all R-410 artifacts", "provenance")
    ...
    lean = command([str(LAKE), "env", "lean", "Tect/R410.lean"], ROOT / "verification/lean")
    ...
    h = hostile["derived"]
    check("hostile spectral shortcuts caught", all(item["mode_constant"] > 0.0 and item["trace_inverse"] <= item["zeta_bound"] + tolerance and item["one_mode_inverse"] < item["trace_inverse"] - tolerance for item in h["selected"]) and abs(h["toy_spectrum"]["sorted_c2"] - h["toy_spectrum"]["unsorted_c2"]) > tolerance and h["toy_spectrum"]["wrong_quadratic_residual"] < -tolerance, h, "sorting, scaling and Fiedler shortcuts rejected", "hostile")
    ...
    payload = {"schema": "tect/pre-a-r410-integrated/1.0", "manifest": str(MANIFEST.relative_to(ROOT)).replace("\\", "/"), "result_id": "R-410", "exploration_id": "EXP-001255", "verdict": "PASS", "checks": checks, "derived": {"primary": p, "independent": i, "hostile": h, "lean": "PASS", "command_outputs": outputs}, "scope": scope, "boundary": manifest["boundary"]}
    atomic_json(args.output if args.output.is_absolute() else ROOT / args.output, payload)
    print(f"INTEGRATED SPECTRAL-COUNTING PASS {len(checks)}/{len(checks)} contexts={p['context_count']} rows={p['comparison_row_count']} c2=[{p['minimum_mode_constant']:.6g},{p['maximum_mode_constant']:.6g}] trace=[{p['minimum_trace_inverse']:.6g},{p['maximum_trace_inverse']:.6g}] zeta=[{p['minimum_zeta_bound']:.6g},{p['maximum_zeta_bound']:.6g}]")
    return 0
