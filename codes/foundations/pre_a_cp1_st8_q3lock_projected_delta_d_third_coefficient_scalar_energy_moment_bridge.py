#!/usr/bin/env python3
"""Primary scalar energy-moment candidate audit for EXP-001076.

This is a commuting surrogate for the mixed P/Q inputs in EXP-001075.  It
does not assert the corresponding inequalities for the unbounded Q3
operators; the noncommutative transfer is an explicit successor gate.
"""
...
def sp_frac(value: Fraction) -> sp.Rational:
    return sp.Rational(value.numerator, value.denominator)


def exact_root_ceiling(m5: Fraction, numerator: int, denominator: int, grid_denominator: int) -> Fraction:
    """Smallest k/grid_denominator with (k/grid_denominator)^denominator >= m5^numerator."""
    k = 0
...
def run() -> dict[str, Any]:
    ...
    r14 = exact_root_ceiling(m5, 1, 4, grid_denominator)
    r320 = exact_root_ceiling(m5, 3, 20, grid_denominator)
    r110 = exact_root_ceiling(m5, 1, 10, grid_denominator)
    ...
    sr14, sr320, sr110, sa = map(sp_frac, (r14, r320, r110, a_max))
    x_ceiling = sp.factor(sr14 + sa * sr320)
    p_ceiling = sp.factor(2 * x_ceiling + sp.Rational(9, 2) * sr110)
    q_ceiling = sp.factor(sp.Rational(3, 2) * sr320)
    p_two = sp.factor(2 * p_ceiling)
    q_two = sp.factor(2 * q_ceiling)
    derived = {
        "m5": m5,
        "a_max": a_max,
        "root_m5_quarter": r14,
    ...
    for key, value in (("X_ceiling", x_ceiling), ("P_sigma_ceiling", p_ceiling), ("Q_sigma_ceiling", q_ceiling), ("two_orientation_P_ceiling", p_two), ("two_orientation_Q_ceiling", q_two)):
        check(f"derived {key}", value == sp.Rational(frac(fixture[f"derived_{key}"]).numerator, frac(fixture[f"derived_{key}"]).denominator), value, fixture[f"derived_{key}"], "bridge")

    # Exact integer fixtures for the scalar order chain A<=K, p^2<=K and
    # A^3*p^4<=K^5.  The grid is a self-test; the all-real implication is the
    # displayed scalar monotonicity argument in the manifest.
    sample_rows = 0
    for q in range(-2, 3):
        for v in range(-2, 3):
            for p in range(-2, 3):
...
def main() -> int:
    ...
    print(f"PRIMARY PROJECTED-DELTA-D-THIRD-SCALAR-ENERGY-MOMENT-BRIDGE PASS {payload['passed']}/{payload['assertion_count']}")
    return 0
