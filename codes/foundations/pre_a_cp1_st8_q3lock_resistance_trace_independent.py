#!/usr/bin/env python3
"""Independent reconstruction of the R-409 resistance trace identity."""
...
def run(output: Path = DEFAULT_OUTPUT) -> dict[str, Any]:
    ...
    payload = {"schema": "tect/pre-a-r409-independent/1.0", "manifest": str(MANIFEST.relative_to(REPO)).replace("\\", "/"), "result_id": "R-409", "exploration_id": "EXP-001254", "verdict": "PASS", "checks": checks, "assertion_count": check_count, "derived": derived, "scope": scope, "boundary": manifest["boundary"]}
    r407i.atomic_json(output, payload)
    print(f"R-409 INDEPENDENT PASS {check_count}/{check_count} systems={len(pairs)} contexts={total_contexts} rows={total_rows} Rbar=[{min(averages):.6g},{max(averages):.6g}] trace=[{min(traces):.6g},{max(traces):.6g}] max_residual={max(abs(value) for value in identity_residuals + transformed_residuals + projector_residuals):.3g}")
    return payload
