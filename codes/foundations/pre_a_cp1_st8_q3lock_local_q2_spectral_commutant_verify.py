#!/usr/bin/env python3
"""Integrated primary/independent/Lean verifier for EXP-001206 / R-364."""
...
LEAN = REPO / "verification/lean/Tect/R364.lean"
...
def compile_lean() -> dict[str, Any]:
    lake = lake_path()
    command = "lake env lean Tect/R364.lean"
    if lake is None:
        return {"status": "UNAVAILABLE", "command": command, "output": "pinned lake executable not found"}
    process = subprocess.run([str(lake), "env", "lean", "Tect/R364.lean"], cwd=LEAN_ROOT, text=True, encoding="utf-8", capture_output=True, check=False)
...
def run() -> dict[str, Any]:
    ...
    check("identity", manifest["exploration_id"] == "EXP-001206" and manifest["result_id"] == "R-364" and not manifest["claim_bearing"], [manifest["exploration_id"], manifest["result_id"], manifest["claim_bearing"]], "EXP-001206/R-364/false")
    ...
    markers = ("spectral_block_offdiagonal_split", "spectral_offdiagonal_reduction", "spectral_scalar_centering", "spectral_block_disjoint", "scope_fixture")
    ...
    registry = json.loads(LEAN_REGISTRY.read_text(encoding="utf-8"))
    entries = [entry for entry in registry["entrypoints"] if entry["path"] == "verification/lean/Tect/R364.lean"]
    ...
    with tempfile.TemporaryDirectory(prefix="q3-spectral-commutant-") as temporary:
        ...
        exact_fields = (
            "context_count", "expected_contexts", "minimum_spectral_group_count", "maximum_spectral_group_count",
            "finite_spectral_pinching_reduction_closed", "finite_state_weighted_commutator_bound_closed", "finite_hilbert_schmidt_residual_comparison_closed",
        ...
        numeric_fields = (
            "max_reduction_error", "max_scalar_center_error", "max_spectral_commutator", "max_b_diagonal_error", "max_spectral_idempotence_error",
            "max_bound_violation", "minimum_bound_slack", "maximum_commutator_expectation", "maximum_weighted_bound", "maximum_b_weighted_norm",
            "maximum_spectral_offdiag_frobenius", "maximum_coordinate_offdiag_frobenius", "minimum_frobenius_gain", "minimum_nonzero_spectral_offdiag_frobenius",
            "maximum_spectral_left_norm", "maximum_spectral_right_norm", "maximum_spectral_to_coordinate_weighted_ratio"
        )
...
def main() -> int:
    ...
    print(f"INTEGRATED LOCAL-Q2 SPECTRAL COMMUTANT PASS {payload['assertion_count']}/{payload['assertion_count']}; Lean={payload['lean']['status']} diff={payload['derived']['max_primary_independent_numeric_difference']:.3e}")
    return 0
