#!/usr/bin/env python3
"""Integrated verifier for EXP-001188."""
...
LEAN = REPO / "verification/lean/Tect/R347.lean"
...
def lean_run() -> dict[str, Any]:
    command = "lake env lean Tect/R347.lean"
    lake = lake_path()
    if lake is None:
        return {"status": "UNAVAILABLE", "command": command, "output": "pinned lake executable not found"}
    process = subprocess.run([str(lake), "env", "lean", "Tect/R347.lean"], cwd=LEAN_ROOT, text=True, encoding="utf-8", capture_output=True, check=False)
...
def compare_rows(primary: list[dict[str, Any]], independent: list[dict[str, Any]], tolerance: float) -> list[dict[str, Any]]:
    ...
    keys = ("volume", "oscillator_dimension", "beta", "max_tail_l4_left", "max_tail_l4_right", "max_history_l4_left", "max_history_l4_right", "max_adjoint_history_l4_left", "max_adjoint_history_l4_right", "max_lhs", "max_bound", "max_ratio", "max_unitary_residual")
...
def run() -> dict[str, Any]:
    ...
    check("identity", manifest["exploration_id"] == "EXP-001188" and manifest["task_id"] == "T-054", [manifest["exploration_id"], manifest["task_id"]], "EXP-001188/T-054")
    ...
    with tempfile.TemporaryDirectory(prefix="finite-unitary-tail-holder-") as temporary:
        ...
        check("finite unitary corollary", p.get("finite_unitary_history_holder_corollary_closed") is True and i.get("finite_unitary_history_holder_corollary_closed") is True, [p.get("finite_unitary_history_holder_corollary_closed"), i.get("finite_unitary_history_holder_corollary_closed")], True)
        check("history-side uniformity", p.get("history_side_l4_uniformity_closed") is True and i.get("history_side_l4_uniformity_closed") is True, [p.get("history_side_l4_uniformity_closed"), i.get("history_side_l4_uniformity_closed")], True)
        check("tail uniformity remains open", p.get("tail_fourth_moment_uniform_closed") is False and i.get("tail_fourth_moment_uniform_closed") is False, [p.get("tail_fourth_moment_uniform_closed"), i.get("tail_fourth_moment_uniform_closed")], False)

    check("scope firewall", scope["finite_unitary_history_holder_corollary_closed"] and scope["finite_tail_leg_symmetry_closed"] and scope["history_side_l4_uniformity_closed"] and not scope["tail_fourth_moment_uniform_closed"] and not scope["modular_domain_transfer_closed"] and not scope["common_alpha_closed"] and not scope["pre_a_closed"], scope, "finite corollary; QFT gates open")
    lean = lean_run()
    check("Lean compile", lean["status"] == "PASS", lean, "PASS")
    return {"schema": "tect/foundation-audit/1.0", "run_kind": "integrated", "audit_id": "PA-CP1-ST8-Q3LOCK-FINITE-UNITARY-TAIL-HOLDER-COROLLARY", "claim_id": manifest["claim_ids"][0], "task_id": manifest["task_id"], "exploration_id": manifest["exploration_id"], "verdict": "PASS", "assertion_count": len(rows), "assertions": rows, "lean": lean, "boundary": scope, "derived": {"primary_summary_rows": p.get("summary_rows", []), "independent_summary_rows": i.get("summary_rows", []), "finite_unitary_history_holder_corollary_closed": True, "history_side_l4_uniformity_closed": True, "tail_fourth_moment_uniform_closed": False, "common_core_promoted": False, "qft_promoted": False}, "recorded_at": datetime.now(timezone.utc).isoformat().replace("+00:00", "Z"), "provenance": {"primary_sha256": primary_hash, "independent_sha256": independent_hash, "manifest_sha256": sha256(MANIFEST), "lean_sha256": sha256(LEAN)}}
...
def main() -> int:
    ...
    print(f"INTEGRATED FINITE-UNITARY-TAIL-HOLDER-COROLLARY PASS {payload['assertion_count']}/{payload['assertion_count']}; Lean={payload['lean']['status']}")
    return 0
