#!/usr/bin/env python3
"""Independent reconstruction of the R-416 log-domain projected stress."""
...
def log_coordinate_distribution(hamiltonian: np.ndarray, basis: np.ndarray, beta: float, dimension: int, volume: int) -> tuple[np.ndarray, np.ndarray]:
    energies, vectors = np.linalg.eigh(r406i.hermitian(hamiltonian))
    shifted = energies - float(np.min(energies))
    transformed = basis.conj().T @ vectors
    coefficients = np.log(np.maximum(np.abs(transformed), np.finfo(float).tiny))
    terms = -beta * shifted[None, :] + 2.0 * coefficients
    logs = np.array([logsumexp(row) for row in terms], dtype=float)
    logs -= logsumexp(logs)
    energy_weights = np.exp(-beta * shifted)
    energy_weights /= float(np.sum(energy_weights))
    direct = np.sum(np.square(np.abs(transformed)) * energy_weights[None, :], axis=1)
    direct = np.maximum(np.real(direct), 0.0)
    direct /= float(np.sum(direct))
    return logs.reshape((dimension,) * volume), direct.reshape((dimension,) * volume)
...
def projected_graph(weights: np.ndarray, momentum: np.ndarray, chi: float) -> dict[str, Any]:
    ...
    if len(values) == 0 or not np.all(np.isfinite(values)) or float(values[0]) <= 0.0:
        raise AssertionError("projected graph is not positive")
    return {"weights": row, "conductance": conductance, "raw_zero_residual": abs(float(raw[0])), "projected_gap": float(values[0])}
...
def run(output: Path = DEFAULT_OUTPUT) -> dict[str, Any]:
    ...
    for dimension in dimensions:
        ...
        q_single, p_single = r406i.q3.oscillator(dimension)
        levels, vectors = np.linalg.eigh(r406i.hermitian(q_single))
        momentum = r406i.hermitian(vectors.conj().T @ p_single @ vectors)
        lower, upper, neutral = r406i.phase_indices(levels)
        ...
        for beta in betas:
            log_reference, direct_reference = log_coordinate_distribution(hamiltonian, basis, beta, dimension, volume)
            ...
            for orientation in orientations:
                ...
                for weights, minimum_log_row in conditional_rows(log_reference, order, dimension, floor):
                    ...
                    schur = r406i.harmonic(weights, data["conductance"], blocks)
                    scaled = projected_graph(weights * scale_factor, momentum, chi)
                    scale_residual = abs(data["projected_gap"] - scaled["projected_gap"])
                    direct_prefix = r406i.marginal(direct_reference, order[:1], dimension) if volume > 1 else direct_reference
                    ...
                    root = np.sqrt(data["weights"])
                    root /= float(np.linalg.norm(root))
                    check("constant projection", float(np.linalg.norm((np.eye(dimension) - np.outer(root, root)) @ root)) <= crosscheck_tolerance, "projected constant", f"<={crosscheck_tolerance}", "projection")
    ...
    derived = {
        "system_count": len(dimensions), "profile_count": total_profiles, "comparison_row_count": total_rows, "cutoff_dimensions": dimensions, "beta_values": betas,
        "minimum_projected_gap": min(projected_gaps), "maximum_projected_gap": max(projected_gaps), "minimum_schur_gap": min(schur_gaps), "maximum_schur_gap": max(schur_gaps),
        "minimum_coarse_schur_gap": min(coarse_gaps), "maximum_coarse_schur_gap": max(coarse_gaps), "minimum_residual_gap": min(residual_gaps), "maximum_residual_gap": max(residual_gaps),
        "maximum_raw_zero_mode_residual": max(raw_zero_residuals), "minimum_raw_zero_mode_residual": min(raw_zero_residuals), "maximum_scale_invariance_residual": max(scale_residuals),
        "minimum_log_conditional_mass": min(log_minima), "maximum_log_condition_number": max(condition_logs), "direct_underflow_rows": direct_underflow_rows, "profiles": profile_records,
        "finite_log_domain_gibbs_closed": True, "finite_log_conditional_rows_closed": True, "finite_constant_mode_projection_closed": True, "finite_projected_schur_gap_stress_closed": True, "finite_cutoff_stress_closed": True, "finite_scale_invariance_closed": True,
    ...
    payload = {"schema": "tect/pre-a-r416-independent/1.0", "manifest": str(MANIFEST.relative_to(REPO)).replace("\\", "/"), "result_id": "R-416", "exploration_id": "EXP-001261", "verdict": "PASS", "checks": checks, "assertion_count": check_count, "derived": derived, "scope": scope, "boundary": manifest["boundary"]}
    atomic_json(output, payload)
    print(f"R-416 INDEPENDENT PASS {check_count}/{check_count} cutoffs={len(dimensions)} profiles={total_profiles} rows={total_rows} projected_gap=[{min(projected_gaps):.6g},{max(projected_gaps):.6g}] schur_gap=[{min(schur_gaps):.6g},{max(schur_gaps):.6g}] raw_zero_max={max(raw_zero_residuals):.6g}")
    return payload
