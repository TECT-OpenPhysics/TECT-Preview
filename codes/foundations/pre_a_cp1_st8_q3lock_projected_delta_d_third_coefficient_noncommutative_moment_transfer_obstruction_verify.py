#!/usr/bin/env python3
"""Integrated verifier for EXP-001077."""
...
LEAN = REPO / "verification/lean/Tect/R259.lean"
...
def lean_run() -> dict[str, Any]:
    lake = lake_path()
    command = "lake env lean Tect/R259.lean"
    if lake is None:
        return {"status": "UNAVAILABLE", "command": command, "output": "pinned lake executable not found"}
    process = subprocess.run([str(lake), "env", "lean", "Tect/R259.lean"], cwd=LEAN_ROOT, text=True, encoding="utf-8", capture_output=True, check=False)
...
def main() -> int:
    ...
    check("identity", manifest["exploration_id"] == "EXP-001077" and manifest["task_id"] == "T-054", [manifest["exploration_id"], manifest["task_id"]], "EXP-001077/T-054")
    check("claim nonbearing", manifest["claim_bearing"] is False, manifest["claim_bearing"], False)
    check("previous authority", previous["exploration_id"] == "EXP-001076" and previous["scope"]["candidate_P_Q_formula_closed"] is True, previous["exploration_id"], "EXP-001076 scalar candidate")
    lean_source = LEAN.read_text(encoding="utf-8") if LEAN.is_file() else ""
    markers = ["pell_first", "pell_second", "pell_third", "k_sum_first", "a_power_first", "q_squared_first", "q_squared_second", "q_squared_third", "candidate_bound_first", "candidate_bound_second", "candidate_bound_third", "ratio_first", "ratio_second", "ratio_third", "state_moment_fixture", "scope_fixture"]
    ...
    with tempfile.TemporaryDirectory(prefix="noncommutative-moment-transfer-") as temporary:
        ...
        keys = ("candidate_Q_squared_bound", "matrix_Q_squared", "violation_ratio", "m5")
        for key in keys:
            check(f"lane agreement {key}", str(pderived.get(key)) == str(iderived.get(key)), [pderived.get(key), iderived.get(key)], "equal")
        fixture_map = {
            "matrix_Q_squared": [fixture["derived_first_matrix_Q_squared"], fixture["derived_second_matrix_Q_squared"], fixture["derived_third_matrix_Q_squared"]],
            "violation_ratio": [fixture["derived_first_violation_ratio"], fixture["derived_second_violation_ratio"], fixture["derived_third_violation_ratio"]],
        }
        for key, expected in fixture_map.items():
            check(f"fixture {key}", [str(item) for item in pderived.get(key, [])] == expected, pderived.get(key), expected)
        check("fixture candidate bound", str(pderived.get("candidate_Q_squared_bound")) == fixture["candidate_Q_squared_bound"], pderived.get("candidate_Q_squared_bound"), fixture["candidate_Q_squared_bound"])
        check("fixture m5", str(pderived.get("m5")) == fixture["m5"], pderived.get("m5"), fixture["m5"])
    ...
    lean = {"status": "SKIPPED", "command": "lake env lean Tect/R259.lean"} if args.skip_lean else lean_run()
    ...
    print(f"INTEGRATED NONCOMMUTATIVE M5-TO-Q TRANSFER OBSTRUCTION PASS {len(rows)}/{len(rows)}; Lean={lean['status']}")
    return 0
