#!/usr/bin/env python3
"""Hostile mutation firewall for the R-459 source-applicability audit."""
...
def run(output: Path = DEFAULT_OUTPUT) -> dict:
    ...
    add(
        "drop generator group",
        owner_candidate(
            base.replace("generator and transfer operator", ""), "note.txt"
        ),
    )
    add(
        "drop heat group",
        owner_candidate(base.replace("heat-root and heat-kernel", ""), "note.txt"),
    )
    add(
        "drop filtration group",
        owner_candidate(
            base.replace("filtration and conditional replica", ""), "note.txt"
        ),
    )
    add(
        "drop current group",
        owner_candidate(base.replace("raw-current spatial intertwiner", ""), "note.txt"),
    )
    add(
        "drop strict q-ledger group",
        owner_candidate(base.replace("q_k ledger and one-use q", ""), "note.txt"),
    )
    add(
        "treat merged bundle as owner",
        owner_candidate(base, "TECT_Math01-35_Merged.txt"),
    )
    add(
    ...
    add(
        "promote source scan to claim",
        mutated_manifest["claim_bearing"] is False,
    )
    if len(mutations) != 8:
        raise AssertionError("unexpected hostile mutation count")
    payload = {
        "schema": "tect/pre-a-contents-owner-audit-hostile/1.0",
        "run_kind": "hostile",
    ...
    print(
        f"R-459 HOSTILE {payload['verdict']} "
...
def main() -> int:
    ...
    if args.self_test:
        ...
        print("R-459 HOSTILE SELFTEST: PASS", flush=True)
    return 0
