#!/usr/bin/env python3
"""Independent source-energy-window stress for EXP-001212 / R-370."""
...
def run() -> dict[str, Any]:
    ...
    energy_windows = [float(Fraction(value)) for value in fixture["energy_gap_windows"]]
    ...
    if not energy_windows or any(value <= 0.0 for value in energy_windows) or energy_windows != sorted(set(energy_windows)):
        raise AssertionError("energy-gap windows must be strictly positive and sorted")
    ...
    check("authority", manifest["exploration_id"] == "EXP-001212" and manifest["result_id"] == "R-370", [manifest["exploration_id"], manifest["result_id"]], "EXP-001212/R-370", "provenance")
    check("scope", manifest["claim_bearing"] is False and manifest["scope"]["shape_uniformity_proved"] is False, manifest["scope"], "finite-energy-window-only", "scope")
    context_count = 0
    max_phase_error = 0.0
    ...
    max_window_decomposition_error = 0.0
    max_window_low_phase_error = 0.0
    max_window_high_phase_error = 0.0
    max_window_low_bound_violation = -float("inf")
    max_window_high_bound_violation = -float("inf")
    max_low_gap_norm = 0.0
    max_high_gap_norm = 0.0
    max_low_gap_bound = 0.0
    max_high_gap_bound = 0.0
    max_high_gap_fraction = 0.0
    min_nonzero_low_gap_norm = float("inf")
    window_summaries = {str(value): {"energy_gap": value, "context_count": 0, "maximum_low_gap_fractional_norm": 0.0, "maximum_high_gap_fractional_norm": 0.0, "maximum_low_gap_fractional_bound": 0.0, "maximum_high_gap_fractional_bound": 0.0, "maximum_high_gap_fraction": 0.0, "maximum_decomposition_error": 0.0} for value in energy_windows}
            ...
            for beta_fraction in betas:
                ...
                for site in sites:
                    ...
                    for bond_term_index in bond_indices:
                        ...
                        row = bond_rows.setdefault(weight_key, {"cutoff": size, "bond_term_index": bond_term_index, "maximum_kubo_fractional_norm": 0.0, "maximum_kubo_fractional_bound": 0.0, "context_count": 0})
                            ...
                            for sign in signs:
                                for prefix_length, prefix in all_prefixes(terms, order, sign, delta, hbar):
                                    for history_adjoint in adjoints:
                                        ...
                                        total_square = float(2.0 * np.sum(kubo_weights * np.abs(deltas) ** (2.0 * theta) * np.abs(transformed) ** 2))
                                        for energy_gap in energy_windows:
                                            window_key = str(energy_gap)
                                            low_mask = np.abs(deltas) <= energy_gap + gap_tolerance
                                            high_mask = ~low_mask
                                            low_norm = float(np.sqrt(max(float(2.0 * np.sum(kubo_weights * np.abs(deltas) ** (2.0 * theta) * np.abs(transformed) ** 2 * low_mask)), 0.0)))
                                            high_norm = float(np.sqrt(max(float(2.0 * np.sum(kubo_weights * np.abs(deltas) ** (2.0 * theta) * np.abs(transformed) ** 2 * high_mask)), 0.0)))
                                            low_change = float(np.sqrt(max(float(2.0 * np.sum(kubo_weights * np.abs(change_transformed) ** 2 * low_mask)), 0.0)))
                                            high_change = float(np.sqrt(max(float(2.0 * np.sum(kubo_weights * np.abs(change_transformed) ** 2 * high_mask)), 0.0)))
                                            low_phase = float(np.sqrt(max(float(2.0 * np.sum(kubo_weights * np.abs(phases * transformed) ** 2 * low_mask)), 0.0)))
                                            high_phase = float(np.sqrt(max(float(2.0 * np.sum(kubo_weights * np.abs(phases * transformed) ** 2 * high_mask)), 0.0)))
                                            low_bound = (2.0 ** (1.0 - theta)) * abs(delta / hbar) ** theta * low_norm
                                            high_bound = (2.0 ** (1.0 - theta)) * abs(delta / hbar) ** theta * high_norm
                                            low_phase_error = abs(low_change - low_phase)
                                            high_phase_error = abs(high_change - high_phase)
                                            decomposition_error = abs(total_square - low_norm ** 2 - high_norm ** 2)
                                            high_fraction = high_norm ** 2 / total_square if total_square > tolerance ** 2 else 0.0
                                            summary = window_summaries[window_key]
                                            summary["context_count"] += 1
                                            row_key = f"d={size}/gap={energy_gap}"
                                            window_row = window_rows.setdefault(row_key, {"cutoff": size, "energy_gap": energy_gap, "maximum_low_gap_fractional_norm": 0.0, "maximum_high_gap_fractional_norm": 0.0, "maximum_high_gap_fraction": 0.0, "context_count": 0})
                                            window_row["maximum_low_gap_fractional_norm"] = max(window_row["maximum_low_gap_fractional_norm"], low_norm)
                                            window_row["maximum_high_gap_fractional_norm"] = max(window_row["maximum_high_gap_fractional_norm"], high_norm)
                                            window_row["maximum_high_gap_fraction"] = max(window_row["maximum_high_gap_fraction"], high_fraction)
                                            window_row["context_count"] += 1
                                            summary["maximum_low_gap_fractional_norm"] = max(summary["maximum_low_gap_fractional_norm"], low_norm)
                                            summary["maximum_high_gap_fractional_norm"] = max(summary["maximum_high_gap_fractional_norm"], high_norm)
                                            summary["maximum_low_gap_fractional_bound"] = max(summary["maximum_low_gap_fractional_bound"], low_bound)
                                            summary["maximum_high_gap_fractional_bound"] = max(summary["maximum_high_gap_fractional_bound"], high_bound)
                                            summary["maximum_high_gap_fraction"] = max(summary["maximum_high_gap_fraction"], high_fraction)
                                            summary["maximum_decomposition_error"] = max(summary["maximum_decomposition_error"], decomposition_error)
                                            max_window_decomposition_error = max(max_window_decomposition_error, decomposition_error)
                                            max_window_low_phase_error = max(max_window_low_phase_error, low_phase_error)
                                            max_window_high_phase_error = max(max_window_high_phase_error, high_phase_error)
                                            max_window_low_bound_violation = max(max_window_low_bound_violation, low_change - low_bound)
                                            max_window_high_bound_violation = max(max_window_high_bound_violation, high_change - high_bound)
                                            max_low_gap_norm = max(max_low_gap_norm, low_norm)
                                            max_high_gap_norm = max(max_high_gap_norm, high_norm)
                                            max_low_gap_bound = max(max_low_gap_bound, low_bound)
                                            max_high_gap_bound = max(max_high_gap_bound, high_bound)
                                            max_high_gap_fraction = max(max_high_gap_fraction, high_fraction)
                                            if low_norm > tolerance:
                                                min_nonzero_low_gap_norm = min(min_nonzero_low_gap_norm, low_norm)
                                            check(f"V={volume} d={size} beta={beta_fraction} site={site} bond={bond_term_index} {order_name} prefix={prefix_length} sign={sign} adj={history_adjoint} gap={energy_gap} decomposition", decomposition_error <= tolerance * 800 * (1.0 + total_square), decomposition_error, f"<={tolerance * 800 * (1.0 + total_square)}", "energy-gap decomposition")
                                            check(f"V={volume} d={size} beta={beta_fraction} site={site} bond={bond_term_index} {order_name} prefix={prefix_length} sign={sign} adj={history_adjoint} gap={energy_gap} low-bound", low_change - low_bound <= tolerance * 800 * (1.0 + low_bound), [low_change, low_bound], "low bound", "energy-gap bound")
                                            check(f"V={volume} d={size} beta={beta_fraction} site={site} bond={bond_term_index} {order_name} prefix={prefix_length} sign={sign} adj={history_adjoint} gap={energy_gap} high-bound", high_change - high_bound <= tolerance * 800 * (1.0 + high_bound), [high_change, high_bound], "high bound", "energy-gap bound")
                                            check(f"V={volume} d={size} beta={beta_fraction} site={site} bond={bond_term_index} {order_name} prefix={prefix_length} sign={sign} adj={history_adjoint} gap={energy_gap} low-phase", low_phase_error <= tolerance * 800 * (1.0 + low_phase), low_phase_error, f"<={tolerance * 800 * (1.0 + low_phase)}", "energy-gap phase")
                                            check(f"V={volume} d={size} beta={beta_fraction} site={site} bond={bond_term_index} {order_name} prefix={prefix_length} sign={sign} adj={history_adjoint} gap={energy_gap} high-phase", high_phase_error <= tolerance * 800 * (1.0 + high_phase), high_phase_error, f"<={tolerance * 800 * (1.0 + high_phase)}", "energy-gap phase")
    ...
    check("energy-gap coverage", all(summary["context_count"] == context_count for summary in window_summaries.values()), {key: summary["context_count"] for key, summary in window_summaries.items()}, context_count, "energy-gap coverage")
    check("energy-gap decomposition", max_window_decomposition_error <= tolerance * 800 * (1.0 + max_low_gap_norm ** 2 + max_high_gap_norm ** 2), max_window_decomposition_error, f"<={tolerance * 800 * (1.0 + max_low_gap_norm ** 2 + max_high_gap_norm ** 2)}", "energy-gap decomposition")
    check("energy-gap low bound", max_window_low_bound_violation <= tolerance * 800 * (1.0 + max_low_gap_bound), max_window_low_bound_violation, f"<={tolerance * 800 * (1.0 + max_low_gap_bound)}", "energy-gap bound")
    check("energy-gap high bound", max_window_high_bound_violation <= tolerance * 800 * (1.0 + max_high_gap_bound), max_window_high_bound_violation, f"<={tolerance * 800 * (1.0 + max_high_gap_bound)}", "energy-gap bound")
    return {
        ...
        "derived": {
            ...
            "energy_gap_windows": energy_windows,
            "window_summaries": list(window_summaries.values()),
            "maximum_window_decomposition_error": max_window_decomposition_error,
            "maximum_low_gap_phase_error": max_window_low_phase_error,
            "maximum_high_gap_phase_error": max_window_high_phase_error,
            "maximum_low_gap_bound_violation": max_window_low_bound_violation,
            "maximum_high_gap_bound_violation": max_window_high_bound_violation,
            "maximum_low_gap_fractional_norm": max_low_gap_norm,
            "maximum_high_gap_fractional_norm": max_high_gap_norm,
            "maximum_low_gap_fractional_bound": max_low_gap_bound,
            "maximum_high_gap_fractional_bound": max_high_gap_bound,
            "maximum_high_gap_fraction": max_high_gap_fraction,
            "minimum_nonzero_low_gap_fractional_norm": min_nonzero_low_gap_norm,
            ...
            "energy_gap_split_finite_checked": True,
            "energy_gap_pythagorean_decomposition_closed": True,
            "weighted_fractional_bound_closed": True,
            "low_gap_cutoff_uniformity_proved": False,
            "high_gap_tail_uniformity_proved": False,
...
def main() -> int:
    ...
    print(f"INDEPENDENT KUBO-MORI ENERGY-WINDOW PASS {payload['assertion_count']}/{payload['assertion_count']} contexts={payload['derived']['context_count']} max_weighted={payload['derived']['maximum_weighted_fractional_norm']:.6f} max_high_gap_fraction={payload['derived']['maximum_high_gap_fraction']:.6f}")
    return 0
