#!/usr/bin/env python3
"""Primary finite Q3 cross-volume tensor-carrier stress test for EXP-001180."""
...
def finite_state(declaration: dict[str, Any], dimension: int, parameters: dict[str, str], amplitude: float, hbar: float) -> dict[str, Any]:
    ...
    def transfer(seconds: float) -> np.ndarray:
        factors = np.exp(-seconds * shifted / hbar)
        return eigenvectors @ np.diag(factors) @ eigenvectors.conj().T
...
def expectation(state: dict[str, Any], observable: np.ndarray, beta: float, hbar: float) -> complex:
    ...
    vectors = state["eigenvectors"]
    weights = np.exp(-beta * energies / hbar)
    partition = float(np.sum(weights))
    diagonal = np.diag(vectors.conj().T @ observable @ vectors)
...
def run() -> dict[str, Any]:
    ...
    for small_name, large_name in fixture["nested_pairs"]:
        ...
        bq, bp = large["observables"]["q"], large["observables"]["p"]
...
def main() -> int:
    ...
    print(f"PRIMARY CROSS-VOLUME-TENSOR-CARRIER PASS {payload['passed']}/{payload['assertion_count']} rows={len(payload['rows'])}")
    return 0
