...
- **T-050** A13: prove the controlled-shell energy one-use bound  _(owner: Codex; claim: A13-CLASSII-RELATIVE-PHASE-SOURCE-BUDGET-OBSTRUCTION; gate: A13-CLASSII-FUTURE-CONTROL-WEIGHTED-INNOVATION-BRACKET)_
  - R-083 closes all three controlled polynomial J_A stopped-FAR rows exactly for C>=3 with both endpoints and predictable drifts retained, and reduces the remaining controlled CFAR to the three Cartan input-scale telescopes with factor 3/(80P). The canonical K-shell coordinate bound spends CM input once, but exact cubic and rational mode-three fixtures show that nonlinear output increments are not pairwise orthogonal, so the R-082 orthogonal Carleson route cannot be invoked. The three linear Pauli--Fierz rows now have exact Gram, heat, secant, conditional-covariance, and nine-block forest algebra; an adapted fixture with rational row zero has negative linear-row zero chaos, so standalone rowwise positivity also fails. Next prove correlated/signed Cartan CFAR and a recombined linear+rational heat-lifted signed NEAR lower bound, then overlap-stable progression, controlled-shell one-use, and q=10/9 Nelson. Sector A remains open.
