...
- **T-054** Pre-A evidence register and candidate-functional tournament  _(owner: Codex; gate: PA-ROUND1-EVIDENCE-ROLE-AND-MINIMUM-MANIFEST-FREEZE)_
  - EXP-000791 freezes the bounded T-053 evidence-role inventory, common categorical matrix, explicit absence fields, and reference/robustness rules. Exact primary and independent audits derive the M2 node Hessian and inserted-lane cone and compute no current admitted microscopic survivor from the hard-row matrix. The visible helium target predates the new stiffness map, so its mismatch has no prospective validation credit and the parent Round-1 gate stays open. Close it only after a genuinely future/blind relation and microscopic observable map are frozen in an earlier immutable commit. In parallel, EXP-000792 advances the ordered ST8/Q3LOCK lane: one phase-independent local polynomial derivation and a source-uniform sharp first weighted-energy cone are proved, while Fourier-second-moment and ordinary basic-resolvent unweighted cutoff routes fail. EXP-000793 records and repairs the initial operator-norm inference gap by adding a nonreal resolvent parameter, fixed-norm Weyl-displaced Schwartz inputs, exact resolvent cancellation, and an explicit Omega(R^3) cutoff norm lower bound. The next parallel gate is PA-CP1-ST8-Q3LOCK-HIGHER-WEIGHTED-ENERGY-MOMENTS-AND-THERMODYNAMIC-CAUCHY-CLOSURE. Common alpha/KMS, algebraic grounds, GNS gap, continuum, physical empty space, C6, CP1, Sector A and Pre-A remain open.
