...
- **T-050** A13: prove the controlled-shell energy one-use bound  _(owner: Codex; claim: A13-CLASSII-RELATIVE-PHASE-SOURCE-BUDGET-OBSTRUCTION; gate: A13-CLASSII-FUTURE-CONTROL-WEIGHTED-INNOVATION-BRACKET)_
  - R-094 proves the regular centered Gram secant, confines 2^(j-4k) to positive quadratic curvature, pays the mixed 2^(j-2k) secant with Young slack 1/3, and pays the value-heat control prefix from a declared feedback-square fraction. Next: for fixed 0<theta<1 prove the reduced-square expectation-inside packet T_Q^>+T_G^>+(1-theta)c^T Bbar c/2 plus conditional mean debts, and embed it once into R-079 with heat, low, paid, trace, and the complete R-063 forest. Then assemble the signed progressive/revisit endpoint into OVERLAP_src/Nelson. Complete H_N, REG, Nelson, and Sector A remain open.
