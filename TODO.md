...
- **T-050** A13: prove the controlled-shell energy one-use bound  _(owner: Codex; claim: A13-CLASSII-RELATIVE-PHASE-SOURCE-BUDGET-OBSTRUCTION; gate: A13-CLASSII-CONTROLLED-SHELL-ENERGY-ONE-USE)_
  - R-137 proves the exact chronologically typed direct t(k)<=r and future t(k)>r split of the finite R-136 raw telescope, with an executable nonzero future Doob column. Its six-row current is the R-079 two-channel connection; trace, forest, low, rational, Cartan, and other signed-owner companions remain separate. R-084 OU normalization is reused and creates no spatial gain. Existing bounded-multiplier and root-orthogonality negatives reject automatic dyadic decay and bookkeeping cross-k orthogonality. The adversarial audit repaired the draft filtration witness, order typing, Sobolev quantifiers, smooth-cylinder derivative scope, future-shell geometry, and R-135 applicability. Next: prove A13-CLASSII-COMPLETE-RAW-DOOB-DIRECT-FUTURE-SPATIAL-ONE-USE-BOUND by instantiating the complete owner, proving the direct t(k)<=r spatial estimate, and either an R-135-compatible future last-insertion estimate with m>=k+5 or a direct integrated signed substitute, followed by one deterministic nonnegative once-owned q_k sum and finite-collar headroom. Both A13 gates and Sector A remain open.
