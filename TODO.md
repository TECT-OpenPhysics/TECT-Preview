...
- **T-050** A13: prove the controlled-shell energy one-use bound  _(owner: Codex; claim: A13-CLASSII-RELATIVE-PHASE-SOURCE-BUDGET-OBSTRUCTION; gate: A13-CLASSII-FUTURE-CONTROL-WEIGHTED-INNOVATION-BRACKET)_
  - R-095 proves that the rootwise future-feedback reserve differs from the global terminal derivative square by an exact sign-indefinite moving-prefix defect, with normalized value -1/4. Fractional perspective positivity holds exactly under 2R>=theta B; the scalar below-threshold fixture has optimized value -1/6. Conditioning leaves resolvent and transformed-mean debts, current separate absolute theta_j schedules have no exponent window, and near T_G^> retains bounded Cartan curvature outside regular Cartan FAR ownership. Next: retain the full rootwise square and prove one expectation-inside moving-prefix, near T_Q^>+T_G^>, heat, low, paid, Wick-trace, R-063-forest, and conditional-mean form. Complete H_N, REG, OVERLAP_src, Nelson, and Sector A remain open.
