...
- **T-054** Pre-A evidence register and candidate-functional tournament  _(owner: Codex)_
  - Primary truth-first Pre-A route. EXP-000696 is the current audit authority and corrects EXP-000694/695. A bounded primary-source programme-sequence audit locates scoped antecedents for every coarse motif but no programme that explicitly selects the causal-origin fork C0 and establishes all applicable N1-N5 links under one compatible microscopic law and state history; under premetric C0-B it would also have to derive causal order/nullness. This is not a world-first or global-novelty proof, and validation/holdouts are a separate V1 gate. PA-H1-DNKG4-v0 now gives candidate-scope fixed-background results: compatible double-null C1 traces yield global finite-rectangle linear reconstruction, Bessel/Volterra stability, and for 0<2*tau<=min(U,V) an injective continuous C1 x C0 slice map with explicit field, spatial-derivative, and momentum bounds, symplectic/Weyl algebraic-state transport, and exact boundary-to-slice energy-flux balance; the phi4 extension is unique only in a common declared contraction ball. Current executable totals are 35 primary, 30 independent, and 49 integrator assertions, 114 combined, with stored child JSON required to equal fresh execution. The causal primitive, physical/Hadamard state, Einstein constraints, event horizon, cosmic high-energy scale, cooling map, spacetime emergence, and Lane-H-to-PA-M2 map remain open. PA-M2 remains a separate 3D T0 benchmark with kernel-restricted morphology and stable/critical Gaussian lambda_L=0; cyclicity is optional downstream. Next select C0-A or C0-B without double-counting inserted structure, then define/select omega_H and prove one state, energy normalization, and r(tau) history feeding both Lane F and Lane Q. In parallel close the PA-M2 complement branch and interacting cutoff test, then test compact-gauge M3.
...
- **T-050** A13: prove the controlled-shell energy one-use bound  _(owner: Codex; claim: A13-CLASSII-RELATIVE-PHASE-SOURCE-BUDGET-OBSTRUCTION; gate: A13-CLASSII-CONTROLLED-SHELL-ENERGY-ONE-USE)_
  - R-159 closes the centered no-past pure-dyadic p:2p:4p local chart at its honest uniform scope: exact Q-Q0 cancellation and finite-covariance-jet compactification give one existential radius independent of cutoff, common real-even covariance-matched |m|<=1 regulator, and retained root at the fixed A1 symbol/floor, with 247/2000>1/10 retained. It supplies no numerical radius, floor removal, alias theorem, realised-past fibrewise bound, growing-root union, or arbitrary predictable nonlinear/revisit estimate. Keep T-050 parked at that true globalization boundary. Resume only for a scheme-independent globalization theorem/counterexample or when a truth-first T-054 survivor pins the complete production law. No phase, BCC, or PDE is privileged.
  - _updated 2026-08-03_
