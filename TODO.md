...
- **T-050** A13: prove the controlled-shell energy one-use bound  _(owner: Codex; claim: A13-CLASSII-RELATIVE-PHASE-SOURCE-BUDGET-OBSTRUCTION; gate: A13-CLASSII-FULL-PROGRESSIVE-REVISIT-EXTENSION)_
  - R-126 differentiates the complete R-125 variance-minus-forest symbol, gives its periodic Euler force, and proves that the legal control object requires strict-past projection plus the actual Cameron-Martin/source Riesz map. It fixes the forward mixed-shell sums and block-Loewner acceptance condition, retains the slower linear mixed scale, and retires only an unrestricted anticipative reverse-band extension. Under a still-unproved production common-terminal/matching-trace hypothesis, the root baseline transports into the low/injected R-079 block with E W_J(A0) and all mean reserves retained; this is an equivalence, not a sign proof. Next construct the predictable CM/source-projected six-row total force and prove one cutoff/chart/control/refinement-uniform forward/legal-reverse/balanced/low block-Loewner estimate with covariance, forest, 2680/729 Cartan, low, injected, and mean owners inside expectation. OVERLAP_src, Nelson, removals, interacting measure, and Sector A remain open.
