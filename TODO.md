...
- **T-050** A13: prove the controlled-shell energy one-use bound  _(owner: Codex; claim: A13-CLASSII-RELATIVE-PHASE-SOURCE-BUDGET-OBSTRUCTION; gate: A13-CLASSII-CONTROLLED-SHELL-ENERGY-ONE-USE)_
  - R-150 installs the first actual A1 +/-k,+/-2k production field/current synthesis. The simultaneous final antipodal pair has exact coincident K=0 and a nonnegative conditional absolute full-output/full-reveal endpoint atom with zero atom-level source/sextic allocation, while the two-point and coefficient crosses are nonzero. Exact fixtures show that a nonlocal projection has separate expectations -1/2 and +1/2, two positive absolute endpoints can have relative secant -3/2, and an earlier-root future-feedback owner can equal -sqrt(6)/8. Continue in the same chart: first compute the relative final endpoint secant with its baseline cross; separately compute the earlier-root common-heat derivative with h2(g1) retained. Then restore R-125 trace/bracket/future variance, R-063 forest, balanced response, historical complete low, source, and sextic incidence once and test eta<5/11 and zeta<13/50. No phase is privileged. T-050, A13, Nelson, and Sector A remain open.
