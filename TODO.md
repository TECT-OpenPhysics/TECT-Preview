...
- **T-050** A13: prove the controlled-shell energy one-use bound  _(owner: Codex; claim: A13-CLASSII-RELATIVE-PHASE-SOURCE-BUDGET-OBSTRUCTION; gate: A13-CLASSII-CONTROLLED-SHELL-ENERGY-ONE-USE)_
  - R-154 derives the complete cross-root endpoint on the first finite disjoint multipair affine production chart. Fourier selection leaves only factor-three edges, the graph is a union of finite paths, and exact Sturm plus independent Bernstein certificates give one global origin-Hessian gap above 1/40 with source paid once and the whole PSD sextic form dropped. T-050 remains open: the next direct chart must permit one overlapping source/target chain and derive its covariance second jet, source reuse, legal adjoint, and any connection terms from the same scalar action before a global row or generalized-eigenvalue test. Test nonzero past or nonlinear feedback only afterward; add low/Feshbach only if a genuine independent low coordinate is declared. R-154 selects no phase and validates or replaces no PDE; T-054 remains the separate primary phase-neutral model-selection route.
  - _updated 2026-08-02_
