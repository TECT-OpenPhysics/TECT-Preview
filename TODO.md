...
- **T-050** A13: prove the controlled-shell energy one-use bound  _(owner: Codex; claim: A13-CLASSII-RELATIVE-PHASE-SOURCE-BUDGET-OBSTRUCTION; gate: A13-CLASSII-FULL-FRAME-POSTERIOR-COVARIANCE-BRACKET)_
  - R-101 proves, for pointwise coefficients with real-even Gaussian covariance and regular strict-past controls, that the moving heat baseline is conditionally orthogonal to the fresh full-Wick increment. Cross-Doob terminalization assigns the complete raw block to one fixed low endpoint plus exactly the already-paid R-100 future-control residual, so no second secant charge is allowed. Exact derivative-current and Taylor ledgers reduce the rational regular owner from old (6.5) to K_R=G^T L c+c^T B_1 c/2 with B_1=(P_(Sigma_tar)B_R)(U+a) after the three linear rows and unshifted current families are assigned. The retained square is heat-lifted. The scalar audit separates the full 40/81 remainder, assigned -25/81 base cubic, and unresolved 65/81 balanced remainder. Next prove the heat-lifted coefficient-balanced rational-current Carleson inequality for K_R uniformly over the admissible target heats while retaining a fixed positive fraction of that same square and the actual Cameron--Martin, heat, trace, q/r, low, and R-063 forest owners exactly once. Unheated or zero-heat-uniform Schur fails; fixed-positive-heat coercivity remains unproved. Complete H_N, REG, progressive/revisit H_A, OVERLAP_src, Nelson, floor removal, measure construction, and Sector A remain open.
