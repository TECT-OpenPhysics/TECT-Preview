...
- **T-050** A13: prove the controlled-shell energy one-use bound  _(owner: Codex; claim: A13-CLASSII-RELATIVE-PHASE-SOURCE-BUDGET-OBSTRUCTION; gate: A13-CLASSII-FULL-FRAME-POSTERIOR-COVARIANCE-BRACKET)_
  - R-100 proves that the complete R-099 owner is payment-gauge invariant and exactly row-additive; the R-098 posterior Schur gap cancels the complete-square gap, and finer coefficient revelation only exchanges covariance with posterior mean. The heat-centred split F_j=L_j+H_j plus a full-Wick R-094 extension pays the entire regular strict-past future-control/Jensen H_j residual with one X/Y allocation. Exact abstract fibres show PSD plus separate X/Y moments is insufficient, while a reduced complete-frame ray with c=Da is H2-subcritical and points to production scale coupling. Next prove the rowwise moving heat-baseline inequality D_H<=M_H+2 eta X+2 zeta Y+C with low, heat, trace, q/r, payment, and R-063 forest retained once. Rational (6.5), H_N, REG, H_A, OVERLAP_src, Nelson, and Sector A remain open.
