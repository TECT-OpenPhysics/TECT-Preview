...
- **T-050** A13: prove the controlled-shell energy one-use bound  _(owner: Codex; claim: A13-CLASSII-RELATIVE-PHASE-SOURCE-BUDGET-OBSTRUCTION; gate: A13-CLASSII-FULL-PROGRESSIVE-REVISIT-EXTENSION)_
  - R-109 proves the all-amplitude one-pair square-first normalizer and fresh-pair supermartingale, repairs the R-108 realized-covariance filtration order, proves the centered diagonal quartic floor and fixed-predictable-PSD signed score-transfer owner, and records the divergent full-pair floor plus exponential Stein-substitution no-go. Next prove one adapted raw complete production-cluster estimate coupling Cameron--Martin mean, predictable square-first covariance, trace, cross-mode baseline/current, rational recovery, all visits, and the random-W R-063 forest with source energy and one terminal sextic paid once. OVERLAP_src, Nelson, removals, interacting-measure construction, and Sector A remain open.
