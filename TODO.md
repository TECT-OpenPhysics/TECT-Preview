...
- **T-050** A13: prove the controlled-shell energy one-use bound  _(owner: Codex; claim: A13-CLASSII-RELATIVE-PHASE-SOURCE-BUDGET-OBSTRUCTION; gate: A13-CLASSII-FULL-FRAME-POSTERIOR-COVARIANCE-BRACKET)_
  - R-099 proves the exact value-plus-target-heat Cartan telescope inside one unchanged root, a sharp causal Doob--Hardy control-coordinate one-use ledger, the complete-frame ordered reveal with mandatory Jensen/covariance residual, and two-sided rational-family recovery in the regular strict-past one-shot class. Two scoped no-gos retire terminal-mixed-only arbitrary revisits and absolute nonlinear-frame square transfer. Next prove the coefficient-unconditioned signed lower form for S_R+C_post/2-P_R-W_0 with every pure-X, frame/Jensen, low, heat, trace, q/r, Cameron--Martin, and R-063 forest owner used exactly once; rational (6.5), H_N, REG, H_A, OVERLAP_src, Nelson, and Sector A remain open.
