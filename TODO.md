...
- **T-050** A13: prove the controlled-shell energy one-use bound  _(owner: Codex; claim: A13-CLASSII-RELATIVE-PHASE-SOURCE-BUDGET-OBSTRUCTION; gate: A13-CLASSII-CONTROLLED-SHELL-ENERGY-ONE-USE)_
  - R-149 fixes the next phase-neutral obstruction precisely. Under the complete R-125 incidence hypotheses, Pcomp=(Forest_ad-Vfuture)/2 while the raw/covariance-normal owner is Forest_ad/2. The exact affine-Gaussian owner depends on the field-current cross synthesis K=Sx Sv*, and identical endpoint marginals can give opposite signs for every positive covariance scale. The separately declared full-complex six-row same-root tensor is positive for K=Gamma, but this does not determine the production derivative/quadrature synthesis or any phase. Continue in the registered old filtration: declare one finite production cylinder with Sx, Sv, K, exhaustive output/spatial projectors, common heat, root/visit incidence, adapted feedback, complement and historical complete-low leakage; compute the polarized predictable kernel bar_tau[p,q]-beta[p,q]-<b_p,b_q> at (Sigma,Delta); then test eta<5/11 and zeta<13/50. BCC, uniform, other modulation, metastability, and effective-action failure remain equally admissible outcomes. T-050, A13, Nelson, and Sector A remain open.
