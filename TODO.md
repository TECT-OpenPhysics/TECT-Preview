...
- **T-054** Pre-A evidence register and candidate-functional tournament  _(owner: Codex)_
  - EXP-000703 is the current C0-A Gaussian authority. PA-C0A-GAUSSIAN-CCR-PAH1-EMBEDDING-v0 upgrades the finite transfer calibration to a strongly continuous reversible Mehler semigroup with unbounded H=dGamma(Omega), unique Gaussian vacuum, gap 3, exact infinite-occupation CCR, quasi-free state, and an explicit three-mode characteristic trace embedding whose pullback is exactly the existing PA-H1 slice map. Primary 119, non-importing independent 26, and integrator 121 checks pass, 266 combined. The transfer is injective and form-strict but has no uniform positive lower bound; zero is spectral accumulation and the logarithm has a proper unbounded domain. The Galerkin covariance estimate is only a cross-algebra characteristic-functional comparison. Finite occupation fails exact CCR; the finite projector is spatially nonlocal; complement extensions are nonunique; the fixture inserts rather than derives KG dispersion and background causal structure. Raw zero-point 13/2 and normal-ordered zero expose the unresolved common energy reference. Mehler-to-Fock reconstruction and characteristic boundary state methods have prior art; the TECT-specific result is only the exact PA-H1 interface and falsifier ledger, with no global novelty claim. This does not select a physical vacuum, beat the no-condensate reference, prove Hadamard/local continuum physics, close C0, or prove Pre-A. Next prove PA-H1-M2-STRICT-COMPOSITION-NOGO-v0: audit unchanged-interface phase-space dimension, interacting quartic energy, linearized spectra, regulator closure, energy-offset identifiability, stationary-vacuum cooling, and causal-symbol compatibility. If the no-go closes, replace unchanged composition by a common finite-regulator T3 parent with one algebra, normalization, boundary, and dynamic clock/control before any cooling or phase-transition claim.
