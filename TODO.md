...
- **T-050** A13: prove the controlled-shell energy one-use bound  _(owner: Codex; claim: A13-CLASSII-RELATIVE-PHASE-SOURCE-BUDGET-OBSTRUCTION; gate: A13-CLASSII-CONTROLLED-SHELL-ENERGY-ONE-USE)_
  - R-134 now survives adversarial repair at the narrower scope: sharp post-cluster root-spatial Pauli floor/action algebra, pointwise six-real D2/D3 jet control, embedded-Pauli D4 logarithmic no-go, pointwise-ellipticity spatial-transfer no-go, and exact conditional gamma=7/12 geometric sums. Next: prove A13-CLASSII-SIGNED-FOREST-TERMINAL-INNOVATION-ONE-USE-HEADROOM-BOUND by retaining each exhaustive cluster direct sum inside its root owner, proving production terminal reservation plus joint value-gradient/spatial data, the owner-preserving R-087-self-atom to R-088-sequential-atom transfer, the once-owned q_k ledger, and collar-uniform near/balanced headroom; then derive production constants, matching energy, strict gap, absolute anchor, and apply R-129. Both A13 gates and Sector A remain open.
