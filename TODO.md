...
- **T-050** A13: prove the controlled-shell energy one-use bound  _(owner: Codex; claim: A13-CLASSII-RELATIVE-PHASE-SOURCE-BUDGET-OBSTRUCTION; gate: A13-CLASSII-FUTURE-CONTROL-WEIGHTED-INNOVATION-BRACKET)_
  - R-085 proves a genuinely nonorthogonal weighted causal Schur lemma: a complete production mixed-Cartan atom estimate with exponent s>1/2 would spend the triangular root/input sum once without output orthogonality. At s=7/12 and eta=1/12 the exact Schur constant is 572.4472106721531... and the gap factor is 2^(-7C/6). The accepted first-order gain is only 1/5; the candidate second-order gain 3/5 requires an unproved complete mixed-principal cancellation. The exact rational endpoint splits into five unshifted form-absorbable families, one signed shifted-Hessian pair, and one retained positive square. A production third-derivative fixture and a rational-only ratio tending to negative infinity exclude deletion, positivity, and fixed-square Schur shortcuts. Next prove the mixed Cartan atom estimate (4.10)--(4.11) and the coupled rational bound (6.5), then REG, OVERLAP, CORE, controlled-shell one-use, and q=10/9 Nelson in order. Sector A remains open.
