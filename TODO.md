...
- **T-050** A13: prove the controlled-shell energy one-use bound  _(owner: Codex; claim: A13-CLASSII-RELATIVE-PHASE-SOURCE-BUDGET-OBSTRUCTION; gate: A13-CLASSII-CONTROLLED-SHELL-ENERGY-ONE-USE)_
  - R-141 checkpoint: the exact complete finite-chart force is g_comp=-g_T/2, and the canonical actual-feature signature Z=(U,J,R) plus global Doob conjugation retain all R-102 product-covariance and cross-reveal blocks. The source-null/low-kernel criterion is e-Lambda_C^2-sigma-kappa0-(a0+tau_C)^2/[4(f-sigma)]. Positive-analysis diagnostics first fit at C=8 and C=10, but move coherent three- and five-layer bands; signed correlation, not magnitude data, decides positivity. No production common-feature factorisation, uniform Loewner bound, low compatibility, or positive graph gap is proved. Research pauses after the verified R-141 checkpoint at operator request; on resume, compute the full paired-replica/global-Doob signed matrix before matching and anchor work.
