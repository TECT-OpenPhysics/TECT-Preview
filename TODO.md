...
- **T-054** Pre-A evidence register and candidate-functional tournament  _(owner: Codex)_
  - Primary truth-first physical-model route. R-157 analytically rejects or requires retuning pinned unconstrained M1 as a nonzero-equilibrium candidate; T-052 is superseded for M1 except as regression. Freeze primary-source discovery and holdout evidence, retain M0, and register at least two structurally distinct candidates such as constrained/compact-target, nonlocal/multicomponent, or conserved/chemical-potential branches before comparison. Build a common falsification matrix and spend T-053 observables only on survivors. R-157 is not a physical-vacuum verdict. Keep only the pinned quantitative-radius or genuine-intrinsic-counterexample T-050 successor in parallel.
