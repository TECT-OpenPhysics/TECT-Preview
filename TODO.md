...
- **T-054** Pre-A evidence register and candidate-functional tournament  _(owner: Codex)_
  - Primary truth-first physical-model route. R-157 analytically rejects pinned unconstrained M1 as a nonzero-equilibrium candidate; T-052 is superseded for M1 except as regression. R-158 solves PA-M1-Q exactly: imposed fixed charge or Omega_mu produces a finite-shell first-order transition, but the state remains above the original neutral zero reference, its local observables are uniform, and no physical conserved charge or reservoir is derived. Freeze PA-M1-Q as CONDITIONAL-MECHANISM, not physical-vacuum selected. EXP-000687 records the structurally distinct NEW-HYPOTHESIS PA-M5-NL3-SV derivation: g>c sigma is the exact continuous-shell gate, kappa_L versus 3u_minus^2/(16v) is the exact finite-torus zero-reference gate, and the bare shell has an anisotropic p_parallel^2 plus |p_perp|^4 dispersion obstruction. Next build its compact candidate manifest and exact certificate; reject before T-053 if symmetry or dynamics cannot repair the common-speed and tangential-dispersion gates. Freeze primary-source discovery and holdout evidence, retain M0, and build the common discriminator matrix. Keep T-050 parked unless a selected model pins the production cylinder or a scheme-independent intrinsic counterexample appears.
