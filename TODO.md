...
- **T-050** A13: prove the controlled-shell energy one-use bound  _(owner: Codex; claim: A13-CLASSII-RELATIVE-PHASE-SOURCE-BUDGET-OBSTRUCTION; gate: A13-CLASSII-CONTROLLED-SHELL-ENERGY-ONE-USE)_
  - R-133 proves the exact affine common-heat Hermite-score identity and covariance costs, the connection-corrected predictable cylindrical score, a declared-finite-energy global score-transfer no-go, the conditional stopped-polynomial physical-response zero at safe collar C=4, the actual periodic rational boundary-layer obstruction to the existing sixth-amplitude gamma=4 route, and the exact positive-gamma aggregate collar threshold. Next: prove A13-CLASSII-BLOCKWISE-SIGNED-SCORE-FOREST-AGGREGATE-COLLAR-BOUND with root/visit/output intertwining, replica-relative variance, covariance/forest matching, all signed rational/Cartan owners, low/source/sextic used once, an aggregate one-use B_(7/12)^out bound, and collar-uniform near/balanced headroom; then derive production constants, matching energy, the absolute anchor, and apply R-129. Both A13 gates, OVERLAP_src, Nelson, removals, the interacting measure, and Sector A remain open.
