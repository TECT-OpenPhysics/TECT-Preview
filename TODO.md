...
- **T-054** Pre-A evidence register and candidate-functional tournament  _(owner: Codex)_
  - EXP-000701 and EXP-000702 are the current C0 fork authorities and continue EXP-000697. C0-A: PA-C0A-RPTM-FS-v0 is only a temporal transfer calibration. A supplied entrywise-Markov, reversible, strictly operator-positive finite transfer and spacing reconstruct H_a, its one-dimensional constant ground space, gap, and unitary group. Negative-, zero-, and operator-positive/non-Markov controls separate P>=0 link reflection positivity from P>0 finite-log reconstruction. Primary 24, non-importing independent 20, and integrator 56 checks pass, 100 combined. C0-B: deterministic natural comparison within one finite automorphism orbit is impossible, so a finite transitive input selects only the empty strict order. The transitive but non-2-transitive C4 fixture and exhaustive 4096-relation audit pass, while two-orbit and random-sector controls keep richer routes open. Primary 52, independent 19, and integrator 60 checks pass, 131 combined. Neither elementary package is scientific novelty or a physical choice between C0-A and C0-B. PA-H1 remains a fixed-background state transporter, not a state selector, and PA-M2 remains a separate 3D low-energy variational benchmark. The causal structure, physical clock and arrow, selected field or Hadamard state, gravity, event horizon, cosmic scale, cooling history, common parent law, and PA-H1-to-PA-M2 composition remain open. Next attack PA-C0A-GAUSSIAN-CCR-PAH1-INTERTWINER-v0 using a strongly continuous Gaussian semigroup and generally unbounded second-quantized generator, then derive the selected quasi-free state, local Klein-Gordon limit, and PA-H1 symplectic intertwiner. Keep C0-B parallel only through a concrete relational dynamics with proved sector structure and continuum causal limit. After that prove one common state, energy normalization, and r(tau) history feeding both Lane F and Lane Q.
