...
- **T-050** A13: prove the controlled-shell energy one-use bound  _(owner: Codex; claim: A13-CLASSII-RELATIVE-PHASE-SOURCE-BUDGET-OBSTRUCTION; gate: A13-CLASSII-CONTROLLED-SHELL-ENERGY-ONE-USE)_
  - R-131 proves the conditional owner-complete response coanalysis identity, the floor-uniform deterministic current H2 component, the mixed-Gram and bounded-multiplier inference no-gos, the exact conditional acceptance simplex, the Xi radial coefficient bound with common-phase full-tangent no-go, and fixed-heat non-uniformity. Next: prove the mixed-replica sextic-transversal successor A13-CLASSII-MIXED-REPLICA-SEXTIC-TRANSVERSAL-RESPONSE-BOUND by constructing the production mixed replica form; derive structure-aware C_mix/C_far and global c_bal; couple Xi radial and wedge channels to an owner-compatible transverse complement controlling the phase-invisible direction; establish D,K,R0,S0, matching energy, and the absolute anchor; then apply R-129 coanalysis and shifted Douglas once. Both A13 gates, OVERLAP_src, Nelson, removals, the interacting measure, and Sector A remain open.
