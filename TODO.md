...
- **T-054** Pre-A evidence register and candidate-functional tournament  _(owner: Codex)_
  - EXP-000712 is the current CP1 structural authority and EXP-000713 is the current units correction. PA-CP1-ST8-CB-v0 proves for every N divisible by four that LT3 is exactly a signed-canonical sum of eight decoupled nearest-neighbour coarse real phi4 species; the eight fine nodes are folded species zero modes, while the 256 minima and finite quantum tensor-product ground are analytic corollaries of the LT3 authorities. For coarse spacing a and fine spacing a/2, the declared harmonic family uses c_LT3(a)=c_phys/a^2, has fixed-band symbol r+c_phys|k|^2 and leading error -c_phys*a^2 sum k_i^4/12, and proves |grad omega|^2<=c_phys/chi, hence speed <=sqrt(c_phys/chi); the corresponding LT3 fine-coordinate speed agrees exactly. Selecting one species, one axis, the transverse-zero sector, PA-H1 circumference pi/2, c/chi=1 and -2r/chi=9 gives only a tuned ordered tangent calibration with frequencies 3,5,5. NG-2026-08-03-PRE-A-CP1-ST8-CONTINUOUS-TIME-EXACT-CONE records the nonzero nearest-neighbour c*t^2/(2chi) analytic tail, and NG-2026-08-03-PRE-A-CP1-ST8-ONE-CONNECTED-SCALAR-EQUIVALENCE excludes only exact invertible equivalence to one same-dimensional connected standard positive-edge scalar. Primary 67, independent 37 and integrated 62 checks pass, 166 combined. Interacting and quantum continuum limits, characteristic reconstruction and same-state restriction, one connected bulk, physical empty space, cooling, CP1 and Pre-A remain open. Next test a registered parity-mixing connected repair and, in parallel, a cutoff-uniform interacting continuum or exact-causal parent with two sheets, corner data and symplectic flux; reject either route if it erases the same-H ordering or PA-H1 normalization ledger.
