...
- **T-050** A13: prove the controlled-shell energy one-use bound  _(owner: Codex; claim: A13-CLASSII-RELATIVE-PHASE-SOURCE-BUDGET-OBSTRUCTION; gate: A13-CLASSII-CONTROLLED-SHELL-ENERGY-ONE-USE)_
  - EXP-000651 and NG-2026-08-02-A13-ZERO-CONTROL-FUTURE-VARIANCE-DOMINATION decisively falsify the EXP-000648 absolute comparison V1<=Q1 in the actual full six-row A1 +/-k,+/-2k cylinder. At zero control, E1(Q1-V1) is exactly the negative coefficient-smoothing variance -E||(C6-E2 C6)D1^(1/2)||_HS^2<0; the linear Pauli rows make it strict, and g1=0 plus continuity gives an open conditional failure set. Park rootwise endpoint positivity and Poincare repair. This does not refute the relative action: the zero-control secant is zero. Next work only in the EXP-000649 event-complete relative coordinate, restore the R-125 forest and complete-low trace/bracket matching before unequal event weights, then test weighted prefix defects with balanced response, source, and sextic exactly once. T-054 remains the phase-neutral physical model-selection track. T-050, A13, Nelson, every phase/PDE verdict, and Sector A remain open.
