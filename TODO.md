...
- **T-054** Pre-A evidence register and candidate-functional tournament  _(owner: Codex)_
  - Primary truth-first physical-model route. R-157 analytically rejects pinned unconstrained M1 as a nonzero-equilibrium candidate; T-052 is superseded for M1 except as regression. R-158 solves PA-M1-Q exactly: imposed fixed charge or Omega_mu produces a finite-shell first-order transition, but the state remains above the original neutral zero reference, its local observables are uniform, and no physical conserved charge or reservoir is derived. Freeze PA-M1-Q as CONDITIONAL-MECHANISM, not physical-vacuum selected. The PA-M5-NL3-SV-v0 certificate now proves exact auxiliary elimination, distinct continuous and finite-torus shell gates, finite-torus coercivity, and the exact neutral-reference threshold. EXP-000688 and NG-2026-08-03-PA-M5-BARE-ISOTROPIC-SHELL-CAUSAL-CONE retain that static mechanism but reject the bare candidate as a joint T-053 survivor: its minimum is a uniform-density plane wave, negative-quartic coexistence is gapped, a critical isotropic shell has rank-one spatial Hessian and tangential-quadratic frequency, and the screened vector is not a local gauge connection. Next register either a genuine compact-gauge M3 candidate or an M5 candidate with symmetry-protected isolated nodes, then apply the common discriminator and evidence matrix. Keep T-050 parked unless a selected model pins the production cylinder or a scheme-independent intrinsic theorem or counterexample appears.
