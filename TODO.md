...
- **T-054** Pre-A evidence register and candidate-functional tournament  _(owner: Codex)_
  - Primary truth-first Pre-A route. EXP-000697 is the current route authority and continues the hostile-audited EXP-000696 baseline. The bounded primary-source programme audit still locates scoped antecedents for every coarse motif but no programme that explicitly selects C0 and establishes all applicable N1-N5 links under one compatible microscopic law and state history; this is not a world-first or global-novelty proof. PA-C0-DYNAMICAL-COMPLETION-NOGO-v0 now retires the static-to-unique-dynamics shortcut: exact gradient and inertial completions share one static critical set and Hessian but differ in temporal order, spectra, reversibility, energy law, and Gaussian node exponent. The PA-M2 critical node gives conditional z=2 versus z=1, its inertial slope contains inserted chi, and its full fourth-order kernel has unbounded ultraviolet group speed. Primary 21, non-importing independent 16, and integrated 42 checks pass, 79 combined. PA-H1-DNKG4-v0 remains a separate fixed-background 1+1 bridge with 114 combined assertions and transports but does not select a boundary state. PA-M2 remains a separate 3D T0 variational benchmark; it is not invalidated, and cyclicity stays optional downstream. The causal primitive, physical clock and arrow, C0-A/C0-B selection, physical/Hadamard state, Einstein constraints, event horizon, cosmic scale, cooling map, spacetime emergence, and Lane-H-to-PA-M2 composition remain open. Next choose one non-circular C0 microscopic model and derive its temporal/causal law; then define/select omega_H and prove one state, energy normalization, and r(tau) history feeding both Lane F and Lane Q. In parallel close the PA-M2 complement branch and interacting cutoff test, then test compact-gauge M3.
