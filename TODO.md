...
- **T-050** A13: prove the controlled-shell energy one-use bound  _(owner: Codex; claim: A13-CLASSII-RELATIVE-PHASE-SOURCE-BUDGET-OBSTRUCTION; gate: A13-CLASSII-FUTURE-CONTROL-WEIGHTED-INNOVATION-BRACKET)_
  - R-084 diagonalises the exact R-083 controlled Cartan square over complete probability roots with coefficient 3/(80P) and gives the conditional far-projected OU-gradient identity retaining coefficient-difference, predictable derivative-injection, and fresh value-innovation channels. An exact cumulative-tree witness proves that root orthogonality or unweighted Poincare alone is not one-use. For regular mutually orthogonal strict-past one-shot controls, all three linear Pauli--Fierz NEAR rows are arbitrarily form-absorbed with five positive Young slacks and worst moment 30/7; this is not positivity and the nonlinear rational row remains open. Next prove the summed production far-projected OU-gradient estimate, then the rational endpoint and complete signed NEAR recombination, overlap-stable progression, controlled-shell one-use, and q=10/9 Nelson. Sector A remains open.
