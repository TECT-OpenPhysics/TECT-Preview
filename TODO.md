...
- **T-054** Pre-A evidence register and candidate-functional tournament  _(owner: Codex)_
  - EXP-000711--EXP-000725 retain the LT3/ST8/Q3LOCK finite-local branch, the CP1a comparator, and the finite-C1 strict-cone boundary. EXP-000728 registered separate CL8 Goursat and semidiscrete Cauchy theorems. EXP-000730 now resolves their mixed boundary-to-lattice gate by scope split through PA-CP1-CL8-CLASSICAL-BOUNDARY-TO-LATTICE-OA2-v0. Periodic-jet-compatible C8 Goursat phase slices initialize the even-M lattice directly; generic admitted traces require a declared larger-circle Hermite fill with separate H_ext and Omega_ext ledgers. Exact sampling and the inherited theorem give O(a^2) grid-phase convergence, real trigonometric reconstruction gives O(a^2) in a common weighted H1 times L2 space including the Nyquist half-weight, and supplied compact classical phase measures converge in W1 at O(a^2). Primary 49, independent 38, integrated 118 checks pass, 205 combined. Generic direct seams, exact arbitrary-frequency sampling, and exact finite-a support remain rejected. The inserted full ordered PA-H1 circumference violates the current one-patch contraction gate with q at least 9pi^2/32, or 9pi^2/64 after an ordered shift; this blocks the certificate, not existence. Next attack PA-CP1-CL8-FULL-CIRCUMFERENCE-GOURSAT-EXISTENCE by multi-patch or global continuation, then PA-CP1-CL8-PREFERRED-STATE-COMPOSITION-SELECTION. Keep preferred classical-measure selection, quantum state composition, fine translation, full 3+1 dependence, physical empty space and the below-empty-space sign, cooling, gravity, C6 advancement, CP1 and Pre-A open.
