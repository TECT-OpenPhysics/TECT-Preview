...
- **T-050** A13: prove the controlled-shell energy one-use bound  _(owner: Codex; claim: A13-CLASSII-RELATIVE-PHASE-SOURCE-BUDGET-OBSTRUCTION; gate: A13-CLASSII-FULL-PROGRESSIVE-REVISIT-EXTENSION)_
  - R-111 proves the stationary scalar two-frequency k:2k square-first inequality for every q and amplitude on both degenerate covariance faces. On the mixed face it gives the exact I_0 normalizer, positive fixed-shape projective limit and first inverse-amplitude correction with a dominated remainder, exact high-q floor reduction, sharp Bessel and factorized tails, and three registered method no-gos. The next scalar step is a projective remainder uniform on covariance-shape charts, uniform local-t and r-to-zero/infinity face patching, and interval certification of the residual mixed core. Even if that closes, the production theorem still requires a subdivision-invariant full-A1 one-fresh-root cluster retaining complete heat, mean, trace, mixed baseline, rational recovery, every visit, random-W double divergence, and the R-063 forest exactly once, followed by a separate one-use source/sextic aggregation. R-103 REG and R-087 fixed-cutoff CORE retain their prior owners. OVERLAP_src, Nelson, removals, interacting measure, and Sector A remain open.
