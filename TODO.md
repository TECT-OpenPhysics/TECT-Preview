...
- **T-054** Pre-A evidence register and candidate-functional tournament  _(owner: Codex)_
  - Primary truth-first physical-model route. R-157 analytically rejects pinned unconstrained M1 as a nonzero-equilibrium candidate; T-052 is superseded for M1 except as regression. R-158 solves PA-M1-Q exactly: imposed fixed charge or Omega_mu produces a finite-shell first-order transition, but the state remains above the original neutral zero reference, its local observables are uniform, and no physical conserved charge or reservoir is derived. Freeze PA-M1-Q as CONDITIONAL-MECHANISM, not physical-vacuum selected. Next formalize the structurally distinct PA-M5-NL3-SV screened-vector/nonlocal candidate and apply its exact zero-reference and causal-shell falsifiers. Freeze primary-source discovery and holdout evidence, retain M0, build the common discriminator matrix, and spend T-053 only on survivors. Keep T-050 parked unless a selected model pins the production cylinder or a scheme-independent intrinsic counterexample appears.
