...
- **T-050** A13: prove the controlled-shell energy one-use bound  _(owner: Codex; claim: A13-CLASSII-RELATIVE-PHASE-SOURCE-BUDGET-OBSTRUCTION; gate: A13-CLASSII-CONTROLLED-SHELL-ENERGY-ONE-USE)_
  - R-153 identifies the exact final-fresh-root strict-past conditional endpoint and Hessian, a spatial L2 collar, and the A1 control-mean gradient constant. A nondegenerate derivative-active Gaussian past rules out a deterministic absolute collar, and event localization rules out an all-predictable-direction averaged escape. T-050 remains open and parked. Reopen the direct matrix only with one actual predictable production chart deriving earlier-root feedback, signed future-variance-minus-trace/forest completion, legal reverse, balanced/root compatibility, genuine low/Feshbach coupling, cross-root sextic, and chart connections from one scalar action exactly once, then test K>=-4I/5 or M>=mu G. R-153 selects no phase and validates or replaces no PDE; T-054 remains the primary phase-neutral physical-model route.
  - _updated 2026-08-02_
