...
- **T-054** Pre-A evidence register and candidate-functional tournament  _(owner: Codex)_
  - EXP-000711 is the current CP1 common-container authority and continues EXP-000707. PA-CP1-LT3-RS-v0 fixes one finite periodic real-scalar Weyl/Hamiltonian family for each N divisible by four and parameter tuple. It proves the exact symbol with eight isolated nodes, cubic Umklapp closure, the same-H classical sign H_min=-Vr^2/(4g)<H(0,0)=0 for r<0, exactly 256 constant-magnitude minima, 6561 kernel stationary fields, a Gram-certified ordered Hessian floor, executable low-energy bounds, and an inertial z=1 scalar Lipschitz speed envelope. A moment-derived product-Gaussian witness is -609/8 at R=20 and proves raw E0<0 for sufficiently negative r, but raw zero is additive-convention dependent. At fixed N the standard confining Schrodinger theorem gives one symmetric ground state per fixed parameter tuple and no selected classical pattern. The deterministic translation-covariant boundary rule from symmetric fixed-N data selects only empty or full site sets; periodicity is not a characteristic or event horizon. Primary 75, non-importing independent 64, and integrator 109 checks pass, 248 combined. Physical empty space and a normalized finite-energy no-condensate reference remain unidentified, and thermodynamic or continuum limits, pure ordered quantum phase, characteristic reconstruction, cooling, CP1, and Pre-A remain open. Next pursue a parent route with a regulator-level causal/locality estimate or a controlled Lorentzian emergence limit, derive two characteristic sheets, corner data, symplectic flux, bulk reconstruction, and state restriction, and then address CP2 with a dynamical r control and total-energy ledger.
