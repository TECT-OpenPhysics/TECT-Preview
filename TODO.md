...
- **T-050** A13: prove the controlled-shell energy one-use bound  _(owner: Codex; claim: A13-CLASSII-RELATIVE-PHASE-SOURCE-BUDGET-OBSTRUCTION; gate: A13-CLASSII-FUTURE-CONTROL-WEIGHTED-INNOVATION-BRACKET)_
  - R-079 closes the exact expectation-level full frame-current and canonical safe-packet Doob decomposition, retaining the conditional low-current block, complete low endpoint, present/future cross term, positive squares, Wick traces, complete R-063 forest, and paid endpoint difference. It proves one spatially weighted Cameron--Martin control square-function use and the predictable base-current heat projection. Next prove the production near/far current-trace estimate: far-root heat-tail gain plus weighted control square function; near-root signed Schur or genuine positive-gain bound with all innovations, compensators, squares, traces, forest terms, and paid subtraction retained. Bound the conditional low current and complete low safe endpoint separately, then apply R-075 graph recovery, assemble controlled-shell one-use, and return to q=10/9 Nelson. Sector A remains open.
