...
- **T-050** A13: prove the controlled-shell energy one-use bound  _(owner: Codex; claim: A13-CLASSII-RELATIVE-PHASE-SOURCE-BUDGET-OBSTRUCTION; gate: A13-CLASSII-FULL-PROGRESSIVE-REVISIT-EXTENSION)_
  - R-108 separates the historical weighted R-085 contract from the direct unweighted R-088 target, proves the rational subdivision quotient and exact quotient-safe complete endpoint/cluster conditional identities, and fixes the order-of-operations boundary. A bare average-before-square ledger requires the explicit nonlinear/sextic repair; square-before-average remains viable only on the one-pair fixture. An absolute arbitrary-selector future-feedback HS/PSD ledger paid solely by source energy and one terminal sextic fails while the signed second jet cancels. Prove the cutoff/chart/control/subdivision-uniform signed complete-cluster lower bound or a legal realized-cluster conditional normalizer with every remainder predeclared. OVERLAP_src, Nelson, removals, interacting-measure construction, and Sector A remain open.
