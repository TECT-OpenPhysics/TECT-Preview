...
- **T-050** A13: prove the controlled-shell energy one-use bound  _(owner: Codex; claim: A13-CLASSII-RELATIVE-PHASE-SOURCE-BUDGET-OBSTRUCTION; gate: A13-CLASSII-NPC-CONE-MARTINGALE-INJECTION-BALANCE)_
  - R-070 A13-CLASSII-WICK-DOOB-TERMINAL-RESOLVENT-REDUCTION proves the exact full-coefficient Wick--Doob terminal current, raw/Wick trace restoration, Abel--Hardy covariance-tail payment, endpoint-defect equivalence, terminal Schur completion, Hilbert--Schmidt trace payment, and the exact full Q_II-weighted symmetric--Cartan linear-frame split. The Cartan and q11 pure-pp pieces are paid. Automatic adapted terminal-resolver centering is false and direct Stein closure needs undeclared Malliavin derivatives. The remaining regular finite-cutoff core has two ordered subgates: the balanced H^{-1-kappa} model attribution for the non-pp symmetric frame, then the coupled nonlinear rational-frame/cross-square bound with the coefficient remainder and terminal square retained; after these come finite-energy extension, controlled-shell one-use, and Nelson.
