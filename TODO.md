...
- **T-054** Pre-A evidence register and candidate-functional tournament  _(owner: Codex)_
  - Primary phase-neutral physical-model route. R-155 is a genuine but origin-only affine A13 advance and supplies no phase or PDE verdict. Freeze primary-source discovery and holdout evidence; register M0/M1 plus at least two structurally distinct candidate functionals; build the common discriminator matrix; use T-052 to reject candidates early; spend T-053 only on survivors. A justified shortlist, non-selection, or evidence against the present PDE is valid. Keep only the minimum nonzero-past or nonlinear/revisit T-050 successor in parallel; do not let new abstract A13 lemmas delay the model tournament.
...
- **T-050** A13: prove the controlled-shell energy one-use bound  _(owner: Codex; claim: A13-CLASSII-RELATIVE-PHASE-SOURCE-BUDGET-OBSTRUCTION; gate: A13-CLASSII-CONTROLLED-SHELL-ENERGY-ONE-USE)_
  - R-155 removes source-target disjointness for the canonical fixed-law affine zero-control chart. The shared Gaussian is defined once on P union 2P; injective doubling and raw-source independence keep the source Gram diagonal and make every distinct-source mixed covariance second jet vanish. Factor-two reuse adds no endpoint edge; exact Sturm and independent Bernstein certificates give a global origin gap above 7/250, with a sharper 147/1000 pure-dyadic corollary, while the sextic Hessian remains one global PSD form. T-050 remains open. Do not add another abstract affine-origin lemma: the minimum direct successor must instantiate actual nonzero past or nonlinear/revisit feedback and derive every owner from one scalar production action before a Loewner test. Add low/Feshbach only for a genuine independent coordinate. Keep T-054 in progress, use T-052 for early candidate rejection, and reserve T-053 for survivors. R-155 selects no phase and validates or replaces no PDE.
  - _updated 2026-08-02_
