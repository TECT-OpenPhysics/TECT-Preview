...
- **T-050** A13: prove the controlled-shell energy one-use bound  _(owner: Codex; claim: A13-CLASSII-RELATIVE-PHASE-SOURCE-BUDGET-OBSTRUCTION; gate: A13-CLASSII-CONTROLLED-SHELL-ENERGY-ONE-USE)_
  - EXP-000648--EXP-000649 complete the two R-150 signed-connection calculations at unregistered exploration grade. The final relative secant retains its B-change and baseline cross. In the actual A1 +/-k,+/-2k cylinder, 4 Gamma(2k)-Gamma(k) is positive definite, so the earlier-root absolute raw endpoint Q1 is nonnegative for every legal measurable h2(g1), but the complete owner is 2 Pcomp,1=Q1-V1. The exact event partition merges retained, direct, and earlier-future connections before subtracting the signed replica secant. Next prove or falsify the actual six-row common-heat comparison V1<=Q1 and its relative weighted event form. If it fails, register the counterexample and park endpoint positivity; if it survives, certify/promote the combined result and restore R-125 trace/bracket/future variance, R-063 forest, balanced response, historical low, source, and sextic exactly once before testing eta<5/11 and zeta<13/50. No phase is privileged; T-050, A13, Nelson, and Sector A remain open.
