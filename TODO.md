...
- **T-050** A13: prove the controlled-shell energy one-use bound  _(owner: Codex; claim: A13-CLASSII-RELATIVE-PHASE-SOURCE-BUDGET-OBSTRUCTION; gate: A13-CLASSII-FULL-PROGRESSIVE-REVISIT-EXTENSION)_
  - Post-R-117 loop audit: primary next step is the direct source-action route. On the smallest legal two-visit, two-output contraction-closed packet, derive or refute a revisit-stable owner-complete operator-valued centered-form factorization; if it exists, apply the R-068 one-use estimate at expectation level over the R-093 directed source union. Use the centered det2 O(N^-3) canonical high-shell cost only after the legal adapted trace operator is identified. Conditional triangular Ramer--Schur is a fold-first fallback, not the primary route. Complete progressive/revisit ownership, one-use aggregation, OVERLAP_src, Nelson, and Sector A remain open. See EXP-000351--EXP-000356.
