...
| **GHAT4-PERTRANSFER** | The per-transfer quartic-difference form factor $\widehat{G^4}(\lvert t\rvert)$ (NOT the sup-kernel): required to decide the quartic-difference endpoint, where the sup-grade inflation eats almost the whole $\times2.6$ margin. Until computed, the third-order lift is UNDETERMINED at the endpoint, not merely marginal. | OPEN | quartic-difference-channel v1.0 |
