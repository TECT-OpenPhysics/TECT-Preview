...
| **G-A0-DUI** | Differentiation-under-integral regularity for $M(m)=\int[m+C(k^2-q_0^2)^2]^{-1}$ (Leibniz under dominated convergence; integrand $k^{-4}$ and $m$-derivative $k^{-6}$ tails dominate for $m\ge m_0>0$). Textbook; the residual of the H-A0 -> H-ANCHOR replacement. | OPEN (textbook, expected routine) | note ha0-removal-pathway v2.0 §L1 |
...
| **H-ANCHOR** | At the production anchor $\mu^2=0.005$, the $A=0$ gap point satisfies $m^*>m_w$ ($m_w=r+15vM_c^2=0.0392407$; closed-form, $\times7.76$ margin), with $M_R>M_c$ ($\times4.12$). The single explicit inequality from which the sign-decomposition theorem (note ha0-removal-pathway v2.0) derives H-A0's $A=0$ uniqueness and zero-at-gap; strictly weaker than the replaced H-A0 (no quadrature-scheme curve-shape certification). | Written-out G-A0-DUI to absorb H-ANCHOR into the A1-KERNEL-CONV anchor dependency; ROBUSTNESS-MU2 for the off-anchor neighbourhood |
