#!/usr/bin/env python3
"""a1_kernel_checks.py -- self-tests for the three A1 kernel cards (v1.7).
...
Passing the gates is scoped certification only after the independent operator
review package has ratified the same config, tolerances, and source hashes.
...
claim("canonical_n001_passes_scoped_t5_manifest", branch,
      ...
      "SCOPED T5 VERIFIED for the pure-Brazovskii scalar-slice manifest; full PDE/BCC/operator theorem excluded")
...
cert = dict(version=__version__, config_source=str(cfg_path.relative_to(REPO)), eta_shell=ETA, scalar_slice=SLICE,
    ...
    gates=dict(delta_Z=dZ, delta_m=dm, delta_r=dr, mu2_shell_positive=MSH > 0, eta_shell_zero=ETA == 0.0,
               ...
               certification="scoped T5 verified for the pure-Brazovskii scalar-slice manifest"),
