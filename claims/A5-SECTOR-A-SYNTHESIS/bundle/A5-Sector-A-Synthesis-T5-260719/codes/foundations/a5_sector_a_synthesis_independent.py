...
DEFAULT_PRIMARY = CLAIM / "runs" / "2026-07-19-t5-capstone-primary-preflight" / "result.json"
DEFAULT_OUTPUT = CLAIM / "runs" / "2026-07-19-t5-capstone-independent-preflight" / "result.json"
...
def main() -> int:
    ...
    check(
        "manifest_records_confirmed_t5_with_exact_capstone_review_gate",
        manifest.get("schema") == "tect/a5-sector-a-synthesis/1.2"
        and manifest.get("source_commit") == "6db0405ff9073078ce695f35caffe664096960c4"
    ...
    check(
        "all_six_published_support_bundle_attestations_are_valid",
        len(bundle_rows) == 6 and all(all(value is True for key, value in row.items() if key != "id") for row in bundle_rows),
        bundle_rows,
        assertions,
    )
