...
| [R-167](#r-167) | Fixed-Ritz DFFR two-level zero-source phases, common-generator zero-temperature transfer and common-dynamics route split | R-167 v3.3 is the current scoped T0 analytic/exact/executed/audited additive result under EXP-000837. It retains every v3.2 child and, for each fixed complete spectral-cluster Ritz label, verifies the explicit DFFR two-level QPS criterion at sufficiently large N, pins coexistence to zero source, and obtains two stable ordered phases with distinct beta-to-infinity ground limits. The result is not uniform in M; full-oscillator cutoff removal, exact-Q3 common alpha and beta-infinity selection, the broken-sector GNS gap, C6, Sector A and Pre-A remain open |
