...
| [R-167](#r-167) | Registered-corridor full-oscillator coexistence, categorical dynamics and exact source-ground transfer | R-167 v4.1 is the current scoped T0 analytic/exact/executed/audited additive result under EXP-000845. It retains every v4.0 child, proves the exact implementing-energy identity and centered bandlimited GNS form-core/Rayleigh reduction for the categorical ground pair, and proves equality of the two parity-related Poincare constants. The full mesoscopic finite-source gaps instead vanish, so a positive fixed-carrier phasewise coercive bound, spatial all-exhaustion common alpha and the broken-sector GNS gap remain open |
