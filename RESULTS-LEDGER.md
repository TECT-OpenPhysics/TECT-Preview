...
| [R-167](#r-167) | Registered-corridor full-oscillator coexistence, categorical dynamics and exact source-ground transfer | R-167 v4.0 is the current scoped T0 analytic/exact/executed/audited additive result under EXP-000844. It retains every v3.9 child and proves that exact-Q3 mesoscopic finite-volume source grounds yield a fixed-witness-separated algebraic ground pair on the zero-source categorical orbit-smear action by closing the combined residual on an explicit bandlimited graph core. Spatial all-exhaustion common alpha, DLR/KMS branch identification and the broken-sector GNS gap remain open |
