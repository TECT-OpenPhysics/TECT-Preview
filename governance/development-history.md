...
## 5.1 Global proof evidence map (GENERATED)

`theory/proof-evidence-map.md` and `verification/proof-evidence-map.json`
project the current claim state, reusable results, append-only proof
explorations, failed routes and reasons, accepted chronology, live tasks, open
gates, and per-claim evidence inventory into one linked roadmap. They are generated by
`verification/scripts/build_proof_evidence_map.py` and are never hand-edited.

The map is complete by reference but cannot move a tier or gate and cannot
replace a proof note, run, claim card, result entry, negative entry, or task.
Its authority and coverage contract is
`governance/proof-evidence-map.md`. Regeneration and staleness checking are
wired into the shared enforcement spine before catalog generation.
...
3. **Is it a formal refutation / dead branch / retraction?** ->
   `negative-results/registry.md`.
4. **Is it a substantive proof-route assessment, objection, assumption check,
   repair, or scope decision that another researcher should not have to
   repeat?** -> append `explorations/log.jsonl` with
   `verification/scripts/exploration.py` (NON-tier-bearing; policy:
   `governance/proof-exploration-ledger.md`). If it later becomes a formal
   result or no-go, link that authority without deleting the exploration.
5. **Is it a binding rule / process?** -> `governance/`.
6. **Is it forward-looking STRATEGY / ANALYSIS / decision-rationale that does
...
7. **Is it the development trace itself?** -> generated `claims/<ID>/LINEAGE.md`
...
- 2026-07-24: added the append-only proof-exploration layer so substantive
  route checks, failed attempts, repairs, and revisit conditions remain
  recoverable even when they do not qualify as formal results or no-gos.
