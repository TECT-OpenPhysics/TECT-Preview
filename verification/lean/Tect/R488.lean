...
/- The second split triangle has oriented edges d0, h01^{-1}, v0^{-1}.
   Its three endpoint incidences cancel independently. -/
def triangle1GaugeShift (ga gc gd : ℤ) : ℤ :=
  (gd - ga) - (gd - gc) - (gc - ga)

theorem triangle1_gauge_shift_cancels (ga gc gd : ℤ) :
    triangle1GaugeShift ga gc gd = 0 := by
  dsimp [triangle1GaugeShift]
  ring

def triangle1Exponent (ud uh01 uv0 ga gc gd : ℤ) : ℤ :=
  (ud + gd - ga) - (uh01 + gd - gc) - (uv0 + gc - ga)

theorem holonomy1_gauge_invariant (ud uh01 uv0 ga gc gd : ℤ) :
    triangle1Exponent ud uh01 uv0 ga gc gd = ud - uh01 - uv0 := by
  dsimp [triangle1Exponent]
  ring
...
/- The finite joint cylinder carries two matter coordinates, ell_a and ell_d. -/
def twoMatterCoordinates (ellA ellD : ℤ) : ℤ × ℤ := (ellA, ellD)

theorem two_matter_coordinates (ellA ellD : ℤ) :
    twoMatterCoordinates ellA ellD = (ellA, ellD) := by
  rfl
