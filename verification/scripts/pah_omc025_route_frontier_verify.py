#!/usr/bin/env python3
"""Integrated replay for PAH-OMC-025."""
...
RESULT = ROOT / "strategy/pa-hyp/PAH-OMC-025-route-frontier-result-v1.json"
REGISTRY = ROOT / "verification/lean/registry.json"
LEAN = ROOT / "verification/lean/Tect/PahOmc025RouteFrontier.lean"
SCRIPTS = {
    ...
    "hostile": ROOT / "codes/foundations/pah_omc025_route_frontier_hostile.py",
}
...
def lean_run(check_only: bool) -> dict[str, Any]:
    ...
    toolchain = registry["toolchain"]["toolchain"]
    if lake is None:
        payload = {"schema": "tect/pah-omc025-lean-run/1.0", "status": "UNAVAILABLE", "toolchain": toolchain, "source_hash": sha(LEAN)}
    else:
        worktree = ROOT / "verification/lean"
        canonical = Path("E:/Dev/TECT/verification/lean")
        if (worktree / ".lake/packages/mathlib").is_dir():
            cwd, source, scope = worktree, "Tect/PahOmc025RouteFrontier.lean", "lane-local-pinned-packages"
        elif (canonical / ".lake/packages/mathlib").is_dir():
            cwd, source, scope = canonical, str(LEAN), "read-only-canonical-pinned-packages"
        else:
            cwd, source, scope = worktree, "Tect/PahOmc025RouteFrontier.lean", "lane-local-missing-packages"
        env = os.environ.copy()
        ...
        payload = {"schema": "tect/pah-omc025-lean-run/1.0", "status": "PASS" if proc.returncode == 0 and "error:" not in output.lower() else "FAIL", "returncode": proc.returncode, "toolchain": toolchain, "source_hash": sha(LEAN), "dependency_scope": scope}
    ...
    if check_only and (not destination.is_file() or destination.read_bytes() != encoded):
        raise SystemExit("PAH-OMC-025 Lean replay mismatch")
    return payload
...
def main() -> int:
    ...
    check(rows, "result identity", result.get("result_id"), "R-565", result.get("result_id") == "R-565")
    ...
    check(rows, "registry entry", any(item.get("path") == "verification/lean/Tect/PahOmc025RouteFrontier.lean" for item in registry.get("entrypoints", [])), True, any(item.get("path") == "verification/lean/Tect/PahOmc025RouteFrontier.lean" for item in registry.get("entrypoints", [])))
    check(rows, "contract result match", contract.get("result_id"), result.get("result_id"), contract.get("result_id") == result.get("result_id"))
    payload = {
        "schema": "tect/pah-omc025-route-frontier-integrated/1.0",
        "audit_id": "PAH-OMC-025-ROUTE-FRONTIER-INTEGRATED-001",
        "result_id": "R-565",
        ...
        "replay": replay,
        "lean": lean,
        # The result card records this integrated run's hash, so hashing RESULT
        # here would create a self-referential replay cycle.  The integrated
        # audit hashes the immutable contract, tooling, Lean source and
        # registry; the result identity/hold/firewall checks above still audit
        ...
        "finding": "All replay roles agree that the three route cuts are required and currently uninstantiated.",
        ...
        "reproduction": "python -X utf8 verification/scripts/pah_omc025_route_frontier_verify.py --check",
    }
    destination = args.output if args.output.is_absolute() else ROOT / args.output
    encoded = (json.dumps(payload, ensure_ascii=True, sort_keys=True, indent=2) + "\n").encode("utf-8")
    if args.check:
        if not destination.is_file() or destination.read_bytes() != encoded:
            raise SystemExit("PAH-OMC-025 integrated replay mismatch")
    else:
        atomic_json(destination, payload)
    print(f"PAH-OMC-025 INTEGRATED: PASS {len(rows)}/{len(rows)}; verdict=HOLD_FOR_EVIDENCE")
    return 0
