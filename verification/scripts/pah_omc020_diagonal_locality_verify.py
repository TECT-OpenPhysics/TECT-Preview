#!/usr/bin/env python3
"""Integrated primary/independent/hostile/Lean replay for R-563."""
...
PRIMARY = ROOT / "verification/scripts/pah_omc020_diagonal_locality.py"
INDEPENDENT = ROOT / "codes/foundations/pah_omc020_diagonal_locality_independent.py"
HOSTILE = ROOT / "codes/foundations/pah_omc020_diagonal_locality_hostile.py"
LEAN_ROOT = ROOT / "verification/lean"
LEAN_SOURCE = LEAN_ROOT / "Tect/PahOmc020DiagonalLocality.lean"
...
DEFAULT_OUTPUT = ROOT / "claims/C6-SPACETIME-SIGNATURE/runs/2026-09-08-pah-omc020-diagonal-locality/integrated.json"
...
def main() -> int:
    ...
    lean_cwd = LEAN_ROOT
    lean_source = "Tect/PahOmc020DiagonalLocality.lean"
    ...
    rows = [{"name": name, "status": "PASS" if result["returncode"] == 0 else "FAIL", "detail": result} for name, result in runs.items()]
    failed = [row for row in rows if row["status"] != "PASS"]
    tools = (PRIMARY, INDEPENDENT, HOSTILE, LEAN_SOURCE)
    payload = {
        "schema": "tect/pah-omc020-diagonal-locality-integrated/1.0",
        "result_id": "R-563",
    ...
    print(f"PAH-OMC-020 DIAGONAL LOCALITY INTEGRATED: {payload['verification']} {payload['checks_passed']}/{len(rows)}")
