#!/usr/bin/env python3
"""lint_claims.py — TECT claim-ledger validator and CLAIMS.md generator.

Usage:
    python verification/scripts/lint_claims.py            # validate, exit 0/1
    python verification/scripts/lint_claims.py --render   # validate + write CLAIMS.md
    python verification/scripts/lint_claims.py --render --check
                                                          # validate + verify CLAIMS.md in sync
...
CLAIMS.md is generated; hand-editing is forbidden (single-source-of-truth).
...
def main():
    ...
    ap.add_argument("--render", action="store_true", help="write CLAIMS.md")
    ap.add_argument("--check", action="store_true",
                    help="with --render: fail if CLAIMS.md is out of sync (date line ignored)")
    args = ap.parse_args()
    ...
    if args.render:
        ...
        if args.check:
            ...
            if strip(old) != strip(content):
                print("RENDER-CHECK: FAIL — CLAIMS.md out of sync; run --render and commit")
                return 1
            print("RENDER-CHECK: PASS — CLAIMS.md in sync")
        else:
            LEDGER.write_text(content, encoding="utf-8")
            print(f"RENDERED: {LEDGER}")
    return 0
