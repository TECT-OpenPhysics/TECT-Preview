#!/usr/bin/env python3
"""Integrated verifier for the PAH-OMC-008 multi-cylinder replay."""
...
RUN_DIR = ROOT / "claims/C6-SPACETIME-SIGNATURE/runs/2026-09-04-pah-omc008-multi-cylinder"
...
LEAN = ROOT / "verification/lean/Tect/R488.lean"
...
REQUIRED_DECLARATIONS = {
    ...
    "triangle1_gauge_shift_cancels",
    "holonomy_gauge_invariant",
    "holonomy1_gauge_invariant",
    "state_count_exact",
    "link_channel_multiplicity",
    "joint_root_identity",
    "two_matter_coordinates",
...
def main() -> int:
    ...
    expected_rows = state_count * int(dims.get("patch_link_edges", 0)) * 2 + (state_count // int(dims.get("radial_placements", 1))) * int(dims.get("radial_degree_sum", 0))
    ...
    check("joint-scope", p_identity.get("levels") == [2, 3] and all(token in contract.get("exact_scope", {}).get("joint_observable", "") for token in ("ell_a", "ell_d", "H_0", "H_1")))
    ...
    entry = next((item for item in registry.get("entrypoints", []) if item.get("path") == "verification/lean/Tect/R488.lean"), {})
    ...
    if lake is None:
        ...
        process = subprocess.run([str(lake), "env", "lean", "Tect/R488.lean"], cwd=LEAN_ROOT, text=True, encoding="utf-8", errors="replace", capture_output=True, check=False, timeout=180)
    ...
    payload: dict[str, Any] = {
        "schema": "tect/pah-omc008-multi-cylinder-integrated/1.0",
