#!/usr/bin/env python3
"""Integrated replay for the PAH-OMC-020 core Duhamel contract."""
...
PRIMARY = ROOT / "verification/scripts/pah_omc020_core_duhamel.py"
INDEPENDENT = ROOT / "codes/foundations/pah_omc020_core_duhamel_independent.py"
HOSTILE = ROOT / "codes/foundations/pah_omc020_core_duhamel_hostile.py"
LEAN = ROOT / "verification/lean/Tect/PahOmc020CoreDuhamel.lean"
REGISTRY = ROOT / "verification/lean/registry.json"
RUN_DIR = ROOT / "claims/C6-SPACETIME-SIGNATURE/runs/2026-09-08-pah-omc020-core-duhamel"
...
def lean_path(cache: Path | None) -> str:
    ...
    local_cache = ROOT / "verification/lean/.lake/packages"
    if local_cache.is_dir():
        for package in sorted(local_cache.iterdir()):
            candidate = package / ".lake" / "build" / "lib" / "lean"
...
def main() -> int:
    ...
    check("contract schema", contract.get("schema"), "tect/pah-omc020-core-duhamel-contract/1.0",
    ...
    check("conditional firewalls", {key: contract["provenance"].get(key) for key in ("claim_bearing", "active_gate_change", "physical_promotion")},
    ...
    check("owner field count", len(contract["required_owner_packet"]), 6, len(contract["required_owner_packet"]) == 6)
    ...
    for name, run in (("primary", primary), ("independent", independent), ("hostile", hostile)):
        ...
        check(f"{name} no promotion", {key: run.get(key) for key in ("claim_bearing", "active_gate_change", "physical_promotion")},
              {"claim_bearing": False, "active_gate_change": False, "physical_promotion": False},
              all(run.get(key) is False for key in ("claim_bearing", "active_gate_change", "physical_promotion")))
    check("primary-independent oracle", primary.get("fixture", {}).get("derived_bound"), independent.get("fixture_bound"),
          primary.get("fixture", {}).get("derived_bound") == independent.get("fixture_bound"))
    check("hostile retained-term checks", hostile.get("checks_passed", 0) >= 19, True,
          hostile.get("checks_passed", 0) >= 19)
    ...
    entry = next((item for item in registry.get("entrypoints", []) if item.get("path") == "verification/lean/Tect/PahOmc020CoreDuhamel.lean"), None)
    check("Lean registry entry", entry is not None, True, entry is not None)
    if entry is None:
        entry = {}
    declarations = {
        "corr_bound_nonnegative", "duhamel_triangle", "compact_time_epsilon", "corr_bound_monotone",
        "omitted_initial_defect_is_unsound", "omitted_residual_is_unsound", "missing_owner_field_blocks",
        "owner_packet_requires_all_fields",
    }
    ...
    payload: dict[str, Any] = {
        "schema": "tect/pah-omc020-core-duhamel-integrated/1.0",
        "audit_id": "PAH-OMC-020-CORE-DUHAMEL-INTEGRATED-001",
        "task_id": "T-079",
        "status": "PASS_INTEGRATED_CORE_DUHAMEL_BOUND",
        ...
        "source_hashes": {
            "strategy/pa-hyp/PAH-OMC-020-core-duhamel-contract-v1.json": digest(CONTRACT),
            "verification/scripts/pah_omc020_core_duhamel.py": digest(PRIMARY),
            "codes/foundations/pah_omc020_core_duhamel_independent.py": digest(INDEPENDENT),
            "codes/foundations/pah_omc020_core_duhamel_hostile.py": digest(HOSTILE),
            "verification/scripts/pah_omc020_core_duhamel_verify.py": digest(Path(__file__)),
            "verification/lean/Tect/PahOmc020CoreDuhamel.lean": digest(LEAN),
            "verification/lean/registry.json": digest(REGISTRY),
        },
        "required_owner_fields": contract["current_status"],
        "finding": "Integrated replay agrees that the Duhamel envelope is a valid conditional transfer bound while the common-space, core, initial-recovery and residual owner inputs remain absent.",
    ...
    if args.check:
        if not destination.is_file() or destination.read_bytes() != encoded:
            raise SystemExit("PAH-OMC-020 core-duhamel integrated replay mismatch")
    else:
        atomic_json(destination, payload)
    print(f"PAH-OMC-020 CORE DUHAMEL INTEGRATED: PASS {payload['passed']}/{payload['assertion_count']}; verdict=HOLD_FOR_EVIDENCE")
    return 0
