#!/usr/bin/env python3
"""Hostile mutation firewall for the PAH-OMC-014 R499 support packet.

Mutations are applied only to an in-memory copy.  The firewall must accept the
unchanged conditional packet and reject source-law injection, status promotion,
hash tampering, model mutation, and missing-hypothesis weakening.
"""
...
OUTPUT = ROOT / "claims/C6-SPACETIME-SIGNATURE/runs/2026-09-05-pah-omc014-witness-stationarity-lean/hostile.json"
AUDIT_ID = "PAH-OMC-014-WITNESS-STATIONARITY-LEAN-HOSTILE-001"
...
def accepts(packet: dict[str, Any]) -> bool:
    ...
    return (
        ...
        and any("positive" in item.lower() and "witness" in item.lower() for item in assumptions)
        and any("absolute error" in item.lower() for item in assumptions)
        and any("source-owned cross-q" in item.lower() for item in missing)
        and any("full-q" in item.lower() for item in missing)
...
def main() -> int:
    ...
    mutations: list[tuple[str, Callable[[dict[str, Any]], None]]] = [
        ...
        ("drop witness hypothesis", lambda p: p["theorem_contract"].update(assumptions=p["theorem_contract"]["assumptions"][:-1])),
        ("erase cross-Q missing input", lambda p: p["theorem_contract"].update(not_supplied=[x for x in p["theorem_contract"]["not_supplied"] if "cross-Q" not in x])),
    ]
    mutation_results = {}
    for label, mutate in mutations:
        ...
        mutation_results[label] = accepted
        check(f"rejects {label}", not accepted, accepted, False)

    check("manifest bytes unchanged after in-memory attacks", digest(MANIFEST) == before, digest(MANIFEST), before)
    check("Lean source bytes unchanged", digest(LEAN) == source_before and source_before == LEAN_PIN, digest(LEAN), source_before)
    ...
    payload = {
        "schema": "tect/pah-omc014-witness-stationarity-lean-hostile/1.0",
        ...
        "failed": len(failed),
        "assertions": rows,
        "mutation_acceptance": mutation_results,
        "source_hashes": {"manifest": before, "R499": source_before},
        "assumptions": ["All mutations are in-memory copies.", "The original packet must remain support-only and hash-pinned."],
        "missing_assumptions": ["A source-owned full-Q weight law and PAH-specific vanishing generator-error estimate remain absent."],
        "non_claims": ["The firewall does not prove a PAH full-Q state, omega, physical Pre-A, spacetime, QFT, gravity, Yang--Mills, mass gap or TOE."],
        "next_question": "Can the source owner supply the missing full-Q law and vanishing generator-error estimate without triggering this firewall?",
    }
