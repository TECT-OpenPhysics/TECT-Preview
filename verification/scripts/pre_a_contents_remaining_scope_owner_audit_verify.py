#!/usr/bin/env python3
"""Integrated primary/independent/hostile verifier for R-460."""
...
PRIMARY = ROOT / "verification/scripts/pre_a_contents_remaining_scope_owner_audit.py"
INDEPENDENT = ROOT / "codes/foundations/pre_a_contents_remaining_scope_owner_audit_independent.py"
HOSTILE = ROOT / "codes/foundations/pre_a_contents_remaining_scope_owner_audit_hostile.py"
DEFAULT_OUTPUT = ROOT / (
    "claims/C6-SPACETIME-SIGNATURE/runs/"
    "2026-08-31-integrated-pre_a_contents_remaining_scope_owner_audit/integrated.json"
)
PRIMARY_OUTPUT = ROOT / (
    "claims/C6-SPACETIME-SIGNATURE/runs/"
...
def run(output: Path = DEFAULT_OUTPUT, contents_root: Path | None = None) -> dict:
    ...
    commands = [
        ...
        ("hostile", HOSTILE, HOSTILE_OUTPUT),
    ]
    command_reports = {}
    for label, script, result_path in commands:
        command = [sys.executable, "-X", "utf8", str(script), "--output", str(result_path)]
        if label != "hostile":
    ...
    for name, key in (
        ...
        ("unique boundary", "unique_content_hashes"),
        ...
        check(name, pscan[key] == expected[key], pscan[key], expected[key], "owner-applicability")
    for name, key in (
        ("merged row boundary", "all_complete_rows_are_merged"),
        ("loose phrase boundary", "loose_qledger_only_complete_rows_are_merged"),
    ):
        check(name, pscan[key] == expected[key], pscan[key], expected[key], "owner-applicability")
    check(
        "hostile firewall",
    ...
    check(
        ...
        [manifest["claim_bearing"], manifest["tier"], manifest["formal_integration"]["no_tier_change"]],
        [False, "T0", True],
        "promotion-firewall",
    )
    payload = {
        "schema": "tect/pre-a-contents-remaining-scope-owner-audit-integrated/1.0",
        "run_kind": "integrated",
        ...
        "primary_summary": {key: pscan[key] for key in keys},
        "hostile_summary": {"mutations_rejected": hostile["mutations_rejected"], "assertion_count": hostile["assertion_count"]},
        "source_hashes": {
    ...
    print(f"R-460 INTEGRATED {payload['verdict']} {len(checks)}/{len(checks)}", flush=True)
    return payload
...
def main() -> int:
    ...
    if args.self_test:
        ...
        print("R-460 INTEGRATED SELFTEST: PASS", flush=True)
    return 0
