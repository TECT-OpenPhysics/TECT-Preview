...
## Current v0.1.42 submission package

The operator requested preparation without external review on 2026-09-09.
The manuscript changes only publication-status and AI-assistance disclosures.
No proof, coefficient, canonical source, claim tier, or external signature is
changed. The current submission entry is `../submission-readiness.md`.

Run `python -X utf8 publish/papers/a2-r157-r158-ensemble-minimizers/verification/submission_package.py --self-test`
after the paper audits, refreshed manifest and visual review. It rejects
missing files, mismatched file/source/PDF identities, nonblank reviewer
signatures and unreviewed PDF replacements. Its transport archives are
submission materials, not a claim-level operator-confirmed capstone.

The canonical suite below retains its fifteen-command contract. The
34-check re-review, PDF QA, and submission-package audit are supplemental.

For an extracted source ZIP (no Git history required), from its `source/`
directory use the following with your actual runtime paths:

```text
python -X utf8 publish/papers/a2-r157-r158-ensemble-minimizers/verification/replay_archived_source.py --self-test --lean-cache PATH_TO_RESOLVED_LEAN_CACHE --tectonic PATH_TO_PINNED_TECTONIC
```

This checks input hashes first, runs the same fourteen nested commands and
writes `verification/runs/archive-replay-current.json`. Install the pinned
requirements and resolve Lean/Mathlib as specified below. The source ZIP does
not distribute Python, TeX or the private dependency cache. The separate
`clean_snapshot_replay.py` command is for a Git checkout, and
`submission_package.py` default/export mode is a maintainer checkout check;
its `--verify-archive PATH_TO_ZIP` mode also works without Git history.
