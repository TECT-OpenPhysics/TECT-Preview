...
from the Dirichlet part of the Class-II density.  To fix the notation used in
...
$L^2(0,T;H^4)$ bound.  Aubin--Lions compactness \cite{AubinLions,Temam} yields strong convergence in
...
particular, $D\Phi:H^2\to L^2\subset H^{-2}$ is locally Lipschitz, while the
solution belongs to $L^2(s,t;H^2)\cap H^1(s,t;H^{-2})$ on every finite interval.
The Bochner chain rule therefore gives
\begin{equation}
 \Phi(u(t))-\Phi(u(s))=\int_s^t\ip{N(u(\tau))}{\partial_\tau u(\tau)}_{\R}\dd\tau.
 \label{eq:nonlinear-chain}
\end{equation}
The quadratic part is $\frac12\norm{L^{1/2}u}_2^2$ and obeys the corresponding
Hilbert-space chain rule under the same time regularity.  Combining the two
identities proves \eqref{eq:energy-id}, not just an energy inequality.  The
...
this last assertion explicit.  Write $\delta=\mu-\mu_t>0$.  Since the
nonnegative Fourier symbol of $L-\lambda_0$ grows quadratically in
$|k|^2$, there are constants $c>0$ and $C_\delta<\infty$ such that
\begin{equation}
 \frac12\ip{\Psi}{(L-\lambda_0)\Psi}_{\R}
 \ge c\norm{\Psi}_{H^2}^2-C_\delta\norm{\Psi}_2^2.
...
convergence in $H^1$ and uniformly in space (after a subsequence).  Hence the
local potential is continuous, while the spectral quadratic form is weakly
lower semicontinuous.  Writing $u_n$ and $u$ for the realifications of
...
repository synchronization gate only; the paper remains at draft pending the
analytic proof audit, specialist literature review, operator confirmation,
capstone bundle, and release checks.
